/*
 *	@(#)ctCursor.c	11.1	10/22/97	12:34:40
 *	@(#) ctCursor.c 61.1 97/02/26 
 *
...
#ident "@(#) $Id: ctCursor.c 61.1 97/02/26 "
...
/*
 * Disable Cursor at Cursor Read/Write Index Register
 */
#define CT_DISABLE_CURSOR() {						\
	unsigned long xx_val;						\
	CT_CRIN(CT_CUR_CONTROL, xx_val);				\
	xx_val = 0x00;							\
	CT_CROUT(CT_CUR_CONTROL, xx_val);				\
}

/*
 * Enable Cursor at Cursor Read/Write Index Register. Set the cursor
 * No blink, No stretch, Coordinate system in upper left.
 */
 ...
 * CT_SET_CURTYPE():
 *
 * Select either 32x32 or 64x64 cursor.
 *
 *	0	32x32
 *	1	64x64
 */
 ...
 * CT_SELECT_CURSOR():
 *
 * Select one of four 32x32 cursors.
 *
 *	0x00	cursor 0
 *	0x01	cursor 1
 *	0x10	cursor 2
...
#define CT_SELECT_CURSOR(idx) {						\
	unsigned long xx_val;						\
	CT_CRIN(CT_CUR_BASELO, xx_val);					\
	xx_val &= 0xf0;							\
	xx_val |= ((idx) & 0x0f);					\
	CT_CROUT(CT_CUR_BASELO, xx_val);				\
}
#else
#define CT_SELECT_CURSOR(idx) {						\
	unsigned long xx_val;						\
...
#define CT_SET_CURPOS(x, y) {						\
	unsigned long xx_val;						\
	xx_val = (x) & 0xff;						\
	CT_CROUT(CT_CUR_XLO, xx_val);					\
	CT_CRIN(CT_CUR_XHI, xx_val);					\
	xx_val = ((x) >> 8) & 0x07;					\
	CT_CROUT(CT_CUR_XHI, xx_val);					\
	xx_val = (y) & 0xff;						\
	CT_CROUT(CT_CUR_YLO, xx_val);					\
	CT_CRIN(CT_CUR_YHI, xx_val);					\
	xx_val = (((y) >> 8) & 0x07) | 0x80;				\
	CT_CROUT(CT_CUR_YHI, xx_val);					\
}
...
#define CT_SET_CURSOR_BASE(addr) {					\
	...
	CT_CRIN(CT_CUR_BASELO, xx_val);					\
	xx_val &= 0x0f;							\
	xx_val |= (((addr >> 12) & 0x0f) << 4);				\
	CT_CROUT(CT_CUR_BASELO, xx_val);				\
	CT_CRIN(CT_CUR_BASEHI, xx_val);					\
	xx_val &= 0xc0;							\
	xx_val |= ((addr >> 16) & 0x3f);				\
	CT_CROUT(CT_CUR_BASEHI, xx_val);				\
}
#else
#define CT_SET_CURSOR_BASE(addr) {					\
	unsigned long xx_val;						\
...
CursorPtr pCursor;
{
	ctCursorPtr ctCursor;
	ctCursorType type;
	int size, width, height, w, wib, srcinc, dstinc, dstride;
...
#ifdef DEBUG_PRINT
		ErrorF("calling scoSWCursorInitialize\n");
#endif
		/*
...
#ifdef DEBUG_PRINT
		ErrorF("Hardware cursor disabled\n");
#endif
		return;
	}

#define CUR_PIXELS_PER_BYTE	0x8
#define CUR_NUM_PLANES		0x2
#define CUR_NUM_CURSORS		0x4
#define CUR_BYTES		\
	(CUR_PIXELS_PER_BYTE * CUR_NUM_PLANES * CUR_NUM_CURSORS)
#define CUR_HEIGHT		64

	/*
	 * Allocate twice as many rows so that we can 4k align it.
	 */
	ctPriv->cursor_chunk = (void *)CT_MEM_ALLOC(pScreen, 	
		CUR_BYTES * 2, CUR_HEIGHT, 16);
	ctPriv->cursor_memory = (unsigned char *)
		(((unsigned)CT_MEM_VADDR(ctPriv->cursor_chunk) 
			+ 4096) & ~0xfff);
#ifdef DEBUG_PRINT
	ErrorF("cursor_memory=0x%x\n", ctPriv->cursor_memory);
#endif /* DEBUG_PRINT */
