 ...
 * ifbImage8.c
 *
 * ifb ReadImage and DrawImage routines for depths 2 through 8.
 */
...
/*
 * ifbReadImage8() - Read a rectangular area of a window into image.
 ...
 */
void 
ifbReadImage8(pbox, image, stride, pDraw)
...
/*
 * ifbDrawImage8() - Draw pixels in a rectangular area of a window.
 ...
 */
void 
ifbDrawImage8(pbox, image, stride, alu, planemask, pDraw)
    BoxPtr pbox;
    unsigned char *image;
    unsigned int stride;
    unsigned char alu;
    ...
    DrawablePtr pDraw;
{
    unsigned long allplanes = (unsigned long)0xFF >> (8 - pDraw->depth);
    int w, h, idx;
    int tmpy;
