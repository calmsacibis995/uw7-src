 ...
 *              get rgbbits from the visual, not grafinfo.
 */
/*
 *   wdCmap.c
 */
...
/*
 * wdSetColor() - set an entry in a PseudoColor colormap
 ...
 */
void
wdSetColor(cmap, index, r, g, b, pScreen)
	unsigned int cmap;
	unsigned int index;
	unsigned int r;
	unsigned int g;
...
#ifdef DEBUG_PRINT
	ErrorF("wdSetColor(cmap=%d, index=%d, r=%d, g=%d, b=%d)\n",
		cmap, index, r, g, b);
#endif
...
/*
 * Called through nfbScrnPriv->LoadColormap && wdSetGraphics()
 */
void
wdLoadColormap(cmap)
...
#ifdef DEBUG_PRINT
	ErrorF("wdLoadColormap(cmap=%d)",cmap );
#endif
