/*
 * @(#) mgaGC.c 11.1 97/10/22
 *
 * Copyright (C) 1994-1995 The Santa Cruz Operation, Inc.
 *
 * The information in this file is provided for the exclusive use of the
 * licensees of The Santa Cruz Operation, Inc.  Such users have the right
 * to use, modify, and incorporate this code into other products for purposes
 * authorized by the license agreement provided they include this notice
 * and the associated copyright notice with any such product.  The
 * information in this file is provided "AS IS" without warranty.
 *
 */

/*
 *	S000	Thu Jun  1 16:52:52 PDT 1995	brianm@sco.com
 *	- New code from Matrox.
 */

/*
 * mgaGC.c
 *
 * Template for machine dependent ValidateWindowGC() routine
 */

#include "X.h"
#include "Xproto.h"

#include "pixmapstr.h"
#include "gcstruct.h"

#include "nfb/nfbDefs.h"
#include "nfb/nfbGCStr.h"
#include "nfb/nfbGlyph.h"
#include "colormap.h"

#include "mgaProcs.h"

extern nfbGCOps mgaSolidPrivOps, mgaTiledPrivOps,
		mgaStippledPrivOps, mgaOpStippledPrivOps;

/*
 * mgaValidateWindowGC() - set GC ops and privates based on GC values.
 *
 * Just set the nfb GC private ops here;
 * nfbHelpValidateGC() will do most of the work.
 */
void
mgaValidateWindowGC(
	GCPtr pGC,
	Mask changes,
	DrawablePtr pDraw)
{
	nfbGCPrivPtr pPriv = NFB_GC_PRIV(pGC);
#ifdef DEBUG_PRINT
	ErrorF("mgaValidateWindowGC()\n" );
#endif

	/* set the private ops based on fill style */
	if (changes & GCFillStyle)
		switch (pGC->fillStyle) {
		    case FillSolid:
			pPriv->ops = &mgaSolidPrivOps;
			break;
		    case FillTiled:
			pPriv->ops = &mgaTiledPrivOps;
			break;
		    case FillStippled:
			pPriv->ops = &mgaStippledPrivOps;
			break;
		    case FillOpaqueStippled:
			if (pGC->fgPixel == pGC->bgPixel)
			    pPriv->ops = &mgaSolidPrivOps;
			else
			    pPriv->ops = &mgaOpStippledPrivOps;
			break;
		    default:
			FatalError("mgaValidateWindowGC: illegal fillStyle\n");
		}

	/* let nfb do the rest */
	nfbHelpValidateGC(pGC, changes, pDraw);
}
