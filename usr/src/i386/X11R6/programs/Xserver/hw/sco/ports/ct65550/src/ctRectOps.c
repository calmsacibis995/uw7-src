/*
 *	@(#)ctRectOps.c	11.1	10/22/97	12:35:01
 *	@(#) ctRectOps.c 61.2 97/02/26 
 *
...
#ident "@(#) $Id: ctRectOps.c 61.2 97/02/26 "
...
#ifdef DEBUG_PRINT
	ErrorF("CopyRect(): dst=(%d,%d)-(%d,%d) src=(%d,%d) alu=%d\n",
...
DrawablePtr pDraw;
{
	 ...
	 */
	CT_SET_BLTCTRL(CT(PatternOps)[alu] | CT_PATSOLIDBIT | CT_PATMONOBIT);
	CT_SET_BLTOFFSET(0, ctPriv->bltStride);
	CT_SET_BLTBGCOLOR(fg);

	while (nbox--) {
#ifdef DEBUG_PRINT
		ErrorF("DrawSolidRects(): pbox=0x%08x nbox=%d fg=0x%08x\n",
			pbox, nbox, fg);
		ErrorF("DrawSolidRects(): alu=%d w=%d, h=%d (%d,%d)\n", alu,
