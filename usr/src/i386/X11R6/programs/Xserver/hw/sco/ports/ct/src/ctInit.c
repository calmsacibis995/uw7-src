/*
 *	@(#)ctInit.c	11.1	10/22/97	12:10:44
 *	@(#) ctInit.c 12.1 95/05/09 
 *      ctInit.c 9.3 93/07/27 SCOINC
 *
...
ctGetInfo()
{
	...
	switch (version & 0xf0) {
	case 0xb0:
		ErrorF("Chips & Technologies 6430x Revision %d\n",
			(version & 0x0f));
		break;
	default:
		ErrorF("Warning: C&T 6430x not detected (version register=0x%02x)\n",
			version & 0xff);
		return;
	}
	...
	case 0x02:
		...
		break;
	case 0x03:
		bus_type = "32-bit Local Bus";
		break;
	}

	switch (memory & 0x03) {
	case 0x00:
		mem_conf = "512Kb VRAM, 16-bit Data Path";
		break;
	case 0x01:
		mem_conf = "1Mb VRAM, 32-bit Data Path";
		break;
	case 0x02:
		mem_conf = "2Mb VRAM, 32-bit Data Path";
		break;
	case 0x03:
		mem_conf = "?Mb VRAM, ??-bit Data Path";
		break;
	}
