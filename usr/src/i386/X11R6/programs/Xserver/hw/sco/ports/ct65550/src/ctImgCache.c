/*
 *	@(#)ctImgCache.c	11.1	10/22/97	12:34:51
 *	@(#) ctImgCache.c 61.2 97/02/26 
 *
...
#ident "@(#) $Id: ctImgCache.c 61.2 97/02/26 "
...
unsigned long fg, bg;
{
...
#ifdef DEBUG_PRINT
		ErrorF("ImageCacheLoad: k=%d chunk_width 0x%x chunk_height 0x%x width 0x%x height 0x%x\n",
		k, chunk_width, chunk_height, image_width, image_height);
#endif
	...
	if (depth == 1) {
		...
		CT(DoDrawMonoImage)(ctPriv->fbPointer, chunk_stride, src_offset,
				image_width, image_height, image, stride, 0);
	} else {
		/*
		...
		/*
		 * All transfers must be qword aligned.
	 	 */
		if ((dwidth * image_height) & 0x1)
			*pdst = 0;
	}
...
#ifdef DEBUG_PRINT
		ErrorF("ImageCacheLoad(): replicating tile\n");
#endif
		/*
...
#ifdef DEBUG_PRINT
		ErrorF("ImageCacheLoad(): replicating tile - done\n");
#endif
	}
