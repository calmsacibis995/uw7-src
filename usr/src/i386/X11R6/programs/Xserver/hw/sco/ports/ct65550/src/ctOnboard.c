/*
 *	@(#)ctOnboard.c	11.1	10/22/97	12:34:57
 *	@(#) ctOnboard.c 59.1 96/11/04 
 *
...
#ident "@(#) $Id: ctOnboard.c 59.1 96/11/04 "
...
ScreenPtr pScreen;
{
	...
	CT_XRIN(0x43, memory_bits);

	switch (memory_bits & 0x02) {
	case 0x00:
		heap->size = 1024 * 1024;	/* 1MB */
		break;
	case 0x02:
		heap->size = 2048 * 1024;	/* 2MB */
		break;
	}
