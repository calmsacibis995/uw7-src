 ...
 * ifbImage16.c
 *
 * ifb ReadImage and DrawImage routines for depths 9 through 16.
 */
...
/*
 * ifbReadImage16() - Read a rectangular area of a window into image.
 ...
 */
void 
ifbReadImage16(pbox, image, stride, pDraw)
    ...
    DrawablePtr pDraw;
{
    ...
    psrc = (unsigned char *)ifbBase[idx]
		    + (pbox->y1 * ifbStride[idx]) + (pbox->x1 << 1);
...
/*
 * ifbDrawImage16() - Draw pixels in a rectangular area of a window.
 ...
 */
void 
ifbDrawImage16(pbox, image, stride, alu, planemask, pDraw)
    BoxPtr pbox;
    unsigned char *image;
    unsigned int stride;
    unsigned char alu;
    ...
    DrawablePtr pDraw;
{
    unsigned long allplanes = (unsigned long)0xFFFF >> (16 - pDraw->depth);
    int w, h, idx;
    int tmpy;
