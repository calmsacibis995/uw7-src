/*
 *	@(#)ctImage.c	11.1	10/22/97	12:34:49
 *	@(#) ctImage.c 62.1 97/03/21 
 *
...
#ident "@(#) $Id: ctImage.c 62.1 97/03/21 "
...
unsigned int stride;
{
	...
	pscan_start = (unsigned char *)(image_addr & ~7);
	delta = image_addr - (unsigned long)pscan_start;
	scanwidth = (int)(((image_addr + wib + 7) & ~7) - (int)pscan_start);
	dwidth = scanwidth >> 2;
	...
	CT_SET_BLTCTRL(CT(PixelOps)[alu] | CT_YINCBIT | CT_XINCBIT |
			CT_BLTMEMORYBIT);
#else
	CT_SET_BLTCTRL(CT(PixelOps)[alu] | CT_BLTMEMORYBIT);
#endif
	...
	ndwords = dwidth * height;
#ifdef DEBUG_PRINT
	ErrorF("Num dwords 0x%x\n", ndwords);
#endif
...
#endif
#endif
		pscan_start += stride;	/* stride MUST be 64-bit padded */
	}
...
DrawablePtr pDraw;
{
...
#ifdef DEBUG_PRINT
	ErrorF("DrawImage(): 0x%08x - 0x%08x\n", pbox->x1, pbox->x2);
#endif /* DEBUG_PRINT */

	if ((planemask & ctPriv->allPlanes) != ctPriv->allPlanes) {
#ifdef DEBUG_PRINT
		ErrorF("No HW planemask support!\n");
#endif /* DEBUG_PRINT */
		/*
	...
	default:
		...
		if ((height != 1) && ((stride & (8 - 1)) != 0)) {
			/*
			 * The BitBlt engine routines require a 64-bit padded
			 * stride.
			 */
			CT(DrawImageFB)(pbox, image, stride, alu, ~0, pDraw);
			return;
		}
		if (((unsigned long)image & 7) ||
		    ((wib != stride) && (height != 1))) {
			/*
	...
	if (height == 1) {
		 ...
		 */
		ndwords = CT_BYTES_TO_QUADS(wib) * 2;
	} else {
		ndwords = CT_BYTES_TO_QUADS(stride) * height * 2;
	}
	...
	CT_START_BITBLT(width, height);
	CT_SET_BLTDATA((unsigned char *)pdst, psrc, ndwords);
...
#ifdef DEBUG_PRINT
	ErrorF("stride 0x%x alu 0x%x planemask 0x%x pdst_start 0x%x\n",
		stride, alu, planemask, pdst_start);
#endif
