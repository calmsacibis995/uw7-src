/*
 *	@(#)ctMono.c	11.1	10/22/97	12:10:48
 *	@(#) ctMono.c 12.1 95/05/09 
 *
...
#ifdef DEBUG_PRINT
	CT(BitBltDebugF)("DoDrawMonoImage(): 0x%08x s=%d w=%d (%d,%d)(%dx%d)\n",
...
DrawablePtr pDraw;
{
	...
	if (startx == 0) {
		...
		CT(DoDrawMonoImage)(ctPriv->fbPointer, ctPriv->bltStride,
				...
				width, height, image, stride);
#if 0
		CT(DoDrawOffsetMonoImage)(ctPriv->fbPointer, ctPriv->bltStride,
				CT_SCREEN_OFFSET(ctPriv, pbox->x1, pbox->y1),
				width, height, image, startx, stride);
#endif
	}
}
