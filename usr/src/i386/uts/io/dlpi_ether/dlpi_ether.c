 ...
 *
 *      L001    ERG   10th April 1997
 *              - Get get message delivered to wrong SAP
 *              - DLrsrv sets SEND_PENDING if SAPs match, if another
 *                processor is in DLsrv then it may well see this
 *                and send the message up as SAPs dont match!
 */
...
DLform_snap( DL_sap_t *sap, mblk_t *mp, uchar_t *src_addr )
{
	...
	hdr->mac_llc.snap.snap_length = 
			htons(msgdsize(mp) + LLC_HDR_SAP_SIZE + SNAP_SAP_SIZE);
...
DLtest_req(queue_t *q, mblk_t *mp)
{
	...
	if (size < sap->min_spdu) {
		if ((tmp = allocb(sap->min_spdu, BPRI_MED)) == NULL) {
			DLuderror_ind(q, mp, DL_UNDELIVERABLE);
			return;
		}
		tmp->b_wptr = tmp->b_rptr + (sap->min_spdu - size);
		linkb(mp,tmp);
	}
...
DLinfo_req( queue_t *q )
{
	...
	if ((resp = allocb (DL_PRIMITIVES_SIZE + DL_ETH_ADDR_LEN + LLC_ADDR_LEN,
							BPRI_MED)) == NULL)
		return;
...
DLbind_req( queue_t *q, mblk_t *mp )
{
	...
	if ((resp = allocb
		(DL_PRIMITIVES_SIZE + DL_ETH_ADDR_LEN, BPRI_MED)) == NULL) {
...
DLsubsbind_req( queue_t *q, mblk_t *mp )
{
	...
	/* LINTED pointer alignment */
	subs_sap = (struct snap_sap *)
			((caddr_t)subs_bind + subs_bind->dl_subs_sap_offset);
	...
	ptmp = (uchar_t *)
		((caddr_t)subs_bind_ack + subs_bind_ack->dl_subs_sap_offset);
...
DLunitdata_req( queue_t	*q, mblk_t *mp )
{
	...
	case DL_CSMACD:
		if (sap->sap_addr == SNAPSAP) {
			...
			llccp = (struct llcc *)
			((caddr_t)data_req + data_req->dl_dest_addr_offset);
...
/* vv L001 vv **
 *
 * Dont set SEND_PENDING in this case as we will send the message
 * in the "Don't forget the one who brougt us." Section below
 * If we set SEND_PENDING we can send this up the wrong SAP as
 * on a SMP box another processor could be in this routine in the
 * "while (nsap)" section below ...
 *
#ifdef ESMP
 ...
 ** ^^ L001 ^^ */
			...
			else if ((sap->sap_addr == PROMISCUOUS_SAP)
						||
					(sap->flags & RAWCSMACD)) {
				continue;
			}
				...
				if ( !(nsap->flags & RAWCSMACD)
					&&
				((nsap->sap_addr == sap_id)
					||
				(sap_id == LLC_GLOBAL_SAP))) {
				    if ((sap_id == SNAPSAP)
						&&
					(! DLis_equalsnap(sap, nsap))) {
						continue;
				    }
#ifdef ESMP
				    nsap->flags |= SEND_PENDING;
...
DLproc_llc( queue_t *q, mblk_t *mp_data )
{
	...
	if (ssap & LLC_RESPONSE) {
		...
		if ( (control == LLC_TEST) || (control == (LLC_TEST|LLC_P)) ) {
			...
			if (canput(sap->read_q->q_next)) {
#endif
				if ( (xmp = copyb(mp_data))
						&&
					(data_ind = DLmk_test_con(
							&hdr->dst,
							&hdr->src,
							dsap,
							ssap,
			...
			bd->mib.ifInDiscards++;         /* SNMP */
			cmn_err(CE_WARN,
				"!DLrsrv: pkt discarded due to flow control");
			return ((mblk_t *)1);
		}
		...
		case LLC_TEST:		/* Need to respond to a TEST command */
		case LLC_TEST |LLC_P:
			...
			/* test response includes data, if any, as echo */
			nmp->b_cont = xmp;

			/* A valid TEST response can now be transmitted */
			break;

		case LLC_XID:		/* Need to respond to a XID command */
		case LLC_XID|LLC_P:
			if ((nmp = allocb(sizeof(DL_mac_hdr_t), BPRI_MED))
									== NULL)
				return NULL;
			...
			newhdr->mac_llc.llc.llc_length =
				htons(LLC_XID_INFO_SIZE+ LLC_HDR_SAP_SIZE);
			nmp->b_wptr =
				nmp->b_rptr + LLC_XID_INFO_SIZE + LLC_HDR_SIZE;
		...
		/*
		 * Only if having response to send.
		 */
		opri = DLPI_LOCK ( bd->bd_lock, plstr ) ;
		if (bd->flags & TX_BUSY) {
			bd->flags |= TX_QUEUED;
...
DLmk_ud_ind( mblk_t *mp_data, DL_sap_t *sap )
{
	...
	case DL_ETHER:
		...
		data_ind->dl_src_addr_offset  =
					DL_UNITDATA_IND_SIZE + DL_ETH_ADDR_LEN;

		/* LINTED pointer alignment */
		llcap = (struct llca *)
			((caddr_t)data_ind + DL_UNITDATA_IND_SIZE);
		...
		llcap->lbf_sap = ( ushort )len_type_field;
		mp->b_wptr =
		mp->b_rptr + (DL_UNITDATA_IND_SIZE + 2 * DL_ETH_ADDR_LEN);
	...
	case DL_CSMACD:
		...
		if ( (sap->flags & RAWCSMACD) || (dsap != SNAPSAP) ) {
			...
			data_ind->dl_src_addr_offset  =
				DL_UNITDATA_IND_SIZE + DL_LLC_ADDR_LEN;
			...
			mp->b_wptr = mp->b_rptr +
				(DL_UNITDATA_IND_SIZE + 2 * DL_LLC_ADDR_LEN);
			...
			data_ind->dl_src_addr_offset =
				DL_UNITDATA_IND_SIZE + sizeof(struct llcb);

			/* LINTED pointer alignment */
			llcbp = (struct llcb *)
			((caddr_t)data_ind + data_ind->dl_dest_addr_offset);
			...
			llcbp->lbf_xsap = 0;
			BCOPY(hdr->mac_llc.snap.snap_org, &llcbp->lbf_xsap,
							SNAP_GLOBAL_SIZE);
			...
			llcbp->lbf_type = SNAP_TYPE(hdr);
			mp->b_wptr = mp->b_rptr +
				DL_UNITDATA_IND_SIZE+(2 * sizeof(struct llcb));
