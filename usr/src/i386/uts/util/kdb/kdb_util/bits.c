#ident	"@(#)kern-i386:util/kdb/kdb_util/bits.c	1.10.2.1"
/* @(#)bits.c	10.3 */
...
as_addr_t
dis_dot(as_addr_t ldot, int mode16)
{
	...
	*/
	...
	if (dp->indirect != TERM) {
		...
		if (opcode1 == 0xD && opcode2 >= 0x8) {
			...
			if (opcode2 == 0xB && mode == 0x3 && opcode3 == 4)
				dp = &opFP5[r_m];
			/* opcode3 == 6 and 7 is new for P6 processor */
			else if (opcode2 == 0xB && mode == 0x3 && opcode3 == 5) {
				sprintf(mneu,"***** Error - bad opcode\n");
				errlev++;
				return loc;
			}
	...
	default:
		sprintf(mneu,"***** Implementation Error - missing case\n");
		errlev++;
		return loc;
	} /* end switch */
}
