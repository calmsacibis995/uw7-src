...
bool aopt,		/* all channels in /etc/default/mapchan */
            ...
            sopt, 		/* set a map */
	    ctty,		/* channel is controlling tty */
            Dopt;		/* Debug flag */
...
get_opts()
{
	...
	aopt = ctty = Dopt = dopt = nopt = oopt = sopt = FALSE;
	*fopt_file = CNULL;
	SHIFT;
	while (argc && **argv == '-') {
		p = (*argv)+1;
...
check_channels()
{
	...
	for (cp = chan_first; cp; cp = cp->chan_next) {
		...
		if (sopt) {
			/*
			 * we will map this channel. Check for writability
			 */
			if (access(name, 2) < 0) {
				error(
				     "you cannot write %s and so cannot map it\n",
				     name); 
				cp->chan_name = BAD_CHANNEL;
				continue;
			}
 ...
 * try again without NDELAY set.  (but special handling of /dev/lp* devices)
...
char *name;
{
	...
	if (errno == EBUSY) { 
		if (ctty == TRUE &&
		    strncmp(name, lpdevnm, strlen(lpdevnm)) == 0 &&
		    (fd = dup(fileno(stdout))) >= 0) {
			return (fd);
		}
