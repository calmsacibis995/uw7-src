...
#ident	"@(#)eac:i386/eaccmd/dosutil/dosformat/bin2c.c	1.1"
...
#define BOOTSZ		512	/* size of boot code in master boot block */
unsigned	char	bootcode[BOOTSZ];
...
char	*argv[];
{
	...
	*(unsigned short *)&bootcode[BOOTSZ-2] = BOOTSIG;

	fprintf(fdout, "#ident	\"@(#)eac:i386at/eaccmd/dosutil/dosformat/MS-DOS_boot.h	1.1\"\n");
	fprintf(fdout, "/*\n");
	fprintf(fdout, " *	This file is generated from the assembly language\n");
	fprintf(fdout, " *	code in the file \"bootstrap.s\"\n");
	fprintf(fdout, " */\n\n\n");
	fprintf(fdout, "unsigned char Bootcode[] = {\n");
	i = 0;
	while(i < BOOTSZ){
		fprintf(fdout, "   ");
		for(j=0; j<10 && i < BOOTSZ; i++, j++){
			ch = (unsigned int) bootcode[i];
