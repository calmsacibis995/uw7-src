...
struct option longopts[] =
{
 ...
 /* {"list",       0, 0, 'l'},    list .z file contents */
    ...
    {"Block",	   0, 0, 'B'}, /* Specify block compress mode */
    {"size",	   1, 0, 's'}, /* Specify block compress mode, block size */
    { 0, 0, 0, 0 }
};
...
unsigned char Blk_magic[] = { 0x19, 0x9e, 'T', 'G' };
...
#else /* !LZW */
#  ifdef NO_DIR
            "usage: %s [-cdfhLtvV19B] [-s block-size ] [file ...]\n",
#  else
            "usage: %s [-cdfhLrtvV19B] [-s block-size ] [file ...]\n",
#  endif
#endif /* LZW */
             progname);
}
    ...
    char **argv;
{
    ...
    while ((optc = getopt_long (argc, argv, "b:cdfhLqrtvVZ123456789Bs:",
				longopts, (int *)0)) != EOF) {
	switch (optc) {
	case 'b':
	    maxbits = atoi(optarg);
	...
	if (file_count != 0) {
		...
		while(optind < argc) {
			if (decompress) tryit_decomp(argv[optind]);
			else tryit(argv[optind]);
			optind++;
		}
	}
	else {
...
#endif
	WARN((stderr,"%s is a directory -- ignored\n", ifname));
	return;
    }
    if (!S_ISREG(istat.st_mode)) {
	WARN((stderr, "%s is not a directory or a regular file - ignored\n",
	      ifname));
	return;
    }
    if (istat.st_nlink > 1 && !to_stdout && !force) {
	WARN((stderr, "%s has %d other link%c -- unchanged\n", ifname,
    ...
    char *name;
{
    ...
    if (p == NULL || p == name || p[-1] == PATH_SEP) return NULL;
    ...
    struct stat *sbuf;
{
	...
	WARN((stderr, "%s: Not a regular file -- ignored\n", ifname));
	return -1;
    }
    return 0; /* ok */
}
...
local int make_ofname()
{
    ...
    if (decompress) {
	if (suff == NULL) {
	    WARN((stderr,"%s -- no z suffix, ignored\n", ifname));
	    return -1;
	}
    ...
    int in;        /* input file descriptor */
{
	...
	WARN((stderr, "trailing garbage ignored in %s\n", ifname));
	return -2;
    }
}
    ...
    struct stat *ifstat;
{
	...
	WARN((stderr, " utime error (ignored)\n"));
    }
#endif
    /* Copy the protection modes */
    if (chmod(ofname, ifstat->st_mode & 07777)) {
	if (!quiet) perror(ofname);
	WARN((stderr, " chmod error (ignored)\n"));
    }
#ifndef NO_CHOWN
	...
	WARN((stderr, " unlink error (ignored)\n"));
    }
}
...
{
	int fd,olen,cnt,i;
	char *mybuf = malloc(Block_size);
	char *myobuf = malloc(Block_size*2);
	BLOCK_HEADER *blk_hdr;
	ulong blocks, remain,size;

	if (n) fd = open(n,O_RDONLY);
	else fd = 0;
	size = lseek(fd,0L,2);
	blocks = size / Block_size;
	remain = size % Block_size;
	blk_hdr = malloc(blocks * sizeof(BLOCK_HEADER));
	if (remain) blocks++;
	lseek(fd,0L,0);
	lseek(1,blocks*sizeof(BLOCK_HEADER)+sizeof(Blk_magic)+sizeof(ulong)*2,0);
	i = 0;
	while((cnt = read(fd,mybuf,Block_size))) {
		mzip(mybuf,cnt,myobuf,&olen);
		blk_hdr[i].ucmp_size = cnt;
		blk_hdr[i].cmp_size = (char *)global_outbuf-myobuf+1;
		blk_hdr[i].daddr = lseek(1,0L,1);
		i++;
		write(1,myobuf,(char *)global_outbuf-myobuf+1);
	}
	lseek(1,0L,0);
	write(1,Blk_magic,sizeof(Blk_magic));
	write(1,&blocks,sizeof(ulong));
	write(1,&Block_size,sizeof(ulong));
	write(1,blk_hdr,sizeof(BLOCK_HEADER)*blocks);
	if (fd) close(fd);
}
...
tryit_decomp(char *n)
{
	...
	read(fd,magic,4);
	if (memcmp(magic,Blk_magic,4) != 0) {
		close(fd);
		fprintf(stderr,"gzip: ");
		if (n) fprintf(stderr,"%s ", n);
		else fprintf(stderr,"stdin ");
		fprintf(stderr,"not in block compressed format.\n");
		return -1;
	}
	read(fd,&blks,sizeof(ulong));
	read(fd,&blksize,sizeof(ulong));
	blk_hdr = malloc(sizeof(BLOCK_HEADER) * blks);
	read(fd,blk_hdr,sizeof(BLOCK_HEADER)*blks);
	for(i = 0; i < blks; i++) {
		obuf = realloc(obuf,blk_hdr[i].ucmp_size);
		ibuf = realloc(ibuf,blk_hdr[i].cmp_size);
		read(fd,ibuf,blk_hdr[i].cmp_size);
		munzip(ibuf,obuf,blk_hdr[i].cmp_size,blk_hdr[i].ucmp_size);
		write(1,obuf,blk_hdr[i].ucmp_size);
	}
	if (fd) close(fd);
	free(obuf);
	free(ibuf);
