 ...
 * to accomodate in-memory decompression.
 */
...
/*  REG10  int        insize; (global) */
	...
	WARN((stderr, "%s: warning, unknown flags 0x%x\n",
		ifname, maxbits & LZW_RESERVED));
    }
    maxbits &= BIT_MASK;
	...
	while (inbits > posbits) {
		...
		if (code > free_ent) {
		    ...
		    if (!test && outpos > 0
			&& write(out, outbuf, outpos) != outpos) {
			write_error();
		    }
	    ...
	    *--stackp =	(char_type)(finchar = tab_suffixof(code));
		...
		if (outpos+(i = (de_stack-stackp)) >= OUTBUFSIZ) {
			...
			if (outpos >= OUTBUFSIZ) {
			    if (!test && write(out, outbuf, outpos) != outpos){
				write_error();
			    }
			    outpos = 0;
			}
    ...
    if (!test && outpos > 0 && write(out, outbuf, outpos) != outpos) {
	write_error();
    }
}
