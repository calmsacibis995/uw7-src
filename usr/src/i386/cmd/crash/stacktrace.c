#ident	"@(#)crash:i386/cmd/crash/stacktrace.c	1.2.1.6"
/* based on kern-i386:util/kdb/kdb_util/stacktrace.c 1.10.1.1 */
 ...
 * SYSDUMP_SYNC, SYSCONTEXT_RETURNED:
...
#endif
#define ASSERT(EX) \
	((void)((EX) || error("error: %s %s line %d\n",#EX,__FILE__,__LINE__)))

#define ST_READ_TEXT(addr, buf, n) \
		(readmem(addr,1,-1,buf,n,"stack backtrace"), 0)

#define ST_READ_STACK(addr, buf, n) \
		st_read_stack(addr, buf, n)

/*
 * Does this lwp have saved extension stack memory? Initialize for
 * ext_read accordingly.  But any stack extension has already been
 * prepended to the ublock stack before calling from crash/u.c,
...
#define ST_PRF_RET_LEN(prf, fmt, arg) \
		st_prf_ret_len(prf, fmt, arg)

#define ST_SHOW_SYM_ADDR(addr, prf) \
		st_show_sym_addr(addr, prf)
...
#define SYSDUMP_SYNC	((ulong_t)sysdump_sync)
#define SYSCONTEXT_RETURNED \
			((ulong_t)syscontext_returned)

static vaddr_t trap, nmi, systrap, sigclean, evt_process;
static vaddr_t sysdump_sync, syscontext_returned;
static vaddr_t *structret_funcs;
static size_t structret_funcs_size;
static ulong_t db_st_offset;
...
void
st_init(ulong_t offset)
{
	vaddr_t addr;
	char *sym;

	/*
	 * Initialize kv-to-uv offset for st_read_stack()
	 */
	db_st_offset = offset;
#ifdef DEBUG
	find_return_verbose = (debugmode > 0);
#endif
	if (syscontext_returned)	/* already fully initialized */
		return;

	addr = symfindval(sym = "structret_funcs_size");
	readmem(addr, 1, -1,
		&structret_funcs_size, sizeof(structret_funcs_size), sym);
	addr = symfindval(sym = "structret_funcs");
	structret_funcs = readmem(addr, 1, -1,
		NULL, structret_funcs_size, sym);

	trap = symfindval("trap");
	nmi = symfindval("nmi");
	systrap = symfindval("systrap");
	sigclean = symfindval("sigclean");
	evt_process = symfindval("evt_process");
	sysdump_sync = symfindval("sysdump_sync");
	syscontext_returned = symfindval("syscontext_returned");
	cr_unfree(structret_funcs);
}

int
st_prf_ret_len(void (*prf)(), const char *fmt, ulong_t arg)
{
	char buf[80];
...
int
st_show_sym_addr(vaddr_t addr, void (*prf)())
{
	char *str;
	int len;
	...
	if (str[len-1] == ')') {
		len -= 2;
		str[len] = '\0';	/* we do our own parentheses */
	}
	(*prf)("%s",str);
	return len;
}
 ...
 */
int
st_read_stack(vaddr_t addr, void* buf, size_t n)
{
	(void)memcpy(buf, (char *)addr + db_st_offset, n);
	return 0;
}
 ...
 *	       int *sp_delta_p, ulong_t fn_start)
	    ...
	    int *sp_delta_p, ulong_t fn_start)
{
	...
	if (fn_start == SYSDUMP_SYNC
	&& start_eip != SYSDUMP_SYNC) {
		ret_type = RT_RET;
		*espp += 20;
		return B_TRUE;
	}
 ...
 * void
 * stacktrace(void (*prf)(),
 *            ulong_t sp_end, ulong_t esp, ulong_t ebp, ulong_t eip,
 ...
 *
 *	stacktrace() finishes if esp reaches sp_end (may be UNKNOWN).
 *
 ...
 */
void
stacktrace(void (*prf)(), ulong_t sp_end, ulong_t esp, ulong_t ebp, ulong_t eip,
	   ...
	   lwp_t *lwpp)
{
	...
	if (eip == UNKNOWN) {
		...
		for (;; esp += sizeof(ulong_t)) {
			...
			if (reg >= sp_end
			|| !find_return(eip, &reg, &ebp, NULL, NULL, fn_start)
			||  reg <= esp || reg >= sp_end)
				continue;
	...
	while (ST_VALID_TEXT_ADDR(eip)) {
		...
		fn_start = ST_SYMVAL(eip);
		if ((reg = esp) >= sp_end
		|| !find_return(eip, &esp, &ebp, prf, (int *)NULL, fn_start)
		||  esp < reg || esp >= sp_end) {
		...
		if (!ST_VALID_TEXT_ADDR(nexteip) ||
			...
			if (prf) {
				nframe(eip, ' ', 0, 0, 0, prf);
				if (nexteip != SYSCONTEXT_RETURNED) {
...
done_silent:
			...
			if (IS_TRAP_ROUTINE(fn_entry))
			    ...
			    (void (*)())NULL, &sp_delta, 0) && sp_delta > 0) {
				...
				narg = st_max_args;
			nframe(eip, tag, nexteip, esp, narg, prf);
			if (IS_TRAP_ROUTINE(fn_entry)) {
