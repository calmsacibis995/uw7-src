#ident	"@(#)crash:i386/cmd/crash/tty.c	1.2.3.1"
...
char *type;
{
	...
	fprintf(fp, "count of %s not found: assuming 1\n", type);
	return 1;
found:
...
gettty()
{
	...
	optind = 1;
	while( ( c = getopt( argcnt, args, "eflw:t:")) != EOF) {
		switch( c) {
			case 'e' :	all = 1;
					break;
			case 'f' :	full = 1;
	...
	if (!giventype && !args[optind]) {
		fprintf(fp, "kd TABLE BASE = %x, SIZE = %d\n", addr, count);
		if ( !full) {
			print_header = 0;
		...
		if(line_disp) {
			...
			readmem((vaddr_t)wsbase,1,-1,&wsbase,
				sizeof(wsbase),"cmux_ws_t *");
			if(wsbase == 0) {
				line_disp = 0;
				goto _prstty;
			}
			readmem((vaddr_t)wsbase,1,-1,&cmux_ws,
				sizeof(cmux_ws_t),"cmux_ws_t");
			if(cmux_ws.w_cmuxpp == NULL) {
				line_disp == 0;
				goto _prstty;
			}
			cmux_array = readmem((vaddr_t)cmux_ws.w_cmuxpp,1,-1,
				NULL, count * sizeof(cmux_t *),"w_cmuxpp");
		}
_prstty:
		for ( slot = 0; slot < count; slot++) {
			prstty( all, full, slot, addr, 0, sheading);
			if(line_disp && *(cmux_array+slot)) {
				readmem((vaddr_t)*(cmux_array+slot),1,-1,&cmux,
	...
	if(args[optind]) {
		do {
			if (getargs(count,&arg1,&arg2))
				...
				prstty(1, full, slot, addr,
					line_disp, sheading);
			else
				prstty(1, full, -1, arg1,
					line_disp, sheading);
		} while(args[++optind]);
	}
	else
		for (slot = 0; slot < count; slot++)
			prstty(all, full, slot, addr,
				line_disp, sheading);
}

/* print streamstty table */
prstty( all, full, slot, addr, ld_type, heading)
int all,full,slot;
long addr;
int ld_type;	/* the line discipline */
char *heading;
{
	struct strtty tbuf;
	struct queue q;
...
got_tbuf:
	...
	if ( !full) {
		if ( ld_type && tbuf.t_state) {
			readmem((vaddr_t)tbuf.t_rdqp, 1, -1,
				&q, sizeof(struct queue), "queue");
...
long addr;
{
	...
	readmem((vaddr_t)q.q_ptr, 1, -1, &ldterm,
		sizeof(struct ldterm_mod), "ldterm_mod");
	...
	fprintf( fp, "\tbytes left for current character %u\n", ldterm.t_eucleft);
...
struct module_info *minfo;
{
	 ...
	 */
	while(addr != 0) {
		readmem(addr, 1, -1, q, sizeof(struct queue), "queue");
		readmem((vaddr_t)q->q_qinfo, 1, -1,
			&qinfo, sizeof(struct qinit), "qinit");
		readmem((vaddr_t)qinfo.qi_minfo, 1, -1,
			minfo, sizeof(struct module_info), "minfo");
		mname = readmem((vaddr_t)minfo->mi_idname, 1, -1,
			mname, namelen, "idname");
