...
#ident	"@(#)deroff:i386/cmd/deroff/deroff.c	1.9.1.6"
#ident "$Header$"
...
char **av;
{
	...
	/* For EUC supplementary code set characters */
	for(i = 0xa1 ; i <= 0xfe ; ++i)
		chars[i] = LETTER;	/* SS1 */

	chars[0x8e] = LETTER;		/* SS2 */
	chars[0x8f] = LETTER;		/* SS3 */
	...
	for( ; ; ){
		if ((unsigned)(lp - line) >= linesize-1){
			...
			if (linesize <= MAXLINELEN){
				linesize += SIZEDELTA;
				save_p = line;
				if ((line = (char *)realloc((char *)line, linesize)) == 0)
					fatal("out of memory\n", (char *)NULL);
				lp = line + (lp - save_p);
			} else
...
int cnst;
{
	...
	while (*s){
		...
		if (*s == '\"')
			s++;
		if (t>s+cnst && chars[ (unsigned char)s[0] ]==LETTER && chars[ (unsigned char)s[1] ]==LETTER){
			while (s < t){
				if (*s == '\"')
					s++;
				else
