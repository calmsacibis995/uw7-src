...
/ misc data
/
fpwinseg:	.value	USER_FP		/ window into kernel FP data
...
e80387:	/proc
	...
	movw	%cs:fpwinseg, %gs	/ set window to kernel FP emul state

	mov	%esp, %gs:restart_esp	/ let kernel find our stack frame
					/ MUST come before any other stack
					/ pointer changes

	push	%fs			/ save remaining segment registers
	push	%es
	push	%ds
	...
	/
	/ copy the current FP state into the restart area,
	/ so the kernel can restart this FP instruction from the beginning
	/

	movw	%gs, %ax
	movw	%ax, %es
	mov	$sr_base, %esi
	mov	$restart_fpstate, %edi
	mov	$[FPSTATE_SIZE \/ 4], %ecx
	rep; movsl %gs

	mov	offset_eip(%ebp), %eax
	mov	%eax, %gs:restart_eip
	...
	testw	$3, %ax
	jz	build_grs1		/ assume that kernel-mode ==> 32 bit
	andl	$0xFFFF, %esp
	andl	$0xFFFF, %ebp
...
build_grs2:
	lea	saved_edi(%ebp), %edi	/ Establishing address for movsl
					/  to move registers from kernel stack
	lea	[offset_edi](%ebp),%esi	/  to current stack frame
	movl	$15, %ecx		/ 15 longs to be moved:
	rep				/  EDI, ESI, EBP, (ESP), EBX, EDX, ECX,
	movsl				/  EAX, DS, ES, FS, GS, EIP, CS, EFL
...
get_opcode:	
	...
	movswl  %si,%esi
	movswl  %di,%edi
	call    *group_handler(%edi)     / call the proper group handler
...
/        -------------------------------------JKD
	...
	movl	%ecx, [offset_eip](%ebp)	/ instruction after the just
						/ executed fp instruction.

	/ indicate that the state has been completely changed, so that
	/ the FP instruction, if interrupted, does not need to be restarted
	mov	$0, %gs:restart_eip
...
no_exception:
	...
	/ Since the VM bit always gets zeroed when pushed, we do not need to
	/ clear it explicitly.
	pushfl
	movw	%ss, %ax
	cmpw	$USER_DS, %ax
	je	stack32		/ assume that USER_DS ==> 32 bit
	cmpw	$3, %ax
	jne	stack32		/ assume that kernel-mode ==> 32 bit
	movzwl	%sp, %eax	/ in case of 16 bit, avoid explicit %esp ref.
	andl	$~0x4000, %ss:(%eax)	/ clear the NT bit in the flags
	jmp	flags_ok
stack32:
	andl    $~0x4000, (%esp)    / clear the NT bit in the flags
flags_ok:
	popfl

	/ restore registers pushed on entry
	popa
	popl	%ds
	popl	%es
	popl	%fs

	/ indicate that all but %gs and the iret have been restored
	mov	$0, %gs:restart_esp
