...
#ident	"@(#)ufs.cmds:i386/cmd/fs.d/ufs/mkfs/mkfs.c	1.9.17.11"
	...
	char *argv[];
{
	...
	if (Cflag && Lflag) {
		pfmt(stderr, MM_ERROR,
			":127:Cannot specify both -o C and -o L\n");
		usage();
	}
	argc -= optind;
	argv = &argv[optind];
	...
	if (!Nflag) {
		...
		if ((statarea.st_mode & S_IFMT) != S_IFBLK &&
			...
			pfmt(stderr, MM_ERROR,
				":128:%s is not special device, cannot mkfs\n",
					fsys);
			exit(RET_FSYS_DEV);
		}
...
next:
	...
	if ((sblock.fs_spc * sblock.fs_cpg) % NSPF(&sblock)) {
		pfmt(stderr, MM_ERROR,
			...
			sblock.fs_nsect, sblock.fs_ntrak, sblock.fs_cpg, sblock.fs_fsize);
		exit(RET_CPG_BAD);
	}
	/*
	...
	if ((i = fssize - j * sblock.fs_fpg) < sblock.fs_fpg &&
		...
		pfmt(stderr, MM_WARNING,
		    ":129:inode blocks/cyl group (%d) >= data blocks (%d) in last cylinder group.\n",
			(cgdmin(&sblock, j) - cgbase(&sblock, j)) / sblock.fs_frag, i / sblock.fs_frag);
		pfmt(stderr, MM_NOSTD,
		    ":130:This implies %d sector(s) cannot be allocated.\n",
			i * NSPF(&sblock));
		sblock.fs_ncg--;
	...
	if (argc > 7) {
		...
		if (sblock.fs_minfree < 0 || sblock.fs_minfree > 99) {
			pfmt(stderr, MM_WARNING,
			    ":93:%s: invalid minfree reset to %d%%\n",
				argv[7], MINFREE);
		...
		if (sblock.fs_minfree < 0 || sblock.fs_minfree > 99) {
			pfmt(stderr, MM_WARNING,
			    ":94:%d: invalid minfree reset to %d%%\n",
				minfree, MINFREE);
			sblock.fs_minfree = MINFREE;
		}
	}
	...
	char *bf;
{
	...
	if (lseek(fsi, bno * DEV_BSIZE, 0) < 0) {
		pfmt(stderr, MM_ERROR, ":46:seek error: %ld\n", bno);
		pfmt(stderr, MM_ERROR,
			":104:rdfs: %s\n", strerror(errno));
		exit(RET_ERR_SEEK);
	}
	n = read(fsi, bf, size);
	if(n != size) {
		pfmt(stderr, MM_ERROR, ":44:read error: %ld\n", bno);
		pfmt(stderr, MM_ERROR,
			":104:rdfs: %s\n", strerror(errno));
		exit(RET_ERR_READ);
	}
...
usage()
{
	...
	(void)pfmt(stderr, MM_NOSTD,
		":111:free=xx,rps=xx,nbpi=xx,opt=x,apc=xx,gap=x\n");
	(void)pfmt(stderr, MM_NOSTD, ":131:[L|C]\n");
        exit(RET_USAGE);
}
...
int ins;
{
	...
	pfmt(ttyp, MM_NOSTD, ":134:If you wish to run such\napplications (without recompiling them), you should restrict the maximum\nnumber of files that may be created to fewer than 65,536.\n\n");
		...
		switch (in[0]) {
			...
			default:
	                        pfmt(ttyp, MM_NOSTD,
				    ":137:Input not understood. Try again.\n");
		}
        }
	return inos;
