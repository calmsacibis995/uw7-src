#ident	"@(#)libelf:i386/coff386.c	1.2"
...
#ifndef NO_NOTE_FOR_COFF
/*	create special .note section
 */

int
_elf_coff386_note(elf, info)
	Elf		*elf;
	Info		*info;
{
	Elf_Data		*d;
	...
	if ((notescn = elf_newscn(elf)) == 0)
		return (int)OK_NO;
	if ((sh = elf32_getshdr(notescn)) == 0)
		return (int)OK_NO;
	sh->sh_name = _elf_coffname(info, NM_NOTE);
	sh->sh_type = SHT_NOTE;
	sh->sh_size = NT_ELFID_SZ;
	sh->sh_link = 0;
	sh->sh_info = 0;
	sh->sh_addr = 0;
	sh->sh_offset = 0;
	...
	if ((d = elf_newdata(notescn)) == 0)
		return (int)OK_NO;
	if ((d->d_buf = malloc(NT_ELFID_SZ)) == 0)
	{
		_elf_err = EMEM_CNOTE;
		return (int)OK_NO;
	}
	d->d_size = NT_ELFID_SZ;
	wp = (Elf32_Word *)d->d_buf;
	*wp++ = NT_NAME_SZ;
	*wp++ = ELFID_DESC_SZ;
	*wp++ = NT_ELFID;
	(void)memset((void *)wp, '\0', NT_NAME_PAD);
	(void)strncpy((void *)wp, NT_NAME, NT_NAME_SZ);
	wp = (Elf32_Word *)((char *)wp + NT_NAME_PAD);
