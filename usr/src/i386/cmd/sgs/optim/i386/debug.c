#ident	"@(#)optim:i386/debug.c	1.6.1.9"
...
typedef enum tok {tok_label, tok_ps_byte, tok_ps_2byte, tok_ps_4byte, 
	...
	/* In the optimizer's simplified view of the world,
	   there are only 7 assembly language tokens:
		tok_label: a label definition, value is pointer to first
			char
		tok_ps_byte, tok_ps_2byte,tok_ps_4byte,tok_ps_8byte,
		tok_ps_string: 
...
char *s; /* passes in the current input pointer */
{
	...
	while(target_attribute==0) { /* loop exits after the tag of an
	    ...
	    */
		...
		if((label=strdup(tok_val))==(char *) NULL)
		    fatal(__FILE__,__LINE__,"parse_debug(): strdup failed\n");
		{ 
		    char *temp;
	    ...
	    tag_value=(short)strtoul(tok_val,(char **)NULL,0);
	...
	while(current_token==tok_ps_2byte) {
	    ...
	    attribute=(short)strtoul(tok_val,(char **)NULL,0);
		...
		case AT_name:
			...
			fatal(__FILE__,__LINE__,"parse_debug(): expected \".string\"\n");
			...
			*tmpstr = '\0';
			...
			fatal(__FILE__,__LINE__,"parse_debug(): coudn't parse fn name\n");
		    current_token=get_token();
		    break;	
		case AT_low_pc:
			...
			fatal(__FILE__,__LINE__,"parse_debug(): expected \".4byte\t<label>\"\n");
		    set_offsets(tmp_ptr,starting_line,linenumber+1);
		    put_entry(tmp_ptr);
		    break;
		default:
		fatal(__FILE__,__LINE__,"parse_debug(): unexpected target_attribute\n");
		}
	    }
	...
	if( found_attribute || (target_attribute == AT_location) ) {
	    ...
	    fatal(__FILE__,__LINE__,"parse_debug(): can't find attribute. \
		   current_line: %s\n",current_line);
        }
    }
} /* parse_debug() */
...
char *label;
{
	...
	if(current_token==tok_end) 
	    fatal(__FILE__,__LINE__,"skip_past_label(): premature EOI\n");
...
D_ptr ptr; 		/* put info in here, if you find it */
{
    ...
    if ((current_token != tok_ps_2byte) || !isdigit(*tok_val))
        fatal(__FILE__,__LINE__,"parse_location(): expected .2byte <const>\n");
    entry_length=(int)strtoul(tok_val,(char **)NULL,0);

    current_token=get_token(); /* OP_REG, OP_BASEREG or OP_ADDR atom */
    if ((current_token != tok_ps_byte) || !isdigit(*tok_val))
        fatal(__FILE__,__LINE__,"parse_location(): expected .byte <atom>\n");
    atom=(short)strtoul(tok_val,(char **)NULL,0);
    current_token=get_token();
	...
	if (current_token != tok_ps_4byte)
	    fatal(__FILE__,__LINE__,"parse_location(): OP_ADDR operand\n");
	break;

    case OP_REG:
    case OP_BASEREG:
        if ((current_token != tok_ps_4byte) || !isdigit(*tok_val))
          fatal(__FILE__,__LINE__,"parse_location(): expected .4byte <reg#>\n");
    
        /* Convert the number to internal register format and enter it */
    
        ptr->regid= (int)strtoul(tok_val,(char **)NULL,0);
    
        if(atom == OP_REG) {		/* Register parm or auto */
            ptr->mode=CPUReg;
	    if(entry_length != 5)
          fatal(__FILE__,__LINE__,"parse_location(): expected 5 byte length\n");
        }
	    ...
	    current_token=get_token();
    	    ...
    	    if ((current_token != tok_ps_byte) || (atom != OP_CONST))
             fatal(__FILE__,__LINE__,"parse_location(): expected .byte OP_CONST\n");
	    current_token=get_token();
    	    if (current_token != tok_ps_4byte)
                fatal(__FILE__,__LINE__,"parse_location(): expected .4byte <displacement>\n");
	    else if ( !isdigit(*tok_val) && (*tok_val != '-'))
                fatal(__FILE__,__LINE__,"parse_location(): bad operand to .4byte\n");
	    ptr->displacement=(int)strtoul(tok_val,(char **)NULL,0);
	    current_token=get_token();
            atom=(int)strtoul(tok_val,(char **)NULL,0);
    	    if ((current_token != tok_ps_byte) || (atom != OP_ADD))
                fatal(__FILE__,__LINE__,"parse_location(): expected .byte OP_ADD\n");
	    if(entry_length != 11)
                fatal(__FILE__,__LINE__,"parse_location(): expected 11 byte length\n");
        }
	break;

    default:
    	fatal(__FILE__,__LINE__,"parse_location(): expected OP_REG or OP_BASEREG\n");
    } /* switch */
			   ...
			   the first token after this attribute. */
{
    ...
    case FORM_NONE: /* error */ 
        fatal(__FILE__,__LINE__,"skip_attribute(): bad attribute format code %d\n",form);
	/* FALLTHRU */
    case FORM_DATA2:
	bytes_to_skip=2;
	break;
	...
	current_token=get_token();
	if ((current_token != tok_ps_2byte) || !isdigit(*tok_val))
	    fatal(__FILE__,__LINE__,"skip_attribute(): expected .2byte <const>\n");
	bytes_to_skip=(short)strtoul(tok_val,(char **)NULL,0);
        break;

    case FORM_STRING:
	current_token=get_token(); 
	if (current_token != tok_ps_string)
	    fatal(__FILE__,__LINE__,"skip_attribute(): expected .string\n");
	break;
	...
	case tok_end:
	    fatal(__FILE__,__LINE__,"skip_attribute(): premature end of entry\n");
	    /* FALLTHRU */
	case tok_ps_byte:
	    skipped++;
	    break;
	...
	case tok_label:
	    break;
	default:
	    fatal(__FILE__,__LINE__,"skip_attribute(): unexpected pseudo_op\n");
	} /* switch */
    } /* while(skipped ... */
    if(skipped != bytes_to_skip)
	fatal(__FILE__,__LINE__,"skip_attribute(): weird byte count\n");
    current_token=get_token();
} /* skip_attribute() */

	static token
...
#define NEWSECTION(x) prev_section=section; section=(x);
    ...
    *t = savechar;			/* Restore saved character. */
	...
	fatal(__FILE__,__LINE__,"get_token(): illegal directive \"%s\".\n",next_char);
	break;
...
{ /* Open intermediate file Dfile */
    ...
    if(filename == NULL)
	fatal(__FILE__,__LINE__,"init_debug: couldn't make tempname.\n");

    Dfile = fopen(filename,"w+");
    if(Dfile == (FILE *)NULL)
	fatal(__FILE__,__LINE__,"init_debug: couldn't open file: %s (%d).\n",filename,errno);

    if(unlink(filename) == -1)		/* Unlink: file will be removed
					   when program ends.	*/
    fatal(__FILE__,__LINE__,"init_debug: couldn't unlink file: %s (%d).\n",filename,errno);
    free(filename);
}
...
/* register */ char *str;
{
	...
	if(fprintf(Dfile,"%s\n",str)<0)
		fatal(__FILE__,__LINE__,"put_Dfile: fprintf failed\n"); /* for now */
	linenumber++;
}
...
short tag_val;
{
    ...
    if(tmp_ptr==NULL)
        fatal(__FILE__,__LINE__,"new_queue_entry(): malloc failed\n");
    tmp_ptr->TAG=tag_val;
    return tmp_ptr;
}
...
{
    if(ptr==NULL)
	fatal(__FILE__,__LINE__,"set_offsets(): nobody home\n");
    ptr->loc_head=start;
    ptr->loc_tail=end;
}
		   ...
		   regal.c. */
{
    ...
    if (! current_fn ) fatal(__FILE__,__LINE__,"mod_debug(): no debugging info for fn\n");
...
/*	    case TAG_inline_subroutine: */
	        fatal(__FILE__,__LINE__,"mod_debug(): weird debug queue\n");
	    ...
	    default: fatal(__FILE__,__LINE__,"mod_debug(): unknown TAG.\n");
	}
	p=p->next;
    }
}
...
FILE *outfp=stdout; /* pointer for output file after in-line subst, we
			   ...
			   brought here from the late inline.c */
...
long *line_num;
{
	...
	while(fgets(s,132,Dfile)) fprintf(outfp,"%s",s);
	    ...
	    else fatal(__FILE__,__LINE__,"print_lines(): fgets() failed\n"); 
        *line_num=limit;
    }
}
...
long *line_num;
{
	...
	while(fgets(s,132,Dfile)) fprintf(outfp,"%s",s);
	    ...
	    if(fgets(s, 132, Dfile) == NULL)
	        fatal(__FILE__,__LINE__,"skip_lines(): fgets() failed\n"); 
	}
        *line_num=limit;
    }
}
...
{
    if(!init_debug_flag)
	fatal(__FILE__,__LINE__,"print_FS_line_info(): no debugging info seen\n");
    printf("\t.section\t.line\n"); 
