...
picode(p) NODE *p;
{
	...
	case BITYPE:
			...
			)
		{
			...
			*/
			NODE *q;
			int stat = p->in.right->in.right->in.sid & (NI_FLSTAT|NI_BKSTAT);
	...
	case UTYPE:
		if (o == STAR && p->in.left->in.op == PLUS) {
			...
			if (q->in.op == ICON && q->in.name != (char *)0) {
			    int stat = q->in.sid & (NI_BKSTAT|NI_FLSTAT);
			    if (stat || num_ucompare(&q->in.c.ival, &num_0) == 0) {
				...
				l = build_tn(ICON, q->in.type, q->in.name, q->tn.sid);
				l->tn.c.ival = num_0;
				l = build_in(PLUS, TPOINT, l, p->in.left->in.left);
				q->in.name = (char *)0;
				q->in.type = TINT;
				p->in.left->in.left = l;
	...
	case LTYPE:
		...
		case ICON:
			...
			stat = p->tn.sid & (NI_FLSTAT|NI_BKSTAT);
			array_ref = num_ucompare(&p->tn.c.ival, &num_0);

			q = build_tn(ICON, TINT, p->tn.name, p->tn.sid);
			q->tn.c.ival = *(stat ? &p->tn.c.ival : &num_0);
			q->tn.strat |= PIC_GOT;
			r = build_in(
				PLUS,
				TPOINT,
				build_tn(REG, TPOINT, (char *)0, BASEREG),
				q);
			...
			if (!stat)  {
				...
				*/
				if (array_ref) {
				    q = build_tn(ICON, TINT, (char *)0, 0);
				    q->tn.c.ival = p->tn.c.ival;
