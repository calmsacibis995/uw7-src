...
static	char	operandbuf[3][OPLEN];	/* to store operands as they	*/
char	*operand[3];	/* to store operands as they	*/
					/* are encountered		*/
static	char	symarr[3][OPLEN];
static char *overreg;	/* save the segment override register if any    */
static unsigned long startloc;
...
#define PRINTLINE() (Silent_mode || printline())
#define CONVERT if (!Silent_mode) convert

#define FUR_INFO(REGNAME, FLAGS) (Optinfo ? Optinfo(dp, (REGNAME), (FLAGS)) : 1)
#define FURFLAGS(IS_SIXTEEN, IS_NOT_EIGHT) (!IS_NOT_EIGHT ? OPTINFO_EIGHTBIT : (IS_SIXTEEN ? OPTINFO_SIXTEEN : 0))

/*
** The tool 'fur' now uses this same routine.  However, fur does not
** give output while disassembling, so all code having to do with output
** is ifdef'ed out for fur.  Also, fur only disassembles one statement
** at a time, so the outer loop is while(0).
*/
...
Elf32_Shdr *shdr;
{
	...
	for (startloc = loc = shdr->sh_addr; ((loc-shdr->sh_addr) < shdr->sh_size) && (errlev < MAXERRS); PRINTLINE()){
		if (!Silent_mode) {
		...
		operand[0] = operandbuf[0];
		operand[1] = operandbuf[1];
		operand[2] = operandbuf[2];
		operand[0][0] = '\0';
		operand[1][0] = '\0';
		operand[2][0] = '\0';
		symarr[0][0] = '\0';
		...
		*/
		...
		case MOVZ:
			...
			FUR_INFO(reg, FURFLAGS(hold16, LONGOPERAND));
			if (!Silent_mode) {
		...
		case IMUL:
			...
			imm_data( OPSIZE(data16,opcode2 == 0x9), 0, 0);
			if (data16)
				reg_name = REG16[reg][LONGOPERAND];
			else
				reg_name = REG32[reg][LONGOPERAND];
			FUR_INFO(reg, FURFLAGS(data16, LONGOPERAND));
			if (!Silent_mode) {
		...
		case MRw:
			...
			FUR_INFO(reg, FURFLAGS(data16, wbit));
			if (!Silent_mode) {
				(void) sprintf(mneu,"%s%s,%s",mneu,operand[0],reg_name);
				if (sflag)
		...
		case DSHIFTcl:
			...
			FUR_INFO(reg, FURFLAGS(data16, LONGOPERAND));
			if (Optinfo) {
				opsinfo[1].fmt = OPTINFO_REGONLY;
				opsinfo[1].reg = ECX;
				opsinfo[1].flags |= OPTINFO_EIGHTBIT;
			}
			if (!Silent_mode) {
		...
		/* 8-bit immediate to memory or register operand */
		case IM8w:
			wbit = WBIT(opcode2);
			get_operand(mode, r_m, wbit, 1);
			if (opcode1 == 8 && opcode2 == 3)
				imm_data(1, 0, 1);
			else
				imm_data(1, 0, 0);
		...
		case IMw:
			...
			if (!got_modrm_byte)
				get_modrm_byte(&mode, &reg, &r_m);
			get_operand(mode, r_m, wbit, 1, 0);
			imm_data(OPSIZE(data16,wbit), 0, 0);
			/* The register is not used, so FUR_INFO(NO_REG is more
			** appropriate, but the code generation side would like
			** to see a register if it is going to have to generate
			** it
			*/
			FUR_INFO(reg, 0);
			if (!Silent_mode) {
		...
		case OA:
			...
			if (Optinfo)
				if (addr16)
					opsinfo[0].flags |= OPTINFO_ADDR_SIXTEEN;
			displacement(OPSIZE(addr16,LONGOPERAND), 0,&lngval);
			reg_name = ( data16 ? REG16 : REG32 )[EAX][wbit];
			FUR_INFO(EAX, FURFLAGS(data16, wbit));
			if (!Silent_mode) {
				(void) sprintf(mneu,"%s%s,%s",mneu,operand[0],reg_name);
				if (sflag) {
		...
		case AO:
			...
			FUR_INFO(EAX, FURFLAGS(addr16, wbit));
			if (!Silent_mode) {
				(void) sprintf(mneu,"%s%s,%s",mneu, reg_name, operand[0]);
				if (sflag) {
					char *symstring = symarr[0];
		...
		case Mv:
			...
			reg_name = vbit ? "%cl," : "" ;
			FUR_INFO(vbit ? ECX : NO_REG, OPTINFO_SIXTEEN);
			if (!Silent_mode) {
				(void) sprintf(mneu,"%s%s%s",mneu, reg_name, operand[0]);
				if (sflag)
					sprintf(symrep,"%s%s", reg_name, symarr[0]);
		...
		case SREG: /* special register */
			...
			if (vbit)
			{
				...
				if (Optinfo) {
					opsinfo[0].fmt = OPTINFO_REGONLY;
					opsinfo[0].flags |= OPTINFO_SPECIAL;
					opsinfo[0].reg = reg;
				}
			}
			else {
		...
		case RA: {
			...
			if (data16) {
				if (Optinfo)
					opsinfo[0].flags |= OPTINFO_SIXTEEN;
				reg_name = REG16[reg][LONGOPERAND];
			}
			else
				reg_name = REG32[reg][LONGOPERAND];
		...
		case IA: {
			...
			imm_data(no_bytes, 0, 0);
			FUR_INFO(EAX, (no_bytes == 4) ? 0 : ((no_bytes == 2) ? OPTINFO_SIXTEEN : OPTINFO_EIGHTBIT));
			if (!Silent_mode) {
				(void) sprintf(mneu,"%s%s,%s",mneu,operand[0], reg_name) ;
				if (sflag)
					sprintf(symrep,"%s,%s",symarr[0],reg_name);
		...
		case SD:
			check_override(0);
			if (Optinfo) {
				opsinfo[1].u.disp.value = 0;
				opsinfo[2].u.disp.value = 0;
				opsinfo[1].fmt = OPTINFO_DISPREG;
				opsinfo[2].fmt = OPTINFO_DISPREG;
				opsinfo[1].reg = ESI;
				opsinfo[2].reg = EDI;
				if (data16) {
					opsinfo[1].flags |= OPTINFO_SIXTEEN;
					opsinfo[2].flags |= OPTINFO_SIXTEEN;
				}
				if (addr16) {
					opsinfo[1].flags |= OPTINFO_ADDR_SIXTEEN;
					opsinfo[2].flags |= OPTINFO_ADDR_SIXTEEN;
				}
			}
			if (addr16) {
				operand[1] = "%si";
			...
			FUR_INFO(NO_REG, 0);
			if (!Silent_mode) {
				(void) sprintf(mneu,"%s%s(%s),(%s)",mneu,operand[0],operand[1],operand[2]);
				if (sflag)
					(void) sprintf(symrep,"%s%s,%s",symarr[0],operand[1],operand[2]);
			}
			continue;
		...
		case SA:
			...
			if (addr16) {
				...
				operand[1] = "%esi";
			FUR_INFO(reg, FURFLAGS(addr16, wbit));
			if (!Silent_mode) {
				(void) sprintf(mneu,"%s%s(%s),%s",mneu,operand[0],
					operand[1], reg_name);
				if (sflag)
					(void) sprintf(symrep,"%s%s,%s",symarr[0],
					operand[1], reg_name);
			}
			continue;
		...
		case BD:
			...
			/* For optimization, must know what register */
			FUR_INFO(ECX, FURFLAGS(data16, 1));
			if (!Silent_mode) {
				compoff(lngval, operand[1]);
				(void) sprintf(mneu,"%s%s%s",mneu, operand[0],
					(lngval == 0) ? "" : operand[1]);
		...
		case I:
			imm_data(OPSIZE(data16,LONGOPERAND), 0, 0);
			FUR_INFO(NO_REG, 0);
			if (!Silent_mode) {
				(void) sprintf(mneu,"%s%s",mneu,operand[0]);
				if (sflag)
		...
		case P:
			check_override(0);
			imm_data(1, 0, 0);
			FUR_INFO(EAX, dp->suffix ? OPTINFO_EIGHTBIT : (data16 ? OPTINFO_SIXTEEN : 0));
			if (!Silent_mode) {
				(void) sprintf(mneu,"%s%s",mneu,operand[0]);
				if (sflag)
		...
		case V:
			check_override(0);
			FUR_INFO(EAX, data16 ? OPTINFO_SIXTEEN : 0);
			if (Optinfo) {
				opsinfo[1].fmt = OPTINFO_REGONLY;
				opsinfo[1].flags |= OPTINFO_SIXTEEN;
				opsinfo[1].reg = EDI;
		...
		case CBW:
			FUR_INFO(EAX, FURFLAGS(data16, 1));
			if (data16)
		...
		case CWD:
			FUR_INFO(EAX, FURFLAGS(data16, 1));
			if (Optinfo) {
				opsinfo[0].flags |= FURFLAGS(data16, 1);
				opsinfo[0].fmt = OPTINFO_REGONLY;
				opsinfo[0].reg = EDX;
			}
			if (data16)
...
{
	if (Optinfo)
		opsinfo[opindex].overreg = overreg;
	if (!Silent_mode) {
		if (overreg) {
...
long *value;
{
	...
	if (Optinfo)
		opsinfo[opindex].u.disp.loc = loc - startloc;
	getbytes(no_bytes, temp, value, 0);
	if (Optinfo && !opsinfo[opindex].fmt) {
		opsinfo[opindex].fmt = OPTINFO_DISPONLY;
		if (data16)
			opsinfo[opindex].flags |= OPTINFO_SIXTEEN;
		opsinfo[opindex].u.disp.value = *value;
	}
	check_override(opindex);
	if (signed_disp) {
		if (!Silent_mode)
...
int opindex;
{
	...
	extern	const char 	*REG16[8][2];	  /* in tables.c */
	extern	const char 	*REG32[8][2];	  /* in tables.c */
	extern	char	*scale_factor[4];	  /* in tables.c */
	int dispsize;   /* size of displacement in bytes */
	int dispvalue = 0;  /* value of the displacement */
	...
	if (Optinfo)
		opsinfo[opindex].u.disp.loc = NO_SUCH_ADDR;
	...
	if (addr16) {
		if (Optinfo)
			opsinfo[opindex].flags |= OPTINFO_ADDR_SIXTEEN;
	...
	if (Optinfo) {
			opsinfo[opindex].flags |= FURFLAGS(data16, wbit);
/*		if (data16)*/
/*			opsinfo[opindex].flags |= OPTINFO_SIXTEEN;*/
		if (s_i_b) {
			opsinfo[opindex].scale = ss;
			if (index != ESP)
				opsinfo[opindex].index = index;
			else
				opsinfo[opindex].index = NO_REG;
				
			if ((mode == 0) && (base == EBP))
				opsinfo[opindex].fmt = OPTINFO_SIB_NOREG;
			else {
				opsinfo[opindex].reg = base;
				opsinfo[opindex].fmt = OPTINFO_SIB;
			}
		}
		else {
			if (mode == REG_ONLY) {
				opsinfo[opindex].fmt = OPTINFO_REGONLY;
				opsinfo[opindex].flags |= FURFLAGS(data16, wbit);
				opsinfo[opindex].reg = r_m;
			}
			else {
				if ((r_m ==EBP) && (mode == 0))
					opsinfo[opindex].fmt = OPTINFO_DISPONLY;
				else {
					opsinfo[opindex].fmt = OPTINFO_DISPREG;
					opsinfo[opindex].flags |= FURFLAGS(data16, wbit);
					opsinfo[opindex].reg = r_m;
				}
			}
...
**
** extend_flag is passed to convert to tell it to print values in sign extended
** form.
*/
getbytes(no_bytes, destbuf, destvalue, extend_flag)
int no_bytes;
char *destbuf;
long *destvalue;
int extend_flag;
{
	...
	CONVERT(shiftbuf, destbuf, LEAD, extend_flag);
}



/*
 *	void imm_data (no_bytes, opindex, extend_flag)
 *
 *	Determine if 1, 2 or 4 bytes of immediate data are needed, then
 *	get and print them.
 *
 *      extend_flag indicates whether to print single byte immediate data
 *      in sign extended form or not
 */

void
imm_data(no_bytes, opindex, extend_flag)
int no_bytes;
int opindex;
int extend_flag;
{
	...
	operand[opindex][len] = '$';
	if (Optinfo) {
		opsinfo[opindex].u.imm.loc = loc - startloc;
		switch (no_bytes) {
		case 1:
			opsinfo[opindex].flags |= OPTINFO_EIGHTBIT;
			break;
		case 2:
			opsinfo[opindex].flags |= OPTINFO_SIXTEEN;
			break;
	...
	getbytes(no_bytes, &operand[opindex][len+1], &value, extend_flag);
	if (!Silent_mode) {
		if (sflag) {
	...
	if (Optinfo) {
		opsinfo[opindex].u.imm.value = value;
		opsinfo[opindex].fmt = OPTINFO_IMMEDIATE;
	}
}
