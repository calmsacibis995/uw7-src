	...
	char * envp[];		/* the environment strings */
{
...
#ifdef DEBUG
	    struct xseg * xp = Xs+i;
	    emprintf("SEG %d: seg %02X attr %02X type %02X "
		     "pos %06lX vs %06lX ps %06lX\n",
		     i,
		     xp->xs_seg, xp->xs_attr, xp->xs_type,
		     xp->xs_filpos,
		     xp->xs_vsize,
		     xp->xs_psize);
#endif
	...
	for ( i = 0; i < Xnumsegs; i++ ) {
		...
		case XS_TTEXT:
			...
			validseg(xp->xs_seg);
#ifdef DEBUG
			emprintf("XS_TTEXT: %X %lX %lX %lX\n",
				 xp->xs_seg, Tfile+xp->xs_filpos,
				 xp->xs_vsize, xp->xs_vsize);
#endif
		...
		case XS_TDATA:
			...
			if ( Stacksel == 0 ) {	/* first data seg has stack */
				copymem( Dfile+xp->xs_filpos, TheStack,
					xp->xs_psize );
#ifdef DEBUG
				emprintf("XS_TDATA (stack): %X %lX %lX %lX\n",
					 xp->xs_seg, TheStack,
					 xp->xs_vsize + Stacksize, NBPS);
#endif
				 ...
				 * set up for use by the x.out floating point
				 * emulator.
				 */
#ifdef DEBUG
				emprintf("XS_TDATA (stack): %X %lX %lX %lX\n",
					 USER_FPSTK, TheStack,
					 btopr(Stackbase + Stacksize), 0);
#endif
				setsegdscr( USER_FPSTK, TheStack,
					btopr( Stackbase + Stacksize ), 0, 3 );
				...
				if (firstText < 0) {
					if (Dsegs[SELTOIDX(Xe->xe_eseg)].base
							== BAD_ADDR) {
					    int segsz = xp->xs_vsize;
					    if (segsz > Tfsize-xp->xs_filpos)
						segsz = Tfsize-xp->xs_filpos;
#ifdef DEBUG
			    emprintf("XS_TDATA: %X %lX(%lX+%lX) %lX %lX\n",
				     Xe->xe_eseg, Tfile+xp->xs_filpos,
				     Tfile, xp->xs_filpos, segsz,
				     xp->xs_vsize);
#endif
