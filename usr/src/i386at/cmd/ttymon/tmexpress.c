...
const	char	*argv[];
{
	...
	read_ttydefs(NULL,FALSE);
        ...
        if ((tmp = getenv("DTR_DELAY")) != NULL)
                dtr_delay = atoi(tmp);
        else
                dtr_delay = DTR_DELAY;

	if ((pmtab->p_device == NULL) || (*(pmtab->p_device) == '\0')) {
		/* No device was specified; use stdin */
/*
...
*/
			...
			if (open(pmtab->p_device, O_RDWR) < 0) {
				log(MM_HALT, badopen, pmtab->p_device,
					strerror(errno));
				exit(1);
			}
/*
...
struct pmtab	*pmtab;
{
	...
	while ((c = getopt(argc, argv, "gd:ht:p:m:l:v")) != -1) {
		switch (c) {
		case 'g':
			gflag = 1;
			break;
		case 'd':
			pmtab->p_device = get_tty(optarg);
			break;
		case 'h':
			pmtab->p_ttyflags &= ~H_FLAG;
			break;
