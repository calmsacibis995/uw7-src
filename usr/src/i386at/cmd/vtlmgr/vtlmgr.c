...
/*
 *	L000	15Apr97		rodneyh@sco.com
 *	- Added call to KDVTPATH if KIOCINFO returns we are on a multiconsole
 *	  device.
 *	L001	10Jul97		rodneyh@sco.com
 *	- Kernel behaviour with repsect to open VT has changed so the new VT
 *	  is no longer auto activated, changes here to force ativate the new
 *	  VT with WS_ACTIVATE and WS_WAITACTIVE in order to retain the same
 *	  observed behaviour as before. Part of the fix for ul97-17852 and
 *	  ul97-18217.
 */ 
...
char **argv;
{
	...
	for (;;) {
		...
		/* L001 begin
		 *
		 * Kernel behaviour with respect to open VT has changed the
		 * new VT is no longer auto activated so we now have to force
		 * the activation here.
		 */
		if(ioctl(0, VT_ACTIVATE, i - K_VTF) == -1){
		...
		cp = (char *) NULL;
		if (home != (char *)NULL && ((strlen(home) + strlen("/.vtlrc")) < (unsigned)VTLRCLEN)) {
