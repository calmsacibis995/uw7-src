...
mca_vend_t	*mca_vendors = NULL;

int
load_mca_data()
{
    static const char	cfg_file[] = "mca.cfg";
    const char		*filename;
    FILE		*fd;
    char		buf[256];
    u_long		line;
	...
	if (*tp == '+')
	{
	    ...
	    if (!(pp = (mca_prod_t *)malloc(sizeof(mca_prod_t))))
	    {
		err = ENOMEM;
		break;
	    }
	    ...
	    for (vp = mca_vendors; vp->next; vp = vp->next)
	...
	else
	{
		...
		*tp++ = '\0';	/* End of key */
	    ...
	    if (!(vp = (mca_vend_t *)malloc(sizeof(mca_vend_t))))
	    {
		err = ENOMEM;
		break;
	    }
	    ...
	    for (vpp = &mca_vendors; *vpp; vpp = &(*vpp)->next)
		;
