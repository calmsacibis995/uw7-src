...
void	PnPvendorID(FILE *out, const char *tab, const u_char *id);
...
report_isa(FILE *out)
{
    ...
    if (!have_isa())
    {
	fprintf(out, "    No ISA Plug-and-Play found\n");
	return;
    }
...
open_PnP()
{
    ...
    {
	printf("Cannot open %s: %s", pnpDev, strerror(errno));
	return errno;
    }
