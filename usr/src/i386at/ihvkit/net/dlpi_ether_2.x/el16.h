...
#ident	"@(#)ihvkit:net/dlpi_ether_2.x/el16.h	1.1"
#ident	"$Header$"
...
#if defined(_KERNEL) || defined(_KERNEL_HEADERS)
 ...
 */
#define SCB_RESET	0x0080	/* reset chip same as hardware reset */
...
#define	CS_MAX_COLLIS	0x0020	/* collisions exceeding 16 */
...
#define	ID_LEN		6	/* length of adapter ID string */
...
#define MAX_RBD_SIZ		1536
#define MAX_RCV_BLK		12
...
typedef struct bdd {
	...
	frame_t		frame[MAX_RCV_BLK];
} bdd_t;

#define MAC_HDR_LEN sizeof(net_addr_t) + sizeof(net_addr_t) + sizeof(ushort_t)
#define EL16_TIMEOUT	(5*HZ)	/* 5 sec */

#define EL16_SET_16BIT_ACCESS(X, Y) \
	{ \
		(Y) = splhi(); \
		outb((X), (inb((X)) | CR_LAD)); \
	}

#define EL16_RESET_16BIT_ACCESS(X, Y) \
	{ \
		outb((X), (inb((X)) & ~CR_LAD)); \
		splx((Y)); \
	}

#define EL16_PRO_ON  1
#define EL16_PRO_OFF 0
