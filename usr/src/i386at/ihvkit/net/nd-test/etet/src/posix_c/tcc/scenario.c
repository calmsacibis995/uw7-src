/* static char sccsid[] = "@(#)scenario.c 1.2 91/12/04 "  SMI; */
...
MODIFICATIONS:
		...
		Revisions for ETET. Much of the new scenario handling
		has been moved to newscen.c
		...
		Andrew Josey, UNIX System Labs, October 1993.
  		...
  		Changed get_rescode_file() to look in both $TET_ROOT as well
  		as $TET_SUITE_ROOT for $TET_RESCODE_FILE files.
		...
		Remove old code from original TET 1.10 that was under #if OLD
		Andrew Josey, Novell UNIX System Labs, March 1994

		
		The functions rescode_name_to_num() and rescode_num_to_index()
		loop through the complete static table of 32 members, when typically
		this table only contains 7 entries. On some architectures this
		can cause problems when a string comparison of unitialised
		static data occurs. The fix is only to do the number of
		comparisons according to the number of entries in the table.
		(see tet bug report tet1.10.008).
		Andrew Josey, Novell UNIX System Labs, March 1994
...
char *fnbc_p;
{
    ...
    /* Enable TET_EXEC_IN_PLACE in tet_run dir if tet_run_set */
    if (exec_in_place == NULL || ((*exec_in_place == 'F') || 
                         (*exec_in_place == 'f')) && (tet_run_set == FALSE))
        eip_set = FALSE;
    else
        eip_set = TRUE;
    ...
    if ((api_compliant_p=get_tetenv ("TET_API_COMPLIANT")) != NULL)
    {

#ifdef DBG2
		(void) fprintf(stderr,
			"get_tetenv (\"TET_API_COMPLIANT\") = <%s>\n", api_compliant_p);
#endif
		if (*api_compliant_p=='T' || *api_compliant_p=='t')
			tet_api_compliant = TRUE;
		else 
		{
			if (*api_compliant_p=='F' || *api_compliant_p=='f')
	    		tet_api_compliant = FALSE;
		}
    }
    ...
    *(strrchr(tc_dir, DIR_SEP_CHAR)) = '\0';
    ...
    on_alarm.sa_handler = (void(*)())exec_sig_hdlr;
    on_alarm.sa_flags = 0;
...
int get_rescode_file()
{
    ...
    g_max_rescode = ctr;
    if ((rcfn_p = get_tetenv("TET_RESCODES_FILE")) == NULL)
		rcfn_p = "tet_code";

	if (strcmp (tet_root, tet_suite_root) != 0)
	{
    ...
    /* pick up values from TET_SUITE_ROOT file */
    rcf_name = (char *)TET_MALLOC(strlen(tet_suite_root)+strlen(rcfn_p)+3);

    (void) strcpy(rcf_name,tet_suite_root);
    if ((rcf_name)[strlen(rcf_name)-1] != DIR_SEP_CHAR)
        (void) strcat(rcf_name,DIR_SEP_STR);
    (void) strcat(rcf_name,rcfn_p);
    ...
    rcf_name = (char *)TET_MALLOC(strlen(tet_suite_root) + strlen(suite_name) +
                strlen(rcfn_p) + 4);
...
char *name;
{
    ...
    for (ctr = 0 ; ((ctr < MAX_RESCODES) && (ctr < g_max_rescode)) ; ctr++)
    {
