...
MODIFICATIONS:
		...
		Split order_tp_results() into make_tprs_list() and parse_tprs_list().
		...
		Various changes and additions for ETET support
		SunSoft TET1.10e update. May 1993.

		TET1.10e review
		Andrew J. Josey, UNIX System Labs, Inc. May 1993

		Remove static char zeetime[] declaration, since the
		data is declared externally. The extern is now in
		tcc_prot.h
		Andrew Josey, UNIX System Labs, 16 May 1993

		Change etet_extended_mode to etet_ext_set which
		is a boolean variable.
		ETET 1.10.2 Update
		Andrew Josey, UNIX System Labs, October 1993.


		Changes for support of a new flag to specify TET API Compliance.
		Based on proposals 1 and 2 of (tetworks 397) 
		API compliance & TC name passing (William Turnbow)
		John Birchfield QualTrak Corporation, January 1994.

		Reduce the memory allocation in the function
		collate_and_sort(), see tet bug report tet1.10.049.
		Andrew Josey, Novell UNIX System Labs, March 1994

		Add additional waitpid() to catch unwaited for child
...
/* this is needed to make sure sys/signal.h is include under SunOS41 --
...
char *env_lines[6] = { NULL,NULL,NULL,NULL,NULL,NULL };
...
void do_nothing(signum)
int signum;
{
/*p	printf("Doing nothing\n");*/
    return;
}


pid_t timed_waitpid(pid, stat_loc, options, seconds)
pid_t pid;
int *stat_loc;
int options;
unsigned seconds;
{
    struct sigaction alarm_act, orig_act;
    int result, err = 0;
    int local_stat;

    sigemptyset(&alarm_act.sa_mask);
    alarm_act.sa_handler = (void(*)()) do_nothing;
#if defined(SunOS41) 
    alarm_act.sa_flags = SA_INTERRUPT;	/* was 0 */
#else
    alarm_act.sa_flags = 0;
#endif
    alarm(0);
    (void) sigaction(SIGALRM, &alarm_act, &orig_act);
    alarm(seconds);
    ...
    alarm(0);
/*     (void) sigaction(SIGALRM, &orig_act, (struct sigaction *) NULL);
/*-*/
/*         siglongjmp(jmpbuf,0);
/*-*/
    }
...
char *pathname, *arg_p;
{
...
#ifdef DBG
    if (arg_p == NULL)
        (void) fprintf(stderr,"start_tc(%s, \"\")\n", pathname);
    else
    ...
    else
    {
        ...
        if (timed_out == 0)
        {
            ...
            while(fgets(linein, sizeof(linein), exec_fp) != NULL)
            {
                if( !strcmp(linein, "TET_EXEC_FAILED"))
                {
                    if(exec_tool_p == NULL || *exec_tool_p == '\0')
                    {
						...
						(void) waitpid((pid_t)cpid, &statloc,0);
                        return(FAILURE);
                    }
                    else
                    {
            ...
            while(timed_waitpid((pid_t)cpid,&statloc,0,(unsigned int)g_timeout) == FAILURE)
            {
            	char buff[128];
                /* If the child recieved a signal */
                if(errno == EINTR)
                    continue;
                sprintf(buff,"waitpid child, errno=%d\n",errno);
                ERROR(buff);
                return(FAILURE);
            }
            ...
            if ( WIFEXITED(statloc))
            {
				 ...
				 *  ACTUALLY - this activity is now based on the
				 *  TET_API_COMPLIANT variable.
                 */
					if (tet_api_compliant == TRUE)
                	{
                    	...
                    	if (res_fp == NULL) 
                    	{
							printf("Error number returned is : %d\n",errno);
							perror("res_fp");
                ...
                if (tet_api_compliant != TRUE)  /* AJJ change */
                {
 ...
 */
extern char *jnl_file_name;     /* journal file name */
int export(env)
ENV_T env[];
{
    char *fn;
	...
	/* possibly a bug should be filed against this line - wat 3/10/93 */
    (void) sprintf(env_lines[2],"%s=%d","TET_ACTIVITY",exec_ctr);
/* was:  (void) sprintf(env_lines[2],"%s=%d","TET_ACTIVITY",context); */
    ...
    /* set TET_JOURNAL_PATH equal to jnl_file_name */
    len = strlen(jnl_file_name) + strlen("TET_JOURNAL_PATH")+3;
    env_lines[5] = (char *)TET_MALLOC((size_t)len);
    (void) sprintf(env_lines[5],"%s=%s","TET_JOURNAL_PATH",jnl_file_name);

    (void) tet_putenv(env_lines[5]);
#ifdef DBG2
    (void) fprintf(stderr,"%s\n", env_lines[5]);
#endif
    if (env_lines[5] != NULL)
        TET_FREE((void *)env_lines[5]);
...
*/
                    if(compress_write(jnl_fd,(void *)buf) == -1)
		    {
                    (void) sprintf(error_mesg,"compress_write() to journal failed: %s\n",buf);
                    BAIL_OUT2(error_mesg);
		    }
