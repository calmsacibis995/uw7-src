 ...
 * documentation, and that the name of OSF, SunSoft, Inc., UI or X/Open not be 
 * used in advertising or publicity pertaining to distribution of the software 
 * without specific, written prior permission.  OSF, SunSoft, Inc., 
 * UI and X/Open make no representations about the suitability of this 
 * software for any purpose. It is provided "as is" without express or 
 * implied warranty.
 *
 * OSF, SunSoft, Inc., UI and X/Open DISCLAIM ALL WARRANTIES WITH REGARD TO 
 * THIS SOFTWARE, INCLUDING ALL IMPLIED WARRANTIES OF MERCHANTABILITY AND 
 * FITNESS, IN NO  EVENT SHALL OSF, SunSoft, Inc., UI or X/Open BE LIABLE FOR 
...
MODIFICATIONS:
	...
	Geoff Clare, UniSoft Ltd, 16 Oct 1991
  	...
  	SunSoft ETET Extensions, May 1993

		Temporary results file handling changed. Conditional
		use of TET_JOURNAL_PATH for name of journal file.

	Andrew Josey, UNIX System Labs Inc, May 24 1993
		Correct printf statement for tet_thistest, should
		be "%d" not "%ld" for systems where int does not equal long.

	Andrew Josey, UNIX System Labs Inc, May 27 1993
		Runtime selection of temporary results file handling
		determined by TET_EXTENDED. If not set the default
		tet_xres files are used. If set, then the ETET extension
		which can cope with tests being ran in parallel is used.

	Ranjan Das-Gupta, UNIX System Labs Inc, Jul 1 1993
		Code review cleanup 

	Andrew Josey, Novell UNIX System Labs, February 1994
		Remove SUNTEST defines from posix_c api.

	Andrew Josey, Novell UNIX System Labs, March 1994
	 	Change behaviour when TET_TMP_DIR="" to use defaults TET_TMP_DIR
		path rather than "." (reported by jss@apollo.hp.com).
************************************************************************/
...
/* The following declares the default names for TET defined codes */
...
tet_openres()
{
	...
	char etet_jnl_mode='F';

	{
 		char * tmp_p = getenv("TET_EXTENDED");
	    if (tmp_p) {
	    	etet_jnl_mode=toupper(*tmp_p);
	    }
	}

    if (etet_jnl_mode !='T')
    {
		/* Strictly TET Spec conforming Journal */
...
/*		(void) unlink(resfile); */
...

	{
		...
		resfile = getenv("TET_JOURNAL_PATH");
	    if ((resfile == NULL) || (resfile[0] == '\0')) 
		{
			resfd = STDOUT_FILENO;
	    } 
	    ...
	    {
			resfd = open(resfile, O_WRONLY|O_APPEND, resmode);
			if (resfd < 0)
			{
				err = errno;
				(void) sprintf(buf, "cannot open results file \"%s\"", resfile);
...
char *data;
{
	...
	(void) sprintf(buf, "%d %ld %ld %ld", tet_thistest, tet_context,
		tet_block, sequence++);
...
int tp_num;
{
	...
	static char *tmpres_name = "tet_tmpres";
	static char *lock_dir_name = ".tmpres";
	static char envstr[40];
	char * t;
	...
	lock_dir=(char *)malloc(strlen(t)+strlen(lock_dir_name)+2);
	strcpy(lock_dir,t);
	strcat(lock_dir,"/");
	strcat(lock_dir,lock_dir_name);
	
	tmpres=(char *)malloc(strlen(t)+strlen(tmpres_name)+2);
	strcpy(tmpres,t);
	strcat(tmpres,"/");
	strcat(tmpres,tmpres_name);
	...
	/* create lock */
	while (1) {
		if (!mkdir(lock_dir,0700)) break;
		if (errno==EEXIST) {
			sleep(1);
			if (--timeout==0) {
				char mess[256];
				sprintf(mess,"could not create lock dir %s",lock_dir);
				tet_error(errno,mess);
				exit(EXIT_FAILURE);
			}
			continue;
		}
		tet_error(errno, "could not create tmp file lock dir");
		return;
	}
