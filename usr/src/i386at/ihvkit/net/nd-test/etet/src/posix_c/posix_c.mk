...
tcc:	nofile
	cd tcc ; $(MAKE) TET_ROOT=../../../../etet CC="$(CC)" COPTS="$(COPTS)" \
		DEFINES="$(DEFINES)" SIG_IGNORE="$(SIG_IGNORE)" \
		SIG_LEAVE="$(SIG_LEAVE)" -f tcc.mk

api:	nofile
	cd api ; $(MAKE)  COPTS="$(COPTS)" DEFINES="$(DEFINES)" \
		SIG_IGNORE="$(SIG_IGNORE)" SIG_LEAVE="$(SIG_LEAVE)" \
		AR="$(AR)" RANLIB="$(RANLIB)" LORDER="$(LORDER)" TSORT="$(TSORT)" -f api.mk 
sun_lib: nofile
