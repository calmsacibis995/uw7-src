...
char *path_p;
{
    ...
    if (path_p[strlen(path_p)-1] == DIR_SEP_CHAR)
    {
        ...
        {
            sprintf(error_mesg, "mkdir(%s) failed", pn);
            COMPLAIN(error_mesg);
            return(FAILURE);
        }
        TET_FREE((void *)pn);
    ...
    else
    {
        ...
        if ( (rc == FAILURE) && (errno != EEXIST))
        {
            sprintf(error_mesg, "mkdir(%s) failed", path_p);
            COMPLAIN(error_mesg);
            return(FAILURE);
        }
    }
...
char object_path[];
{
    ...
    else
    {
        ...
        else
        {
            char buf[256];
            int ret;
            sprintf(buf,"set_uid  %s %s", new_dir,temp_dir);
            ret = system(buf);
            if ( (WIFEXITED(ret) == 0) || (WEXITSTATUS(ret) != 0) )
            {
               (void) sprintf(error_mesg,"set_uid IFEXITED %d exit status %d\n",WIFEXITED(stat_loc), WEXITSTATUS(stat_loc)); 
                ERROR(error_mesg);
                return(FAILURE);
             }
            /* free shared lock */
