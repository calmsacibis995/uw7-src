#ident	"@(#)kern-pdi:io/target/sdi/sdi_qm.c	1.1"
 ...
 * position of each access so that next accesses via qm_next_and_rotate()
...
/*
 * void
 * qm_initialize  ( qm_anchor_t *anchor_ptr, ulong_t link_offset )
 *
 * Calling/Exit State
 *	None.
 */
...
/*
 * void
 * qm_move  ( qm_anchor_t *from_anchor_ptr, qm_anchor_t *to_anchor_ptr )
...
/*
 * boolean_t
 * qm_is_member( qm_anchor_t *anchor_ptr, void *entry_ptr )
...
qm_is_member( qm_anchor_t *anchor_ptr, void *entry_ptr )
{
	...
	for ( q_entry_ptr = QM_HEAD( anchor_ptr );
		  q_entry_ptr != NULL;
		  q_entry_ptr = QM_NEXT( anchor_ptr, q_entry_ptr ) ) {
...
/*
 * void
 * qm_add_head ( qm_anchor_t *anchor_ptr, void *entry_ptr )
...
qm_add_head ( qm_anchor_t *anchor_ptr, void * entry_ptr )
{
	...
	qm_link_t * next_links_ptr;
	qm_link_t * new_links_ptr;
	...
	if( anchor_ptr->links.next == NULL ) {
		...
		next_links_ptr =
			QM_ENTRY_PTR_TO_LINK_PTR( anchor_ptr, anchor_ptr->links.next );
    }
	new_links_ptr = QM_ENTRY_PTR_TO_LINK_PTR( anchor_ptr, entry_ptr );
...
/*
 * void
 * qm_add_tail ( qm_anchor_t *anchor_ptr, void *entry_ptr )
...
qm_add_tail ( qm_anchor_t *anchor_ptr, void * entry_ptr )
{
	...
	if( anchor_ptr->links.next == NULL ) {
		...
		prev_links_ptr =
        QM_ENTRY_PTR_TO_LINK_PTR( anchor_ptr, anchor_ptr->links.prev );
	}
...
/*
 * void
 * qm_next_and_rotate ( qm_anchor_t *anchor_ptr)
...
qm_next_and_rotate( qm_anchor_t *anchor_ptr)
{
	qm_link_t * return_value;
	qm_link_t * next_links_ptr;

	return_value = anchor_ptr->rotor;
    if (return_value != NULL){
    	next_links_ptr = QM_ENTRY_PTR_TO_LINK_PTR( anchor_ptr, return_value );
...
/*
 * void *
 * qm_remove_head ( qm_anchor_t *anchor_ptr )
...
qm_remove_head ( qm_anchor_t *anchor_ptr )
{
	...
	if( return_value != NULL ) {
		anchor_ptr->size -= 1;
		removed_links_ptr = QM_ENTRY_PTR_TO_LINK_PTR( 
								anchor_ptr, return_value );
		if( removed_links_ptr->next == NULL ) {
			next_links_ptr = &anchor_ptr->links;
		}
		else {
			next_links_ptr = QM_ENTRY_PTR_TO_LINK_PTR(
...
/*
 * void *
 * qm_remove_tail ( qm_anchor_t *anchor_ptr )
...
qm_remove_tail ( qm_anchor_t *anchor_ptr )
{
	...
	if( return_value != NULL ) {
		...
		if( removed_links_ptr->prev == NULL ) {
			...
			prev_links_ptr = QM_ENTRY_PTR_TO_LINK_PTR(
...
/*
 * void
 * qm_remove ( qm_anchor_t *anchor_ptr, void *entry_ptr )
...
qm_remove ( qm_anchor_t *anchor_ptr, void * entry_ptr )
{
	...
	anchor_ptr->size -= 1;
	removed_links_ptr = QM_ENTRY_PTR_TO_LINK_PTR( anchor_ptr, entry_ptr );
	if( removed_links_ptr->prev == NULL ) {
		prev_links_ptr = &anchor_ptr->links;
	}
	else {
