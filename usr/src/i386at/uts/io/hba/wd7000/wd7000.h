...
#ident	"@(#)kern-pdi:io/hba/wd7000/wd7000.h	1.5"
#ident	"$Header$"
...
#define HBA_PREFIX	wd7000	/* needed to fake out macro defs in hba.h */
...
#ifdef __STDC__
extern void wd_intr(int);
extern int wd_init(void);
extern long wd_send(struct sb2 *);
extern long wd_icmd(struct sb2 *);
extern int wd_xlat(struct sb2 *, int, struct proc *, int);
extern struct hbadata *wd_getblk(int);
extern long wd_freeblk(union sb_item *);
extern void wd_getinfo(struct scsi_ad *, struct hbagetinfo *);
extern int wd_open(dev_t *, int, int, cred_t *);
extern int wd_close(dev_t, int, int, cred_t *);
extern int wd_ioctl(dev_t, int, caddr_t, int, cred_t *, int *);
#else /* __STDC__ */
extern void wd_intr();
extern int wd_init();
extern long wd_send();
