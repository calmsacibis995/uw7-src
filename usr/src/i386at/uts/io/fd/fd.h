...
#define EMIN(x)		(geteminor(x))
#define EMAJ(x)		(getemajor(x))
...
#define FDIOCTL          ('R'<<8)
#define FDSPARAM    (FDIOCTL | 00)   /* set the drive paramaters */
#define FDGPARAM    (FDIOCTL | 01)   /* get the drive paramaters */
#define FDFMTTRK    (FDIOCTL | 02)   /* format a single track */

/*
 * Definitions for SCO Everest Unix (SVR3.2.5) compatibility ioctls.
 */

#define FLIOC		('F'<< 8)
#define FLIOCFTRK	(FLIOC|1)	/* format a track 	*/
#define FLIOCSIZE	(FLIOC|2)	/* get floppy size 	*/
#define FLIOCPRT	(FLIOC|3)	/* enable/disable format msgs	*/

#define	FLBUSY		1		/* Device busy			*/
#define FLNOWR		2		/* Device RO			*/
#define FLILLTN		3		/* Illegal track number		*/
#define FLNOTDS		4		/* Illegal side number 		*/
#define FLIOERR		5		/* Device error during format	*/ 
...
#define SPECIFY     0x03
#define	PERPENDICULAR	0x12	/* Perpendicual recording mode, 2.88Mb */
#define	FDC_CONFIG	0x13	/* CONFIGURE command                   */
...
/* Track formats supported */
/* Add new format types here and in space.c */
#define FMT_5H    0
#define FMT_5D9   1
#define FMT_5D8   2
#define FMT_5D4   3
#define FMT_5D16  4
#define FMT_5Q    5
#define FMT_3D    6		/* 3.5", 720Kb	*/
#define FMT_3H    7		/* 3.5", 1.44Mb */
#define FMT_3E    8     	/* 3.5", 2.88Mb */
#define FMT_3M    9     	/* Unisys 3-Mode 1.2Mb 3.5" 512-BPS */
#define FMT_3N    10    	/* Unisys 3-Mode 1.2Mb 3.5" 1024-BPS */
#define FMT_3P    11    	/* Microsoft 21 secs/track format */ 
#define FMT_MAX   12
#define FMT_AUTO  FMT_MAX
#define FMT_UNKNOWN 0xff

#define	FDNSECT	FMT_MAX	  /* no. of sector size/count types supported */
...
/* Types of floppy drive as defined in th*/
#define DRV_NONE  0x00
#define DRV_5D    0x01
#define DRV_5H    0x02
#define DRV_3H    0x04
...
struct fdsectab {
	...
	unsigned char	fd_tre;		/* transfer rate encoding */
	unsigned char	fd_trk;		/* enable/disable track buffer reads */
								/* 0 = disable, 1 = enable	*/ 
};
...
struct fdstate {
	...
	unsigned char  fd_interleave; 	/* media interleave */
	char		   *fd_intlv_tbl; /* Pointer to interleave table */
};
...
#define F_READID	(FIOC|64)	/* Readid */
...
/*
 * SCO OS/5 compatibility. Get disk geometry/format disk track.
 * Used as output from FLIOCSIZE, input to FLIOCFTRK. Deprecated interface 
 * use V_FORMAT instead.
 */

struct fmtfl {			/* FLIOCFTRK	/	FLIOCSIZE 	*/
        char    fm_trk;         /* track number	/ 	number of tracks */
        char    fm_hd;          /* side number	/ 	number of sides */
        char    fm_sec;         /* reserved	/ 	sectors per track */
        char    fm_size;        /* reserved	/ 	BPS code : 2 = 512 */
        char    fm_il;          /* interleave			*/
        char    fm_status;      /* return status, 0 <=> SUCCESS	*/
};
