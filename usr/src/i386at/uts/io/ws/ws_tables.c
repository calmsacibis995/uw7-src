 ...
 *	  Added ALT_LOCK to kb_shifttab[] array.
 *
 */
...
keymap_t kdkeymap = { USL_TABLE_SIZE, {		/* Number of scan codes */
/*                                                         ALT    SPECIAL    */
...
/* 19*/ {{'p',   'P',   A_DLE, A_DLE, K_ESN, K_ESN, K_PNC, K_NOP },0x0f, L_C },
...
/*
 * Because the escape table above steals some of the scan-codes that the
 * Kana keyboards use, we need to map those raw scan-codes to other free
 * key-map table indices.
 */

esctbl2_t kdesctbl2 = {                                                 /*USLP*/
        0x73, 0x56, /* IBM 5576-A01 '\' key */                          /*USLP*/
        0x79, 0x5b, /* IBM 5576-A01 Conversion key */                   /*USLP*/
        0x7b, 0x5a, /* IBM 5576-A01 No Conversion key */                /*USLP*/
        0x7d, 0x5c, /* IBM 5576-A01 Yen symbol key */                   /*USLP*/
};                                                                      /*USLP*/
...
/*
 * int
 * xlate_keymap(keymap_t *, keymap_t *, int)
 *
 * Calling/Exit State:
 *	- charmap spin lock is held on entry/exit.
 */
int
xlate_keymap(keymap_t *src_kmap, keymap_t *dst_kmap, int format)
{
	...
	case  USL_FORMAT:
		/*
		 * Copy and translate the SCO keymap table to USL keymap
		 * table. Here the <src_kmap> is of sco format and the
		 * <dst_kmap> is of usl format.
		 */
		...
		for (i = 0; usl_sco_extkeys[i].escape_code >= 0; i++) {
			...
			ASSERT(sco_index <= src_kmap->n_keys &&
			       usl_index <= dst_kmap->n_keys);
	...
	case  SCO_FORMAT:
		/*
		 * Copy and translate the USL keymap table to SCO keymap
		 * table. Here the <src_kmap> is of usl format and the
		 * <dst_kmap> is of sco format.
		 */
		...
		for (i = 0; usl_sco_extkeys[i].escape_code >= 0; i++) {
			usl_index = usl_sco_extkeys[i].usl_ext_key;
			sco_index = usl_sco_extkeys[i].sco_ext_key;
			ASSERT(usl_index <= src_kmap->n_keys &&
			       sco_index <= dst_kmap->n_keys);
			if (usl_index >= 0 && sco_index >= 0)
				bcopy(&src_key[usl_index], &dst_key[sco_index],
							sizeof(keyinfo_t));
		}
		break;
