    ...
    char		*authorizationName;
{
    ...
    Debug("SetProtoDisplayAuthorization\n");
    ...
    int		    count;
{
    ...
    Debug("SaveServerAuthorizations\n");
    mask = umask (0077);
    ...
    struct display  *d;
{
   ...
   int i;
    ...
    if (auth && *auth) {
            Debug ("SetAuthorization: Got 0x%x (%d %*.*s) ", auth,
                (*auth)->name_length, (*auth)->name_length,
                (*auth)->name_length, (*auth)->name);
            Debug ("\n");
            for (i = 0; i < (int)(*auth)->data_length; i++)
...
struct verify_info	*verify;
{
	...
	Debug("envname =%s new_name=% \n", envname?envname:"NULL", new_name?new_name:"NULL");

	envname = name;
	if (link (new_name, name) == -1) {
	    Debug ("link failed %s %s\n", new_name?new_name: "NULL", name?name:"NULL");
    ...
    struct verify_info	*verify;
{
	...
	if (link (new_name, name) == -1) {
	    ...
	    Debug ("1: new is in place, go for it!\n");
	    unlink (new_name);
	}
    }
    XauUnlockAuth (name);
