...
#ifdef TESTGUI
 printf("%s (%d) - StartServer is disabled for now\n", __FILE__, __LINE__);
 fflush(stdout);
 return(TRUE);
#endif
    i = 0;
...
serverPauseUsr1 (n)
    int n;
{
    Debug ("serverPauseUsr1: display manager paused til SIGUSR1\n");
    ++receivedUsr1;
    Longjmp (pauseAbort, 1);
}
    ...
    struct display  *d;
{
    ...
    for (i = 0; i < (d->openRepeat > 0 ? d->openRepeat : 1); i++) {
    	...
    	if (!Setjmp (openAbort)) {
	    if(d->name) Debug ("Before XOpenDisplay(%s)\n", d->name);
	    errno = 0;
    ...
    Display	    *alternateDpy;
{
    ...
    if (!alternateDpy)
	alternateDpy = dpy;
    oldError = XSetIOErrorHandler ((XIOErrorHandler)PingLostIOErr);
    oldAlarm = alarm (0);
