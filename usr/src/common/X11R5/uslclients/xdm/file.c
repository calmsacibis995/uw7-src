...
int		numAcceptable;
{
	...
	Debug ("Found existing display:  %s %s %sn", d->name, d->class? d->class: "<NULL-class>", type);
	freeArgs (d->argv);
    }
    else
    {
	d = NewDisplay (name, class);
	Debug ("Found new display:  %s %s %sn", d->name, d->class ? d->class : "<NULL-class>", type);
    }
