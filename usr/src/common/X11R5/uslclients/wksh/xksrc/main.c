...
static const char	S_label[]={"UX:ksh"} ;
static const char	S_uxue []={"uxue"} ;
...
register char *v[];
{
	...
	if(sh.userid!=sh.euserid || sh.groupid!=sh.egroupid)
	{
		...
		if(!is_option(PRIVM) && sh.euserid < P_SUID)
		{
		...
		if(sh.login_sh && eq(v[0],v[1]))
			sh_fail(gettxt(E_PROHIBITED),NIL);
#endif /*SHELLMAGIC*/
	}
	else
		off_option(PRIVM);
			...
			else
			{
				...
				if(strmatch(sim,e_devfdNN))
				{
					...
					if(fstat(input,&statb)<0)
						sh_fail(st.cmdadr,gettxt(E_OPEN));
					sim = st.cmdadr;
				...
				&& statb.st_atime <= statb.st_mtime)
			{
				...
				else
				{
						...
						|| statb.st_size > lastmail.st_size))
					{
						...
						sh.lastarg = cp;
						p_str(mac_try(qp==0?gettxt(E_MAILMSG):qp+1),NL);
						sh.lastarg = save;
					}
					lastmail = statb;
					break;
