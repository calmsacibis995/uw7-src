#pragma ident	"@(#)m1.2libs:DtWidget/SpinBox.c	1.8"
/*
 * DtWidget/SpinBox.c
 */
/*
...
static char SpinBoxTranslationTable[] = "\
...
static char SpinBoxChildTranslationTable[] = "\
...
/* #5: Label get focus */
static char SpinBoxLabelTranslationTable[] = "\
        <Key>osfUp:             SpinBoxUp(child) \n\
        <Key>osfDown:           SpinBoxDown(child) \n\
        <Key>osfRight:          SpinBoxRight(child) \n\
        <Key>osfLeft:           SpinBoxLeft(child) \n\
...
static XmPartResource resources[] = {
    ...
    {DtNspinBoxChildType, DtCSpinBoxChildType, DtRSpinBoxChildType, 
	 ...
	 DtOffset(child_type), XmRImmediate, (XtPointer)XmSTRING},
...
#endif
{
    if ((ArrowSensitivity(spin) != DtARROWS_SENSITIVE) &&
	(ArrowSensitivity(spin) != DtARROWS_DECREMENT_SENSITIVE) &&
	(ArrowSensitivity(spin) != DtARROWS_INCREMENT_SENSITIVE) &&
	(ArrowSensitivity(spin) != DtARROWS_INSENSITIVE)) {
...
#endif
{
	...
	if (available_width > (short)LabelMaxLength(spin))
...
#endif
{
    ...
    if ((Editable(spin) == FALSE) && 
	(height > (short)LabelMaxHeight(spin)))
...
#endif
{
	...
	else if (ArrowLayout(new) == DtARROWS_SPLIT) {
...
#endif
{
	 ...
	 * use InitLabel.
	 */
	xm_string = (Items(spin)) ? (Items(spin))[index] : InitLabel;
	XtSetArg(arg, XmNlabelString, xm_string);
	XtSetValues(Label(spin), &arg, 1);
    }
}
		...
		Boolean crossed)
{
	...
	xm_string = (Items(spin)) ? (Items(spin))[position] : InitLabel;
...
#endif
{
   ...
   /* Getting Focus */
     ...
     */
    if ((ChildType(spin) == XmSTRING) && (Items(spin) == NULL))
	return;
