#pragma ident	"@(#)m1.2libs:DtWidget/ComboBox.c	1.9"
/*
 * DtWidget/ComboBox.c
 */
/*
...
#endif /* _NO_PROTO */
{
    ...
    */    
    ...
    if (InitLabel == NULL)
	InitLabel = XmStringCreateLocalized("ComboBox");
...
#endif
{
    ...
    activate_cb((Widget)Arrow(combo), (XtPointer)combo, NULL);
}
...
#endif
{
    ...
    if (Type(combo) == DtDROP_DOWN_COMBO_BOX && SelectedItem(combo))
...
#endif
{
    ...
    if (Label(new) && ((Items(new) != Items(current)) || 
	...
	(SelectedItem(new) != SelectedItem(current)) ||
	(Label(new) != Label(current)) ||
 ...
 */
	...
	!item && list_items)
	item = list_items[combo_p->selected_position];
...
#endif
{
	...
	XtCallCallbackList((Widget)w, SelectionCallback(combo_w), 
			   (XtPointer)&cb);
    }
}
 ...
 */
		   ...
		   (item_pos >= item_count) )
		{
			...
			if ( item_pos ==0 && event->xbutton.y <item_height/2) 
 ...
 */
    ...
    if (width < Width(combo_w))
	{
		...
		n = 0;
		width = Width(combo_w);
		list_width = ( ((root_x + width)>disp_width ||
				ScrollBarVisible(sb) )?
    ...
    _XmGrabPointer(Shell(combo_w), True, ButtonPressMask,
 ...
 */
	...
	XmStringByteCompare(SelectedItem(combo), xmStr) !=True)
    {
    	SelectedPosition(combo) = 0;
	if (SelectedItem(combo))
		XmStringFree( SelectedItem(combo) );
...
#endif /* _NO_PROTO */
{
    ...
    XtFree(s);
    i = argCount;
    XtSetArg (Args[i], XmNscrollBarDisplayPolicy, (XtArgVal )XmAS_NEEDED); i++;
