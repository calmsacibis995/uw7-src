...
#include <dlfcn.h>
#define gethostbyname (*gethostbyname_p)
#define DL_ERROR() { fprintf(stderr,"%s\n", dlerror()); return -1; }
static struct hostent * (*gethostbyname_p)();
#endif /* SVR4 */
...
#if defined (SVR4) && !defined (ARCHIVE)
        if( ! gethostbyname_p ) {
            if((libsocket_handle = dlopen("libsocket.so", RTLD_NOW) ) != NULL )
            {
               if(( gethostbyname_p =(struct hostent* (*)()) dlsym(
                             libsocket_handle,"gethostbyname" )) == NULL )
                                   DL_ERROR()
            }
           else 
               DL_ERROR()
