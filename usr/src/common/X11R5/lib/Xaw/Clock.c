...
int tick_units;
{
	 ...
	 *	    2 ------- 3
	 */
	wc = (double)width * cosangle;
	ws = (double)width * sinangle;
	SetSeg(w,
	       x = w->clock.centerX + round((double)length * sinangle),
	       y = w->clock.centerY - round((double)length * cosangle),
...
int tick_units;
{
	...
	SetSeg(w, w->clock.centerX + round((double)offset *sinangle),
	       w->clock.centerY - round((double)offset * cosangle), /* 2-----3 */
	       w->clock.centerX + round(ms + wc), 
	       w->clock.centerY - round(mc - ws));
