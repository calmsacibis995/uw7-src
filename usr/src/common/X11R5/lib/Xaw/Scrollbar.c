  ...
  ScrollbarWidget w;
{
    ...
    float    len = w->scrollbar.length;	/* for removing ANSI C warnning */
					/* Can't just cast top/shown to */
					/* Position... */

    oldtop = w->scrollbar.topLoc;
    oldbot = oldtop + w->scrollbar.shownLength;
    newtop = len * w->scrollbar.top;
    newbot = newtop + len * w->scrollbar.shown;
