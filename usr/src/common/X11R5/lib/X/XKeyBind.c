#pragma ident	"@(#)R5Xlib:XKeyBind.c	1.3"
...
/**  Search for SER to find changes for Numeric Keypad/Shift_Lock fix  **/
...
#define KeyPadKey(x) ((((x) >= XK_KP_0) && ((x) <= XK_KP_9)) || \
	((x) == XK_KP_Decimal))

/** SER:  End of Numeric keypad fix **/
	...
	*keysym_return = NoSymbol;
	...
	if ((per == 1) || (syms[1] == NoSymbol))
	    ...
	    *keysym_return = syms[0];
	       ...
	       (dpy->lock_meaning != XK_Caps_Lock)) {
	/* come here if shift is on or (the only case of lock off left
   	   from above is with shift on) or if lock is on and it is Shift lock)
	*/
	if ((per == 1) || ((usym = syms[1]) == NoSymbol))
	    XConvertCase(dpy, syms[0], &lsym, &usym);
	/**  SER - Fix for Numeric keypad.  When ShiftLock is on
	 **  we do not need to shift these keys.  This makes the
	 **  functionality compatible with DOS.
	 **/
	/**
	 **  use the lower (keypad numeric code) if this is a keypad. Note, This
	 **  is the case (the lower is the keypad) if numlock is on and
	 **  is a mode_shift (there would have been 4 keysyms) or 
         **  we are on a terminal with no numlock and they have defined
	 **  2 keysyms with the lower one = keypad numeric code.
         **  we only want to do this if just the lock is on. That is, if just
	 **  shift is on we want the 'upper' and if both shift and shift_lock
	 **  are on we also want the 'upper'
	 **/
	if ((((modifiers & (LockMask | ShiftMask)) == LockMask)) &&
	    KeyPadKey(syms[0]))
		*keysym_return = syms[0];
	else
		*keysym_return = usym;
	/** SER:  End of Numeric Keypad changes **/
    } else {
