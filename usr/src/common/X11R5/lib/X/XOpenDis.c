...
#ifdef VARIABLE_BUFSIZE
	/* QOS:marcel -
	 * This code allows the user to adjust the default buffer-size
	 * as used in Xlib.  Simply compile with -DVARIABLE_BUFSIZE
	 * and set the variable XLIB_BUFSIZE in the clients environment.
	 * This will provide a performance boost for certain clients,
	 * but may produce altered behavior by clients which expect
	 * Xlib to automatically flush requests after every 2K of protocol.
	 * Set XLIB_BUFSIZE_VERBOSE to report buffer size on allocation.
	 */
	{
	    unsigned long _xlib_bufsize;
	    char *xbufsize;

	    if ((xbufsize = getenv("XLIB_BUFSIZE")) == NULL) {
		_xlib_bufsize = 2048;
	    } else {
		_xlib_bufsize = atoi(xbufsize);
		if (_xlib_bufsize < 2048) {
		    _xlib_bufsize = 2048;
		} else if (_xlib_bufsize > 64*1024) {
		    _xlib_bufsize = 64*1024;
		}
	    }
	    if (xbufsize && getenv("XLIB_BUFSIZE_VERBOSE")) {
		fprintf(stderr,"Xlib: buffer size = %d\n",_xlib_bufsize);
	    }
	    /* Set up the output buffers. */
	    if ((dpy->bufptr = dpy->buffer = Xmalloc(_xlib_bufsize)) == NULL) {
	        OutOfMemory (dpy, setup);
		return(NULL);
	    }
	    dpy->bufmax = dpy->buffer + _xlib_bufsize;
	}
#else
	/* Set up the output buffers. */
