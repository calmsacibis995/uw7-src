#pragma ident	"@(#)R5Xlib:Xsi/XLocalIM.c	1.10"
...
int *len;
{
    ...
    *len = 0;
    ...
    for (i = 0, p = work_tbl, total = 0, p->state = 0; i < cnt; i++) {
	if (plp = is_state(k[i])) {
		p->state |= plp;
		continue;
	}
...
XipLocalCvt *cvt;
{
	...
	for(dist = 0; dist < cvt->cnt; dist++) {
		...
		if((code_p->keysym == cvt->buf[base].keysym) &&
		   ((!(code_p->state | (cvt->buf[base].state & ~(ShiftMask|LockMask)))) ||
		    (code_p->state == (cvt->buf[base].state & ~(ShiftMask|LockMask))))) {
