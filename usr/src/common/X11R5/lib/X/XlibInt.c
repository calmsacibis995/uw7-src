#ident	"@(#)R5Xlib:XlibInt.c	1.9.3.1"
/* $Header$ */
/*
 ...
 *
 */
static void _doXRead16 (dpy, data, size, packbuffer)
        register Display *dpy;
        register short *data;
        register long size;
	char *packbuffer;
	...
	register long size;
{
	...
	while ((bytes_read = ReadvFromServer (dpy->fd, iov, 2)) != size) {
		...
		size -= bytes_read;
		    ...
		    (char *) iov[1].iov_base -= iov[0].iov_len;
		    iov[0].iov_len = 0;
		    }
	    	else
		    (char *) iov[0].iov_base += bytes_read;
	    	}
	    else if (ETEST(errno)) {
		_XWaitForReadable(dpy);
		errno = 0;
...
register xEvent *event;	/* wire protocol event */
{
	...
	/* The original code (_SysErrorMsg) uses two undocumented globals,
	 * sys_nerr and sys_errlist. This can cause problem when moving
	 * from release to release especially in sbird or R6, this will
	 * create problem when people are writing multi-thread application.
	 * the old routine shall be replaced by a libc equivalent, strerr(),
	 * and shall ask X consortium to buy-in this change especially this
	 * function is protected with USL_SHARELIB!!! */
#define USE_STRERR
#ifdef USE_STRERR

	/* strerror() is defined in strings.h, this file is already included
	 * this header indirectly... */
    ...
    FILE *fp;
{
	...
	for (ext = dpy->ext_procs; ext; ext = ext->next) {
		...
		(int)ext->codes.first_error < (int)event->error_code &&
