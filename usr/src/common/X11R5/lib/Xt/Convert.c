    ...
    XtCacheRef			*cache_ref_return;
{
	...
	if (from_type == p->from && to_type == p->to) {
		...
		args = (XrmValue*)
		    ...
		    ALLOCATE_MAYBE(num_args*sizeof(XrmValue),locBuf,locBufInUse); /* PERF */
	    ...
	    if (args)/* DEALLOCATE_LOCAL( (XtPointer)args ); PERF */
		FREE_MAYBE((XtPointer)args,locBuf,locBufInUse); /* PERF */
	    return retval;
	}
    }
