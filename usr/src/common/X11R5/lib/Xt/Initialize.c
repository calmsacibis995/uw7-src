...
#endif
{
	...
	if (!(search_list = (XrmHashTable*)
	      ...
	      ALLOCATE_MAYBE(SEARCH_LIST_SIZE*sizeof(XrmHashTable),localBuf,localBufInUse)))/*PERF*/
	    _XtAllocError(NULL);
	name_list[0] = pd->name;
	class_list[0] = pd->class;
	...
	/* DEALLOCATE_LOCAL( search_list ); PERF */
