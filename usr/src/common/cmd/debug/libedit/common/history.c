#ident	"@(#)debugger:libedit/common/history.c	1.5"
...
retry:
	...
	if((fd=debug_open(histname,O_APPEND|O_RDWR|O_CREAT,S_IRUSR|S_IWUSR))>=0)
	{
...
static void hist_trim(n)
{
	...
	off_t old,new=0;
	int fdo = hist_ptr->fixfd;
	uid_t e_uid = geteuid ();	/* retrieve effective uid */
	...
	/* If the effective user is not the same as the owner of the file
	or the file cannot be opened, don't trim the history file */
	if(e_uid != stat_info.st_uid || !hist_open())
	{
...
char *string;
{
	...
	do
	{
		...
		for(cp=(unsigned char*)string;*cp;cp++)
		{
			...
			if(flag && c == *(unsigned char*)string && offset<0)
#ifdef MULTIBYTE
...
register int	ptrname;
{
	...
	if(fd==hist_ptr->fixfd && ptrname==0 && (fp->flag&IOREAD) && offset<hoffset)
	{
...
ksh_init()
{
	...
	if ((io_ftable = (struct fileblk **)calloc(open_max, 
		sizeof(struct fileblk *))) == 0)
	{
		/* can't call new_handler from C file */
