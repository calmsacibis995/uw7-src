...
static genchar lastline[MAXLINE];
...
addin:
	...
	case '_':		/** append last argument of prev command **/
		save_v();
		{
			char histbuf[MAXLINE];
			if(repeat_set==0)
				repeat = -1;
			p = (genchar*)hist_word(histbuf,repeat);
			if(p==0) {
				bell;
				break;
			}
