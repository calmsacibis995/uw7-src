...
#define DISPLAYQL	'\4'
#define RMJOB		'\5'
#define DISPLAYQS_SEC	'\6'	/* SCO value added */
#define DISPLAYQL_SEC	'\7'	/* SCO value added */
#define MVJOB		'8'	/* SCO value added */
#define	QUERY_ENABLE	'9'	/* SCO value added */

#define LPD_PROTO_MSG(c) (((c) >= PRINTJOB && (c) <= DISPLAYQL_SEC) || (c) == MVJOB || (c) == QUERY_ENABLE)
...
#define MAX_AUTHENTICATION_ATTEMPTS 1
...
#define MAX_TITLE_LEN 79      /* max length of title in 'T' control file line */
...
char	* lp_gethostname(void);
char	* getitem(char *, char);
char	* s_cancel(char *);
char	* s_get_status(char *);
char	* s_mvjob(char *);
char	* s_print_request(char *);
char	* s_nuc_print_request(char *);
char	* s_nuc_cancel(char *);
char	* s_nuc_get_status(char *);
