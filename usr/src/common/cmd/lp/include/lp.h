 ...
 * FILENAME:    lp.h
 *
 * DESCRIPTION: The main include file for the lp system
 *
 * SCCS:	lp.h 1.3  10/23/97 at 15:34:22
 *
 * CHANGE HISTORY:
 *
 * 23-10-97  Paul Cunningham        ul97-28215
 *           This MR requests that the value of the #define for BIGGEST_REQID be
 *           changed from 999 to 999999, so that an ICL customer can print a
 *           large number of very small jobs in quick succession (eg. > 999 
 *           jobs).
 *           But for the remote BSD protocol the request id is limited to 3
 *           digits (see rfc1179 sections 6.2 and 6.3) so the change can not be
 *           done. The effect of doing the change would be:
 *             1. request ids greater than 3 digits would be truncated, thus
 *                on a job with a request id of > 3 digits we can not 
 *                "lpstat -t" it as it has a different request id at the server.
 *             2. If the id number is increased to greater than 3 digits, on a 
 *                BSD remote printer's server the print jobs could end up with 
 *                the same id, eg. at client the ids 895, 1895, 901895, etc,
 *                would all end up on the server with the same id of 895 and 
...
#define	C_LOCALE	"C" /* name of locale default of last resort*/
#define	POSIX_LOCALE	"POSIX" /* name of locale default of last resort*/
...
#define CHARSETFILE	"LCL_CHARSET"
 ...
 *
 * NOTE: 1. the BIGGEST_REQID value should not be greater than a 3 digit number
 *          otherwise it invalidates the rfc1179 (remote BSD printer protocol) 
 *          spec, see comments in the CHANGE HISTORY above for MR ul97-28215.
 *       2. if BIGGEST_REQID is changed the file 
 *          /usr/spool/lp/tmp/<sysname>/.SEQF needs to be removed from the 
 *          system before this will take effect.
 */
...
char *		getlocalefile ( char * locale, char * type );
char *		getlocaledir ( void );
char **		get_locales ( void );
char **		get_charset ( char * locale );
...
extern void		(*lp_alloc_fail_handler)();
