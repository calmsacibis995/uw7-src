...
#endif
{
	...
	/* Remove any remote requests we are caching for the printer */
	purge_remote(pps->system, pps, 1);
	...
	/* Free any cached remote printer status */
	if (pps->rmt_form) {
		Free(pps->rmt_form);
		pps->rmt_form = NULL;
	}
...
#endif
{
	...
	*/
	...
	} else if ((pps = search_ptable(printer)) != NULL) {
...
SendBackStatus:
	for (pps = walk_ptable(1); pps; pps = walk_ptable(0)) {
		if (!(pps->status & PS_REMOTE))
			continue;
		if (*printer && !STREQU(printer, NAME_ALL))
			if (!STREQU(pps->printer->name, printer))
				continue;
		if (found)
			(void) mputm(md,
				...
				MOKMORE,
				found->printer->name,
				(found->rmt_form ? found->rmt_form : ""),
				(found->rmt_pwheel ? found->rmt_pwheel : ""),
				(found->rmt_dis_reason ? found->rmt_dis_reason : ""),
				(found->rmt_rej_reason ? found->rmt_rej_reason : ""),
				found->rmt_status,
				(found->rmt_req_id ? found->rmt_req_id : ""),
				found->rmt_dis_date,
				found->rmt_rej_date);
		found = pps;
	}
