 ...
 * FILENAME:    init.c
 *
 * DESCRIPTION: The initialise routes for lpsched
 *
 * SCCS:	init.c 1.4  9/24/97 at 16:24:34
 *
 * CHANGE HISTORY:
 *
 * 24-09-97  Paul Cunningham        ul97-17601
 *           Change function init_requests() so that the NOTIFYs are send 
 *           correctly (this fix was suggested by Nihon Unisys) during the
 *           startup of lpsched to a remote client box (S5).
 *
 *******************************************************************************
 */
...
RSTATUS		*Remote_Request_List;	/* Queue of known remote requests */
...
#endif
{
	...
	while((name = next_file(sysdir, &sysaddr)) != NULL)
	{
	    ...
	    *(table[count]->request) = *r;
	    ...
	    if ((r->outcome & (RS_CANCELLED|RS_FAILED|RS_PRINTED)) &&
		!(r->outcome & RS_NOTIFY))
	    {
		rmfiles(table[count], 0);
		freerstatus(table[count]);
