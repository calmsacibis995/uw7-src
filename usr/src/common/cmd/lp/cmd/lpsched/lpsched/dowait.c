 ...
 * FILENAME:    dowait.c
 *
 * DESCRIPTION: lpsched task to clean up child that has finished, and then 
 *              reschedule another task.
 *
 * SCCS:	dowait.c 1.3  10/13/97 at 10:57:00
 *
 * CHANGE HISTORY:
 *
 * 13-10-97  Paul Cunningham        ul97-26801
 *           Change function dowait() in case EX_SLOWF, so that it checks the
 *           prs pointer is not null before continueing. If the lp system has 
 *           just been restarted there may be remote requests that have not been
 *           properly cleaned up leaving the request pointer as null, ignore the
 *           request if the pointer is null.
 *
 *******************************************************************************
 */
...
#endif
{
	...
	while (DoneChildren > 0) {
		...
		case EX_SLOWF:
		{
		  ...
		  /* Before continueing check that the prs pointer is not null.
		   * If the lp system has just been restarted there may be 
		   * remote requests that have not been properly cleaned up 
		   * leaving the request pointer as null, ignore the request if
		   * the pointer is null.
		   */
		  if ( prs != NULL)
		  {
