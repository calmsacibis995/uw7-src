 ...
 * FILENAME:    set_size.c
 *
 * DESCRIPTION: Out put the printer control codes to the printer to set 
 *              parameter sizes for Page Length (L) and width (W)
 *
 * SCCS:	set_size.c 1.3  8/26/97 at 13:22:47
 *
 * CHANGE HISTORY:
 *
 * 26-08-97  Paul Cunningham        ul94-28782
 *           Change function set_size() so that if the printer is a Unisys 
 *           AP9210 (LaserJet) it outputs the control character sequence for
 *           the default page length of 66 lines as this printer requires the
 *           default length to be set otherwise it doesn't print correctly. The
 *           #define AP9210LJ was added to define the name used by terminfo for
 *           this printer.
 *
 *******************************************************************************
 */
...
#define AP9210LJ "AP9210-lj"
	/* defines the printers terminfo name string
	 */
...
#endif
{
	...
	char			*rest,
				...
				*finale,
				*slines;
	char			*p_term;	/* ptr to printer name string */
	...
	p_term = getenv("TERM"); /* get printer type name */

	if (( size == 0) && ( which == 'L'))
	{	
	  /* If the printer is a Unisys AP9210 Printer we need to send the
	   * default page length to the printer, so if either null or 0 was
	   * passed to this function continue to set default page length.
	   * but other printer types do not need this so return successfuly now
	   */
	  if ( strcmp( p_term, AP9210LJ) != 0)
	  {
	    /* use default page length (nothing to send to printer)
	     */
	    return( E_SUCCESS);
	  }
	}
	...
	if (( strcmp( p_term, AP9210LJ) == 0) && ( which == 'L'))
	{	
	  /* AP9210 Printer, so always set page length
	   */
	  isize = R(size);
	  if ( isize == 0)
	  {
