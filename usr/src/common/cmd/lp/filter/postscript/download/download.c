...

{
	...
	if (((ext = strrchr(fontname, DOT)) == NULL) || (strcmp(ext, PFB_EXT)))
		return FALSE;
	return TRUE;
}
/*****************************************************************************/
...
/*
 *
 * Copies *file to stdout. Returns FALSE if there was a
...
static int procid;
static  int myside, yourside;

int
call_pfb2pfa( int infd )
{
	char		ibuf[BUFSIZ];
	struct stat	statbuf;
	int		buflen;
	...
	if((procid = fork()) == 0) {
		...
		(void) close(yourside);
		(void) execl("/bin/sh", "sh", "-c", PFB2PFA, 0);
		return 1 ;
	}
	if(procid == -1)
	{
	...
	while ((buflen = read(infd, ibuf, BUFSIZ)) > 0 )
	{
		if (write(myside, ibuf, buflen) != buflen)
		{
...
int fd;
{
	...
	signal(SIGQUIT, SIG_IGN);
	signal(SIGHUP, SIG_IGN);
	while((r = wait(&status)) != procid && r != -1)
		;
	if(r == -1)
		status = -1;
	return status ;
