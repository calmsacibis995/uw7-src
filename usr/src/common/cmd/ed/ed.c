...
const struct {const sev; const char *msg, *id; } msgtab[] =
{
	...
	MM_ERROR,	"Maximum of 1023 characters in pathname",":1204",/* 24 */
	...
	MM_WARNING,	"Too many files",			":1205",/* 68 */
	0
};
...
static char expandbuf[FNSIZE + sizeof(echocmd)];
...
int		gotre;		/* set once curre describes an RE */
regex_t		curre;		/* current regular expression */
regmatch_t	currm[10];	/* only need "&" and "\1"-"\9" */
char		*restr;		/* current RE string for substitution */
...
char	**argv;
{
	...
	*argv = p1 + 1;
	...
	tfname = tempnam("","ea");
	if (!tfname) {
		pfmt(stdout, MM_ERROR, ":602:%s failed: %s\n", "tempnam", strerror(errno));
		exit(1);
	}
...
swch:
	...
	case 'c':
		...
		/* POSIX.2 says that if nothing inserted, leave dot AFTER */
		if (dot == addr1-1 && dot < dol)
			dot = addr1;
		continue;
	...
	caseread:
		...
		if ((io = eopen(file, O_RDONLY, NULL, 0)) < 0) {
			lastc = '\n';
...
filename(comm)
{
	...
	*p1++ = 0;
	...
	if (!rflg && gscan(file)) /* e,E,f,r,w and 'W' */
		fexpand(file);
...
putm(code)
{
	...
	if(code != INT_MAX) {
		if(code < 0) {
			regerror(-code, &curre, msgre, sizeof(msgre));
			s = msgre;
			sev = MM_ERROR | MM_NOGET;
		} else {
			s = gettxt(msgtab[code].id, msgtab[code].msg);
			sev = msgtab[code].sev | MM_NOGET;
		}
		pfmt(stdout, sev, s);
		putchar('\n');
	}
...
getchr()
{
	...
	i = read(0, &c, 1);
	if (i == 0)
		sleep (1);      /* allow time for SIGHUP to arrive */
	if (i <= 0)
		return(lastc = EOF);
...
unixcom()
{
	...
	*pcurcmd++ = 0;
	...
	if (env_sh = getenv("SHELL"))
		strncpy(shell, env_sh,30);
...
long atl;
{
	register unsigned long bno, off;
	register char *p1, *p2;
	register int n;
	...
	if (iof==READ) {
		if (ichanged) {
			if(xtflag)
				if(run_crypt(0L, ibuff, IOBSIZE, tperm) == -1)
					error(63);
	...
	if (oblock>=0) {
		if(xtflag) {
			...
			while(n--)
				*p2++ = *p1++;
			if(run_crypt(0L, crbuf, IOBSIZE, tperm) == -1)
				error(63);
...
int (*iofcn)();
{
	...
	if ((*iofcn)(tfile, buf, IOBSIZE) != IOBSIZE) {
...
substitute(inglob)
{
	...
	for (a1 = addr1; a1 <= addr2; a1++) {
		...
		subnewa = putline();
		/* Need to preserve the mark bit -- POSIX.2 */
		if ((marked = a1->cur & 01) != 0)
			a1->cur &= ~01;
		if (anymarks) {
...
dosub()
{
	...
	if(gsubf > 0 && gsubf < 999)
		{numpass++;
		if(gsubf != numpass){restr += currm[0].rm_eo; return;}
		}
	ocerr1++;
	ocerr2++;
	lp = linebuf;
	sp = genbuf;
	rp = rhsbuf;
	lp1 = &restr[currm[0].rm_so];
	lp2 = &restr[currm[0].rm_eo];
	while (lp < lp1)
		*sp++ = *lp++;
	while (c = *rp++) {
		if (c=='&') {
...
register wchar_t seof;
{
	...
	*cp = '\0';
	...
	if((bkt = regcomp(&curre, genbuf, REG_ANGLES | REG_ESCNL)) != 0)
		error1(-bkt);
	if(genbuf[0] == '^')
		gotre = -1;
	else
...
int sf;
{
	...
	if((k = regexec(&curre, p, (size_t)sf, &currm[0], k)) == 0)
		return 1;
...
wchar_t ac;
{
	...
	if ( listf ) {
		if ((width = scrwidth(c)) == 0 || c == '\\') {
			/*
			 * POSIX.2 requires C-like escapes for these.
			 */
			switch (c) {
			default:
				putoctal(c);
...
globaln(k)
{
	...
	register LINE a1;
	int  nfirst;
	int  savpflag, savlistf, savlistn;
	char globuf[GBSIZE];
	char multic[MULTI_BYTE_MAX];
	...
	nfirst = 0;
	newline();
	savpflag = pflag; pflag = 0;
	savlistf = listf; listf = 0;
	savlistn = listn; listn = 0;
	for (a1=zero; a1<=dol; a1++) {
		if (a1->cur & 01) {
			a1->cur &= ~01;
			dot = a1;
...
eopen(string, rw, alt_shell, noerr)
char *string;
register rw;
char *alt_shell;
int noerr;
...
xerr:		error(0);
	if((i = fork()) == 0) {
		...
		if (noerr)
			(void) close(2);
		execlp(alt_shell ? alt_shell : "/usr/bin/sh",
					"sh", "-c", string, (char *) 0);
		exit(1);
	}
	if(i == (pid_t)-1)
...
int up;
{
	...
	havespec = fsprtn = 0;
	for(fsp=(unsigned char *)line; *fsp && *fsp != '\n'; fsp++)
		switch(*fsp) {
...
char *name;
{
	unsigned max  = FNSIZE + sizeof(echocmd);
	unsigned total = 0;
	unsigned count ;
	int fd ;

	/* execute "echo name" */
	(void) strcpy (expandbuf, echocmd);
	(void) strcat (expandbuf, name);
					/* val = getenv("SHELL") */
	Xqt++;
	fd = eopen(expandbuf, O_RDONLY, (val && *val) ? val : NULL, 1);
	do {
		count = read (fd, expandbuf + total, max - total);
		if ((int)count <= 0)
			break ;
		else
			total += count;
	...
	if (total == 0)
		return ;
	expandbuf[total-1] = '\0';
	if (strrchr(expandbuf, ' ') != NULL) {
		/* Too many files */
		error(68);
	} else {
		if (total-1 >= FNSIZE)
			error(24);
		(void) strcpy(name, expandbuf) ;
	}
}

/*
 * Scan filename for shell metacharacters.
...
char *filename;
{
	...
	for (cp = (unsigned char *)filename; *cp; cp++)
		if (strchr("~{[*?$`'\"\\", *cp))
			return (1);
	return (0);
}
