...
char		*tmp_name;
char		def_tmp_name[] = "/tmp/pgXXXXXX";
...
readch ()
{
	...
	if (read (fileno(stdout), (char*)&ch, 1) < 0)
		end_it();
	return (ch);
}
...
register char *fs;
{
	...
	if ((stbuf.st_mode & S_IFMT) == S_IFREG) {
		...
		if (f != stdin) {
			...
			tmp_name = tempnam((char *)0, "pg");
			if (tmp_name == (char *)0) {
				tmp_name = def_tmp_name;
...
regex_t *p;
{
	...
	if (p != 0) {
		id = ":1359";
		str = "RE error: %s";
		regerror(i, p, msg, sizeof(msg));
	} else if (i == REG_NOMATCH) {
		id = ":427";
		str = "Pattern not found";
	} else {
		id = ":199";
		str = "No remembered search string";
	}
	error(id, str, msg);
...
register int n;
{
	...
	if (setjmp(restore) <= 0) {
		if (buf[0] == '\0') {
			if (gotre == 0)
				re_error(0, (regex_t *)0);
		} else {
			if (gotre)
				regfree(&expbuf);
			gotre = 0;
			if ((err = regcomp(&expbuf, buf, REG_ANGLES | REG_ESCNL | REG_NOSUB)) != 0)
				re_error(err, &expbuf);
			gotre = 1;
		}
		...
		while (find(1,direction) != END_COND){
			if (brk_hit)
				break;
			if ((err = regexec(&expbuf, Line, (size_t)0,
				(regmatch_t *)0, 0)) == REG_NOMATCH)
			{
				continue;
			} else if (err != 0) {
				re_error(err, &expbuf);
		...
		re_error(REG_NOMATCH, (regex_t *)0); /* Pattern not found */
	}
	BEEP();
	return(0);
}
