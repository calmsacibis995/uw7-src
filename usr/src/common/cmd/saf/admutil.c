...
char *tname;
{
	...
	if ( attr_ret ) {
			...
			if ( lvlfile(tname, MAC_SET, &level) != 0) {
				Saferrno = E_NOPRIV;
				(void) unlink(tname);
				pfmt (stderr, MM_ERROR, MSG9, fname);
				quit();
			}
		}
		if (chmod(tname, attr.st_mode) != 0 ||
			...
			Saferrno = E_SYSERR;
			(void) unlink(tname);
			pfmt (stderr, MM_ERROR, MSG10, fname);
			quit();
		}
	...
	if ( rename(tname, fname) < 0) {
		Saferrno = E_SYSERR;
		(void) unlink(tname);
		pfmt (stderr, MM_ERROR, MSG11, tname);
		quit();
	}
}
	...
	if (script) {
		...
		if (access(script, 0) == 0) {
			if (stat(script, &statbuf) < 0) {
				Saferrno = E_SYSERR;
				pfmt (stderr, MM_ERROR, MSG1, script);
				return(1);
			}
			if ((statbuf.st_mode & S_IFMT) != S_IFREG) {
				pfmt (stderr, MM_ERROR, MSG2, script);
				return(1);
			}
		}
		else {
			Saferrno = E_NOEXIST;
			pfmt (stderr, MM_ERROR, MSG3, script);
			return(1);
		}
		ifp = fopen(script, "r");
		if (ifp == NULL) {
			
			pfmt (stderr, MM_ERROR, MSG4, script);
			Saferrno = E_SYSERR;
			return(1);
		}
		...
		if ( new ) {
			if (chmod(origname, (mode_t) 0644) != 0 ||
				...
				Saferrno = E_SYSERR;
				pfmt (stderr, MM_ERROR, MSG15, origname);
				quit();
			}
			...
			if (lvlin("SYS_PRIVATE", &level) == 0) {
				if (lvlfile(origname, MAC_SET, &level) != 0) {
					Saferrno = E_NOPRIV;
					pfmt (stderr, MM_ERROR, MSG16, origname);
					quit();
				}
			} else {
				if (mac_check()) {
					Saferrno = E_SYSERR;
					pfmt (stderr, MM_ERROR, MSG17, origname);
					quit();
				}
			}
		}
