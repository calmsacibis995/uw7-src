 ...
 */
			...
			if (tsp->sc_valid) {
				...
				(void) sprintf(Scratch, gettxt(MSGID109,MSG109), tsp->sc_tag);
				log(Scratch);
			}
			else {
	...
	while (sp) {
		...
		case STARTING:
			...
			if (sendsig(sp, SIGTERM))
				(void) sprintf(Scratch, gettxt(MSGID110,MSG110),sp->sc_tag);
			else
				(void) sprintf(Scratch, gettxt(MSGID111,MSG111), sp->sc_tag);
			log(Scratch);
