 ...
 */
		...
		case AC_START:
# ifdef DEBUG
			(void) sprintf(Scratch, gettxt(MSGID22,MSG22), ap->ac_tag);
			log(Scratch);
# endif
 ...
 */
		...
		case AC_KILL:
# ifdef DEBUG
			(void) sprintf(Scratch, gettxt(MSGID23,MSG23), ap->ac_tag);
			log(Scratch);
# endif
			if ((sp = findpm(ap->ac_tag)) == NULL) {
				ak->ak_pid = ap->ac_pid;
			...
			case DISABLED:
				...
				if (sendsig(sp, SIGTERM)) {
					(void) sprintf(Scratch, gettxt(MSGID24,MSG24), sp->sc_tag);
					log(Scratch);
					ak->ak_pid = ap->ac_pid;
				...
				ak->ak_size = 0;
				sendack(ak);
				(void) sprintf(Scratch, gettxt(MSGID25,MSG25), sp->sc_tag);
				log(Scratch);
				(void) sigprocmask(SIG_SETMASK, &cset, NULL);
				break;
			}
 ...
 */
		...
		case AC_ENABLE:
# ifdef DEBUG
			(void) sprintf(Scratch, gettxt(MSGID26,MSG26), sp->ac_tag);
			log(Scratch);
# endif
			if ((sp = findpm(ap->ac_tag)) == NULL) {
				ak->ak_pid = ap->ac_pid;
 ...
 */
		...
		case AC_DISABLE:
# ifdef DEBUG
			(void) sprintf(Scratch, gettxt(MSGID27,MSG27), ap->ac_tag);
			log(Scratch);
# endif
			if ((sp = findpm(ap->ac_tag)) == NULL) {
				ak->ak_pid = ap->ac_pid;
...
# ifdef DEBUG
			(void) sprintf(Scratch,gettxt(MSGID29,MSG29), a->ak_size);
			debug(Scratch);
# endif
 ...
 */
		...
		case AC_PMREAD:
# ifdef DEBUG
			(void) sprintf(Scratch,gettxt(MSGID32,MSG32), ap->ac_tag);
			log(Scratch);
# endif
			if ((sp = findpm(ap->ac_tag)) == NULL) {
				ak->ak_pid = ap->ac_pid;
 ...
 */
		...
		default:
			(void) sprintf(Scratch,gettxt(MSGID33,MSG33), ap->ac_tag);
			log(Scratch);
			ak->ak_pid = ap->ac_pid;
...
# endif
	if (write(sp->sc_fd, sm, sizeof(struct sacmsg)) != sizeof(struct sacmsg)) {
		(void) sprintf(buf, gettxt(MSGID35,MSG35), sp->sc_tag);
		log(buf);
	}
}
...
# endif
	...
	if (fd < 0) {
		(void) sprintf(Scratch, gettxt(MSGID36,MSG36), sp->sc_tag);
		log(Scratch);
		return((pid_t)0);
	}
