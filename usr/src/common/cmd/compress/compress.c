/*	copyright	"%c%"	*/
#ident	"@(#)compress:compress.c	1.4.10.2"
...
#else
	    pfmt(stderr, MM_ACTION,":1009:Usage: compress [-dcCfnFqvV] [-b maxbits] [file ...]\n");
#endif /* DEBUG */
	    exit(1);
    	}
    } else if (uflg) { 	/* uncompress */
         while ((c = getopt(argc, argv, "qcfP:v")) != EOF)
		    switch (c) {
		    case 'c':
			zcat_flg = 1;
			break;
         ...
         if (errflg) {
		pfmt(stderr, MM_ACTION,":1010:Usage: uncompress [-cfvq] [file ...]\n");
		exit(1);
    	 }
    } else { 	/* zcat */
    ...
    *fileptr = NULL;
	...
	":791:%s: compressed with %1$d bits, can only handle %2$d bits\n",
		...
		if (stat(ofname, &statbuf) == 0) {
			...
			if (err != 0) {
				regerror(err, &yesre, response, MAX_INPUT);
			...
			pfmt(stderr, MM_NOSTD,
			 ":1129:do you wish to overwrite %s (%s/%s)? ",ofname,yesp, nop);
			fflush(stderr);
			...
			if ( i > 0)
				response[i] = '\0';
			if (regexec(&yesre, response, (size_t)0,
				(regmatch_t *)0, 0) != 0) {
		...
		(void) fclose(stdin);
		if (fdopen(sstdin, "r") == NULL) {
    			pfmt(stderr, MM_ERROR, ":1011:Can't (re)open stdin: %s\n", 
					strerror(errno));
			exit_stat++;
			continue;
		}
		(void) fclose(stdout);
		if (fdopen(sstdout, "w") == NULL) {
    			pfmt(stderr, MM_ERROR, ":1012:Can't (re)open stdout: %s\n", 
					strerror(errno));
			exit_stat++;
			continue;
		}
...
decompress() {
	...
	while ( code >= 256 ) {
	    *stackp++ = tab_suffixof(code);
	    if (stackp >= ((char_type *)&htab[HSIZE]) - 1)
		oops();
