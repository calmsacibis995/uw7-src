...
static int	syncpad = 0;	/* character used to pad by conv=sync */
...
/* old messages */
static char o_trunc1blk[] =
	":1160:1 truncated block\n";
static char o_truncnblks[] =
	":1161:%u truncated blocks\n";

/* POSIX.2 messages*/
static char p_trunc1blk[] =
	":1360:1 truncated record\n";
static char p_truncnblks[] =
	":1361:%u truncated records\n";

static char *trunc1blk = o_trunc1blk;
static char *truncnblks = o_truncnblks;
...
char **argv;
{
	...
	if (getenv(posix_var) != 0) {
		trunc1blk = p_trunc1blk;
		truncnblks = p_truncnblks;
		posix = 1;
	}
