...
static unsigned int	nrules;	/* number of rules */
...
char *	string;
{
	...
	** on BSD systems.
	*/
	(void) pfmt(stderr, MM_ERROR, ":1:\"%s\", line %d: %s",
		filename, linenum, string);
	if (rfilename != NULL)
		(void) pfmt(stderr, MM_NOSTD, ":2: (rule from \"%s\", line %d)",
			rfilename, rlinenum);
	(void) pfmt(stderr, MM_NOGET|MM_NOSTD, "\n");
	++errors;
}
...
usage(int flag)
{
	(void) pfmt(stderr, flag,
":3:Usage:\n\t%s [ -v ] [ -d directory ] [ -l localtime ] [ filename ... ]\n",
		progname);
	if (flag == MM_INFO)
		exit(0);
	else
...
char *	argv[];
{
			...
			case 'd':
				if (directory == NULL)
				    ...
				    (void) pfmt(stderr, MM_ERROR,
				      ":4:More than one -d option specified\n");
				    usage(MM_ACTION);
				}
				break;
			case 'l':
				if (lcltime == NULL)
				    ...
				    (void) pfmt(stderr, MM_ERROR,
				      ":5:More than one -l option specified\n");
				    usage(MM_ACTION);
				}
				break;
			case 'v':
	...
	if (optind == argc - 1 && strcmp(argv[optind], "=") == 0)
		usage(MM_INFO);	/* usage message by request */
	if (directory == NULL)
		directory = TZDIR;
	...
	*/
	if (chdir(directory) != 0) {
		(void) pfmt(stderr, MM_ERROR, ":6:Cannot chdir to %s: %s\n",
	...
	for (i = 0; i < nlinks; ++i) {
		...
		if (link(links[i].l_from, links[i].l_to) != 0) {
			(void) pfmt(stderr, MM_ERROR,
			    ":7:Cannot link %s to %s: %s\n",
			    links[i].l_from, links[i].l_to, strerror(errno));
			exit(1);
		}
	}
	if (lcltime != NULL) {
		...
		if (link(lcltime, TZDEFAULT) != 0) {
			(void) pfmt(stderr, MM_ERROR,
			    ":7:Cannot link %s to %s: %s\n",
			    lcltime, TZDEFAULT, strerror(errno));
			exit(1);
		}
	}
	exit((errors == 0) ? 0 : 1);
...
associate()
{
	...
	if (nrules != 0)
		qsort((char *) rules, nrules, sizeof *rules, rcomp);
	for (i = 0; i < nzones; ++i) {
		zp = &zones[i];
		zp->z_rules = NULL;
		zp->z_nrules = 0;
		...
		if (zp->z_nrules == 0) {
			...
			*/
			...
			zp->z_stdoff = gethms(zp->z_rule,
				gettxt(":8", "ruleless zone"), TRUE);
			/*
			** Note, though, that if there's no rule,
			** a '%s' in the format is a bad thing.
			*/
			if (strchr(zp->z_format, '%') != 0)
				error(gettxt(":9", "%s in ruleless zone"));
		}
	}
	if (errors)
		exit(1);
...
char *	name;
{
	...
	if (strcmp(name, "-") == 0) {
		...
		(void) pfmt(stderr, MM_ERROR, ":10:Cannot open %s: %s\n",
			name, strerror(errno));
		exit(1);
	}
	wantcont = FALSE;
	for (num = 1; ; ++num) {
		...
		*cp = '\0';
		...
		if (nfields == 0) {
			...
			if (lp == NULL)
			    error(gettxt(":12", "input line of unknown type"));
	...
	if (ferror(fp)) {
		(void) pfmt(stderr, MM_ERROR, ":14:Error reading %s: %s\n",
			filename, strerror(errno));
		exit(1);
	}
	if (fp != stdin && fclose(fp)) {
		(void) pfmt(stderr, MM_ERROR, ":15:Error closing %s: %s\n",
			filename, strerror(errno));
		exit(1);
	}
	if (wantcont)
		error(gettxt(":16", "expected continuation line not found"));
}
...
register char **	fields;
{
	...
	if (nfields != RULE_FIELDS) {
		error(gettxt(":17", "wrong number of fields on Rule line"));
		return;
	}
	if (*fields[RF_NAME] == '\0') {
		error(gettxt(":18", "nameless rule"));
		return;
	}
	r.r_filename = filename;
	r.r_linenum = linenum;
	r.r_stdoff = gethms(fields[RF_STDOFF],
		gettxt(":19", "invalid saved time"), TRUE);
...
register char **	fields;
{
	...
	if (nfields < ZONE_MINFIELDS || nfields > ZONE_MAXFIELDS) {
		error(gettxt(":20", "wrong number of fields on Zone line"));
		return FALSE;
	}
	if (strcmp(fields[ZF_NAME], TZDEFAULT) == 0 && lcltime != NULL) {
		(void) sprintf(buf,
		   gettxt(":21", "\"Zone %s\" line and -l option are mutually exclusive"),
		   TZDEFAULT);
		error(buf);
		return FALSE;
	}
	for (i = 0; i < nzones; ++i)
		if (zones[i].z_name != NULL &&
			strcmp(zones[i].z_name, fields[ZF_NAME]) == 0) {
				(void) sprintf(buf,
				  gettxt(":22", "duplicate zone name %s (file \"%s\", line %d)"),
					fields[ZF_NAME],
					zones[i].z_filename,
					zones[i].z_linenum);
				error(buf);
...
register char **	fields;
{
	if (nfields < ZONEC_MINFIELDS || nfields > ZONEC_MAXFIELDS) {
		error(gettxt(":23", "wrong number of fields on Zone continuation line"));
		return FALSE;
	}
...
register char **	fields;
{
	...
	z.z_linenum = linenum;
	z.z_gmtoff = gethms(fields[i_gmtoff],
		gettxt(":24", "invalid GMT offset"), TRUE);
	if ((cp = strchr(fields[i_format], '%')) != 0) {
		if (*++cp != 's' || strchr(cp, '%') != 0) {
			error(gettxt(":25", "invalid abbreviation format"));
			return FALSE;
		}
	}
...
error(gettxt(":26", "Zone continuation line end time is not after end time of previous line"));
			return FALSE;
		}
	}
...
register char **	fields;
{
	...
	if (nfields != LINK_FIELDS) {
		error(gettxt(":27", "wrong number of fields on Link line"));
		return;
	}
	if (*fields[LF_FROM] == '\0') {
		error(gettxt(":28", "blank FROM field on Link line"));
		return;
	}
	if (*fields[LF_TO] == '\0') {
		error(gettxt(":29", "blank TO field on Link line"));
		return;
	}
...
char *			timep;
{
	...
	if ((lp = byword(monthp, mon_names)) == NULL) {
		error(gettxt(":30", "invalid month name"));
		return;
	}
	...
	rp->r_tod = gethms(timep, gettxt(":31", "invalid time of day"), FALSE);
	/*
	** Year work.
	*/
	cp = loyearp;
	if ((lp = byword(cp, begin_years)) != NULL) switch ((int) lp->l_value) {
		...
		rp->r_loyear < min_year || rp->r_loyear > max_year) {
			if (noise)
				error(gettxt(":32", "invalid starting year"));
	...
	if ((lp = byword(cp, end_years)) != NULL) switch ((int) lp->l_value) {
		...
		rp->r_hiyear < min_year || rp->r_hiyear > max_year) {
			if (noise)
				error(gettxt(":33", "invalid ending year"));
	...
	if (rp->r_loyear > rp->r_hiyear) {
		error(gettxt(":34", "starting year greater than ending year"));
		return;
	}
	if (*typep == '\0')
		rp->r_yrtype = NULL;
		...
		if (rp->r_loyear == rp->r_hiyear) {
			error(gettxt(":35", "typed single year"));
			return;
		}
	...
	*/
	if ((lp = byword(dayp, lasts)) != NULL) {
		...
		if (rp->r_dycode != DC_DOM) {
			*cp++ = 0;
			if (*cp++ != '=') {
				error(gettxt(":36", "invalid day of month"));
				return;
			}
			if ((lp = byword(dayp, wday_names)) == NULL) {
				error(gettxt(":37", "invalid weekday name"));
				return;
			}
			rp->r_wday = lp->l_value;
		}
...
char *	name;
{
	...
	if (ferror(fp) || fclose(fp)) {
		(void) pfmt(stderr, MM_ERROR, ":40:Write error on %s: %s\n",
			fullname, strerror(errno));
		exit(1);
	}
}
...
struct zone *	zpfirst;
{
	...
	register time_t			starttime = 0, untiltime;
...
time_t	starttime;
{
	...
	if (timecnt >= TZ_MAX_TIMES) {
		error(gettxt(":41", "too many transitions?!"));
		exit(1);
	}
...
char *	abbr;
{
	...
	*/
	if (typecnt >= TZ_MAX_TYPES) {
		error(gettxt(":42", "too many local time types"));
		exit(1);
	}
	gmtoffs[i] = gmtoff;
	isdsts[i] = isdst;
...
char *	type;
{
	...
	if (result == 1 << 8)
		return FALSE;
	error(gettxt(":43", "Wild result from command execution"));
	(void) pfmt(stderr, MM_NOSTD, ":44:command was '%s', result was %d\n",
		buf, result);
	for ( ; ; )
		exit(1);
}
	...
	for ( ; ; ) {
		...
		do {
			if ((*dp = *cp++) != '"')
				...
				if (*dp != '\0')
					++dp;
				else	error(gettxt(":45", "Odd number of quotation marks"));
		} while (*cp != '\0' && *cp != '#' &&
...
register int		wantedy;
{
	...
	if (m == TM_FEBRUARY && i == 29 && !isleap(y)) {
		if (rp->r_dycode == DC_DOWLEQ)
			...
			error(gettxt(":47", "use of 2/29 in non leap-year"));
			exit(1);
		}
	}
	--i;
...
#define LDAYS_PER_WEEK	((long) DAYS_PER_WEEK)
		...
		*/
		...
		if (i < 0 || i >= len_months[isleap(y)][m]) {
			error(gettxt(":48", "no day in month matches rule"));
			exit(1);
		}
	}
	if (dayoff < 0 && !tt_signed) {
		if (wantedy == rp->r_loyear)
			return min_time;
		error(gettxt(":49", "time before zero"));
		exit(1);
	}
...
char *	string;
{
	...
	if (charcnt + i >= TZ_MAX_CHARS) {
		error(gettxt(":50", "too many, or too long, time zone abbreviations"));
		exit(1);
	}
		...
		*cp = '\0';
		if (access(name, 0) != 0) {
			...
			if (mkdir(name, 0755) != 0) {
				(void) pfmt(stderr, MM_ERROR,
					":51:Cannot create directory %s: %s\n",
					name, strerror(errno));
				return -1;
			}
		}
...
eitol(i)
{
	...
	if (i < 0 && l >= 0 || i == 0 && l != 0 || i > 0 && l <= 0) {
		(void) pfmt(stderr, MM_ERROR,
			":52:%d did not sign extend correctly\n", i);
		exit(1);
	}
	return l;
}
