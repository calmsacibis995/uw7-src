 ...
 *	All Rights Reserved
 */
#ident	"@(#)fmli:vt/vinit.c	1.30.3.5"
...
set_term_ioctl()
{
	...
	if (ioctl(0, TCGETA, &tbuf) == 0)
		...
		fd = 2;
	if (fd >= 0) {
		Erase_character = tbuf.c_cc[VERASE];
                /* this is required in order to allow function keys which
                   have the VINTR character as part of its string */
