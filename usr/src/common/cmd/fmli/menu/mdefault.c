...
#ident	"@(#)fmli:menu/mdefault.c	1.25.3.5"
...
char	*arg;
{
	...
	itemwidth = max(itemwidth, (int)strlen(ml.highlight));
	if (ml.description && *(ml.description))
    ...
    else	    /* full name fits, truncate description if needed */
	{
		...
		if (ml.description) 	
		{
		    ...
		    descwidth = max(descwidth, (int)strlen(ml.description));
		}
	    }
	}
    ...
    menuwidth = max(menuwidth, (int)strlen(title) + FIXED_TITLE - 2); /* abs f16 */
...
int   reserved_cols;
{
    ...
    if ((int)strlen(str) > max_len && max_len >= LEN_TRUNC_STR)
