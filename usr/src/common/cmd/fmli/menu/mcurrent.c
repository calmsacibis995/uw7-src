...
#ident	"@(#)fmli:menu/mcurrent.c	1.13.3.5"
...
int	high;
{
	...
	s = ml->highlight;
        /*
         *  If nothing is highlighted, then return without doing anything
         */
        if ( s == NULL )
                return;
	...
	else
	{
	    ...
	    if (s && len2 > 0) {
		wputs(" - ", NULL);
		for (tot = min(len2, (int)strlen(s)); tot > 0; tot--, len2--)
