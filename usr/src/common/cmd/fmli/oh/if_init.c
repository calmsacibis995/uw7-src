 ...
 *	All Rights Reserved
 */
#ident	"@(#)fmli:oh/if_init.c	1.36.3.6"
...
int index;
{
	...
	if (tmp && (!Init_single_tab[index].def ||
...
set_def_colors()
{
	...
	refresh_scr = FALSE;
	if (Col_active_bord && Col_inactive_bord &&
		strcmp(Col_active_bord, Col_inactive_bord) == 0)
...
set_def_status()
{
	...
	Work_col = c - (int)strlen(Work_msg);
	if (Banner) {
