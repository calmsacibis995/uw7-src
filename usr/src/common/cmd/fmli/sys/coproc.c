 ...
 *	All Rights Reserved
 */
#ident	"@(#)fmli:sys/coproc.c	1.23.3.4"
...
IOSTRUCT *errstr;		/* abs s14 */
{
    ...
    if (!(Cotable[Cur_corec].readfp) && ((Cotable[Cur_corec].readfp = fopen(Cotable[Cur_corec].r_path, "r")) == NULL))
    {
    ...
    Readfp = Cotable[Cur_corec].readfp;
    return;
}
