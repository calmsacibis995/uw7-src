...
#ident	"@(#)truss:common/cmd/truss/main.c	1.5.10.8"
#ident  "$Header$"
#include <sys/types.h>
#include <sys/times.h>
#include <sys/fstyp.h>
...
static	void	prtim( volatile timestruc_t * );
...
"\t[-m [!]faults] [-[rw] [!]fds] [-o outfile]  command | -p pid ...\n");
	...
	if (argc > 0) {		/* create the controlled process */
		...
		no_inherit = fflag ? Pset(Pr, PR_FORK) : Preset(Pr, PR_FORK);
		created = Pr->upid;
		PR = Pr;	/* for abend() */
	}
	...
	if (stdout->__cnt == 0)		/* stdio bug, compensate */
		stdout->__cnt = BUFSIZ;
	 ...
	 */
	/*
	 * Make sure that child processes do not create zombie processes.
	 */
	...
	if (fflag || ngrab > 1) {	/* multiple processes */
		...
		"%s: *** semaphores are not configured in this system ***\n",
			...
			if (semctl(semid, 0, SETVAL, semarg) == -1
			 || semctl(semid, 1, SETVAL, semarg) == -1) {
	...
	(void) Psysexit(Pr, SYS_fork1,  TRUE);
	(void) Psysexit(Pr, SYS_forkall,TRUE);
	...
	if (!isemptyset(&readfd)) {
		...
		(void) Psysexit(Pr, SYS_pread, TRUE);
	...
	if (!isemptyset(&writefd)) {
		...
		(void) Psysexit(Pr, SYS_pwrite, TRUE);
	...
	for (;;) {		/* run until termination */
		...
		if ((Pr->why.pr_flags&PR_PTRACE)
		 && !wasptraced
		 && Pr->why.pr_lwp.pr_why != PR_SYSENTRY) /* freshly ptrace()d */
			isptraced();

		if (!(Pr->why.pr_lwp.pr_flags & PR_ISTOP))
			Pr->state = PS_RUN;
		else {
			int runopt = 0;
...
void
accumulate(ap, ep, sp)
	register volatile timestruc_t *ap;
...
static void
prtim(tp)
	register volatile timestruc_t *tp;
{
...
{
	struct psinfo prpsinfo;
	char procname[100];
	int fd, rc;

	(void) sprintf(procname, "%s/%d/psinfo", procdir, Pr->upid);
	if ((fd = open(procname, O_RDONLY)) == -1)
		perror(procname);
	else if (read(fd, (char *)&prpsinfo, sizeof(prpsinfo)) == -1)
		perror("psinfo read");
	else {
...
long
fetchval(ulong_t addr)
{
	process_t *Pr = &Proc;
	long retval;
	Pread(Pr, addr, &retval, sizeof retval);
	return(retval);
}
char *
fetchstring(ulong_t addr, int maxleng)
{
	process_t *Pr = &Proc;
	int nbyte;
	int leng = 0;
	...
	int new;
{
	...
	sprintf(procname, "%s/%d/cred", procdir, Pr->upid);
	fd = open(procname, O_RDONLY);
	(void) read(fd, (char *)&cred, sizeof(cred));
	close(fd);
	...
	pid_t pid;
{
	...
	if (mypid != 0) {
		/*
		 * Parent waits for signal from child, then carries on.
		 * Use sighold() to avoid race.
		 */
	...
	descendent = TRUE;
	(void) close(Pr->asfd);		/* forget old process */
	(void) close(Pr->ctlfd);
	(void) close(Pr->statusfd);
	...
	if (Pr->why.pr_lwp.pr_why != PR_SYSEXIT ||
	    (Pr->why.pr_lwp.pr_what != SYS_fork &&
	     ...
	     Pr->why.pr_lwp.pr_what != SYS_forkall))
		(void) printf("%s\t*** Expected fork SYSEXIT\n", pname);
	...
	pid_t pid;
{
	...
	if ((Pr->state != PS_STOP || Pr->why.pr_lwp.pr_why != PR_REQUESTED) &&
	    !(Pr->why.pr_lwp.pr_flags & PR_DSTOP)) {
#if 0
		(void) fprintf(stderr,
...
bad:
	if (errno == ENOENT) {
		...
		if (access(procname, 0) != 0) { /* file doesn't exists */
	...
	pid_t pid;
{
	...
	/* open the /proc/upid/ctl file */
	(void) sprintf(procname, "%s/%d/ctl", procdir, pid);
	...
	CONST char *dir;	/* this is filthy */
{
	...
	*p++ = '0';
	...
	/* return TRUE iff we have a directory with mode 555 */
	return ((stat1.st_mode&S_IFMT) == S_IFDIR
	  && (stat1.st_mode&0777) == 0555);
}
	...
	struct stat * buf;
{
	...
	struct argdes argd[3];		/* arg descriptors for xstat() */
	register struct argdes *adp;

	if (Pr == (process_t *)NULL)	/* no subject process */
		return stat(path, buf);

	adp = &argd[0];		/* xstat version argument */
	adp->value = _STAT_VER;
	adp->object = 0;
	adp->type = AT_BYVAL;
	adp->inout = AI_INPUT;
	...
	rval = Psyscall(Pr, SYS_xstat, 3, &argd[0]);
