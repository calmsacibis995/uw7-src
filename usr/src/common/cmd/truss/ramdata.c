...
#ident	"@(#)truss:common/cmd/truss/ramdata.c	1.3.6.3"
#ident  "$Header$"
#include <stdio.h>
#include <signal.h>
#include <sys/types.h>
...
volatile struct counts * Cp = NULL; /* for counting: malloc() or shared mem */
...
volatile int timeout = FALSE;	/* set TRUE by SIGALRM catchers */
volatile int interrupt = FALSE;	/* interrupt signal was received */
volatile int sigusr1 = FALSE;	/* received SIGUSR1 (release process) */
volatile int sigsys = FALSE;	/* received SIGSYS (no semaphores or shmem) */
