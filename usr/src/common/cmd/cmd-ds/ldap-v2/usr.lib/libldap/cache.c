...
ldap_flush_cache( LDAP *ld )
{
	...
	logDebug( LDAP_LOG_CLNT, "=> ldap_flush_cache\n",0,0,0);
...
ldap_uncache_request( LDAP *ld, int msgid )
{
	logDebug( LDAP_LOG_CLNT,
	    "=> ldap_uncache_request, id(%d) ld_cache(%x)\n",
	    msgid, ld->ld_cache, 0 );

	uncache_entry_or_req( ld, NULL, msgid );
	
	logDebug( LDAP_LOG_CLNT,"<= ldap_uncache_request\n",0,0,0);
}
...
{
	logDebug( LDAP_LOG_CLNT,
	    "=> ldap_uncache_entry, dn(%s) ld_cache(%x)\n",
	    dn, ld->ld_cache, 0 );

	uncache_entry_or_req( ld, dn, 0 );

	logDebug( LDAP_LOG_CLNT,"<= ldap_uncache_entry\n",0,0,0);
}
	...
	int msgid )		/* request to uncache (if dn == NULL) */
{
	...
	if ( ld->ld_cache == NULLLDCACHE ) {
	    logDebug( LDAP_LOG_CLNT,"<= uncache_entry_or_req\n",0,0,0);
	    return;
	}
...
void
ldap_add_request_to_cache( LDAP *ld, 
	...
	BerElement *request )
{
	...
	logDebug( LDAP_LOG_CLNT,"=> ldap_add_request_to_cache\n",0,0,0);
	...
	if (( new = (LDAPMessage *) calloc( 1, sizeof(LDAPMessage) ))
	    != NULL ) {
		if (( new->lm_ber = ldap_alloc_ber_with_options( ld )) == 
		    NULLBER ) {
...
ldap_add_result_to_cache( LDAP *ld, LDAPMessage *result )
{
	...
	logDebug( LDAP_LOG_CLNT,
	    "=> ldap_add_result_to_cache, id(%d), type(%d)\n", 
	...
	if ( result->lm_msgtype != LDAP_RES_SEARCH_ENTRY &&
		 ...
		 */
		logDebug( LDAP_LOG_CLNT,
		    "<= ldap_add_result_to_cache (%s)\n",
		    "only caching search & compare operations",0,0);
	...
	if ( m != NULLMSG ) {	/* found request; add to end of chain */
		...
		if (( new = msg_dup( result )) != NULLMSG ) {
			...
			logDebug( LDAP_LOG_CLNT,
   	    "(ldap_add_result_to_cache) result added to cache request chain",
			    0,0,0);
		...
		if ( result->lm_msgtype == LDAP_RES_SEARCH_RESULT ||
			...
			if ( !keep ) {
				logDebug( LDAP_LOG_CLNT,
	"(ldap_add_result_to_cache) not caching result with error %d\n",
				    err, 0, 0 );
				...
				*mp = req;
				...
				logDebug( LDAP_LOG_CLNT,
	"(ldap_add_result_to_cache) cached result with error %d\n",
				    err, 0, 0 );
			}
		}
	} else {
	...
	logDebug( LDAP_LOG_CLNT, "<= ldap_add_result_to_cache\n",0,0,0);
}
...
ldap_check_cache( LDAP *ld, unsigned long msgtype, BerElement *request )
{
	...
	logDebug( LDAP_LOG_CLNT, "=> ldap_check_cache\n",0,0,0);
	...
	for ( m = ld->ld_cache->lc_buckets[ hash ]; m != NULLMSG; m = next ) {
		...
		logDebug( LDAP_LOG_CLNT,
		    "(ldap_check_cache) examining id(%d), type(%d)\n",
