...
#ifdef LDAP_DNS
	if (( ld->ld_options & LDAP_FLAG_DNS ) != 0 && ldap_is_dns_dn( dn )) {
...
#ifdef LDAP_DEBUG
		if ( ldapdebug_level & LDAP_LOG_CLNT ) {
			LDAPServer	*srv;
...
ldap_free_connection( LDAP *ld, LDAPConn *lc, int force, int unbind )
{
	...
	logDebug(LDAP_LOG_CLNT,"=> ldap_free_connection\n",0,0,0);
...
ldap_dump_connection( LDAP *ld, LDAPConn *lconns, int all )
{
	...
	for ( lc = lconns; lc != NULL; lc = lc->lconn_next ) {
		...
		fprintf( stderr, "  last used: %s\n",
		    ctime( (time_t *)&lc->lconn_lastused ));
		if ( !all ) {
			break;
		}
	}
...
ldap_chase_referrals( LDAP *ld, LDAPRequest *lr, char **errstrp, int *hadrefp )
{
	...
	logDebug( LDAP_LOG_CLNT, "=> ldap_chase_referrals\n",0,0,0);
	...
	*hadrefp = 0;
	...
	if ( *errstrp == NULL ) {
		logDebug( LDAP_LOG_CLNT,
		    "<= ldap_chase_referrals (unitialised structure)\n",0,0,0);
		return( 0 );
	}
	...
	if ( lr->lr_parentcnt >= ld->ld_refhoplimit ) {
		...
		logDebug(LDAP_LOG_CLNT,
   "No of referral hops is greater than limit (%d), stopping referral chase\n",
	...
	for ( ref = p; rc == 0 && ref != NULL; ref = p ) {
	    ...
	    "(ldap_chase_referrals) chasing LDAP referral(%s)\n",ref,0,0);
			...
			logDebug( LDAP_LOG_CLNT,
	    "(ldap_chase_referrals) chasing DX referral(%s)\n",ref,0,0);
