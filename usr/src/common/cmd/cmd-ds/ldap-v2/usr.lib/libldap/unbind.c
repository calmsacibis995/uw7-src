...
ldap_ld_free( LDAP *ld, int close )
{
	...
	/*
  	 * ld_passwd and ld_redn may be pointing to allocated
 	 * strings that define the orignal password and bind dn.
	 * If so, then release the memory
	 */
	if (ld->ld_passwd!=NULL)
		free(ld->ld_passwd);
	if (ld->ld_redn!=NULL)
		free(ld->ld_redn);
#endif /* LDAP_REFERRALS */
