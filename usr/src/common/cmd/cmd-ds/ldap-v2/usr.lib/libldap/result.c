	...
	LDAPMessage **result )
{
	...
	logDebug( LDAP_LOG_CLNT, "=> ldap_result\n",0,0,0 );
	...
	while ( rc == -2 ) {
		...
		if ( ld->ld_sb.sb_ber.ber_ptr >= ld->ld_sb.sb_ber.ber_end ) {
			...
			if ( rc == 0 || ( rc == -1 && (( ld->ld_options &
			    LDAP_FLAG_RESTART ) == 0 || errno != EINTR ))) {
#else
			if ( rc == -1 || rc == 0 ) {
#endif
...
#else /* !LDAP_REFERRALS */
#ifdef LDAP_DEBUG
		if ( ldapdebug_level & LDAP_LOG_NETWORK ) {
		...
		if ( lc == NULL ) {
			...
			if ( rc == -1 ) {
			    logDebug( LDAP_LOG_CLNT,
		    "(wait4msg) do_ldap_select returned -1: errno(%d)\n",
			        errno, 0, 0 );
			}
#endif
    ...
    LDAPMessage **result )
{
	...
	if ( tag != LDAP_RES_SEARCH_ENTRY ) {
		if ( ld->ld_version >= LDAP_VERSION2 &&
			    ( lr->lr_parent != NULL ||
			    ( ld->ld_options & LDAP_FLAG_REFERRALS ) != 0 )) {
			...
			if ( ber_scanf( &tmpber, "{iaa}", &lderr,
				...
				if ( lderr != LDAP_SUCCESS ) {
					...
					refer_cnt = ldap_chase_referrals( ld, 
					    lr, &lr->lr_res_error, &hadref );
				}
