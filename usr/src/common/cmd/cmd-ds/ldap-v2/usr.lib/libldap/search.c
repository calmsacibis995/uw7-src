	...
	char **attrs, int attrsonly )
{
	...
	logDebug( LDAP_LOG_CLNT, "=> ldap_search\n",0,0,0);
...
put_filter( BerElement *ber, char *str )
{
	...
	logDebug(LDAP_LOG_CLNT, "=> put_filter (%s)\n",str,0,0);
	...
	while ( *str ) {
		...
		case '(':
			...
			default:
				logDebug( LDAP_LOG_CLNT,
				    "(put_filter) simple filter found\n",
				    0, 0, 0 );
