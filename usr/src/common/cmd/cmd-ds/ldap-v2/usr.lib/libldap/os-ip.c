 ...
 */
{
	...
	logDebug( LDAP_LOG_CLNT, "=> ldap_connect_to_host: %s:%d\n",
	...
	for ( i = 0; !use_hp || ( hp->h_addr_list[ i ] != 0 ); i++ ) {
		if (( s = socket( AF_INET, SOCK_STREAM, 0 )) < 0 ) {
		   ...
		   "<= ldap_connect_to_host, could not create socket\n",
			    0,0,0);
		...
		if ( async && ioctl( s, FIONBIO, (caddr_t)&status ) == -1 ) {
			...
			logDebug( LDAP_LOG_CLNT, 
			  "(ldap_connect_to_host) FIONBIO ioctl failed on %d\n",
			  s, 0, 0 );
...
#endif /* EAGAIN */
				logDebug( LDAP_LOG_CLNT,
			"(ldap_connect_to_host) connect would block...\n",
				    0,0,0);
	...
	if ( connected ) {
		...
		logDebug( LDAP_LOG_CLNT,
		    "(ldap_connect_to_host) sd %d connected to: %s\n",
		    s, inet_ntoa( sin.sin_addr ), 0 );
	}

	logDebug( LDAP_LOG_CLNT,"<= ldap_connect_to_host\n",0,0,0);
