...
)
{
	...
	logDebug( LDAP_LOG_LDBMBE, "=> ldbm_back_add\n",0,0,0);
	...
	if ( (p = dn2entry( be, dn, &matched )) != NULL ) {
		...
		logDebug( LDAP_LOG_LDBMBE,
		   "(ldbm_back_add) Entry already exists\n",0,0,0);
		logDebug( LDAP_LOG_LDBMBE, "<= ldbm_back_add\n",0,0,0);
	...
	if ( global_schemacheck && oc_schema_check( e ) != 0 ) {
		...
		logDebug( LDAP_LOG_LDBMBE,
		    "(ldbm_back_add )entry failed schema check\n", 0, 0, 0 );
	...
	if ( (pdn = dn_parent( be, dn )) != NULL ) {
		...
		if ( (p = dn2entry( be, pdn, &matched )) == NULL ) {
			...
			logDebug( LDAP_LOG_LDBMBE, 
			    "(ldbm_back_add) parent does not exist\n",0,0,0);
		...
		if ( ! access_allowed( be, conn, op, p, "children", NULL,
			...
			logDebug( LDAP_LOG_LDBMBE,
			    "(ldbm_back_add) no access to parent\n",0,0,0);

			send_ldap_result( conn,
			    op,LDAP_INSUFFICIENT_ACCESS,"","");
		...
		if ( ! be_isroot( be, op->o_dn ) ) {
			...
			logDebug( LDAP_LOG_LDBMBE,
			    "(ldbm_back_add) no parent & not root\n",0,0,0);
	...
	if ( id2children_add( be, p, e ) != 0 ) {
		...
		logDebug( LDAP_LOG_LDBMBE, 
		    "(ldbm_back_add) id2children_add failed\n",0,0,0);

		send_ldap_result( conn,
		    op, LDAP_OPERATIONS_ERROR,"","");
	...
	if ( index_add_entry( be, e ) != 0 ) {
		...
		logDebug( LDAP_LOG_LDBMBE, 
		    "(ldbm_back_add) index_add_entry failed\n",0,0,0);
