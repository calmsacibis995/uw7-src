...
)
{
	...
	if ( (rc = ldbm_cache_delete( db, bkey )) != 0 ) {
		...
		logDebug( LDAP_LOG_LDBMBE,
		    "(idl_change_first) ldbm_delete of (%s) returns %d\n",
		    bkey.dptr, rc, 0 );
	...
	if ( (rc = idl_store( be, db, bkey, b )) != 0 ) {
		logDebug( LDAP_LOG_LDBMBE,
		    "(idl_change_first) idl_store of (%s) returns %d\n",
		    bkey.dptr, rc, 0 );
		return( rc );
	}
...
)
{
	...
	if ( (tmp = idl_fetch_one( be, db, k2 )) == NULL ) {
		...
		logDebug( LDAP_LOG_LDBMBE,
		    "(idl_insert_key) nonexistent continuation block (%s)\n",
		    k2.dptr, 0, 0 );
		return( -1 );
	}
	...
	case 0:		/* id inserted ok */
		if ( (rc = idl_store( be, db, k2, tmp )) != 0 ) {
			...
			logDebug( LDAP_LOG_LDBMBE,
			    "(idl_insert_key) idl_store of (%s) returns %d\n",
			    k2.dptr, rc, 0 );
	...
	case 3:		/* id not inserted - block is full */
		...
		if ( !first && idl->b_ids[i + 1] != NOID ) {
			...
			if ( (tmp2 = idl_fetch_one( be, db, k2 )) == NULL ) {
				...
				logDebug( LDAP_LOG_LDBMBE,
		    "(idl_insert_key) idl_fetch_one (%s) returns NULL\n",
				    k2.dptr, 0, 0 );
			...
			case 0:		/* id inserted */
				if ( rc == 2 ) {
					logDebug( LDAP_LOG_LDBMBE,
			    "(idl_insert_key) id %d already in next block\n",
					    id, 0, 0 );
				}
				free( kstr );
				idl_free( tmp );
...
)
{
	...
	if ( ALLIDS( a ) ) {
		n = idl_alloc(((struct ldbminfo *)be->be_private)->li_min_maxids);
		ni = 0;
