#ident	"@(#)acpp:common/token.c	1.73.6.8"
...
#endif

static Ext_Token *ext_token_list;
static void
alloctokens(n)
	register int n;
/*
** Given the (positive) number of Tokens to be malloc()-ed,
** this routine gets  more space, creates a linked list of
** Tokens, and assigns them to the free Token list.  It
** also adds to the permanent list of all known tokens.
** This list is maintained for the sole purpose of
** adjusting text buffer pointers after a pp_realloc
** ( see the function adjust() in buf.c ).
*/
{
	Ext_Token * xtp;
	Token *tp;
	...
	DBGASSERT(n > 0);
	xtp = (Ext_Token *)pp_malloc(sizeof(Ext_Token) * n);
	freetokens = 0;
	for (i=1; i <= n; i++)
	{
		tp = &(xtp->tk);
		...
		tp->next = freetokens;
		freetokens = tp;
		xtp->next_ext_token = ext_token_list;
		ext_token_list = xtp;
		xtp++;
	}
}
...
*/
{
	...
	do
	{
		...
		case C_BadInput:
			TKERROR(gettxt(":524","invalid token in directive"), tp);
			break;
		...
		default:if (warned == 0)
			{
				warned++;
				WARN(gettxt(":525","tokens ignored at end of directive line"));
			}
			break;
		}
...
#define GOTO_FOURCHAR	goto fourchar
...
/* Note that the user has requested that '$' be allowed in identifiers. */
{
	will_allow_dollar = 1;
}
#endif
...
int
tk_chk_if_dollar_allowed_in_id()
/* Check if we are to accept '$' in identifiers.  If yes, update the
** chtypes array and return "true".
*/
{
	if (will_allow_dollar) {
		dollar_allowed_in_id = 1;
		setchtypes((unsigned char *) "$", CH_IDENT);
		if ((pp_flags & (F_Xc|F_DOLLAR_WARNED)) == F_Xc)
		{
			WARN(gettxt(":1771","'$' in an identifier accepted as an extension"));
			pp_flags |= F_DOLLAR_WARNED;
		}
	}
	return will_allow_dollar;
}
...
/* Initialize data structures in token.c. */
{
	...
	setchtypes((unsigned char*) " \t\f\v\r", CH_WHITE); /* \r for DOS */
