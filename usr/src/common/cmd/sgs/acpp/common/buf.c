#ident	"@(#)acpp:common/buf.c	1.73.1.15"
...
long	bf_seqno = 0;		/* running line sequence number. */
...
static void	adjust(   /* char *, char*, ptrdiff_t */ );
...

static void
adjust(first, last, diff)
	char *first, *last;
	ptrdiff_t diff;
/*
** This routine adjusts by diff the character pointer
** of any Token ever placed in the allocation pool,
** if that pointer points into the old buffer.  The
** routinge only gets called after a pp_realloc that
** has actually moved the buffer.
*/
{
Ext_Token *xtp;
extern Ext_Token *get_ext_token_list();
#ifdef DEBUG
	if (DEBUG('b') > 1)
 		(void)fprintf(stderr, 
	 	"adjust() fix pointers: diff=%d\n", diff);
#endif
	for ( xtp = get_ext_token_list(); xtp; xtp = xtp->next_ext_token) {
		Token *tp;
		tp = &xtp->tk;
			if(first <= tp->ptr.string && tp->ptr.string < last) {
#ifdef DEBUG
	if (DEBUG('b') > 3)
		fprintf(stderr,
			"Old ptr x%x: %s\n",
			tp->ptr.string, tp->ptr.string);
#endif
				tp->ptr.string += diff;
#ifdef DEBUG
	if (DEBUG('b') > 3)
...
{
	buf_len = 4 * BUFSIZ;
	bf_ptr = pp_malloc((unsigned)buf_len + 1);
	bufchars['?']	= B_NEXTLINE;
...
again: 
	...
	for (;;)
	{
		...
		case '$':
			if (   dollar_allowed_in_id
			    || tk_chk_if_dollar_allowed_in_id())
				goto identifier;
			code = C_Dollar;
			goto onechar;
...
fourchar:				lp += 4;
		...
		case '!':
			...
			case '>':
				if (lp[2] == '=')
				{
					code = C_NotGreaterEqual;
					goto threechar;
				}
				code = C_NotGreaterThan;
				goto twochar;
			case '<':
				if (lp[2] == '=')
				{
				...
				if (lp[3] == '=')
				{
					code = C_NotLessGreaterEqual;
					goto fourchar;
				}
				code = C_NotLessGreater;
				goto threechar;
			}
			break;
#else /*!NO_NCEG*/
		...
		case '#':
			...
			lp++;
sharp:			if (TST(D_no|D_other|D_include))
			/*EMPTY*/	;
			else if (TST(D_null))
			{
...
threechar:				lp += 3;
		...
		case '<':
			if (TST(D_include))
			{
				...
				while (*++lp != '>')
				{
					if (*lp == '\0')
					{
						WARN(gettxt(":555", "#include <... missing '>'" ));
						*lp++ = '>';
						goto out;
					}
				}
		...
		case '/':
				...
				for (lp++; ; )
				{
					...
					if (*lp == '\0')
					{
						...
						*lp = '\n';
						...
						if (new == 0)
						{
							WARN(gettxt(":110","EOF in comment"));
							*lp = '\0';
							break;
						}
			...
			case '/':
				if ((pp_flags &
					(F_Xc|F_TWO_COMMENTS|F_2CMNTS_WARNED))
						== F_Xc
					&& !fl_stdhdr())
				{
					WARN(gettxt(":1629","//-style comments accepted as an extension"));
					pp_flags |= F_2CMNTS_WARNED;
				}
				code = C_WhiteSpace;
				lp = strchr(lp, '\0');
				if ((!(pp_flags & F_KEEP_COMMENTS))
		...
		case '\f':
		case '\v':
		case '\r': /* allow CR as white space for DOS source files */
			code = C_WhiteSpace;
			if (ISDIRECTIVE())
			{	
				...
				do {
					if (*lp == ' ' || *lp == '\t' || hadwarned)
						continue;
					WARN(gettxt(":1489","invalid white space character in directive"));
					hadwarned = 1;
...
identifier:
		...
		case '0':
			...
			if (lp[1] == 'x' || lp[1] == 'X')
			{
				...
				case '.':
					savlp = lp;
					while (tk_chtypes[(unsigned char)*++lp]
						& CH_HEX)
						;
					if (lp - cp <= 3
						|| *lp != 'p' && *lp != 'P')
...
#ifndef NO_BASE2CONST
			else if (lp[1] == 'b' || lp[1] == 'B')
			{
				lp++;
				while (*++lp == '0' || *lp == '1')
					;
...
string_lit:		 while (*++lp != '"')
			{
				...
				if (*lp == '\0')
				{
					if (TST(D_include))
					{
						WARN(gettxt(":247","newline in string literal"));
						*lp++ = '"';
						CHECKDIR();
						goto out;
					}
...
*/
{
	...
	if (mode & B_invocation)
		WARN (gettxt(":1490", "directive within macro argument list might not be portable"));
#endif
	tp = head.next;
	...
	case C_I_Constant:
		...
		if (!fl_dotisource())
			...
			if ((pp_flags & F_Xc) && !fl_stdhdr())
				WARN(gettxt(":1491", "interpreted as a #line directive" ));
		dirnum = N_lineinfo;
		break;
	...
	default:
		if (GR_TRUEPART())
			UERROR( gettxt(":1492","invalid directive") );
		else
			WARN(gettxt(":1492", "invalid directive" ));
	/* FALLTHROUGH */
	...
	if (fl_dotisource() && ! preprocess_doti())
	{
		...
		{
			UERROR(gettxt(":1493","invalid compiler control line in \".i\" file"));
			tk_rml(head.next);
			return;
		}
...
#endif
	{
		if ((NON_ANSI_DIRECTIVE(dirnum)) && (pp_flags & F_Xc) && !fl_stdhdr() )
			WARN(gettxt(":1494","directive is an upward-compatible ANSI C extension"));
		if ((mode == B_invocation) && DISALLOWED_IN_ARGLIST(dirnum))
		{
			if (GR_TRUEPART())
				UERROR(gettxt(":554",inarglistmsg));
			else
				WARN(gettxt(":554",inarglistmsg));
			tk_rml(head.next);
			return;
		}
...
fillup:				if (end - eod < BUFSIZ)
				{
					...
					end = pp_realloc(bf_ptr, 1 + (unsigned int) (buf_len += BUFSIZ));
#ifdef DEBUG
					if (DEBUG('n') > 1) 
					{
						(void)fprintf(stderr,
					...
					eod[0] = BF_SENTINEL;
					if(diff)
						adjust(bf_ptr-diff, end-diff-BUFSIZ, diff);
				}
				{
					register int n;
	
					if ((n = read((int)fileno(fl_curfile()), eod, BUFSIZ)) < 0)
						FATAL(gettxt(":1495","read error: "), "");
					else if (n == 0)
					{
						if (cur == eod)
						{
			...
			{
				UERROR(gettxt(":1496", "null character in input" ));
				cur[0] = ' ';
				cur++;
			}
			continue;
		...
		case '\n':
			...
			if (cur[-2] == '\\')
				cur -= 2;
			else if (cur[-2] == '\r') /* handle dos-style CR LF */
			{
				if (cur - beg < 3 || cur[-3] != '\\')
				{
					cur[-2] = '\0';
					goto out; /* both CR and LF tossed */
				}
				cur -= 3;
				backcrlf = 1;
			}
		...
		case '?':
			...
			if (pp_flags & F_Xt)
			{
				...
				bf_lineno += bf_newlines;
				bf_seqno  += bf_newlines;
				WARN(gettxt(":1497", "trigraph sequence replaced" ));
				bf_lineno -= bf_newlines;
				bf_seqno  -= bf_newlines;
			}
#endif
		...
		}
		{
			...
			if (DEBUG('n') > 2)
			{
				(void)fprintf(stderr,
					"nextline(): copy %d bytes\n",
					eod - cur + (backcrlf ? 3 : 2));
				fflush(stderr);
				fflush(stdout);
			}
#endif
			sav_cur = cur;
			if (backcrlf) /* ugh...remove 3 bytes */
			{
				eod -= 3;
				for ( ; cur < eod; cur++)
					cur[0] = cur[3];
...
eof:	gr_check();
	...
	if (&bf_ptr[bf_cur] < cur)
		WARN(gettxt(":1498","newline not last character in file"));
	else
...
out:;
	...
	*/
	...
	if (DEBUG('n') > 3)
	{
		...
		(void)fprintf(stderr, "\tbeg: %#lx\tbf_prev: %ld\n", beg, bf_prev);
		(void)fprintf(stderr, "\tcur: %#lx\tbf_cur:  %ld\n", cur, bf_cur);
		(void)fprintf(stderr, "\teod: %#lx\tbf_eod:  %ld\n", eod, bf_eod);
		(void)fprintf(stderr, "\tend: %#lx\tbuf_len:  %lu\n\n", end, buf_len);
		fflush(stderr);
		fflush(stdout);
	}
#endif
