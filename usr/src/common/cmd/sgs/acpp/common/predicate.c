#ident   "@(#)acpp:common/predicate.c	1.22.6.2"
/* predicate.c */
...
*/
{
	...
	if ((tp = itp) == 0)
	{
		UERROR(gettxt(":478", "empty #assert directive" ));
		return (Token *)0;
	}
	else if (tp->code != C_Identifier)
	{
		UERROR(gettxt(":479", "\"#assert identifier\" expected" ));
		return tp;
	}
	tp = (prev = tp)->next;
	...
	{
		UERROR(gettxt(":480", "\"#assert identifier (...\" expected" ));
		return itp;
	}
	else
	{
		...
		if ((prev->next = tp = tk_rmws(tp)) == 0)
		{
			UERROR(gettxt(":481", "no tokens following \"#assert name (\"" ));
			return itp;
		}
		else if (tp->code == C_RParen)
		{
			UERROR(gettxt(":482", "missing tokens between parentheses" ));
			return itp;
		}
		else
		{
			...
			for (parendepth = 1; ; tp = (prev = tp)->next)
			{
				if (tp == 0)
				{
					UERROR(gettxt(":483", "\"#assert\" missing \")\"" ));
					return itp;
				}
				switch (tp->code)
				{
...
*/
{
	...
	case C_Minus:
		...
		if ((tp = tk_rm(tp)) != 0)
		{
		WARN(gettxt(":484", "tokens after -A- are ignored" ));
			tk_rml(tp);
		}
		break;

	default:UERROR(gettxt(":485", "identifier or \"-\" expected after -A" ));
		tk_rml(tp);
		break;
	}
}
...
*/
{
	...
	{
		UERROR(gettxt(":486", "identifier expected after \"#\"" ));
		return tk_bool(1);
	}
	else if (tp->code != C_Identifier)
	{
	...
	{
		UERROR(gettxt(":487", "\"(\" expected after \"# identifier\""));
		return tk_bool(1);
	}
	else if (tp->code != C_LParen)
	{
	...
	if ((tp = tk_rmws(tk_rm(tp))) == 0)
	{
		UERROR(gettxt(":488", "tokens expected after \"# identifier (\""));
		return tk_bool(1);
	}
	else if (tp->code == C_RParen)
	{
		UERROR(gettxt(":489", "tokens expected between parentheses" ));
		return tk_bool(1);
	}
	prev = argp = tp;
...
*/
{
	...
	if ((tp = itp) == (Token *)0)
	{
		UERROR(gettxt(":491", "empty #unassert directive" ));
		return (Token *)0;
	}
	if (tp->code != C_Identifier)
	{
		UERROR( gettxt(":492","#unassert requires an identifier token" ));
		return tp;
	}
	prev = tp;
	tp = tp->next;
	...
	/*	case C_NewLine:	*/
			{
				...
				case S_lparen:
					UERROR( gettxt(":493","tokens expected after \"(\"" ));
					return itp;
	
				case S_pptoken:
					UERROR(gettxt(":494", "\")\" expected" ));
					return itp;
				}
			}
			/*FALLTHRU*/
		...
		case C_RParen:
			...
			{
			case S_lparen:
				UERROR(gettxt(":495", "empty predicate argument" ));
				return itp;
		...
		default:if (state == S_predicate)
			{
				UERROR(gettxt(":496", "\"(\" expected after first identifier" ));
				return itp;
			}
