#ident	"@(#)acpp:common/syms.c	1.82.1.5"
...
static Token	verstk;			/* replacement list for __STDC_VERSION__ 	*/
static char	timearray[] = "\"        \"";	/* `"hh:mm:ss"' */
static Token	timetk;			/* replacement list for __TIME__	*/
#ifdef VERSION_ID
static Token	version_id;		/* token list for __SCO_VERSION */
static Token	version_value;		/* replacement list for __SCO_VERSION__ */
#endif
...
static Macro pdmac[] = {
	...
	{"__STDC_VERSION__", 16, OBJECT_LIKE, &verstk, (Macro *)0, M__STDCVERSION|M_MANIFEST },
	...
	else if ( !(ST_ISUNDEF(mp)) )
	{
		if (ST_ISCONSTRAINED(mp))
		{
			if (ST_ISPREDEFINED(mp))
				WARN(gettxt(":497", "ANSI C predefined macro cannot be redefined" ));
			else
			{
				COMMENT(ST_ISUNARYOP(mp));
				WARN(gettxt(":498", "cannot define \"defined\"" ));
			}
			COMMENT(tp == inputp);
			tk_rml(tp);
			return;
		...
		TKWARN(gettxt(":499","macro redefined"), inputp);
		tk_rml(mp->replist);
		tp = inputp;
	...
	register Token *tp;
{
	...
	if (ST_ISCONSTRAINED(mp))
	{
		if ( (pp_flags & F_Xt) && (mp->flags & M__STDC) )
			WARN(gettxt(":500", "undefining __STDC__" ));
		else {
			if (ST_ISPREDEFINED(mp))
				WARN(gettxt(":501","ANSI C predefined macro cannot be undefined" ));
			else
			{
				COMMENT(ST_ISUNARYOP(mp));
				WARN(gettxt(":502", "cannot undefine \"defined\"" ));
			}
			return;
		}
	}
...
*/
{
	...
	*/
	...
	if (tp->code != C_Identifier)
		UERROR(gettxt(":503", "-D option argument not an identifier" ));
	else
	{
		...
		else
		{
			...
			{
				WARN(gettxt(":504", "-D option argument not followed by \"=\"" ));
				tk_rml(tp);
				name->next = tk_bool(1);
			}
		}
...
*/
{
	...
	tp = tk_tokenize(arg);
	if (tp->code != C_Identifier)
		UERROR(gettxt(":505", "-U option argument not an identifier" ));
	else
	{
		...
		{
			WARN(gettxt(":506", "tokens ignored after \"-U{identifier}\"" ));
			tk_rml(tp);
		}
	}
}
...
#	define	S_furtherop	0x20
	...
	*/
	if ((tp = list) == 0)
	{
		UERROR(gettxt(":507","empty #define directive line"));
		return (0);
	}
	if (tp->code != C_Identifier)
	{
		UERROR(gettxt(":508","#define requires macro name"));
		return (tp);
	}
	len = tp->rlen;
	prev = tp;
	...
	*/
	...
	else
	{
		...
		for (;;)
		{
			...
			if (tp == 0)
			{
				UERROR(gettxt(":509","incomplete #define macro parameter list"));
				return (list);
			}
		again:;
			switch (tp->code)
			{
			default:
				TKERROR(gettxt(":510","invalid token in #define macro parameters"), tp);
				return (list);
			...
			case C_Identifier:
				...
				{
				badlist:;
					UERROR(gettxt(":511","syntax error in macro parameters"));
					return (list);
				}
				SET(S_gotformal);
				CLR(S_gotrparen);
				...
				for (i=0;  i<nformals;  i++ ) {
				    if (tp->rlen == fp->rlen
				        ...
				        TKWARN(gettxt(":512","duplicate formal parameter"),tp);
					break;
				    }
				    fp = fp->next;
				}
	...
	*/
	...
	while (tp != 0)
	{
		...
		CASE(C_Sharp)
			...
			{
				WARN(gettxt(":513","bad use of \"#\" or \"##\" in macro #define"));
			}
		...
		CASE(C_Paste)
			...
			if (fp == 0)
				WARN(gettxt(":514","cannot begin macro replacement with \"##\""));
		...
		CASE(C_Identifier)	/* check for formal parameter */
		{
			...
			if (i < nformals) {
				...
				UERROR(gettxt(":515","non-formal identifier follows \"#\" in #define"));
			CLR(S_gotsharp|S_gotpaste);
			break;
		}
		CASE(C_WhiteSpace)
			...
			else
			{
				    ...
				    ) )
				{
					...
					if (pp_flags & F_Xt)
					{
						WARN(gettxt(":516","comment is replaced by \"##\""));
						tp->ptr.string = "##";
						tp->rlen = 2;
						tp->code = C_Paste;
						tp->aux.ws = 0;
						...
						WARN(gettxt(":517", "comment does not concatenate tokens" ));
				}
#endif
				SET(S_zapwhite);
				tp->ptr.string = " ";
...
*/
			...
			no = gettxt(":518","no ");
			body = gettxt(":519","macro replacement within a "); /*WARN*/
			str = gettxt(":115","string literal");
			chr = gettxt(":116","character constant");
			if (nformals > 0)
			{
		...
		if (TST(S_gotsharp))
		{
			UERROR(gettxt(":520","\"#\" must be followed by formal identifier in #define"));
	...
	if (TST(S_gotpaste|S_gotsharp))
		WARN(gettxt(":521","cannot end macro replacement with \"#\" or \"##\""));
	if (fp != 0)
	{
...
*/
{
	...
	if (predefined != 0 && nodefaults == 0 )
	{
		...
		do
		{
				/* Enter all macros beginning with
				** "__" regardless of pp_flags setting.
				** This  means __USLC__ will always get
				** predefined if listed in PREDMACROS.
				*/
			if(pp_flags & F_Xc) {
				if(strlen(*cpp) < 2
					|| (*cpp)[0] != '_' || (*cpp)[1] != '_')
	...
	if (nodefaults == 0)
	{
		version_value.ptr.string = VERSION_ID;
		version_value.rlen = sizeof(VERSION_ID)-1;
		version_value.code = C_I_Constant;
		TK_SETREPL(&version_value);
		version_id.ptr.string = "__SCO_VERSION__";
		version_id.rlen = sizeof("__SCO_VERSION__")-1;
		version_id.aux.argno = OBJECT_LIKE;
		version_id.code = C_Manifest;
		version_id.next = &version_value;
		enter(&version_id);
	}
#endif
	myclock = time(0);
	...
	TK_SETREPL(&linetk);
	verstk.ptr.string = "199409L";
	verstk.rlen = 7;
	verstk.code = C_I_Constant;
	TK_SETREPL(&verstk);
	for (mp = pdmac; mp < &pdmac[sizeof(pdmac)/sizeof(pdmac[0])]; mp++)
	{
...
/* Prints out symbol table performance information. */
{
	...
	if ((perfile = fopen("st_perf.out", "a")) == NULL)
		FATAL( gettxt(":734","cannot open st_perf.out") );
	for (nmacros = 0, hval = 0; hval<HASHTABSZ; hval++)
	{
		nmacros += (n = macro_cnt(hashtab[hval]));
		if (n >= bszlimit)
			FATAL( gettxt(":735","bucketsz[] too small" ));
		else
			bucketsz[n]++;
	}
...
*/
{
	...
	if (tp == 0)
	{
		UERROR(gettxt(":522","empty #undef directive, identifier expected"));
		return (Token *)0;
	}
	if (tp->code == C_Identifier)
	{
		...
		UERROR(gettxt(":523", "identifier expected after #undef" ));
	return tp;
}
	
void
