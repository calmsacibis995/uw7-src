#ident	"@(#)acpp:common/expand.c	1.99.1.4"
/* macro expansion */
				...
				if (tp->ptr.string[0] == '\n')
				{
					...
					if ((eflags & E_directive) == 0
					 && ((tp->next = bf_tokenize(gotlparen ?
				 	 ...
				 	 && gotlparen != 0)
					{
						TKERROR(gettxt(":1499","EOF in argument list of macro"), itp);
						goto bad_arglist;
					}
				}
				continue;
...
*/
{
	...
	for(state = S_defined; ; tp = (pretp = tp)->next)
	{
		...
		case C_Marker:
			...
			/*FALLTHRU*/
error:		default:UERROR(gettxt(":1500", "invalid use of \"defined\" operator" ));
			isdefined = 1;
			tk_rml(inputp);
			tp = (Token *) 0;
			goto errout;
			...
			if (chkargs(tp))	/* any arguments? */
			{
				...
				if ((ax != mp->nformals) && !(mp->nformals == 0
		 	 	&& ax == 1 && argbase[0] == (Token *)0))
					WARN(gettxt(":433", "argument mismatch" ));
				warned_recursion = 0;
				pretp->next = replace(tp, mp);
				initax();
...
ret:
	...
	if ( (nformals != 0) && tokenless && (pp_flags  & F_Xc) )
		WARN(gettxt(":434","token-less macro argument"));
	return tp;
not_invocation:
...
#define MAX_RECURSION 2000
	...
	if (++recursion_level > MAX_RECURSION)
		FATAL(gettxt(":435","macro nesting too deep"), (char *)0);
#ifdef TRANSITION
	if (st_recurse(mp) && !warned_recursion)
	{
	        WARN(gettxt(":436","macro recursion"), (char *)0);
		warned_recursion = 1;
        }
#endif
...
*/
{
	...
	for((eos = &head)->next = 0; tp != 0; )
	{
		...
		{
			if ((eos->next = tk_cpl(argbase[tp->aux.argno])) == 0)
			{
				tp = tk_rm(tp); /* skip paste */
			}
		}
		else if (tp->code == C_Formal)
			eos->next = expand(tk_cpl(argbase[tp->aux.argno]));
		else if (tp->code == C_UnexpFormal)
		{
	...
	if (DEBUG('e') > 0)
	{
		...
		(void)fprintf(stderr,"subst<%3d%c> returns",
		 subdepth, mark[((subarray[(subdepth%DEPTH)])-1)%10] );
		tk_prl(head.next);
	}
#endif
ret:
