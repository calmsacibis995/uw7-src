...
void callopt(){
	...
	if( (finput=fopen(pfxname(filepfx, TEMPNAME),"r")) == NULL )
...
static void aoutput(){ /* this version is for C */
	...
	arout(pfxname(sympfx, "act"), amem, (maxa-amem)+1);
	arout(pfxname(sympfx, "pact"), indgo, nstate);
	arout(pfxname(sympfx, "pgo"), pgo, nnonter+1);
...
static void arout( s, v, n ) char *s; int *v, n; {
	...
	(void) fprintf(ftable, "YYCONST %stabelem %s[]={\n", sympfx, s);
	for( i=0; i<n; ){
