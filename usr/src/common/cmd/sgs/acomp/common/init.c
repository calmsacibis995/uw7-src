#ident	"@(#)acomp:common/init.c	55.1.1.15"
/* init.c */

#include "p1.h"
#include <stdio.h>			/* for debug output */
#include <unistd.h>			/* for debug output */
...
*/
{
    ...
    */
	...
	cerror(gettxt(":231","confused in_end(), level %d"), in_sp);
	...
	*/
	...
	break;
    default:
	cerror(gettxt(":232","can't handle case %d in in_end()"), in_kind);
    }
...
*/
{
	...
	case TY_UCHAR:
	case TY_SCHAR:
	    if ((p->sid & TR_ST_WIDE) == 0) /* cannot be wide */
		return 1;
	    break;
	default:
	    if (TY_EQTYPE(t, T_wchar_t) > 0 && p->sid & TR_ST_WIDE)
		return 1;
	    break;
	}
    }
...
warn:;
	    if (II(0).in_lcflag == 0)
		WERROR(gettxt(":233","{}-enclosed initializer required"));
	break;
    }
	...
	if (   II(0).in_lcflag != 0
	    ...
	    */
		...
		UERROR(gettxt(":234","struct/union-valued initializer required"));
		t1free(s);
		t1free(p);
	    }
	    else {
		...
		*/
		if (TY_ISINTTYPE(stype) && TY_ISNUMTYPE(p->type)) {
			...
			*new = *p;
			...
			p->left = new;
			p->flags = 0;
			p->type = TY_ISUNSIGNED(stype) ? TY_ULLONG : TY_LLONG;
			...
			*/
			...
			in_chkval(p, stype, (int)TY_SIZE(stype), 0, (char *)0);
		}
#ifdef LINT
    		ln_setgoal(EFF);
#endif
...
/* Saw initializer tree p. */
{
    ...
    if (p == ND1NIL)
	cerror(gettxt(":235","in_init() called with NIL"));
    ...
    */
	...
	SIZE asize = II(in_sp-1).in_nelem; /* length of array */
	BITOFF stsize = p->c.size;	/* length of initializing string */
	...
	*/
	...
	if (asize != TY_NULLDIM) {
	    ...
	    */
	    ...
	    if (stsize > asize) {
		WERROR(gettxt(":1621","%ld extra byte(s) in string literal initializer ignored"),
			stsize-asize);
		stsize = asize;
	    }
	}
	...
	*/
	if (TY_ISINTTYPE(t) && TY_ISNUMTYPE(p->type)) {
		...
		p = tr_conv(p, (T1WORD) (TY_ISUNSIGNED(t) ? TY_ULLONG : TY_LLONG),
			gettxt(":737","can't convert??"), TR_CAST);
	    p = op_init(p);
	    if (p->op == ICON && p->sid == ND_NOSYMBOL)
		in_chkval(p, t, (int)II(in_sp).in_size, isfield, (char *)0);
	}
	else {
	    p = tr_conv(p, t, gettxt(":738","initialization type mismatch"), TR_ASSIGN);
	    p = op_init(p);
	}
	switch( p->op ) {
	case ICON:
	...
	case EH_OFFSET:
	    ...
	    break;
	case PLUS:
	    /* Special check for initializations of the form:

			char *ptr = &("Message String"[7]);
	    */
	    ...
	    /*FALLTHRU*/
	default:
	    UERROR(gettxt(":238","non-constant initializer: op \"%s\""), opst[p->op]);
	}
...
int isfield;
char *mesg;
/* Check whether an initializer (or the equivalent) fits in the space
** available (len bits).  p points to an unnamed ICON node.  totype is
** the type of the result.  Essentially, do the checking as if there
** were an assignment from p to a len-bit object of type totype.  Warn
** if the value doesn't fit, based on how strict we're being.
*/
{
    int signedcheck, plainchar;
    NumSize szinfo;

    num_size(&p->c.ival, &szinfo);
    plainchar = 0;
    if ((signedcheck = TY_ISSIGNED(totype)) < 0) {
	/* Change some "plain"s to unsigned as appropriate. */
#ifndef SIGNEDFIELDS
	if (isfield)
	    signedcheck = 0;
#endif
	if (!isfield && TY_EQTYPE(totype, TY_CHAR) > 0) {
	    plainchar = 1;
	    if (!chars_signed)
		signedcheck = 0;
	}
    ...
    if (signedcheck) {
	if (szinfo.sz_nsbit <= len)
	    return;
	if (plainchar) {
	    /* When the target is a plain char, don't complain when we're
	    ** not verbose or strict (-Xc) and the value would have fit in
	    ** and unsigned char.  This covers a LOT of X code that has
	    ** large initialized bitmaps based on plain char arrays.
	    */
	    if (!verbose && !(version & V_STD_C) && szinfo.sz_nubit <= len)
		return;
	}
    }
    else {
	if (szinfo.sz_nubit <= len) {
	    /* When the target is a full unsigned long long, we can't tell
	    ** whether the value fits just fine or doesn't really fit
	    ** because the source was a negative signed value.  So, we warn
	    ** regardless of fitting when the source is signed and the
	    ** value looks negative and when either verbose or strict (-Xc).
	    */
	    if (!verbose && !(version & V_STD_C)
		|| !TY_ISSIGNED(p->type)
		|| num_scompare(&p->c.ival, &num_0) >= 0) return;
	}
	else if (!verbose && !(version & V_STD_C)) {
	    /* When we're not verbose and not strict (-Xc), don't complain
	    ** if the source is not unsigned and source and target have the
	    ** same size.  There is no parallel to the above "plainchar" case.
	    */
	    if (!TY_ISUNSIGNED(p->type) && TY_SIZE(totype) == TY_SIZE(p->type))
		return;
	}
    }
    if (mesg == 0)
	mesg = gettxt(":237", "initializer does not fit"); /*ERROR*/
    WERROR(gettxt(":359", "%s: %s"), mesg,
	(*(TY_ISUNSIGNED(p->type) ? &num_tohex : &num_tosdec))(&p->c.ival));
}
...
/* Exit one level of initialization. */
{
	...
	cerror(gettxt(":241","in_pop():  too much popping"));
...
*/
{
	...
	if (in_sp == IN_EMPTY)
	    cerror(gettxt(":242","confused stack in in_nextelem()"));
	...
	if (ttype == TY_STRUCT || ttype == TY_UNION) {
	    ...
	    */
		...
		UERROR(gettxt(":243","too many struct/union initializers"));
		...
		*/
		...
		cerror(gettxt(":244","insane structure member in nextelem()"));
		...
		&& II(in_sp).in_celem >= II(in_sp).in_nelem
	        ...
	        UERROR(gettxt(":245","too many array initializers"));
	    ...
	    */
		...
		UERROR(gettxt(":246","too many initializers for scalar"));
	    break;			/* hit scalar */
	}
    } while (!onelevel);
