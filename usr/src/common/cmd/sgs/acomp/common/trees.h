#ident	"@(#)acomp:common/trees.h	52.16"
/* trees.h */
...
extern int tr_truncate();	/* pass value through "knothole" */
extern ND1 * tr_evalconexp();
extern ND1 * tr_sizeof();
extern ND1 * tr_copy();
extern ND1 * tr_return();
extern ND1 * tr_newnode();
extern ND1 * tr_cbranch();
...
#define FF_NOEXCEPT	0100000	/* relational does not permit exceptions */
#define FF_UNDEFNAME	0200000	/* NAME node was undefined symbol */

/* Bits for STRING node (in "sid"). */
#define	TR_ST_WIDE	0x1	/* Set for wide string, else 0. */
#define	TR_ST_ICON	0x2	/* Set if node should be treated as ICON
				** upon code generation, 0 if as NAME.
				*/
