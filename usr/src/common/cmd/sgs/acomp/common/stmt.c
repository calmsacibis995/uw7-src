#ident	"@(#)acomp:common/stmt.c	55.4.2.18"
/* stmt.c */
...
*/
{
    ...
    if (sm_sp <= 0)
	cerror(gettxt(":300","block stack underflow, %d"), sm_sp);
	...
	if (cur_default == SM_NOLABEL)
	    ...
	    cerror(gettxt(":301","overwriting default"));
    }
#ifdef LINT
    /*
...
*/
{
    if (!sm_reached) {
	if (isloop)
	    WERROR(gettxt(":302","loop not entered at top"));
	else
#ifdef LINT
	    BWERROR(9);
#else
	    WERROR(gettxt(":303","statement not reached"));
#endif
...
*/
{
    ...
    if (sm_sp != 0)
	cerror(gettxt(":304","bad sm_sp in sm_begf(), %d"), sm_sp);
    ...
    cg_setcurfunc(sid);
#ifndef LINT
    if(!get_function_fixed_frame_flag())
	SY_FLAGS(sid) |= SY_NO_FIXED_FRAME;
#endif
...
{
    if (sm_sp != 0)
	cerror(gettxt(":305","bad sm_sp in sm_endf(), %d"), sm_sp);
...
*/
{
	...
	if (flag && (SY_FLAGS(sid) & SY_DEFINED))
	    UERROR(gettxt(":306","label redefined: %s"), s);
    }
...
*/
{
    ...
    */
    ...
    return -1;
#else /*!LINT*/
    /* Following is a hack to catch controlling expressions
    ** of the form st, st a struct.  The bizarre way op_optim
    ** treats these causes them to be converted to pointer
    ** nodes, so the check in tr_cbranch succeeds incorrectly.
    ** Since we have to leave the check in tr_cbranch, we only
    ** add a very specialized check here, otherwise we will
    ** get multiple error messages for the same error.
    */
    if (TY_ISSU(p->type) && !TY_ISSCALAR(p->type)) 
	WERROR(gettxt(":362","controlling expressions must have scalar type"));
    p = op_optim(p);

    /* Figure out if we know what's going to happen in the loop:
    ** is there a constant controlling expression?
    ** NOTE:  With the introduction of weak symbols, it is possible
    ** that the address of a NAME is zero.
    */
    if (p->op == ICON) {
	conval = num_ucompare(&p->c.ival, &num_0) != 0;
	if (p->sid != ND_NOSYMBOL && conval == 0)
	    conval = -1;
    } else if (p->op == FCON)
	conval = !FP_ISZERO(p->c.fval);
...
*/
{
	...
	*/
	...
	case LL_DUP:
		...
		** (We have to wait until now in case tr_cbranch() saw the
		** error.)
		*/
	...
	default:
	    cerror(gettxt(":307","bad code %d in sm_lp_start()"), codetype);
	}
...
*/
{
    ...
    */
    ...
    {
	WERROR(gettxt(":308","end-of-loop code not reached"));
	sm_reached = 1;
    }
	...
	*/
	if (cur_control->op != CBRANCH)
	    cerror(gettxt(":309","confused sm_lp_end()"));

	cur_control->c.label = cur_top;	/* want to branch to loop top */
...
/* Generate end of do-while loop with controlling expression p. */
{
    ...
    */
	...
	cur_control = tr_cbranch(p, cur_top);
	if (nerrors != 0)
	    cur_control = ND1NIL;
    }
...
*/
{
	...
	UERROR(gettxt(":310","\"break\" outside loop or switch"));
    else if (wasreached) {
	sm_gengoto(cur_break);
...
/* Generate jump to continue label. */
{
	...
	UERROR(gettxt(":311","\"continue\" outside loop"));
    return;
}
...
/* Return (optional) expression p from current function. */
{
	...
	WERROR(gettxt(":312","function expects to return value: %s"), SY_NAME(sm_curfunc));
...
*/
{
	...
	*/
	...
	if (TY_ISFPTYPE(t))
	    WERROR(gettxt(":394",mesg));		/* just warn and convert value */
	else
	    UERROR(gettxt(":394",mesg));		/* can't handle */
	change = 1;
    }

    if (change) {
	p = tr_conv(p, TY_INT, gettxt(":739","invalid switch expression type"), TR_CAST);
	t = TY_INT;
    }
...
/* Add case to current switch statement. */
{
	...
	UERROR(gettxt(":313","\"case\" outside switch"));
	return;
    }
#ifdef LINT
    ...
    */
	...
	/*
	** There are two potential size or signedness problems with
	** case expressions:
	**  (1) The controlling expression was widened to int-size
	**	and thus a case label value that cannot fit in the
	**	original smaller size cannot be reached.
	**  (2) This case label expression has a value that cannot
	**	be represented in the controlling expression's type.
	** Item (1) is always checked.  Item (2) is checked when in
	** verbose mode or when this expression's type is bigger
	** than the controlling expression's.
	*/
	if (TY_SIZE(p->type) > TY_SIZE(cur_swtype) || verbose)
	    p2 = tr_copy(p);
	p = tr_evalconexp(tr_conv(p, cur_swtype,
		gettxt(":740","sm_sw_case1??"), TR_CAST));
	if (TY_SIZE(cur_swtype) > TY_SIZE(cur_swotype)) {
	    in_chkval(p, cur_swotype, (int)TY_SIZE(cur_swotype), 0,
		gettxt(":314", "unreachable case label")); /*ERROR*/
	    if (p2)
		t1free(p2);
	} else if (p2) {
	    p2 = tr_evalconexp(p2);
	    in_chkval(p2, cur_swtype, (int)TY_SIZE(cur_swtype), 0,
		gettxt(":315", "case label affected by conversion"));
	    t1free(p2);
	}
	cg_deflab(lab);
	cg_swcase(&p->c.ival, lab); /* cgstuff checks for duplicates */
    } else {
	/* Bogus expression type.  Disregard case but pretend following
	** code is reachable.  Free the expression tree.
	*/
	UERROR(gettxt(":316", "non-integral case expression"));
    }
    t1free(p);
    sm_reached = 1;
}
...
{
    if (cur_swtype == TY_NONE)
	UERROR(gettxt(":317","\"default\" outside switch"));
    else if (cur_default != SM_NOLABEL)
	UERROR(gettxt(":318","duplicate \"default\" in switch"));
    else
