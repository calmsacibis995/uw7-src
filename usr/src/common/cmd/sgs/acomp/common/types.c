#ident	"@(#)acomp:common/types.c	52.61"
/* types.c */
...
*/
{
	...
	TT(newt, ts_ptr)->ts_dbscope = 0;	/* debugging info scope */
	if (t < TY_NBASE)
	    ty_ptrs[t] = newt;
	else
...
*/
{
    ...
    tp->ts_dbscope = 0;			/* scope of debugging info */
...
#ifndef SAME_ERETTYPE
#define SAME_ERETTYPE 0
#elif defined(LINT)
#undef SAME_ERETTYPE
#define SAME_ERETTYPE 0
#endif

int same_erettype = SAME_ERETTYPE; /* also set by -1e */
...
/* Create function-returning type t. */
{
    ...
    tp->ts_dbscope = 0;			/* no debugging scope yet */
...
** that qualifies the underlying type.  Otherwise, create a newly 
** qualified type.
**
...
*/
{
	...
	*/
    ...
    TY_DBSCOPE(newt) = 0;		/* no debugging info scope */
    return( newt );
}
...
static int sizeofhidden; /* set by ty_size() for ty_chksize() */
...
*/
{
	...
	case TY_ARY:
		...
		size = ty_sizesub(t);
	    if (TT(t, ts_ary)->ts_spec & TY_THIDDEN)
		sizeofhidden = 1;
	    break;
	}
    }
...
#define XX(l,r) (l*TT_LASTBASE+r)
	    ...
	    case XX(	TY_LLONG,	TY_SLLONG ):
	    case XX(	TY_SLLONG,	TY_LLONG ):
		return( 1 );
...
/* Set number of elements for array t, adjust final array size. */
{
	...
	UERROR(gettxt(":155","array too big"));
	elemsize = 1;
    }
...
*/
{
    ...
    case TY_VOID:
	if (flags & TY_CVOID) {
	    ULERROR(lineno, gettxt(":374","cannot have void object: %s"), name);
	...
	if (TY_SIZE(t) == 0) {
		...
		WLERROR(lineno, gettxt(":158","null dimension: %s"), name);

	    if (flags & TY_CSUE) {
		while (TY_TYPE(t) == TY_ARY)
		    t = TY_DECREF(t);
		if (TY_ISSUE(t) && !TY_HASLIST(t)) {
		    ULERROR(lineno, gettxt(":375","incomplete struct/union/enum %s: %s"),
	...
	if (sizeofhidden)
	    WLERROR(lineno, gettxt(":1575","using out of scope dimension: %s"), name);
	break;
    } /* end switch */
...
*/
{
	...
	if (tc == tdecl && nelem == TY_NELEM(tl)) {
	    if (hiding && tc == tdecr && nelem == TY_NELEM(tr))
		hiding = 0;
	    tc = tl;
	}
