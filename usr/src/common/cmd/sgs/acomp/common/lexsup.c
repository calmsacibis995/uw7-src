#ident	"@(#)acomp:common/lexsup.c	1.20"
/* lexsup.c */
...
static unsigned long dochar();
static unsigned long doesc();
    ...
    */
	...
	if (htp->lexcode == L_ASM) {
		...
		if (!warned && !in_system_header()) {
		    WERROR(gettxt(":253","__asm is an extension of ANSI C"));
		    warned = 1;
		}
	    }
	}
...
#define	WCSIZE (sizeof(wchar_t))
	...
	if (*s == '\\') {
	    unsigned long escchar;

	    ++s;
	    escchar = doesc(&s);
	    if (iswide) {
		clen = WCSIZE;
		if (escchar > T_UWCHAR_MAX)
		    WERROR(gettxt(":254","character escape does not fit in wide character"));
	    }
	    else {
		clen = 1;
		if (escchar > T_UCHAR_MAX)
		    WERROR(gettxt(":255","character escape does not fit in character"));
	    }
	    c = escchar;
	}
	else if (iswide) {
	...
	if ((newlen = outpos + clen+WCSIZE) >= strbufl) {
		...
		if (tbuf == 0)
		    cerror(gettxt(":257","out of space in doescape()"));
...
*/
{
    unsigned long c = *(unsigned char *)*ps;
	...
	if (version & V_CI4_1)
	    WERROR(gettxt(":258","\\a is ANSI C \"alert\" character"));
	c = C_ALERT;
	break;
    case 'b':	c = '\b'; break;
    case 'f':	c = '\f'; break;
    ...
    default:
    if (isprint(c))
	WERROR(gettxt(":259","dubious escape: \\%c"), c);
    else
	WERROR(gettxt(":260","dubious escape: \\<%#x>"), c);
    }
    return( c );
}
...
*/
{
	...
	*/
    ...
    if (base == 16) {
	if (version & V_CI4_1)
	    WERROR(gettxt(":261","\\x is ANSI C hex escape"));
	else if (nchars == 0)		/* got no hex digits */
	    WERROR(gettxt(":262","no hex digits follow \\x"));
	if (nchars == 0)		/* got no hex digits */
	    retval = 'x';		/* treat as character x */
	if (toobig)
	    WERROR(gettxt(":263","overflow in hex escape"));
    }
    return( retval );
}
...
*/
{
    ...
    default:
	cerror(gettxt(":264","unknown wchar_t size"));
	/*NOTREACHED*/
