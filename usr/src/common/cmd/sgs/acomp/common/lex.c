...
#define TKC(x) case TK_ENUMNO(x)
    ...
    TKC(C_I_Constant):
			if (!insharp)
			    yylval.nodep = tr_int_const(tk->LXCH, tk->rlen);
			return( L_INT_CONST );
    TKC(C_F_Constant):
			if (!insharp) {
...
string_literal: ;
    ...
    TKC(C_NotGreaterThan):yylval.intval = NGT;	return( L_RELOP );
    TKC(C_GreaterEqual):yylval.intval = GE;	return( L_RELOP );
    TKC(C_NotGreaterEqual):yylval.intval = NGE;	return( L_RELOP );
    TKC(C_LessThan):	yylval.intval = LT;	return( L_RELOP );
    TKC(C_NotLessThan):	yylval.intval = NLT;	return( L_RELOP );
    TKC(C_LessEqual):	yylval.intval = LE;	return( L_RELOP );
    TKC(C_NotLessEqual):yylval.intval = NLE;	return( L_RELOP );
    TKC(C_LessGreater):	yylval.intval = LG;	return( L_RELOP );
    TKC(C_NotLessGreater):yylval.intval = NLG;	return( L_RELOP );
    TKC(C_LessGreaterEqual):yylval.intval = LGE;return( L_RELOP );
    TKC(C_NotLessGreaterEqual):yylval.intval = NLGE;return( L_RELOP );
    ...
    TKC(C_LBrace):	db_curline = PP_CURSEQNO(); /* for start of function */
			lx_getcur = 1;
			return( L_LC );
...
#define	OUTLEN 20			/* maximum amount to show in msg. */
	...
	if (firstchar == 'L')
	    firstchar = tk->LXCH[1];
	if (firstchar == '"') {
	    UERROR(gettxt(":247","newline in string literal"));
	    skiptonl();
	    if (!insharp)
		goto string_literal;
	}
	else if (firstchar == '\'') {
	    UERROR(gettxt(":248","newline in character constant"));
	    skiptonl();
	    if (!insharp)
		...
		if (isprint(firstchar))
		    UERROR(gettxt(":118","invalid source character: '%c'"), firstchar);
		else
		    UERROR(gettxt(":119","invalid source character: <%#x>"), firstchar);
	    }
	}
	else {
	    /*FALLTHRU*/
	...
	*/
	    UERROR(gettxt(":249","invalid token: %.*s%s"),
...
*/
{
	...
	*/
	if (! warned && (oldiswide ^ iswide) != 0) {
	    if (oldiswide)
		WERROR(gettxt(":381",mesg));
	    else
		UERROR(gettxt(":381",mesg));
	    warned = 1;
	}
    }
...
/* Done fetching characters. */
{
	...
	cerror(gettxt(":250","lx_e_getc() out of sync"));
    return;
}
...
*/
{
	...
	cerror(gettxt(":251","lx_ungetc() out of sync"));
...
{
    if (curtoken)
	cerror(gettxt(":252","non-null curtoken in lx_s_sharp()"));
