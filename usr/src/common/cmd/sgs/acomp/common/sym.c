#ident	"@(#)acomp:common/sym.c	55.1.4.25"
/* sym.c */
...
/*	puts("allocating new string heap"); */
	retval =
        ...
        if ( retval == 0 ) cerror(gettxt(":319","st_save() out of memory"));
...
/*	    puts("allocating new string hash table"); */
		...
		== 0)
		cerror(gettxt(":320","st_lookup() out of memory"));
	}
    }
    /*NOTREACHED*/
}
...
*/
{
    ...
    */
    ...
    sp->sy_name = name;
#ifdef LINT
    sp->sy_line_id.sy_lineno = er_getline();
    sp->sy_file = st_lookup(er_curname());
#else
    sp->sy_line_id.sy_lineno = PP_CURSEQNO();
#endif
...
*/
{
	...
	*/
	...
	if (sp->sy_type == TY_NONE)
	    cerror(gettxt(":321","%s undefined"), sp->sy_name);
	/* unlink from hash list and scope list */
	sy_scopehead = sp->sy_scopelink;
	if (*hashslot != i)
	    cerror(gettxt(":322","removing wrong entry"));
	*hashslot = sp->sy_next;
#ifdef	SY_1STNONLEV0
	if (i == sy_1stnonlev0)
	...
	if (   lev == SL_EXTERN
	    ...
	    WERROR(gettxt(":323","static function called but not defined: %s()"), sp->sy_name);
    ...
    */
	...
	for (i = sy_labtab; i != SY_NOSYM; i = SY(i).sy_next) {
		...
		if (!( SY(i).sy_flags & SY_DEFINED))
			UERROR(gettxt(":324","undefined label: %s"), SY(i).sy_name);
		else if (verbose && !(SY(i).sy_flags & SY_REF))
			WERROR(gettxt(":325","unused label: %s"), SY(i).sy_name); 
	    }
	    last = i;
	}
	if (sy_labtab) {
...
#ifdef LINT
    pnew->sy_line_id.sy_lineno = er_getline();
    pnew->sy_file = st_lookup(er_curname());
#else
    pnew->sy_line_id.sy_lineno = PP_CURSEQNO();
#endif
...
*/
{
	...
	cerror(gettxt(":326","hiding non-top-level symbol"));
...
{
    if (sy_scopelevel == 0)
	cerror(gettxt(":327","attempt to leave scope level %d"), sy_scopelevel);
...
#ifdef __STDC__
#define ST_INIT(s)	st_ ## s = st_nlookup(#s, sizeof(#s))
#else
#define ST_INIT(s)	st_/**/s = st_nlookup("s", sizeof("s"))
#endif

function_type
sy_function_type(sid)
SX sid;
{
	function_type retval;
	char *name;
        static int first = 1;
        static char 
		/* alloca inlined only if inline_alloca() is true */
		...
		*st_strcpy, *st_strncpy, *st_strlen, *st_strcmp,
		*st_strncmp, *st_memcmp, *st_strcoll, *st_memchr,
		*st_strchr, *st_strcspn, *st_strpbrk, *st_strrchr, 
		*st_strspn, *st_strstr, *st_memset, *st_memcpy,

		/* MATH FUNCTIONS */

		*st_sin, *st_cos, *st_tan, *st_asin, *st_acos, *st_atan, 
		*st_atan2, *st_cosh, *st_tanh, *st_sinh, *st_ldexp, 
		*st_log10, *st_frexp, *st_modf, *st_exp, *st_log, 
		*st_sqrt, *st_pow, *st_ceil, *st_fabs, *st_floor, 
		*st_fmod, *st_atof, *st_atoi, *st_atol,

		/* GENERAL UTILITIES */

		*st_getenv, *st_abs, *st_div, *st_mblen, *st_labs,
		*st_ldiv,

		/* DATE/TIME FUNCTIONS */

		*st_clock, *st_difftime, *st_mktime, *st_asctime,
		*st_ctime, *st_gmtime, *st_localtime;
	int check_fp;

	if (sid <= 0)
		/*
		** sid will be 0 for goofy stuff like (int (*))24,
		** i.e., calls to an absolute address
		*/
		return func_unknown;
	if (SY_CLASS(sid) == SC_STATIC)
		return func_unknown;
	if (SY_FLAGS(sid) & SY_NOT_BUILTIN)
		return func_unknown;
	if (!hosted() && !(SY_FLAGS(sid) & SY_BUILTIN)) 
		return func_unknown;
	retval = func_unknown;
	name = SY_NAME(sid);
	check_fp = 0;
		...
		ST_INIT(alloca);
#endif
		ST_INIT(strcpy); ST_INIT(strncpy); ST_INIT(strlen);
		ST_INIT(strcmp); ST_INIT(strncmp); ST_INIT(memcmp);
		ST_INIT(strcoll); ST_INIT(memchr); ST_INIT(strchr);
		ST_INIT(strcspn); ST_INIT(strpbrk); ST_INIT(strrchr);
		ST_INIT(strspn); ST_INIT(strstr); ST_INIT(memcpy);
		ST_INIT(memset);

		ST_INIT(sin); ST_INIT(cos); ST_INIT(tan); ST_INIT(atoi);
		ST_INIT(asin); ST_INIT(acos); ST_INIT(atan); ST_INIT(atan2);
		ST_INIT(cosh); ST_INIT(sinh); ST_INIT(tanh); ST_INIT(ldexp);
		ST_INIT(frexp); ST_INIT(modf); ST_INIT(pow); ST_INIT(ceil);
		ST_INIT(fabs); ST_INIT(floor); ST_INIT(fmod); ST_INIT(exp); 
		ST_INIT(log); ST_INIT(log10); ST_INIT(sqrt); ST_INIT(atof);

		ST_INIT(getenv); ST_INIT(abs); ST_INIT(div); ST_INIT(labs);
		ST_INIT(ldiv); ST_INIT(mblen);

		ST_INIT(clock); ST_INIT(difftime); ST_INIT(mktime);
		ST_INIT(asctime); ST_INIT(ctime); ST_INIT(gmtime);
		ST_INIT(localtime);
        }
        /* Hash on first character */
	/* Note that any function which depends on the environment
	** is said to have side effects, e.g., clock().
	*/
        switch (name[0]) {
	case 'a':
		if (name == st_asin || name == st_acos || name == st_atan ||
		    name == st_atan2 || name == st_atof || name == st_atoi ||
		    name == st_atol)
			check_fp = 1;
		else if (name == st_abs )
			retval = func_no_side_effects;
		else if (name == st_asctime)
			retval = func_side_effects;
#ifndef LINT
		else if (name == st_alloca) {
			if(inline_alloca()) retval = func_side_effects;
		}
#endif
		break;
	case 'c':
		if (name == st_cos || name == st_cosh)
			check_fp = 1;
		else if (name == st_ceil)
			retval = func_no_side_effects;
		else if (name == st_clock || name == st_ctime)
			retval = func_side_effects;
		break;
	case 'd':
		if (name == st_div)
			retval = func_no_side_effects;
		else if (name == st_difftime)
			retval = func_side_effects;
		break;
	case 'e':
		if (name == st_exp)
	...
	case 'f':
		if (name == st_frexp)
			retval = func_side_effects;
		else if (name == st_floor)
			retval = func_no_side_effects;
		else if (name == st_fmod || name == st_fabs)
			check_fp = 1;
		break;
	case 'g':
		if (name == st_getenv || name == st_gmtime)
			retval = func_side_effects;
		break;
	case 'l':
		if (name == st_ldexp || name == st_log || name == st_log10)
			check_fp = 1;
		else if (name == st_labs || name == st_ldiv)
			retval = func_no_side_effects;
		else if (name == st_localtime)
			retval = func_side_effects;
		break;
	case 'm':
		if (name == st_memchr || name == st_memcmp || name == st_mblen)
			retval = func_no_side_effects;
		else if (name == st_modf || name == st_mktime || 
			     name == st_memcpy || name == st_memset)
			retval = func_side_effects;
		break;
	case 'p':
		if (name == st_pow)
		...
		break;
        case 's':
		if (name[1] != 't' || name[2] != 'r')
			goto not_str;
                if (name == st_strcpy || name == st_strncpy) 
			retval = func_side_effects;
		else if (name == st_strlen || name == st_strcmp ||
			 name == st_strncmp || name == st_strcoll ||
			 name == st_strchr || name == st_strcspn ||
			 name == st_strpbrk || name == st_strrchr || 
			 name == st_strspn || name == st_strstr)
			retval = func_no_side_effects;
		break;
	not_str:
		if (name == st_sin || name == st_sinh || name == st_sqrt)
			check_fp = 1;
		break;
	case 't':
		if (name == st_tan || name == st_tanh)
			check_fp = 1;
		break;
        }
	if (check_fp) {
        ...
        if (retval == func_unknown)
		SY_FLAGS(sid) |= SY_NOT_BUILTIN;
	else if (SY_FLAGS(sid) & SY_DEFINED) {
		WERROR("Function illegally defined in hosted mode: %s",name);
		retval = func_unknown;
		SY_FLAGS(sid) |= SY_NOT_BUILTIN;
		SY_FLAGS(sid) &= ~SY_BUILTIN;
	} 
	else
		SY_FLAGS(sid) |= SY_BUILTIN;
	return retval;
