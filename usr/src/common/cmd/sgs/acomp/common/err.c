...
/* { */
	...
	return;
    fflush(stdout);
    pfmt(stderr,MM_WARNING, ":376:\"%s\", line %d: ", filename, elineno);
...
/* { */
    ...
    fflush(stdout);
    pfmt(stderr,MM_ERROR, ":377:\"%s\", line %d: ", filename, elineno);
    (void) vfprintf(stderr, fmt, args);
    putc('\n',stderr);
    va_end(args);
    fflush(stderr);
...
va_dcl
{
	...
	return;
    fflush(stdout);
    pfmt(stderr,MM_WARNING, ":376:\"%s\", line %d: ", filename,
...
/* Print a warning with an explicit line number and explicit filename.
   If the line number is < 0, then use the current line number.  Likewise,
   if the filename is NULL, use the current filename.
*/

void
#ifdef __STDC__
wlferror(int lineno, const char *filenm, const char *fmt,...) {
    va_list args;
    va_start(args, fmt);
#else	/* non-ANSI C case */
wlferror(lineno, filenm, fmt, va_alist)
int lineno;
char *filenm;
char *fmt;
va_dcl
{
	...
	return;
    fflush(stdout);
    pfmt(stderr,MM_WARNING, ":376:\"%s\", line %d: ",
		filenm ? filenm : filename,
		lineno > 0 ? lineno : elineno);
    (void) vfprintf(stderr, fmt, args);
    putc('\n',stderr);
    va_end(args);
...
va_dcl
{
    ...
    fflush(stdout);
    pfmt(stderr,MM_ERROR, ":377:\"%s\", line %d: ", filename,
		lineno > 0 ? lineno : elineno);
    (void) vfprintf(stderr, fmt, args);
    putc('\n',stderr);
    va_end(args);
...
/* { */
	...
	pfmt(stderr,MM_ERROR, ":378:\"%s\", line %d: internal compiler error:  ",
		    filename, elineno);
        (void) vfprintf(stderr, fmt, args);
	if (nerrors != 0) {
	    putc(']',stderr);
    ...
    if (nerrors != 0)
	UERROR(gettxt(":226","cannot recover from previous errors"));
    else if (err_dump)
	abort();
...
/* Stub routine for yacc's error reporting. */
{
    ...
    UERROR(gettxt(":227","%s before or at: %.*s"), s, toklen, token);
    return;
}
