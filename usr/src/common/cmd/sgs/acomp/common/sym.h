#ident	"@(#)acomp:common/sym.h	55.2.6.16"
/* sym.h */
...
#define SY_NOT_BUILTIN	(1<<18)		/* symbol has been found not to be
					** an ansi function.  This is used
					** to save compile time.
					*/
#define SY_ADDRTAKEN	(1<<19)		/* set if address assigned or passed
					   in function call. Only calculated
					   if ADDRTAKEN is defined in CG
					*/
#define SY_AMIGO_OBJECT	(1<<20)		/* set for NAMES for which AMIGO
					** has created an object_info
					*/
#define SY_ASM_FULL_OPT (1<<21)		/* fully optimizable asm */
#define SY_ASM_PARTIAL_OPT (1<<22)	/* partially optimizable asm */

#define SY_ARTIFICIAL	(1<<23)		/* symbol is compiler generated. */
#define SY_PRIVATE	(1<<24)		/* access mode is private. */ 
#define SY_PROTECTED	(1<<25)		/* access mode is protected. */

#define SY_HAS_ASM (1<<26)
#define SY_HAS_SAFE_ASM (1<<27)
#define SY_HAS_SETJMP (1<<28)
#define SY_HAS_LONGJMP (1<<29)
#define SY_NO_FIXED_FRAME (1<<30)
...
struct symtab {
    ...
    union {
        int sy_lineno;	/* lineno symbol defined at - used by acomp*/
	int sy_debug_id; /* number of debug entry - used by cplusbe */
	SX  sy_orig_sym;	/* index of original symbol - used by inliner */
    } sy_line_id;
#ifdef LINT
    ...
    SX sy_sameas;	/* symbol at more outer scope that this is linked to */
    ...
    int sy_args_in_regs; /* Bit vector: args passed in regs */
    union {
	struct Funct * func;
	int arg;
    } sy_aux;		/* used by inliner */
#endif
    I32 sy_offset;	/* offset/enum/label/packed field */
};
...
#define SY_LINENO(sid)  (SY(sid).sy_line_id.sy_lineno)     /* int SY_LINENO(SX sid)  */
#define SY_DEBUG_ID(sid) (SY(sid).sy_line_id.sy_debug_id)  /* int SY_DEBUG_ID(sid) */
#define SY_ORIG_SYM(sid) (SY(sid).sy_line_id.sy_orig_sym)  /* SX SY_ORIG_SYM(sid) */
#ifdef LINT
...
#define	SY_ARGS_IN_REGS(sid)	(SY(sid).sy_args_in_regs)
...
typedef enum { 
	func_unknown, 		/* Function known to compiler */
	func_no_side_effects, 	/* Does not change any location pointed
				** to by arguments.  Does not change externs.
				** Does not change environment.  This may
				** also be set for floating point functions
				** under an aggressive optimization mode.
				*/
	func_side_effects 	/* Known to compiler, but does not meet
				** criteria for func_no_side_effects.
				*/
} function_type;

extern function_type sy_function_type();
extern SX sy_hide();
