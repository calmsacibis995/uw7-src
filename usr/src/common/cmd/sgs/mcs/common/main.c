...
char *cur_file;
{
	...
	if ((cmd != ELF_C_NULL) && ((err = elf_errno()) != 0))
        {
                (void) fprintf(stderr,
                        "%smcs: libelf error: %s\n",SGS, elf_errmsg(err) );
                (void) fprintf(stderr,
...
Elf32_Sym *p;
{
	unsigned char info = p->st_info;

	if (ELF32_ST_BIND(info) == STB_GLOBAL)
		return ELF32_ST_INFO(STB_WEAK, STT_NOTYPE);

	if (ELF32_ST_BIND(info) == STB_LOCAL)
		return ELF32_ST_INFO(STB_LOCAL, STT_NOTYPE);
...
{
        (void) fprintf(stderr,
        "%smcs: Usage: %smcs -?Vcdp [-a 'string'] [-n name] files...\n", SGS, SGS);

	if (errflag)
	{
		(void) fprintf(stderr,
	"\t[-V]  print the version number of mcs on stderr\n\
	[-c]  compress the contents of the comment section of the object files\n\
	[-d]  delete the contents of the comment section from the object files\n\
	[-p]  print the contents of the comment section on stdout\n\
	[-a 'string']  append string to the comment section of the object files\n\
	[-n name]  specify the name of the comment section to access if other\n\
		than .comment\n");
	}
