#ident	"@(#)dump:common/dump.c	1.101"
#include <stdio.h>
...
char       *filename;
{
	...
	int           num=1;
	static int    num_scns;
	SCNTAB		*p_dwarf2_debug = 0;
	SCNTAB		*p_dwarf2_line = 0;
	...
	while ( (scn = elf_nextscn(elf_file, scn) ) != 0)
	{
		...
		if (strcmp(s_name, ".line") == 0)
			p_line = buffer;
		if (strcmp(s_name, ".debug_info") == 0)
			p_dwarf2_debug = buffer;
		if (strcmp(s_name, ".debug_abbrev") == 0)
			p_abbreviations = buffer;
		if (strcmp(s_name, ".debug_line") == 0)
			p_dwarf2_line = buffer;
 ...
 */
	...
	if (p_dwarf2_debug && D_flag)
	{
		dump_dwarf2_debug(elf_file, filename, p_dwarf2_debug);
	}
	if(p_line && l_flag)
	{
		dump_line(elf_file, filename);
	}
	if (p_dwarf2_line && l_flag)
	{
		dump_dwarf2_line(elf_file, filename, p_dwarf2_line);
	}
	if (s_flag)
	{
	...
	for(num=1; num<num_scns; num++, p++)
	{
		...
		else
		{
			switch (p->p_shdr->sh_type)
			{
				...
				case SHT_MOD: Printf("MOD "); break;
				case SHT_DELAY_REL: Printf("DREL"); break;
				default:
...
char * filename;
{
	...
	for (i = 1; i < num_scns; i++,p++)
	{
		...
		*/
		...
		if (strcmp(p->scn_name, ".debug_abbrev") == 0)
		{
			p_abbreviations = p;
			dump_dwarf2_abbreviations(elf_file, filename);
			continue;
		}
		if (strcmp(p->scn_name, ".debug_info") == 0)
		{
			dump_dwarf2_debug(elf_file, filename, p);
			continue;
		}
		if (strcmp(p->scn_name, ".debug_line") == 0)
		{
			dump_dwarf2_line(elf_file, filename, p);
			continue;
		}
