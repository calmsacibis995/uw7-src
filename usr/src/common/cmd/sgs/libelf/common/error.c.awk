#ident	"@(#)libelf:common/error.c.awk	1.2.6.2"
...
END	{
		...
		print ""
		print ""
		print "static const struct msgchr{"
		print ""
		print "		char *msgnum;"
		print "		char *msgstr;"
		print ""
		...
		for (j = 1; j <= major; ++j)
		{
			print ""
			print ""
			printf "static const struct msgchr\t%s[] =\t/* E%s_... */\n",\
				info[j MAJOR_DATA],\
				info[j MAJOR_ENUM]
			print "{"
x = j MAJOR_PREF
			printf "\t/* 0 */\t{\"uxlibelf:%d\",\t\"%s: reason unknown\"},\n",number, info[x]
			++number
			for (k = 1; k <= info[j MAJOR_NUMB]; ++k)
			{
				printf "\t/* %s */\t{\"uxlibelf:%d\",\t\"%s: %s\"},\n",\
		...
		print "{"
		print "	const struct msgchr 	*m_list;\t/* message list */"
		...
		print "	static  char  msgid[15]=\"uxlibelf:\";"
		print ""
		print "	if (err == 0)"
		print "	{"
		...
		print "	{"
		print "		if ((err = _elf_err) == 0){"
		print "			return(gettxt(\"uxlibelf:1\", \"ELF error 0\"));"
		print "		}"
		print "	}"
		...
		print "			minor = 0;"
		print "		return(gettxt(msg[major].m_list[minor].msgnum, msg[major].m_list[minor].msgstr));"
		print "	}"
		print "	return(gettxt(\"uxlibelf:2\", \"Unknown ELF error\"));"
		print "}"
		print ""
		print ""
		print "int"
