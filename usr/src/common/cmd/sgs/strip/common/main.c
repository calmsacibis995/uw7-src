#ident	"@(#)strip:common/main.c	1.28.1.6"
/**************************** CI5/elf-based STRIP *****************************/
...
#include <memory.h>
#include <ccstypes.h>
#include <locale.h> 	/* for setlocale, LC_*, etc... */
...
static const char Usage [] =
	":882:usage: %sstrip [-?Vlx] file(s)...\n";

static const char open_temp [] =
	":858:%s: cannot open temporary file\n";

static const char close_fail [] =
	":859:%s: close system failure\n";

static const char nosect [] =
	":860:%s: no section header table\n";

static const char read_fail [] =
	":861:%s: read failure\n";

static const char write_fail [] =
	":862:%s: write system failure: %s: file not stripped\n";

static const char elferr [] =
	":863:libelf error: %s\n";

static const char malloc_fail [] =
	":864:malloc failure\n";

static const char calloc_fail [] =
	":865:memory allocation failure on calloc\n";

static const char strip_fail[] =
	":866:%s: Cannot strip file\n";

static const char sect_remove[] =
	":867:%s: %s linked to a section that has been removed \n";

static const char old_opt[] =
	":868:the -%c option has no effect and will be removed in the next release.\n";
...
char 	*argv[];
{
	...
	(void) sprintf(label, "UX:%sstrip", SGS);
	(void) setlabel(label);
	(void) setcat("uxcore");
        ...
        if (elf_version(EV_CURRENT) == EV_NONE) 
	{
			...
			":869:elf_version() failed - libelf.a out of date.\n") ;
                exit(FAILURE);
        }
	...
	while ((optchar = getopt(argc, argv, optstr)) != -1) {
		...
		case 'r':	(void) pfmt(stderr, 
					MM_WARNING, old_opt, 'r');
				break;
		...
		case 'b':	(void) pfmt(stderr, 
					MM_WARNING, old_opt, 'b');
				break;

		case '?':       errflag++; usage(); break;
                default:	break;
                }
        }
...
usage()
{
        (void) pfmt(stderr, MM_ACTION, Usage, SGS);

        if (errflag)
        {
                (void) fprintf(stderr,
        "\t[-V]  print the version number of strip on stderr\n\
...
each_file()
{
	...
	{
		(void) pfmt(stderr, MM_ERROR, elferr, elf_errmsg(-1) );
		(void) elf_end(arf);
	...
	{
		(void) pfmt(stderr, MM_ERROR,
			":872:%s: cannot strip a COFF file\n", fname);
		(void) elf_end(arf);
                (void) close(fd);   /* done processing this file */
                return(FAILURE);
        } 
	else if ((elf_kind(arf) == ELF_K_AR))
	{
		...
		{
			(void) pfmt(stderr, MM_ERROR, open_temp, artmpfile);
			(void) elf_end(arf);
                        (void) close(fd);
			exit(FAILURE);	
		}
	...
	while ((elf = elf_begin(fd, cmd, arf)) != 0)
	{
		if (ar_file) /* get header info */
		{
			...
			if ((cur_filenm = (char *)malloc( (strlen(fname) + 3
                        ...
                        {
                                (void) pfmt(stderr, MM_ERROR, malloc_fail) ;
                                strip_exit(FAILURE);
                        }
	...
	if ( err != 0)
	{
		(void) pfmt(stderr, MM_ERROR, elferr, elf_errmsg(err));
		(void) pfmt(stderr, MM_NOSTD,
			":876:%s: file not stripped\n", fname) ;
		return(FAILURE);
	}
	...
	{
		(void) pfmt(stderr, MM_ERROR, close_fail, fname);
		strip_exit(FAILURE);
	}
	...
	if (ar_file)
	{
		...
		if (found_ar_sym_tab)
		{
			(void) pfmt(stderr, MM_WARNING,
				":877:%s: symbol table deleted from archive \n", fname);
			(void) pfmt(stderr, MM_ACTION,
					":878:execute  `ar -ts %s` to restore symbol table.\n", fname) ;
		}
        }
	else
...
char *fname;
{
	...
	char *buf;
        ...
        {
		(void) pfmt(stderr, MM_ERROR, open_temp, elftmpfile);
                strip_exit(FAILURE);
        }
	...
	{
		(void) pfmt(stderr, MM_ERROR, read_fail, elftmpfile);
               	strip_exit(FAILURE);
	}
	...
	{
		(void) pfmt(stderr, MM_ERROR, write_fail,
					artmpfile, fname);
		strip_exit(FAILURE);
	}
...
char *fname;
{
	...
	if ( strcmp(mem_header->ar_name,"/") != 0 )
	{
		...
		{
			(void) pfmt(stderr, MM_ERROR, read_fail, fname);
                	strip_exit(FAILURE);
        	}
...
char *temp_file_name;
{
	...
	{
		(void) pfmt(stderr, MM_ERROR, open_temp, temp_file_name);
               	strip_exit(FAILURE); 
        }
	...
	{
		(void) pfmt(stderr, MM_ERROR, read_fail, temp_file_name);
		strip_exit(FAILURE);
	}
#if 0
	...
	{
		(void) pfmt(stderr, MM_ERROR,
			":880:%s: cannot open file for writing\n",
				 fname);
		strip_exit(FAILURE); 
        }
#if 0
...
initialize()
{
	...
	{
		(void) pfmt(stderr, MM_ERROR, calloc_fail) ;
		strip_exit(FAILURE);
	}
...
build_segment_table()
{
	...
	while ((scn = elf_nextscn(elf, scn)) != 0)
	{
		...
		{
			(void) pfmt(stderr, MM_ERROR, nosect, cur_filenm);
			return(FAILURE);
		}
 ...
 * should not be deleted from a relocatable file */
	...
	while ((scn = elf_nextscn(elf, scn)) != 0) 
	{
		...
		if ( shdr->sh_type == SHT_REL || shdr->sh_type == SHT_RELA )
		{
			...
			if (shdr->sh_info != SHN_UNDEF && 
				...
				if ((temp_shdr = elf32_getshdr(temp_scn)) != 0)
				{
					...
					if (((is_debug_section(temp_name)
						&& !lflag)
					     || is_line_section(temp_name))
					   &&
					     ( y != IN )
					   &&
					     ( x != IN ) )
		...
		else if (((is_debug_section(name) && !lflag) ||
				is_line_section(name)) &&
				sec_table[scn_no] == 0 &&
		...
		if ( sec_table[scn_no] != -1 )
		{
			...
			} else if ( shdr->sh_type == SHT_REL || shdr->sh_type == SHT_RELA )
			{
				...
				if (sec_table[shdr->sh_info] < 0) {
					elf_shdr->sh_info = 0;
					(void) pfmt(stderr, MM_WARNING,
						":881:%s: %s info field set to a section that has been removed \n"
						,fname, name);
				}
...
/* returns 1 if the section name refers to a debugging
 * information section, but not a line section, else
 * returns 0
 */
static int
is_debug_section(sname)
char	*sname;
{
...
/* returns 1 if the section is a line number section, else 0 */
static int
is_line_section(sname)
char *sname;
{
	return((strcmp(sname, ".line") == 0) ||
		(strcmp(sname, ".debug_line") == 0));
}
