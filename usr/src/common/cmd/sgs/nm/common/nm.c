...
#define OPTSTR	"?CVhnoplruvx"	/* option string for getopt() */
	...
	char *argv[];
{
	...
	{
		(void) fprintf(stderr, 
		"%s%s: Usage: -l option must be used with the -p option\n", 
			SGS, prog_name, SGS, prog_name);
		exit(NOARGS);
	}
...
usage()
{
	...
	"%s%s: Usage: %s%s [-%s] file(s) ...\n", 
		SGS, prog_name, SGS, prog_name, OPTSTR);
	if (errflag)
	{
		(void) fprintf(stderr,
...
Elf32_Word  link;
{
	...
	if( (buf = (SYM *)malloc(num * sizeof(SYM)) ) == NULL)
	{
	...
	/* demangle C++ symbol names */
		...
		for(i=1; i<num; i++, buf++) {
			...
			if (*buf->name != 0) {
				if ((namebuf =  (char *) malloc(NAMELEN)) == 0) {

					(void)fprintf(stderr, "%s: cannot malloc space\n", prog_name);
					errflag++;
					return NULL;
				}
			       if (((len = demangle(buf->name, namebuf, NAMELEN)) != (size_t)-1) && (len > NAMELEN)) {
					free(namebuf);
					if ((namebuf = (char *)malloc(len)) == 0) {
						(void)fprintf(stderr, "%s: cannot malloc space\n", prog_name);
						errflag++;
						return NULL;
					}
					len = demangle(buf->name, namebuf, len);
