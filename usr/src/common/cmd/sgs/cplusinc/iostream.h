#ident	"@(#)cplusinc:iostream.h	1.11"
/*******************************************************************************
...
class streambuf {
	...
	int		allocate()
	{
		if ( !unbuffered() && x_base== 0 ) return doallocate() ;
		else			  	 return 0 ;
	}
...
public:	
	...
	ostream&	operator<<(unsigned char c) throw()
	{
		if (ospecial || osfx_special) {
			return ls_complicated(c);
		}
	...
	ostream&	operator<<(const char*) throw();
#ifdef _BOOL
	ostream&	operator<<(bool n) throw();
#endif
	ostream&	operator<<(int a) throw(); 
	ostream&	operator<<(long) throw();	
#if !__STDC__
	ostream&	operator<<(long long) throw();
#endif
	ostream&	operator<<(double) throw();
	ostream&	operator<<(long double) throw();
	ostream&	operator<<(float) throw();
	ostream&	operator<<(unsigned int a) throw();
	ostream&	operator<<(unsigned long) throw();
#if !__STDC__
	ostream&	operator<<(unsigned long long) throw();
#endif
	ostream&	operator<<(void*) throw();
/*	ostream&	operator<<(const void*);   add this later */
	ostream&	operator<<(streambuf*) throw();
	ostream&	operator<<(short i) throw() { return *this << (int)i ; }
	ostream&	operator<<(unsigned short i) throw()
			{ return *this << (int)i  ; }

	ostream&	operator<< (ostream& (*ffff)(ostream&) throw()) throw()
			{ return (*ffff)(*this) ; }
	ostream&	operator<< (ios& (*f)(ios&) ) throw();
...
public:
	...
	iostream_withassign&	operator=(iostream_withassign&) ;
...
ios&		dec(ios&) ; 
ostream&	endl(ostream& i) throw() ;
ostream&	ends(ostream& i) throw() ;
