#ident	"@(#)alint:common/msgbuf.c	1.6"
#include "msgbuf.h"
/*
...
/*14 */ { "assignment causes implicit narrowing conversion",
	  "%s = %s",
	  LONE,
	  "assignment causes implicit narrowing conversion: %s = %s" },
...
/*17 */ { "called lacking visible prototype declaration",
	  "%s",
	  MULTI,
...
/* 19 */ {"missing argument ( number ) for positional conversion",
	  "#%-3lu %-16s \t%s(%d)",
	  LONE,
	  "missing argument %lu for positional conversion: %s in %s(%d)" },
};
#endif
