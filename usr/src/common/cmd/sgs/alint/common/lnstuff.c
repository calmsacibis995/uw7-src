#ident	"@(#)alint:common/lnstuff.c	1.10.1.15"
/*
...
ND1 *tr;
{
    ...
    /*
    ** The following code assumes that fputwc() can be intermixed
    ** with the single byte I/O functions.
    */
    pos = ftell(file4);
    if ((len = tr->c.size) != 0) {
	sp = tr->string;
	if (tr->sid & TR_ST_WIDE) {
	    do {
		fputwc(lx_mbtowc(sp), file4);
		sp += sizeof(wchar_t);
	    } while (--len != 0);
	} else {
	    fputs(sp, file4);
...
int goal;
{
    ...
    */
    if (!IS_SYMBOL(ret->left) || !TY_ISFTN(SY_TYPE(ret->left->sid)) ||
	LN_DIR(LINTLIBRARY))
	    return;
    ...
    */
    ...
    if (SY_CLASS(tr->sid) == SC_AUTO)
	return;
    else 
	ln2_outdef(tr->sid, (goal==EFF) ? LUE : LUV, args);
...
*/
static int
ln2_chkform(ty, flag, isproto, iswide, args)
T1WORD ty;
int flag, isproto, args;
{
    T1WORD argty;
    LNBUG(ln_dbflag > 1, ("ln2_chkform: %ld %d %d %d %d",ty,flag,isproto,iswide,args));
    ...
    */
    if (TY_ISPTR(argty)) {
	argty = TY_TYPE(TY_DECREF(argty));
	if (argty == iswide ? T_wchar_t : TY_CHAR) {
	    LN_DIR(VARARGS) = flag;
	    return flag;
	}
    }
    WERROR(iswide ?
	"argument %d must be \"wchar_t *\" for WPRINTFLIKE/WSCANFLIKE"
	: "argument %d must be \"char *\" for PRINTFLIKE/SCANFLIKE", flag);
    return args;
}
...
int args, isproto;
{
    ...
    */
	...
	if (args < LN_DIR(PRINTFLIKE)) {
	    ...
	    LN_DIR(PRINTFLIKE) = -1;
	} else args = ln2_chkform(ty, LN_DIR(PRINTFLIKE), isproto, 0, args);
    }
    /* Same for WPRINTFLIKE, except check for a wchar_t *. */
    else if (LN_DIR(WPRINTFLIKE) != -1) {
	if (args < LN_DIR(WPRINTFLIKE)) {
	    WERROR(arg_mis, "WPRINTFLIKE");
	    LN_DIR(WPRINTFLIKE) = -1;
	} else args = ln2_chkform(ty, LN_DIR(WPRINTFLIKE), isproto, 1, args);
    }
    /*
    ...
    */
	...
	if (args < LN_DIR(SCANFLIKE)) {
	    ...
	    LN_DIR(SCANFLIKE) = -1;
	} else args = ln2_chkform(ty, LN_DIR(SCANFLIKE), isproto, 0, args);
    }
    /* Same for WSCANFLIKE, except check for a wchar_t *. */
    else if (LN_DIR(WSCANFLIKE) != -1) {
	if (args < LN_DIR(WSCANFLIKE)) {
	    WERROR(arg_mis, "WSCANFLIKE");
	    LN_DIR(WSCANFLIKE) = -1;
	} else args = ln2_chkform(ty, LN_DIR(WSCANFLIKE), isproto, 1, args);
    }
    ...
    */
    ...
    /* Don't apply [W]PRINTFLIKE or [W]SCANFLIKE to asms. */
    if (class == SC_ASM) {
	static char asm_use[] =
	...
	if (LN_DIR(PRINTFLIKE) > 0) {
	    ...
	    LN_DIR(PRINTFLIKE) = -1;
	} else if (LN_DIR(WPRINTFLIKE) > 0) {
	    WERROR(asm_use, "WPRINTFLIKE");
	    LN_DIR(WPRINTFLIKE) = -1;
	} else if (LN_DIR(SCANFLIKE) > 0) {
	    WERROR(asm_use, "SCANFLIKE");
	    LN_DIR(SCANFLIKE) = -1;
	} else if (LN_DIR(WSCANFLIKE) > 0) {
	    WERROR(asm_use, "WSCANFLIKE");
	    LN_DIR(WSCANFLIKE) = -1;
	}
    }
    ...
    */
    ...
    skip:
    /* reset [W]PRINTFLIKE/[W]SCANFLIKE flags to default */
    LN_DIR(PRINTFLIKE) = LN_DIR(SCANFLIKE) = -1;
    LN_DIR(WPRINTFLIKE) = LN_DIR(WSCANFLIKE) = -1;
}
...
ln2_chkdir()
{
	...
	if (LN_DIR(WPRINTFLIKE) >= 0) {
	    WERROR(baduse, "WPRINTFLIKE");
	    LN_DIR(WPRINTFLIKE) = -1;
	}
	...
	if (LN_DIR(WSCANFLIKE) >= 0) {
	    WERROR(baduse, "WSCANFLIKE");
	    LN_DIR(WSCANFLIKE) = -1;
	}
    }
}
...
int code, args;
{
    ...
    /*
    ** If [W]PRINTFLIKE or [W]SCANFLIKE set, or in code.
    */
    if (LN_DIR(PRINTFLIKE) > 0)
	rc.l.decflag |= LPF;
    else if (LN_DIR(WPRINTFLIKE) > 0)
	rc.l.decflag |= LPF|LWF;
    else if (LN_DIR(SCANFLIKE) > 0)
	rc.l.decflag |= LSF;
    else if (LN_DIR(WSCANFLIKE) > 0)
	rc.l.decflag |= LSF|LWF;
	...
	fprintf(stderr,"  sid: %d   name: %s   type: %d   extra: %ld   ",
			sid, SY_NAME(sid), rc.l.type.aty, rc.l.type.extra.pos);
	fprintf(stderr,"dcl_mod: %lu  args: %d   line: %d   ", rc.l.type.dcl_mod,rc.l.nargs, rc.l.fline);
	...
	switch (code) {
	    ...
	    case LND: s = "LND"; break;
	    default:  s = "BAD-CODE"; break;
	}
	fprintf(stderr,"code: %s", s);
	if (rc.l.decflag & (LPF|LSF)) {
	    if (rc.l.decflag & LWF) fprintf(stderr,"|LWF");
	    fprintf(stderr, "|%s\n", rc.l.decflag & LPF ? "LPF" : "LSF");
	} else putc('\n', stderr);
    }
#endif
}
