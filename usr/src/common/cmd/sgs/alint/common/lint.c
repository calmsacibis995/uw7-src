#ident	"@(#)alint:common/lint.c	1.18"
#include <stdio.h>
#include <ctype.h>
#include "p1.h"
#include "lnstuff.h"
...

/*
** Return address of the ICON or FCON node if node p is a constant.
** In all uses of this routine, constant folding will have already been
** completed; the only spot where constants expressions may not be
** flattened into a single node is if within a COMOP operator.
*/
ND1 *
iscon(p)
ND1 *p;
...
ND1 *p;
{
    ...
    if ((p = iscon(p)) == NULL)
	return 0;
    if (p->op == ICON)   /* p->sid == ND_NOSYMBOL tested in iscon() */
	return num_scompare(&p->c.ival, &num_0) < 0;
    if (p->op == FCON)
	return FP_CMPX(p->c.fval, flt_0) == FPEMU_CMP_LESSTHAN;
    return 0;
}
...
ND1 *p;
{
	...
	return 0;
    if (p->op == ICON)   /* p->sid == ND_NOSYMBOL tested in iscon() */
	return num_ucompare(&p->c.ival, &num_0) == 0;
    if (p->op == FCON)
	return FP_ISZERO(p->c.fval);
    return 0;
}
...
ND1 *l, *r;
{
	...
	if (l->sid != r->sid || num_ucompare(&l->c.ival, &r->c.ival) != 0)
	    return 1;
    }
    else if (l->op == FCON && r->op == FCON) {
	if (FP_CMPX(l->c.fval, r->c.fval) != FPEMU_CMP_EQUALTO)
	    return 1;
    }
    return 0;
}
