#ident	"@(#)alint:common/messages.c	1.10"
#include "p1.h"
#include <stdio.h>
#include <varargs.h>
#include <string.h>
...
va_dcl
{
    ...
    va_start(args);
    prerror(WARN, er_getline(), NULL, args);
    va_end(args);
}
...
va_dcl
{
    ...
    va_start(args);
    prerror(WARN, ( line > 0 ? line : er_getline() ), NULL, args);
    va_end(args);
}
...
/* VARARGS */
void
lwlferror(line, filename, va_alist)
int line;
char * filename;
va_dcl
{
    ...
    va_start(args);
    prerror(WARN, ( line > 0 ? line : er_getline() ), filename, args);
    va_end(args);
}
...
va_dcl
{
    ...
    va_start(args);
    prerror(ERR, er_getline(), NULL, args);
    va_end(args);
    ++nerrors;
}
...
va_dcl
{
    ...
    va_start(args);
    prerror(ERR, ( line > 0 ? line : er_getline() ), NULL, args);
    va_end(args);
    ++nerrors;
}
...
va_dcl
{
    ...
    va_start(args);
    prerror(INFO, er_getline(), NULL, args);
    va_end(args);
} 
...
*/
static void
prerror(code, line, ln_filename, args)
int code;
int line;
char *ln_filename;
va_list args;
{
    ...
    /* If no specific file name provided, use the current file name. */
    if (ln_filename == NULL)
        ln_filename = er_curname();
    fmt = va_arg(args, char *);
...
int code;
{
    ...
    */
    if (   (code == ERR)
        && (igline == (line - 1))
        && ikwid_file_name
        && (strcmp(ikwid_file_name, er_curname()) == 0)) {
	LN_DIR(IKWID) = 0;
	free(ikwid_file_name);
	ikwid_file_name = NULL;
    ...
    */
	...
	LN_DIR(IKWID) = 0;
	WERRORLNF(igline, ikwid_file_name,
		  "lint suppression directive not used");
	free(ikwid_file_name);
	ikwid_file_name = NULL;
	return 1;
    } 
