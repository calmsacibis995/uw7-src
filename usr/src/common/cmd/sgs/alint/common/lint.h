#ident	"@(#)alint:common/lint.h	1.14"
...
extern void ln_setflags(char *, unsigned int);
#else
...
enum {
	ARGSUSED, VARARGS, PRINTFLIKE, WPRINTFLIKE,
	SCANFLIKE, WSCANFLIKE, PROTOLIB, CONSTANTCONDITION,
	LINTLIBRARY, NOTREACHED, IKWID, EMPTY, FALLTHRU,
	NUMDIRECTS
};
...
** From directives.c
*/
extern char* ikwid_file_name;
			/* pointer to filename for last IKWID directive. */
...
#ifdef __STDC__
void ln_postop( ND1 *, int, int, int);
extern void ln_precedence(ND1 *);
extern int isnegcon(ND1 *);
extern ND1 * iscon(ND1 *);
