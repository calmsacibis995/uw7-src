#ident	"@(#)alint:common/lpass2.c	1.23"
#include "p1.h"
#include <stdio.h>
#include <varargs.h>
#include <string.h>
...
char *argv[]; 
{
    ...
    ** read options
    */
    while((c=getopt(argc,argv,"abhkmpsuvxyFX:1:2:T:VK:")) != EOF)
	switch(c) {
...

#define ISINT(t)  ((t) == LN_INT || (t) == LN_SINT || (t) == LN_UINT || \
                   (t) == LN_LONG || (t) == LN_SLONG || (t) == LN_ULONG || \
                   (t) == LN_LLONG || (t) == LN_SLLONG || (t) == LN_ULLONG)
/* 
...
int nostrict;
{
    ...
    */
	...
	if ((SZINT == SZLONG) && !LN_FLAG('p'))
	    ...
	    return 1;
	else if ((b2 == LN_ULLONG) && (b1 == LN_LLONG))
	    return 1;
    } else if (con2 && ISINT(b1)) {
	/*CONSTANTCONDITION*/
	if ((SZINT == SZLONG) && ISINT(b1) && !LN_FLAG('p'))
	    return 1;
	else if ((b1 == LN_UINT) && (b2 == LN_INT))
	    return 1;
	else if ((b1 == LN_ULONG) && (b2 == LN_LONG))
	    return 1;
	else if ((b1 == LN_ULLONG) && (b2 == LN_LLONG))
	    return 1;
    }

    /*
    ** Simple promotions: SINT->INT, SLONG->LONG, SLLONG->LLONG.
    */
    b1 = promote(b1);
    b2 = promote(b2);
