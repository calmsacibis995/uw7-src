#ident	"@(#)alint:common/postopt.c	1.27"
/*
...
#define MAXPRNT	512
static char 	str1[MAXPRNT];	/* used to hold the strings of types	*/
static char	str2[MAXPRNT];
...
static void str_type(char *, T1WORD);
static void pass1_type(T1WORD);
static void pass2_type(T1WORD);
#else
...
static int ln_cpexp();
static void str_type();
static void pass1_type();
static void pass2_type();
#endif
...
**
** Also, the NCEG relational operators on integers can produce always
** true/false situations.
*/
...
ND1 *tr;
{
    ...
    if (TY_ISINTTYPE(tr->type)) {
	switch (op) {
	case LGE:
	case ULGE:
	    WERROR(exptrue, opst[op]);
	    break;
	case NLGE:
	case UNLGE:
	    WERROR(expfalse, opst[op]);
	    break;
	}
    }
...
ND1 *p;
{
	...
	if(IS_SYMBOL(p) && (TY_SIZE(ty) < TY_SIZE(ptype)))
	    return 0;
	else if(op == CALL || op == UNARY CALL) {
	    /* Check size of return value, not the promoted value */
	    p = p->left;
	    ptype = p->type;
	    if(TY_ISPTR(ptype) && TY_ISFTN(TY_DECREF(ptype))
		&& TY_SIZE(ty) < TY_SIZE(TY_DECREF(TY_DECREF(ptype))) )
		return 0;
	}
    }
    return 1;
...
ND1 *tr;
{
    ...
    */
	...
	str_type(str1, l->type);
	str_type(str2, r->type);
	BWERROR(14, str1, str2);
    }
...
int goal;
{
    ...
    if (TY_ISARY(ty)) {
	unsigned long nelem = TY_NELEM(ty);
	long subsc;
	...
	*/
	if ((nelem == TY_NULLDIM) || (nelem == TY_ERRDIM))
	    return;
	if (num_toslong(&tr->c.ival, &subsc)) {
	    WERROR("array subscript too big for pointer arithmetic: %s",
		num_tosdec(&tr->c.ival));
	} else {
	    static const char sub_gt[] = "array subscript cannot be > %d: %d";

	    subsc /= (int)BITOOR(TY_SIZE(TY_DECREF(ty)));
	    /*
	    ...
	    */
		...
		if ((subsc >= nelem))
		    WERROR(sub_gt, (nelem-1), subsc);
	    }
	    /* allow for pointers one past the last element (valid) */
	    else if (subsc > nelem)
		WERROR(sub_gt, nelem, subsc);
	}
    }
}
...

/*
** Called as the trees are being built (before op_optim and constant folding)
**
** If a bit-wise expression is seen.
**    Tree expected:
**
**         BIT-OP   or   COMPL
**         /    \        /
**	  X1    X2      X1
**
** check for LGOP on either X1 or X2
*/
extern void
ln_precedence(p)
ND1 *p;
...
register ND1 *node;
    ...
    if (   !LN_FLAG('h')
        ...
        if (   (   LGOP((node = p->left)->op)
                && (! (node->flags&FF_PAREN)))
            || (   LGOP((node = p->right)->op)
                && (! (node->flags&FF_PAREN)))) {
...
/*
** Format the type as an ASCII string
*/
static char typestr[MAXPRNT];

static void
str_type(char *str, T1WORD ty)
{
    LNBUG(ln_dbflag > 2, ("str_type"));
    typestr[0] = '\0';
    pass1_type(ty);
    pass2_type(ty);
    strcpy(str, typestr);
...
pass1_type(T1WORD ty)
{
	...
	case TY_FUN:
	case TY_ARY:
	    pass1_type(TY_DECREF(ty));
	    break;

	case TY_STRUCT:
	    strcat(typestr, "struct ");
	    goto add_sue_tag;
	    /* NOTREACHED */
	case TY_UNION:
	...
	case TY_ENUM:
	    strcat(typestr, "enum ");
add_sue_tag:
	    ts = SY_NAME(ty_suetag(ty));
	    break;

    /* Basic type below here. */
...
pass2_type(T1WORD ty)
{
    ...
    } else if (TY_ISARYORFTN(ty)) {
	if (TY_ISARY(ty))
	    strcat(typestr, "[]");
	else
	    strcat(typestr, "()");
	pass2_type(TY_DECREF(ty));
    }
}
