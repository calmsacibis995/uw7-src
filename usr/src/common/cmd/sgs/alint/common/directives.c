#ident	"@(#)alint:common/directives.c	1.14"
#include "p1.h"
#include <string.h>
...
char* ikwid_file_name;	/* pointer to filename for last IKWID directive. */

static char baduse[]=
	"directive must be outside function: /* %s */";
static char badcombo[]=
	"cannot use multiple \"...LIKE\" directives on single declaration";
static char implies[]=
	"dubious use of /* VARARGS */ with: /* %s */";
static char needs_arg[]=
	"directive requires argument: /* %s */";
...
unsigned int len;
{
    ...
    */
	...
	if (res == IKWID_DIRECT) {
		...
		char *filename = er_curname();
		LN_DIR(IKWID) = er_getline();
		if (   ! ikwid_file_name
		    || (strcmp(ikwid_file_name, filename) != 0)) {
		    if (ikwid_file_name)
			free(ikwid_file_name);
		    ikwid_file_name = malloc(strlen(filename) + 1);
		    strcpy(ikwid_file_name, filename);
		}
	    }
	    return;
	...
	*/
	...
	case 'V' :
		...
		if (LN_DIR(SCANFLIKE) > 0)
		    ...
		    WERROR(implies, "WSCANFLIKE");
		else if (LN_DIR(WPRINTFLIKE) > 0)
		    WERROR(implies, "WPRINTFLIKE");
		else if (infunc)
	    ...
	    break;
	case 'W' :
	    if ((strncmp(str,"WPRINTFLIKE",11)==0) &&
		((num=ln_getnum(str+11)) != -1)) {
		rval = ln_chkusage(WPRINTFLIKE,"WPRINTFLIKE",num,infunc);
	    }
	    else if ((strncmp(str,"WSCANFLIKE",10)==0) &&
		((num=ln_getnum(str+10)) != -1)) {
		rval = ln_chkusage(WSCANFLIKE,"WSCANFLIKE",num,infunc);
	    }
	    break;
...
int infunc;
{
    ...
    if (LN_DIR(PRINTFLIKE) > 0 || LN_DIR(WPRINTFLIKE) > 0
	    || LN_DIR(SCANFLIKE) > 0 || LN_DIR(WSCANFLIKE) > 0) {
	WERROR(badcombo);
	LN_DIR(direct) = -1;
	return NO_DIRECT;
    }
