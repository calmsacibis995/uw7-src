#ident	"@(#)ld:common/globals.h	1.50"
...
/* truncate A to previous B boundary */
#define TRUNCATE(A, B) ((A) & ~(((B)?(B):1) - 1))
...
typedef struct bss_cpy Bss_cpy;
typedef struct needed_so Second_level_needed_so;
...
struct infile {			/* Input file control structure */
	CONST 	char	*fl_name;	/* The file name */
	CONST	char	*so_name;	/* The SONAME if it's a shared object
					   that has an entry for SONAME */
	Half	fl_e_type;		/* The corresponding file type from 
					   the file header */
	...
	Word	fl_numsyms;		/* Number of symbols in file symbol table */
...
struct sg_desc {		/* Output segment descriptor */
	...
	Ldsym	*sg_basesym;		/* Points to the base addr symbol for this segment. */
	Boolean	sg_downseg;		/* is this a "down" segment */
};	
...
struct ldsym {			/* Symbol table node */
	Sym		*ls_syment;	/* pointer to file symtab representation
					   of a symbol */
	CONST char	*ls_name;	/* Keep a copy of the symbol name here 
					   for performance */
	unsigned long	ls_hashval;	/* The pure hash value of this symbol */
	Deflevel 	ls_deftag;	/* `Level' of this definition */
	...
	Word		ls_GOTndx;	/* Index into global offset table for 
					   this symbol */
	Word		ls_PLTndx;	/* Index into procedure linkage table 
					   for this symbol */
	Word		ls_PLTGOTndx;	/* index of the GOT entry for the PLT 
					   indirection */
	unsigned char	ls_def_binding; /* keep a copy of the definition's 
					   binding for weak symbol solution */
	Boolean		ls_not_alloc;	/* for weak symbol solution, has symbol
					   been allocated */
	Boolean		ls_COPYalloc;	/* has copy reloc entry been allocate 
					   for this symbol */
	Boolean		ls_PLTdone;	/* plt entry created for this symbol */
	Boolean		ls_GOTdone;	/* got entry created for this symbol */
	Boolean		ls_COPYdone;	/* copy reloc entry created for this 
					   symbol */
	Boolean 	ls_Bsymbolic;	/* references to this symbol should be 
					   handled like -Bsymbolic */
	Boolean		ls_Bexport;	/* this symbol should be exported, i.e.
					   be made visible by placing it in the 
					   global part of the special dynamic 
					   symbol table of a dynamic executable
					   */
	Boolean		ls_Bhide;	/* This symbol should be hidden */
};

struct bss_cpy {
	CONST char	*sh_lib_name;	/* name of shared library that contains
...
struct needed_so {
	char *so_name;	
	char *parent_so_name;  /* needed for error messages */
};
...
extern Boolean
	...
	sflag,			/* Strip all syms and debug info */
	xflag,			/* Strip local syms and debug info */
	...
	Bbind_now,		/* Add DT_BIND_NOW to .dynamic section */
	Bsortbss,		/* true if bss symbols have to be sorted
				** by input file in the output sections */
	local_common,		/* we need to allocate bss for local
				 * commons - happens when we hide
				 * a global common in a dot-o
				 */
	
	check_cplus_version,	/* check that we are not loading an
				 * old C++ object
				 */
	create_defs_for_weak_refs,	
				/* compatibility mode - create
				 * definition of absolute symbol
				 * with value 0 for undefined
				 * weak references
				 */
	processing_so;
...
extern SetBexportstate Bflag_export;	/* export global symbols */
extern SetBexportstate Bflag_hide;	/* hide defined global symbols */
extern SetBexportstate qflag_hide;	/* hide defined global symbols */
...
extern Word	count_hidden;		/* Number of output hidden symbols */
...
extern Word	count_delay_rel;		/* Total number of delayed relocation entries in output file */

extern Word	count_strsize;		/* Size in bytes of output symtab strtab for global symbols */
extern Word	count_locstr;		/* Size in bytes of output symtab strtab for local symbols */
...
extern Os_desc	*eh_ranges_sect;	/* output eh_ranges section */
extern Boolean	wrong_cplus_version;	/* attempt to load old C++ object
					 */
...
extern Insect	*first_delay_rel_sect;	/* First output delay relocation section */
...
extern char	**bexport_list;		/* list of symbols to be exported */
extern char	**bhide_list;		/* list of symbols to be hidden */
extern char	**qhide_list;		/* symbols for "quiet" hiding */

extern Boolean	shared_object;

extern	Bss_cpy	*bss_cpy_ptr;
extern	List	bss_cpy_list;

extern	Second_level_needed_so *second_level_needed_so_ptr;
extern	List	second_level_needed_so_list;
extern	char	*LD_ROOT_path;
...
PROTO(void process_files, (int, char **, int));
PROTO(void process_flags, (int, char **));
PROTO(void process_infile, (char *));
PROTO(void process_arg_file, (char *));
...
PROTO(void add_exports_to_dynsymtab, (void));
PROTO(void add_export_list_to_dynsymtab, (void));
PROTO(void mark_hidden_symbols, (char **, int));
PROTO(void mark_def_globals_as_hidden, (void));
PROTO(Insect *getIsectByName, (List *, char *));
PROTO(void build_link_map_for_eh, (void));
PROTO(void update_link_map_for_eh, (void));

PROTO(void process_second_level_needed_so, (void));
PROTO(int force_platform, (CONST char *));
PROTO(void no_note_found, (void));
PROTO(Boolean check_note, (Elf_Scn*, Shdr*, Elf_Data*));

#ifdef	DEBUG
PROTO(void infile_list_print, (void));
