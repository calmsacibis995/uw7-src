#ident	"@(#)ld:common/outfile.c	1.60"
/*
...
build_odynsym()
{
	...
	for (bkt = 0; bkt < NBKTS; bkt++)
		for (LIST_TRAVERSE(&(symbucket[bkt]), np1, sym)) {
			if(sym->ls_deftag == REF_DYN || sym->ls_deftag == REF_DEFN || sym->ls_Bexport == TRUE){
...
build_osymtab()
{
	...
	if (Gflag) { /* will hash the large symbol table */ 
		...
		chain = &hashtab[2 + dynbkts];
		hashtab[0] = dynbkts;
		hashtab[1] = count_outglobs + 1;
			/* 1 for null first entry */
		if (!xflag)
			hashtab[1] += count_hidden + count_outlocs + 1;
			/* 1 for FILE symbol */
	}
	...
	if (!xflag) {
		/* build local part of symtable */
		/* Add a file symbol to the symtab. */

		symptr->st_info = ELF_ST_INFO(STB_LOCAL, STT_FILE);
		symptr->st_shndx = SHN_ABS;

		/* Add its character string to the string table
		 * for the symbol table
		 */

		symptr->st_name = strtab_strptr - 
			(char *)strtab_sect->is_rawbits->d_buf;
		...
		for (LIST_TRAVERSE(&seg_list, np1, sgp))
			for (LIST_TRAVERSE(&(sgp->sg_osectlist), np2, 
				osp)) {
				symptr->st_info = ELF_ST_INFO(STB_LOCAL,
					STT_SECTION);
		...
		/* We must now traverse each file's local symbols 
		 * and add the  local to the output symbol table.
			...
			for (locsym = &(ifp->fl_locals[1]);
				...
				if (ELF_ST_TYPE(locsym->ls_syment->st_info) == STT_FILE)
					...
					if (locsym->ls_syment->st_shndx == SHN_ABS)
						symptr->st_shndx = SHN_ABS;
					else if (locsym->ls_syment->st_shndx
						== SHN_UNDEF)
						symptr->st_shndx = SHN_UNDEF;
					else if ( locsym->ls_scnptr
						   && locsym->ls_scnptr->is_outsect_ptr )
		...
		/*  If we have any symbols to hide in a shared object or 
		** relocatable, add the hidden 
		**  symbols to the to the output symbol table and change the TYPE to 
		**  STB_LOCAL from STB_GlOBAL.
		*/
		
		if ((Gflag || rflag) && count_hidden ){ 
			/* Add a file symbol, _fake_hidden,  
			 * to the symtab 
			 */
			symptr->st_info = ELF_ST_INFO(STB_LOCAL, 
				STT_FILE);
			...
			/* Add its name to the string table */

			symptr->st_name = strtab_strptr - 
				(char *)strtab_sect->is_rawbits->d_buf;
			strcpy(strtab_strptr,FAKE_NAME);
			strtab_strptr += strlen(FAKE_NAME) + 1;
			symptr++;

			for (bkt = 0; bkt < NBKTS; bkt++)
			   for (LIST_TRAVERSE(&(symbucket[bkt]), np1,
				sym)) {
				if( sym->ls_deftag >= REF_DEFN && sym->ls_Bhide) {
					*symptr = *(sym->ls_syment);
					...
					symptr->st_info = 
						...
						ELF_ST_TYPE(symptr->st_info));
					symptr++;
				}
			}
		}
		/* We can now fill in the index of the first global symbol. */
		symtab_sect->is_outsect_ptr->os_shdr->sh_info = 
			count_hidden + count_outlocs + 2;
	} else {
		/* not putting out locals */
		symtab_sect->is_outsect_ptr->os_shdr->sh_info = 1;
	}
...
finish_dyn()
{
	...
	if (Bbind_now) {
		dynbits[dyn_entries].d_tag = DT_BIND_NOW;
		dynbits[dyn_entries].d_un.d_val = 0;
		dyn_entries++;
	}
	...
	/* If C++ EH */
	if (eh_ranges_sect) {
                dynbits[dyn_entries].d_tag = DT_RANGES;
                dynbits[dyn_entries].d_un.d_ptr = eh_ranges_sect->os_shdr->sh_addr;
                dyn_entries++;
		dynbits[dyn_entries].d_tag = DT_RANGES_SZ;
                dynbits[dyn_entries].d_un.d_val = eh_ranges_sect->os_shdr->sh_size;
		dyn_entries++;
        }
	if (count_delay_rel) {
                dynbits[dyn_entries].d_tag = DT_DELAY_REL;
                dynbits[dyn_entries].d_un.d_ptr = first_delay_rel_sect->is_newVAddr;
                dyn_entries++;
		dynbits[dyn_entries].d_tag = DT_DELAY_RELSZ;
                dynbits[dyn_entries].d_un.d_ptr = count_delay_rel;
		dyn_entries++;
		dynbits[dyn_entries].d_tag = DT_DELAY_RELENT;
                dynbits[dyn_entries].d_un.d_ptr = sizeof(Rel);
		dyn_entries++;
	}
...
finish_out()
{
	...
	my_elf_update(outfile_elf, ELF_C_IMPURE_WRITE);
} 
...
open_out()
{
	...
	if ((outfile_fd = open(outfile_name, O_RDWR | O_CREAT | O_TRUNC,
		(mode_t)0666)) < 0) {
		outfile_name = NULL;
		lderror(MSG_FATAL, gettxt(":1116","cannot create output file"));
	}
	...
	if(out_type == ET_EXEC /* || out_type == ET_DYN */ ){
		...
		{
			/* outfile_name is the name of a regular file */
	...
	if ((outfile_ehdr = elf_newehdr(outfile_elf)) == NULL)
		lderror(MSG_FATAL, gettxt(":1117","cannot open output file elf header"));
