...
infile_setup()
{
	...
	if(cur_file_ptr == NULL)
		lderror(MSG_SYSTEM,gettxt(":1051","ld internal error:  null cur_file_ptr"));

	/* check version, etc. */
	if (cur_file_ehdr->e_machine != MACHINE_TYPE)
		lderror(MSG_FATAL, gettxt(":1052","%s: wrong machine type"), cur_file_name);

	if (cur_file_ehdr->e_version > libver)
		lderror(MSG_FATAL,
			gettxt(":1053","%s: creator ELF library version is higher than the maximum version known to ld"),
			cur_file_name);

	if (cur_file_ehdr->e_ident[EI_CLASS] != MACHINE_CLASS)
		lderror(MSG_FATAL, gettxt(":1054","%s: wrong machine class"), cur_file_name);

	if (cur_file_ehdr->e_ident[EI_DATA] != BYTE_ORDER)
		lderror(MSG_FATAL, gettxt(":1055","%s: wrong machine byte order for data encoding"), cur_file_name);
	...
	Shdr		*shdr;
{
	...
	if (shared_object) {
		tscn_name = mymalloc(strlen(scn_name)+1);
		strcpy(tscn_name,scn_name);
		tip->is_name = tscn_name;
	}
	else
        ...
        Shdr		*shdr;
{
	...
	Insect		*tip;
        ...
        if (shdr->sh_type != SHT_STRTAB){
        	lderror(MSG_SYSTEM, gettxt(":1056","section %s: string table for symbol names not of type stringtable"),
				scn_name);
		return;
	}
        data = NULL;
	...
	if (data->d_size != 0 && (((char *)(data->d_buf))[0] != '\0'
		...
		lderror(MSG_WARNING,gettxt(":1057","section %s: malformed string table, initial or final byte"),
			scn_name);

	tip = insect_setup(scn,shdr);

	if(shared_object) {
		tdata = mymalloc(sizeof(*data));
		*tdata = *data;

		/* assign enough space and memcpy data->d_buf */
		tdata->d_buf = mymalloc(data->d_size);
		(void) memcpy(tdata->d_buf,data->d_buf, data->d_size);

        	tip->is_rawbits = tdata;
		/* this assignment assumes 1 symbol-string table pair per file 
		   in .so's should this change, keep a pointer to the stringtab
		   bits in the symtab insect */
		symname_bits = tdata;
	...
	char	*fname;
{
	...
	case ELF_K_ELF:
		...
		case ET_DYN:
			...
			break;
		default:
			lderror(MSG_FATAL, gettxt(":1060","is an illegal elf file type"));
			break;
		}
		break;
	case ELF_K_NONE:
		elf_end(cur_file_ptr);
		free(cur_file_name);
		cur_file_name = fname;
	...
	if(shared_object)
	{
		if (elf_end(cur_file_ptr) != 0)
			lderror(MSG_FATAL,
				gettxt(":1518","failed to close shared object"));
	}
	else
...
process_relobj()
{
	...
	while((scn = elf_nextscn(cur_file_ptr,scn)) != NULL)
	{
		...
		case SHT_SHLIB:
			if ( (dmode && Gflag) || (!dmode && rflag) ){
				lderror(MSG_FATAL,
					gettxt(":1062","static shared library section `%s` valid only when building an executable"),
					scn_name);
			} else
				goto savesec;
			/*FALLTHROUGH*/
		case SHT_NOTE:
			note_data = my_elf_getdata(scn, NULL);
			if (check_note(scn, shp, note_data) == TRUE)
			{
				/* a special note section 
				 * do not add it to output
				 */
				special_note = TRUE;
				break;
			}
		...
		case SHT_PROGBITS:
			...
			if(strip_debug && 
				((strncmp(scn_name, ".debug", 6)
					== SAME) ||
				(strcmp(scn_name,".line") == SAME))) {
				cur_infile_ptr->fl_insect_ndx[scn_ndx] = (Insect*)(-1);
				break;
			}
			/*FALLTHROUGH*/
		case SHT_MOD:
	...
	savesec:
		...
		case SHT_RELA:
		case SHT_REL:
			tip = cur_infile_ptr->fl_insect_ndx[shp->sh_info];
			if (strip_debug && tip == (Insect *)-1)
				/* associated section stripped */
				break;
			if(tip == 0) {
				...
				save_scn_name = scn_name;
                                scn_name = (char *)scn_name_bits->d_buf + (size_t)(nshp->sh_name);
				if(strip_debug && ((strncmp(scn_name, 
					".debug", 6) == SAME) ||
					(strcmp(scn_name,".line") == SAME))) {
					break;
				}
			...
			break;
		case SHT_DYNAMIC:
			lderror(MSG_FATAL,gettxt(":1063", "found a DYNAMIC section in relocatable object file"));
			/*NOTREACHED*/
		default:
			if(shp->sh_type > SHT_HIUSER)
				lderror(MSG_WARNING,gettxt(":1064", "section type 0x%x may conflict with system-defined type"), shp->sh_type);
			else if(shp->sh_type < SHT_LOUSER )
				lderror(MSG_WARNING,gettxt(":1065", "unknown section type 0x%x"), shp->sh_type);
			goto savesec;
		}
	}
	if (special_note == FALSE)
		no_note_found();
	if (sym_scn){
...
process_shobj(fname)
	char* fname;
{
	register Elf_Scn *scn; 		/* the section */
	Elf_Scn		*nscn;
	register Shdr	*shp, *tshp, *t1shp;  /* current section header table 
					      entry */
	Shdr		*nshp;
	Elf_Scn 	*sym_scn;	/* the symbol table */
	Insect		*tip;		/* temp input section ptr */
	size_t		scn_ndx;
	char		*save_scn_name;
	Elf_Data 	*scn_name_bits;
	Elf_Data	*note_data;
	Elf_Data 	*dyndata;	/* Input dynamic section data */
	Dyn		*tdyn;		/* temp dynamic element pointer */
	char		*name_recorded = NULL;	/* override name of this shared
						object (perhaps) */
	char		*cur_so_name, *tname, *needed_name, *need_name, *SONAME;
	Boolean		special_note = FALSE;
	Second_level_needed_so	*tneeded_so;
	...
	while((scn = elf_nextscn(cur_file_ptr,scn)) != NULL)
	{
		...
		case SHT_SHLIB:
			lderror(MSG_FATAL,
				gettxt(":1066","static shared library section `%s` appears in a shared object"),
					scn_name);
			break;
		case SHT_MOD:
		case SHT_NOBITS:
		...
		case SHT_DELAY_REL:
			...
			if (shared_object) { /* malloc space since the elf_end
						will unmap the space for shp */
				tshp = mymalloc(sizeof(*shp));
				*tshp = *shp;
				tip = insect_setup(scn,tshp);
		...
		case SHT_SYMTAB:
			if( cur_infile_ptr->fl_insect_ndx[shp->sh_link] == 0 ){
				...
				save_scn_name = scn_name;
				scn_name = (char *)scn_name_bits->d_buf + (size_t)(nshp->sh_name);
				if (shared_object) { /* malloc space since the 
							elf_end will unmap the 
							space for shp */
					t1shp = mymalloc(sizeof(*nshp));
					*t1shp = *nshp;
					process_strtab(nscn,t1shp);
				}
				else
                                	process_strtab(nscn,nshp);
			}
		...
		case SHT_DYNAMIC:
			...
			for( tdyn = (Dyn*)dyndata->d_buf; tdyn->d_tag !=DT_NULL; tdyn++){
				if( tdyn->d_tag == DT_SONAME && !processing_so){
					if((name_recorded = 
elf_strptr(cur_file_ptr, shp->sh_link,(size_t)tdyn->d_un.d_val))!=NULL){
						SONAME = mymalloc(strlen(name_recorded)+1);
						strcpy(SONAME, name_recorded);
						cur_infile_ptr->so_name = SONAME;
			...
			for( tdyn = (Dyn*)dyndata->d_buf; tdyn->d_tag !=DT_NULL; tdyn++){
				if (tdyn->d_tag == DT_NEEDED){
				  if((need_name = elf_strptr(cur_file_ptr, 						shp->sh_link,(size_t)tdyn->d_un.d_val))!=NULL){
					needed_name = 
						mymalloc(strlen(need_name)+1);
					strcpy(needed_name, need_name);

					tneeded_so = (Second_level_needed_so *)mymalloc( sizeof(Second_level_needed_so));
					tneeded_so->so_name = needed_name;
					tneeded_so->parent_so_name = cur_file_name;
					(void)list_append(&second_level_needed_so_list,tneeded_so);
	...
	if( !name_recorded )
		...
		if(shared_object) {
			tname = mymalloc(strlen(name_recorded)+1);
			strcpy(tname, name_recorded);
		}
		(void)list_append(&soneeded_list,shared_object? tname : name_recorded);
