...
/*	There are a number of builtin cost names:
	...
	NEGRANGE n	constants in the range [-(2**n), -1]
	*/
...
struct optb {		/* operation table */
	...
	long	tyop;	/* the type associated with the op node */
...
static int buildshape(int, int, int, int, long);
static int fillshape(int, int, int, long, RST);
...
%token LCOUNT RCOUNT
%token USERCOST DCOST SHAPES OPCODES EXCEPT
%token CONVAL NCONVAL POSRANGE SGRANGE NEGRANGE NONAME
...
int lval;				/* non-zero if left shape used in asgop */
{
    ...
    long lshp, rshp;			/* corresponding members of pshp[] */
    ...
    */
	...
	long ltype = ((plshp->ssh & SPTYPE) ? TANY : plshp->ssh) & lshty->ty;
	long rtype = ((prshp->ssh & SPTYPE) ? TANY : prshp->ssh) & rshty->ty;
					...
					*/
{
	...
	weight = get_weight(s);		/* get this node's weight */
...
int shno;				/* shape number */
{
    ...
    weight = get_weight(shp[shno].sleft) + get_weight(shp[shno].sright);
...
yylex()
{
	...
	for(;;)
	{
		...
		case '!':
			...
			if( oplook(0) )
			{
				switch (yylval.ival) /* handle 3+ char ops */
				{
				case LS: /* >>=, too */
					if ((c = getchar()) != '=')
						ungetc(c, stdin);
...
unsigned long sh;			/* special shape info */
{
    ...
    case SNRANGE:	spstring = "NEGRANGE"; break;
...
int left;				/* left child */
int right;
long special;				/* special shape bits */
{
...
static long rmssh[NRMERGE];		/* register node types in tree */
