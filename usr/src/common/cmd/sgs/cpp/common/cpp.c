#ident	"@(#)cpp:common/cpp.c	1.45.2.6"
...
# define NPREDEF 20
# endif
STATIC	unsigned char **prespc = NULL;	/* Pointer to beginning of array of
					   pointers to predefined preprocessor
					   symbols */
STATIC	unsigned char **predef = NULL;	/* Pointer to the next available
					   array element. */
STATIC  int	predef_cnt = 0;		/* Current predefined symbol array
					   size. */
STATIC	unsigned char **punspc = NULL;	/* Pointer to beginning of array of
					   pointers to undefined preprocessor
					   symbols */
STATIC	unsigned char **prund = NULL;	/* Pointer to the next available
					   array element. */
STATIC	int	prund_cnt = 0;		/* Current undefined symbol array
					   size. */
STATIC	int	exfail;
struct symtab
{
...
# endif
	if(exfail < 0) fprintf(stderr,"warning: ");
...
STATIC unsigned char **
realloc_ptr_array( arr_ptr, ptr_next_avail_ptr, ptr_curr_sz)
	unsigned char ** arr_ptr;
	unsigned char *** ptr_next_avail_ptr;
	int * ptr_curr_sz;
{
	register unsigned char ** new_array;
	register int new_size;

	new_size = *ptr_curr_sz + NPREDEF;
	new_array = (unsigned char **)realloc((void *)arr_ptr,
					new_size * sizeof(unsigned char **));
	if (new_array) {
		*ptr_next_avail_ptr = new_array + *ptr_curr_sz;
		*ptr_curr_sz = new_size;
	}  /* if */ 
	return new_array;
}
...
#endif
			case 'D':
				if ( predef >= (prespc + predef_cnt) )
				{
					cp2 = realloc_ptr_array( prespc,
								 &predef,
								 &predef_cnt);
					if (!cp2) {
						pperror( "unable to allocate memory, ignoring %s", argv[i] );
						continue;
					} else {
						prespc = cp2;
					}  /* if */
				...
				continue;
			case 'U':
				if ( prund >= (punspc + prund_cnt) )
				{
					cp2 = realloc_ptr_array( punspc,
								 &prund,
								 &prund_cnt);
					if (!cp2) {
						pperror( "unable to allocate memory, ignoring %s", argv[i] );
