...
/*
 * Macro used for VPATH
 */
#define NAME(p) ((p)->vpath?((p)->vpath):((p)->namep))
	...
	for (lp = p->linep; lp; lp = lp->nextline) {
		...
		for (q = lp->depp; q; q = q->nextdep) {
			...
			if(IS_ON(UCBLD) || ptime < td1 )
				appendq((CHAIN) &qchain, NAME(q->depname));
		}
...
*/
			...
			if (lp->shp) { /* shell line specified? */
				if (explcom){	/*  specified before? */
					if(IS_OFF(WARN))
						pfmt(stderr, MM_WARNING, ":263:Too many command lines for `%s' (bu10)\n",
					    p->namep);
				}else
					explcom = lp->shp;
			}
								...
								if ( dp = lp->depp )
									for (; dp; dp = dp->nextdep)
										if ( dp->depname) {
											...
											/*	appendq((CHAIN) &qchain, NAME(p2)); osr5make */
											tdep = MAX(tdep, td);	/* max of all lines */
#ifdef MKDEBUG
						if (IS_ON(DBUG)) {
							blprt(reclevel);
	...
	if ( rebuild && !errstat ) {
		...
		if (explcom)
			...
			if (p1 = SRCHNAME(".DEFAULT")) {
				...
				fatal1(":271:don't know how to make %s (bu42)", p->namep);
...
*/
		...
		pfmt(stderr, MM_ERROR,
			":264:`%s' not remade because of errors (bu14)\n",
				p->namep);
...
DEPBLOCK dep_line;
{
	...
	if (IS_ON(TOUCH)) {
		...
		if (IS_OFF(SIL))
			pfmt(stdout, MM_INFO, ":266:touch(%s)\n", s);
		if (IS_OFF(NOEX))
			touch(s);
	} else {
		char	*string;
		...
		/* Allow recursive makes only if NOEX flag is set */
			...
			if ( !(sindex(q->shbp, "$(BM)")    == -1 &&
		    	...
		    	strPos = 0 ;
		    	string = subst(q->shbp, string, &strLen, &strPos) ;
...
register CHARSTAR s;
{
	...
	if (utime(s, NULL) ) {
		if ((fd = creat(s, (mode_t)0666)) < 0)
			fatal1(":273:Cannot touch %s (bu25)", s);
		(void)close(fd);
	}
