    ...
    DEPBLOCK nextdbl;       /* final value for chain */
{
	extern char *execat() ;
	char n15[PATHSIZE+1];
	char * n15end = &n15[PATHSIZE];
	DIR * dirf;
	char * dirname, * dirpref, * endir, * filepat, * p;
	char temp[PATHSIZE+1];
	char fullname[PATHSIZE+1];
	char * p1, * p2;
	struct varblock *	viewVar;
	char viewPat[PATHSIZE+1];
	char *	vpath;
	struct nameblock * q;
...
/*
 *	Results are not to be remembered. So just do search and return 
 *	immediately if pattern is already on list of patterns.
 */
	if( !mkchain )
	...
	if(*pat=='/') /*Do not use view paths for absolute pathname patterns*/
		vpath=":";
	else
	{
		viewVar=varptr("VPATH");
		if(viewVar->varval.charstar==0)
			vpath=":";
		else
			vpath=viewVar->varval.charstar;
	}

	do
	{
		vpath=execat(vpath, pat, viewPat);
	 ...
	 */
		...
		firstpat = patp;
		patp->patval = copys(viewPat);
	
	/*
	 *	endir points to end of directory portion of path name
	 */
		for(endir=0, p=viewPat; *p!=CNULL; ++p)
			if(*p==SLASH)
				endir = p;
	
	/*
	 *	dirname = directory name part of pattern
	 *	filepat = filename part of pattern
	 */
		dirpref=viewPat+strlen(viewPat)-strlen(pat);
		if(dirpref==(endir+1) || endir==0)
			dirpref="";
		...
		{
			*endir = CNULL;
			dirname = concat(viewPat, "/", temp);
			filepat = endir+1;
		}
	/*
	 *	Search for directory matching directory name part of pattern
	 */
		for(dirf=NULL, od=firstod ; od!=0; od = od->nextopendir)
		{
			if(STREQ(dirname, od->dirn))
			{
				dirf = od->dirfc;
	...
	/*
	 *	If directory not found, add it to list
	 */
		if(dirf == NULL )
		{
			...
			if (dirf != NULL) {
				if( ++opendirs < MAXODIR) 
				{
					...
					dirofl = 1;	/* Close directory afterwards	*/
			}
		}
		...
		/*
		 *	Scan directory, adding names to list of symbol names.
		 */
			while ((d_ent = readdir(dirf)) != NULL)
			{
				if(d_ent->d_ino!= 0)
				{
					p1 = d_ent->d_name;
					p2 = n15;
					while( (p2<n15end) && (*p2++ = *p1++)!=CNULL ) ;
