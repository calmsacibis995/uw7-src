	...
	char **av;
{
	...
	for (bl--; bl >= 0; bl--) {
		...
		for ( ; bp >= buf; bp--) {
			if(bp->ut_type == INIT_PROCESS
			   && strncmp(bp->ut_name, "rc", 2) == 0 
			   && (bp->ut_name[2] == '0' || bp->ut_name[2] == '5'
			   || bp->ut_name[2] == '6')) 
				crmsg = "shutdown";
			if(bp->ut_type == BOOT_TIME) {
				boottime = bp->ut_xtime;
				crmsg = "crash";
				memset(&ttnames, 0, sizeof ttnames);
			}
			print = want(bp);
			if (print) {
