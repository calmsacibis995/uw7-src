...
/* Interrupt has been ignored flag. */
int IntIgnored = 0;
...
EightBitCharHelp() {
	...
	pfmt(stderr, MM_WARNING, ":214:There are no extended characters available for your US-ASCII terminal.\n\n");
	pfmt(stderr, MM_NOSTD, ":215:If you are actually using a terminal or terminal emulator with a richer\ncharacter set, you must use the '-a' option or the 'MM_CHARSET' environment\nvariable to inform this program of that fact.\n");
    } else {
	static char *kbd1[] =
	  {
	    "1234567890-=",
...
TildeHelp() {
    ...
    fp = fopen(TmpName, "w");
    if (!fp) fp = stdout;
    pfmt(fp, MM_INFO, ":3:The following tilde escapes are BSD-mail-compatible:\n");
    pfmt(fp, MM_NOSTD, ":4:~? Show help on tilde escapes\n");
    pfmt(fp, MM_NOSTD, ":5:~| RESERVED FOR BSD MAIL COMPATIBILITY\n");
    pfmt(fp, MM_NOSTD, ":6:~f RESERVED FOR BSD MAIL COMPATIBILITY\n");
    pfmt(fp, MM_NOSTD, ":7:~m RESERVED FOR BSD MAIL COMPATIBILITY\n");
    pfmt(fp, MM_NOSTD, ":8:~! Shell escape\n");
    pfmt(fp, MM_NOSTD, ":9:~~ Enter text line starting with a tilde\n");
    pfmt(fp, MM_NOSTD, ":10:~. Send the mail and exit\n");
    pfmt(fp, MM_NOSTD, ":11:~c Add to Cc list\n");
    pfmt(fp, MM_NOSTD, ":12:~d Read from %s (or named file, ~d filename)\n", deadLetter);
    pfmt(fp, MM_NOSTD, ":13:~e Edit message being composed\n");
    pfmt(fp, MM_NOSTD, ":14:~h Edit the headers\n");
    pfmt(fp, MM_NOSTD, ":15:~p Print out the message so far\n");
    pfmt(fp, MM_NOSTD, ":16:~q Quit, copying to %s\n", deadLetter);
    pfmt(fp, MM_NOSTD, ":17:~r Read the named text file into the message\n");
    pfmt(fp, MM_NOSTD, ":18:~s Reset the subject\n");
    pfmt(fp, MM_NOSTD, ":19:~t Add to To list\n");
    pfmt(fp, MM_NOSTD, ":20:~v Edit using VISUAL editor\n");
    pfmt(fp, MM_NOSTD, ":21:~w Write message to named file\n");
    fprintf(fp, "\n");
    pfmt(fp, MM_NOSTD, ":22:The following tilde escapes are unique to this program:\n");
    pfmt(fp, MM_NOSTD, ":23:~/<number> Set maximum size before message is split into multiple parts\n");
    pfmt(fp, MM_NOSTD, ":24:~?+ Show help on extended (eight-bit) characters\n");
    pfmt(fp, MM_NOSTD, ":25:~> Indent Left Margin\n");
    pfmt(fp, MM_NOSTD, ":26:~< Unindent Left Margin\n");
    pfmt(fp, MM_NOSTD, ":27:~<R Indent Right Margin\n");
    pfmt(fp, MM_NOSTD, ":28:~>R Unindent Right Margin\n");
    pfmt(fp, MM_NOSTD, ":29:~+ Enter 8-bit mode for non-ASCII characters\n");
    pfmt(fp, MM_NOSTD, ":30:~- Leave 8-bit mode (return to ASCII)\n");
    pfmt(fp, MM_NOSTD, ":31:~^ Toggle \"Upside-down\" (right-to-left) mode.\n");
    pfmt(fp, MM_NOSTD, ":32:~* Add non-text data (pictures, sounds, etc.) as a new MIME part (try it!)\n");
    pfmt(fp, MM_NOSTD, ":33:~b Toggle bold mode\n");
    pfmt(fp, MM_NOSTD, ":34:~i Toggle italic mode\n");
    pfmt(fp, MM_NOSTD, ":35:~j Alter Justification (~jc = center, ~jl = flushleft, ~jr = flushright.)\n");
    pfmt(fp, MM_NOSTD, ":36:~k Toggles whether or not to keep a 'blind' copy of your mail\n");
    pfmt(fp, MM_NOSTD, ":37:~n Force newline (hard line break)\n");
    pfmt(fp, MM_NOSTD, ":38:~Q Toggle quotation (excerpt) mode\n");
    pfmt(fp, MM_NOSTD, ":39:~S Toggle Semitic text mode (combines 8-bit and left-to-right)\n");
    pfmt(fp, MM_NOSTD, ":40:~u Toggle underline mode\n");
    pfmt(fp, MM_NOSTD, ":41:~z Add the contents of %s as a TEXT signature.\n", signature);
    pfmt(fp, MM_NOSTD, ":42:~Z Add the contents of %s as a NON-TEXT (MIME-format) signature.\n", mimeSignature);
    pfmt(fp, MM_NOSTD, ":43:For further information, read the man page.\n");
    fprintf(fp, "\n");
    if (fp != stdout) {
	fclose(fp);
...
InitUserName()
{
	...
	pfmt(stderr, MM_ERROR, ":44:Unable to find UserName in environment or UUCP Config file\n");
	exit(10);
    }
    strcpy(userName, cp);
    if ((cp = FindConfig("NodeName")) == NULL) {
	pfmt(stderr, MM_ERROR, ":45:Unable to find NodeName in UUCP Config file\n");
	exit(10);
    }
    strcpy(myAddress, cp);
    if ((cp = FindConfig("DomainName")) == NULL) {
	pfmt(stderr, MM_ERROR, ":46:Unable to find DomainName in UUCP Config file\n");
	exit(10);
    }
    strcat(myAddress, cp);
...
nomemabort() {
    pfmt(stderr, MM_ERROR, ":47:Out of memory\n");
    cleanexit(-1);
}
...
char **argv;
{
	...
	i = atoi(sdum);
	if (i < MINCHUNKSIZE) {
	    pfmt(stderr, MM_WARNING, ":48:Ignoring SPLITSIZE environment variable of %d -- the minimum value is %d\n", i, MINCHUNKSIZE);
	} else {
	    SplitSize = i;
	}
    }
	...
	if (argv[i][0] == '-') {
		...
		case 'a':
			...
			pfmt(stderr, MM_ACTION, ":49:-a requires a character set specification to follow\n");
			cleanexit(-1);
		    }
		    break;
		case 'c':
		...
		case 'r':
			...
			pfmt(stderr, MM_ACTION, ":50:-r requires a Message-ID to follow\n");
			cleanexit(-1);
		    }
		    break;
		case 's':
			...
			pfmt(stderr, MM_ERROR, ":51:-s requires a subject specification to follow\n");
			cleanexit(-1);
		    }
		    break;
		default:
		    pfmt(stderr, MM_ACTION, ":52:Unrecognized option %s\n", argv[i]);
		    cleanexit(-1);
	    }
	} else {
	...
	pfmt(stderr, MM_ERROR, ":53:Unsupported character set: %s\n", CharacterSet);
	exit(-1);
    }
    if (strcmp(CharacterSet, "us-ascii")
#ifdef TRUE /*JALE*/
	 && strncmp(CharacterSet, "iso-2022-jp", 11)
#endif
	 && strncmp(CharacterSet, "iso-8859-", 9)) {
	pfmt(stdout, MM_INFO, ":54:Composing mail in character set %s\n", CharacterSet);
    }
    ProcessMailcapFiles();
    if (!ToList) {
    ...
    if (!fpout) {
	pfmt(stderr, MM_ERROR, ":55:Cannot open temporary file %s\n", CurrentPart->filename);
	cleanexit(-1);
    }
    do {
	IntIgnored = 0;
	while (!AllDone && fgets(LineBuf, MAX_LINELENGTH, stdin) != NULL) {
		...
		switch (LineBuf[1]) {
			...
			break;
		    case '!':
			if (!V_quiet) pfmt(stdout, MM_INFO, ":56:Executing: %s\n", start);
			SYSTEM(start);
			fprintf(stdout,"!\n");
			break;
		    case '>':
			...
			i = atoi(start);
			if (i < MINCHUNKSIZE) {
			    pfmt(stderr, MM_WARNING, ":57:Ignoring splitsize setting of %d -- the minimum value is %d\n", i, MINCHUNKSIZE);
			} else {
			    SplitSize = i;
			    if (!V_quiet) pfmt(stdout, MM_INFO, ":58:Set splitsize to %d\n", i);
			}
			break;
		    case '+':
			if (!strcmp(CharacterSet, "us-ascii")) {
			    pfmt(stderr, MM_ERROR, ":59:No 8-bit characters allowed in ASCII mail\n");
			} else {
			    EightBitMode = 1;
			    if (!V_quiet) pfmt(stdout, MM_INFO, ":60:Entering text in eight-bit mode\n");
			}
			break;
		    case '-':
			EightBitMode = 0;
			if (!V_quiet) pfmt(stdout, MM_INFO, ":61:Entering text in seven-bit (normal) mode\n");
			break;
		    case '^':
			RightToLeftMode = ! RightToLeftMode;
			if (!V_quiet) pfmt(stdout, MM_INFO, ":62:%s right-to-left mode\n", RightToLeftMode ? "Entering" : "Exiting");
			break;
		    case '*':
			{
			...
			if (!V_quiet) pfmt(stdout, MM_ERROR, ":64:Cc field is now: %s\n", CCList);
			break;
		    case 'd':
			{
			...
			fpin = fopen(fnam, "r");
			if (!fpin) {
			    pfmt(stderr, MM_ERROR, ":65:Cannot open file %s\n", fnam);
			    break;
			}
			TempCloseStyles(fpout);
			fclose(fpout);
			...
			fclose(fpin);
			fclose(fpout);
			pfmt(stdout, MM_INFO, ":67:Included contents of %s\n", fnam);
			CurrentPart->next = NewPart();
			CurrentPart->next->prev = CurrentPart;
			CurrentPart = CurrentPart->next;
			fpout = fopen(CurrentPart->filename, "w");
			...
			switch (*start) {
				...
				break;
			    default:
				pfmt(stderr, MM_WARNING, ":68:Unrecognized justification state: %c\n", *start);
				NewJustificationState = JustificationState;
			}
			if (JustificationState == NewJustificationState) {
			    if (!V_quiet) pfmt(stderr, MM_INFO, ":69:No change in justificiation\n");
			} else {
			...
			if (!V_quiet) if (V_keepblind) pfmt(stderr, MM_INFO, ":70:Will keep a blind copy of this message\n");
				    else pfmt(stderr, MM_INFO, ":71:Won't keep a blind copy of this message\n");
			break;
		    case 'n':
			CurrentPart->isrich = 1;
			fputs("<nl>", fpout);
			if (!V_quiet) pfmt(stderr, MM_ERROR, ":72:Inserted line break\n");
			break;
		    case 'p':
			{
			FILE *fptmp;
		    ...
		    case 'r':
			if (! *start) {
			    pfmt(stderr, MM_WARNING, ":73:No file name given.\n");
			    break;
			}
			fpin = fopen(start, "r");
			if (!fpin) {
			...
			fclose(fpin);
			if (!V_quiet) pfmt(stderr, MM_INFO, ":67:Included contents of %s\n", start);
			break;
		    case 'S':
			...
			if (EightBitMode) {
			    ...
			    if (!strcmp(CharacterSet, "us-ascii")) {
				pfmt(stderr, MM_WARNING, ":59:No 8-bit characters allowed in ASCII mail\n");
				break;
			    }
			    EightBitMode = 1;
			    RightToLeftMode = 1;
			...
			if (!V_quiet)
			    if (RightToLeftMode) pfmt(stderr, MM_INFO, ":76:Entering Eight-bit and right-to-left modes\n");
			    else pfmt(stderr, MM_INFO, ":77:Exiting Eight-bit and right-to-left modes\n");
			break;
		    case 's':
			{
			char *subj=start;
			...
			if (!V_quiet) pfmt(stderr, MM_INFO, ":78:To field is now: %s\n", ToList);
			break;
		    case 'u':
			...
			TempCloseStyles(fptmp);
			if(fclose(fptmp)) {
			    pfmt(stderr, MM_ERROR, ":79:Could not write file %s\n", fname);
			} else {
			    pfmt(stdout, MM_INFO, ":80:Wrote draft to %s\n", fname);
			}
			fpout = fopen(CurrentPart->filename, "a");
			if (!fpout) {
			    pfmt(stderr, MM_ERROR, ":55:Cannot open temporary file %s\n", CurrentPart->filename);
			...
			fclose(fpin);
			fclose(fpout);
			pfmt(stderr, MM_INFO, ":67:Included contents of %s\n", fnam);
			CurrentPart->next = NewPart();
			CurrentPart->next->prev = CurrentPart;
			CurrentPart = CurrentPart->next;
			fpout = fopen(CurrentPart->filename, "w");
		    ...
		    default:
			pfmt(stderr, MM_ERROR, ":81:Unrecognized tilde escape: %c\n", LineBuf[1]);
			break;
		}
	    } else {
...
struct mailpart *FirstPart;
{
	...
	struct passwd *p = getpwuid(getuid());
	if (!p) {
	    pfmt(stderr, MM_ERROR, ":84:Can't find your user id to keep a blind copy\n");
	} else {
	...
	while(FirstPart) {
	    ...
	    if (!fpin) {
		pfmt(stderr, MM_ERROR, ":85:Cannot read temporary file %s\n", FirstPart->filename);
		cleanexit(-1);
	    }
	...
	fpin = fopen(FirstPart->filename, "r");
	if (!fpin) {
	    pfmt(stderr, MM_ERROR, ":86:Can't read temporary file %s\n", FirstPart->filename);
	    cleanexit(-1);
	}
	(void) TranslateInputToEncodedOutput(fpin, fp, FirstPart->encoding_type_needed, FirstPart->content_type);
	fclose(fpin);
...
char *envstartstr;
{
    ...
    Stack[StackSize++] = t;
    if (!V_quiet) pfmt(stderr, MM_INFO, ":86:Beginning: %s\n", t);
}
...
char *s;
{
    ...
    PartEndsWithNewline=0;
    if (!V_quiet) pfmt(stderr, MM_ERROR, ":87:Ending: %s\n", s);
}
...
{
    if (AlreadyInStyle(name)) {
	pfmt(stderr, MM_WARNING, ":88:Already in %s style\n", name);
    } else {
	part->isrich = 1;
...
struct mailpart *part;
{
	...
	pfmt(stderr, MM_WARNING, ":89:You aren't in a %s style\n", name);
    }
}
...
int signum;
{
    ...
    if (signum == SIGINT) {
	if (V_ignore) {
	    pfmt(stderr, MM_INFO, ":90:Interrupt ignored because 'ignore' is set.  Use ~q if you want to quit.\n");
	    IntIgnored++;
	    return;
	} else {
	    (void) WriteDeadLetter();
...
WriteDeadLetter()
{
	...
	pfmt(stderr, MM_INFO, ":80:Wrote draft to %s\n", deadLetter);
	return(0);
    }
#else
    char DeadFile[1000];
    ...
    WriteOutMessage(fp, ToList, Subject, CCList, FirstPart);
    if(fclose(fp)) {
	pfmt(stderr, MM_ERROR, ":92:Could not write ~/dead.letter: %s\n", strerror(errno));
	return(-1);
    } else {
	pfmt(stderr, MM_ERROR, ":93:Wrote draft to ~/dead.letter\n");
	return(0);
    }
#endif
}
...
FILE *fp;
{
    ...
    *rawentry = '\0';
    ...
    s = index(rawentry, ';');
    if (!s) {
	pfmt(stderr, MM_WARNING, ":94:Ignoring invalid mailcap entry: %s\n", rawentry);
	free(rawentry);
	return(NULL);
    }
    *s++ = '\0';
...
/*	    if (*arg) fprintf(stderr, " Ignoring invalid mailcap flag: %s\n", arg);  */
	}
	s = t;
    }
    free(rawentry);
...
ProcessMailcapFiles()
{
	...
	while (fp && !feof(fp)) {
		...
		pfmt(stderr, MM_WARNING, ":95:A mailcap file is configured to allow the generation\n");
		pfmt(stderr, MM_NOSTD, ":96:of a content-type with a wildcard.  This is probably a mistake.\n");
		pfmt(stderr, MM_NOSTD, ":97:The relevant mailcap entry is in the file %s\n", path);
		pfmt(stderr, MM_NOSTD, ":98:and is for content-type %s.\n\n", mc->contenttype);
	    }
...
CreateNewPart() {
    ...
    mp = NewPart();
    if (!mp) return(NULL);
    pfmt(stderr, MM_NOSTD, ":99:Please choose which kind of data you wish to insert:\n\n");
    pfmt(stderr, MM_NOSTD, ":100:0: A raw file, possibly binary, of no particular data type.\n");
    pfmt(stderr, MM_NOSTD, ":101:1: Raw data from a file, with you specifying the content-type by hand.\n");
    i = 1;
    while (mc) {
	if (mc->composecommand || mc->composetypedcommand) {
		...
		pfmt(stderr, MM_NOSTD, ":102:%d: data in '%s' format\n", i, mc->contenttype);
	    }
	}
	mc = mc->next;
    }
    pfmt(stderr, MM_NOSTD, ":103:\n\nEnter your choice as a number from 0 to %d: ", i);
    fflush(stdout);
	...
	if (! *sdum) {
	    pfmt(stderr, MM_INFO, ":105:Data insertion cancelled\n");
	    return(NULL);
	}
	fpi = fopen(sdum, "r");
	if (!fpi) {
	    pfmt(stderr, MM_ERROR, ":106:Cannot read %s, data insertion cancelled\n", sdum);
	    return(NULL);
	}
	fpo = fopen(mp->filename, "w");
	if (!fpo) {
	    pfmt(stderr, MM_ERROR, ":107:Cannot open temporary file, data insertion cancelled\n");
	    return(NULL);
	}
	...
	if (ans == 1) {
		...
		if (index(LineBuf, '/')) {
		    ...
		    if (!yesexpr) yesexpr = "^(Y|y)";
		    pfmt(stderr, MM_NOSTD, ":109:The MIME content-type '%s' is not listed in your local mailcap files,\nand may not be a valid MIME type.  Do you want to use it anyway [no] ? ", LineBuf);
		    s = gets(AnsBuf);
	...
	pfmt(stderr, MM_INFO, ":112:Included data in '%s' format\n", mp->content_type);
	SetTextFlags(mp);
	return(mp);
    } else if (ans<0 || ans > i) {
	pfmt(stderr, MM_INFO, ":105:Data insertion cancelled\n");
    ...
    pfmt(stderr, MM_INFO, ":114:Executing: %s\n", CmdBuf);
    resultcode = SYSTEM(CmdBuf);
    if (resultcode) {
	pfmt(stderr, MM_INFO, ":115:Command execution failed, nothing included\n");
	free(CmdBuf);
	return(NULL);
    }
    if (access(mp->filename, R_OK)) {
	pfmt(stderr, MM_INFO, ":116:No file was created, nothing included!\n");
	free(CmdBuf);
	return(NULL);
    }
    mp->istext = 0;
	...
	if (!fp) {
	    pfmt(stderr, MM_ERROR, ":117:Cannot read file %s, no data included.\n", mp->filename);
	    free(CmdBuf);
	    return(NULL);
	}
	...
	if (!ctypebuf) {
	    pfmt(stderr, MM_ERROR, ":118:The command (%s) did not return a content-type value, so no data could be included.\n", CmdBuf);
	    free(CmdBuf);
	    return(NULL);
	}
	...
	if (cencbuf) {
		...
		pfmt(stderr, MM_ERROR, ":119:The command (%s) returned an unrecognized content-transfer-encoding value, so no data could be included.\n", CmdBuf);
		free(CmdBuf);
		return(NULL);
	    }
	} else {
	...
	if (!fpout) {
	    pfmt(stderr, MM_ERROR, ":120:Cannot open file %s for writing, no data included.\n", newfilename);
	    free(CmdBuf);
	    return(NULL);
	}
...
char *Buf, *controlstring, *TmpFileName;
{
	...
	if (prefixed) {
		...
		case 's':
		    ...
		    break;
		default:
		    pfmt(stderr, MM_ERROR, ":122:Ignoring unrecognized format code in mailcap file: %%<%c\n", *from);
		    break;
	    }
	} else if (*from == '%') {
	    prefixed = 1;
...
int UseVisual;
{
    ...
    mp = FirstPart;
    while (mp) {
	pfmt(stderr, MM_INFO, ":124:Part #%d is of type '%s'.\n", partct, mp->content_type);
	...
	if (CmdBuf2[0]) {
	    pfmt(stderr, MM_NOSTD, ":125:Which command do you want to use to edit it?\n\n");
	    pfmt(stderr, MM_NOSTD, ":126:1: %s\n", CmdBuf2);
	    pfmt(stderr, MM_NOSTD, ":127:2: %s\n", CmdBuf);
	    pfmt(stderr, MM_NOSTD, ":128:\n\nEnter 1 or 2, or 0 to not edit it: ");
	    fflush(stdout);
	    fgets(LineBuf, sizeof(LineBuf), stdin);
	    ans = atoi(LineBuf);
	} else ans = 2;
	if (ans == 1) {
	    pfmt(stderr, MM_INFO, ":53:Executing: %s\n", CmdBuf2);
	    SYSTEM(CmdBuf2);
	} else if (ans == 2) {
	    pfmt(stderr, MM_INFO, ":53:Executing: %s\n", CmdBuf);
	    SYSTEM(CmdBuf);
...
int DoSet;
{
	...
	*s++ = '\0';
	if (!strcmp(cmd, "askcc")) {
	    ...
	    i = atoi(cmd+10);
	    if (i < MINCHUNKSIZE) {
		pfmt(stderr, MM_ERROR, ":129:Ignoring splitsize mailrc setting of %d -- the minimum value is %d\n", i, MINCHUNKSIZE);
	    } else {
		SplitSize = i;
	    }
	} else {
...
char *aliasline;
{
    ...
    if (!*s2) {
	if (s != s2) pfmt(stderr, MM_WARNING, ":130:ignoring bad alias line in init file: %s\n", aliasline);
	free(s);
	free(tmpalias);
	return;
    }
...
char *hdr;
{
    ...
    if (firstnonascii) {
	if (!strcmp(CharacterSet, "us-ascii")) {
	    pfmt(stderr, MM_WARNING, ":131:The prevailing character set is ASCII.  Your %s header has\nnon-ASCII or non-printing ASCII characters which may be unreadable.\n", hdr);
	}
	while (firstnonascii) {
...
SwitchToEuropean() {
    pfmt(stderr, MM_WARNING, ":132:You have entered 8-bit characters in what is supposed to be\n");
    pfmt(stderr, MM_NOSTD, ":133:plain ASCII text.  If you are using a non-ASCII character set, you should\n");
    pfmt(stderr, MM_NOSTD, ":134:declare this to be the case with the MM_CHARSET environment variable.\n");
    pfmt(stderr, MM_NOSTD, ":135:For now, I am assuming you are using the iso-8859-1 character set,\n");
    pfmt(stderr, MM_NOSTD, ":136:but this may be false.\n");
