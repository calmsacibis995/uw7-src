...
char *FindParam();
extern FILE *popen();
static char *nomem = ":47:Out of memory!\n";
...
char **argv;
{
	...
	if (!InputFP) ExitWithError(":156:Cannot read input file\n");
...
int nestingdepth;
{
	...
	if (!boundary) {
	    ExitWithError(":157:Bad message format -- multipart message has no boundary parameter!\n");
	}
	if (boundary[0] == '"') {
	...
	if (DoDebug) pfmt(stderr, MM_INFO, ":137:Handling multipart as built-in here.  Boundary: %s\n", boundary);
	...
	if (IsAlternative) {
		...
		pfmt(stderr, MM_WARNING, ":138:Cannot handle any part of multipart/alternative message\n");
	    }
	}
	return(0);
    }
	...
	if (MightAskBeforeExecuting
		...
		switch(ans) {
			...
			while (needname) {
			    needname = 0;
			    pfmt(stderr, MM_NOSTD, ":143:Please enter the name of a file to which the data should be written\n(Default: %s) > ", suggestedname);
			    fflush(stdout);
			    fgets(Fname, sizeof(Fname), stdin);
				...
				if (!p) {
				    MkTmpFileName(Fname);
				    pfmt(stderr, MM_WARNING, ":144:Cannot figure out what ~ means, using temporary file %s instead\n", Fname);
				} else {
				...
				char *s, AnsBuf[50];
				int overwriteans = -1;
				char *yesexpr = 0, *noexpr = 0;
				regex_t re;
				yesexpr = nl_langinfo(YESEXPR);
				noexpr = nl_langinfo(NOEXPR);
				if (!yesexpr) yesexpr = "^(Y|y)";
				if (!noexpr) noexpr = "^(N|n)";
				do {
				    pfmt(stderr, MM_NOSTD, ":145:File %s exists.  Do you want to overwrite it (y/n) ?\n", Fname);
				    s = gets(AnsBuf);
				    if (!s) {
					overwriteans = 0;
				    } else {
					...
					if (len > 0) {
						...
						regcomp(&re, noexpr, REG_EXTENDED|REG_NOSUB);
						status =  regexec(&re, s, (size_t)0, NULL, 0);
						if (status == 0) {
						    /* answered no,
						       get new filename */
				    ...
				    if (overwriteans == -1) pfmt(stderr, MM_ERROR, ":146:Please answer yes or no.\n");
...
char **argv;
{
    ...
    if (DoDebug) pfmt(stderr, MM_INFO, ":153:Metamail Version %s, debugging turned on.\n", mmversion);
	...
	if (argv[i][0] == '-') {
		...
		case 'B':
#ifdef MSDOS
		    pfmt(stderr, MM_ERROR, ":154:metamail warning: -B flag not supported on this system\n");
#else
		    DoInBackground = 1;
#endif
		    break;
		...
		case 'h':
		    ...
		    if (!tmpfp) {
			ExitWithError(":155:Cannot execute lpr command\n");
		    }
		    fflush(stdout);
		    close(1);
		    dup2(fileno(tmpfp), 1);
		...
		case 'r':
#ifdef MSDOS
		    pfmt(stderr, MM_WARNING, ":166:-r flag not supported on this system\n");
#else
		    RunAsRootOK = 1;
#endif
		    break;
		case 'R':
		    ...
		    if (DoDebug) pfmt(stderr, MM_INFO, ":167:Executed reset\n");
#else
		    pfmt(stderr, MM_WARNING, ":168:-R flag not supported on this system\n");
#endif
		    break;
		case 's':
		    if (++i >= argc) usage();
	...
	pfmt(stderr, MM_ACTION, ":169:-B is incompatible with -p, -P, and MM_USEPAGER\n");
	usage();
    }
    if (!Is822Format && !ContentType) {
	pfmt(stderr, MM_ACTION, ":170:-b requires -c.\n");
	usage();
    }
    if (DeleteSourceFileWhenDone && !SourceFileName) {
	pfmt(stderr, MM_ACTION, ":171:-z requires -f.\n");
	usage();
    }
#if !defined(AMIGA) && !defined(MSDOS)
    if (!RunAsRootOK && (getuid() == 0 || geteuid() == 0)) {
	pfmt(stderr, MM_ERROR, ":172:You can not run MetaMail as root unless you use -r or set MM_RUNASROOT\n.");
	usage();
    }
#endif
    if (SourceFileName) {
...
usage() {
    pfmt(stderr, MM_ACTION, ":173:Usage:  metamail [-b] [-B] [-d] [-e] [-h] [-r] [-R] [-p]  [-P] [-x] [-y] [-z] [-c content-type] [-E content-transfer-encoding] [-f from-name] [-m mailername] [-s subject] [message-file-name]\n");
    ExitWithError((char*)NULL);
}
...
int argc;
{
	...
	if (!fptmp) ExitWithError(":158:Can't open temporary file\n");
	...
	*SourceFileNamePtr = TmpName;
    ...
    DefinitelyNotTty = 0;
    SetUpEnvironment();
    if (DoDebug) pfmt(stderr, MM_INFO, ":114:Executing: %s\n", FullCmd);
    fflush(stdout); fflush(stderr);
#ifdef MSDOS
    system(FullCmd);
...
char *SquirrelFile;
{
    ...
    if (DoDebug) pfmt(stderr, MM_WARNING, ":175:Looking for '%s' in built-in content-type handling settings.\n", ContentType);
...
char *file, *SquirrelFile;
{
    ...
    if (DoDebug) pfmt(stderr, MM_INFO, ":176:Looking for '%s' in mailcap file '%s'.\n", ContentType, file);
    while (fp && !feof(fp)) {
	mc.needtofree = 0;
...
eatmore:
    ...
    /* point at beginning of non-space */
    oldprogname = progname;
    /* get rid of leading things to skip */
    ...
    /*  skip args on thing to be skipped */
    if (*progname == '-') {
	++progname;
    ...
    /*
     * When we cat data to a command, we want to know what
     * the command is, not just that we're cat-ing somewhere.
     */
    if (!strncmp(FullProgName, "cat", strlen("cat"))) {
	return(FullProgName);
    }
...
char *SquirrelFile;
{
    ...
    if (DoDebug) pfmt(stderr, MM_INFO, ":177:Trying mailcap entry for '%s'.\n", mc.contenttype);
...
char *SquirrelFile;
{
    ...
    if (!outfp) {
	pfmt(stderr, MM_ERROR, ":178:Cannot open %s to squirrel away a portion of a multipart/alternative\n", SquirrelFile);
	return(-1);
    }
    ...
    if (fclose(outfp)) {
	ExitWithError(":159:fclose failed\n");
    }
    return(0);
}
...
struct MailcapEntry mc;
{
    ...
    if (DoDebug) pfmt(stderr, MM_INFO, ":179:Match!  Built command %s.\n", cmd);
	...
	if (s && strncmp(s, "metamail", 8)) {
		...
		pfmt(stderr, MM_WARNING, ":180:'less' without '-r' behaves badly for some mail types,\n\tnotably richtext.\n");
	    }
#endif
	} else {
	    strcat(cmd, "more");
	...
	if (!resultcode) {
	    ...
	    pfmt(stderr, MM_INFO, ":181:Command failed: %s\n", FullCmd);
	...
	if (DoDebug) pfmt(stderr, MM_INFO, ":182:Not executing command.\n");
...
struct MailcapEntry *mc;
{
    ...
    if (DoDebug) pfmt(stderr, MM_ERROR, ":183:Executing test command: %s\n", cmd);
    result = system(cmd);
#ifdef MSDOS
...
struct MailcapEntry *mc;
{
    ...
    *rawentry = 0;
    ...
    s = index(rawentry, ';');
    if (!s) {
	pfmt(stderr, MM_ERROR, ":184:Ignoring invalid mailcap entry: %s\n", rawentry);
	free(rawentry);
	return(0);
    }
    *s++ = 0;
...
/*        if (t) *t++ = 0; */
	...
	if (!strcmp(arg, "needsterminal")) {
	    ...
	    if (*arg && DoDebug) pfmt(stderr, MM_INFO, ":94:Ignoring mailcap flag: %s\n", arg);
	}
	s = t;
    }
    free(rawentry);
...
char *txt;
{
    if (txt) pfmt(stderr, MM_ERROR, txt);
    exit(-1);
}
...
yankagain:
    ...
    *t = 0;
	...
	if (nestingdepth) {
	    ...
	    if (YankMode) {
		ExitWithError(":160:Could not extract a MIME message from the body\n");
	    } else {
		ExitWithError(":161:Could not find end of mail headers\n");
	    }
	}
    }
...
PrepareMessage() {
	...
	if (!lc2strcmp(ContentEncoding, "base64")) {
	    ...
	    pfmt (stderr, MM_WARNING, ":186:Using nonstandard %s encoding, trying uuencode algorithm.\n", ContentEncoding);
		...
		pfmt(stderr, MM_WARNING, ":187:Ignoring unrecognized Content-Transfer-Encoding value: %s\n", ContentEncoding);
	    }
	}
    }
...
#define putenv(var)        newenviron[i++] = var;
    ...
    environ = newenviron;
    if (DoDebug) {
	pfmt(stderr, MM_INFO, ":188:Here is the environment:\n\n");
#ifdef SYSV
	system("env");
#else
	system("printenv");
...
char *ctype, *progname, *label;
{
	...
	printf("\n");
	if (label) {
	    pfmt(stderr, MM_NOSTD, ":189:This message contains %s.\nDo you want to view it using the '%s' command (yes/no) [yes] ? ", label, ShortCommand(progname));
	} else {
	    pfmt(stderr, MM_NOSTD, ":190:This message contains '%s'-format data.\nDo you want to view it using the '%s' command (yes/no) [yes] ? ", ctype, ShortCommand(progname));
	}
	s = gets(AnsBuf);
	if (!s) return(0); /* EOF */
	while (*s && isspace((unsigned char) *s)) ++s;
	...
	pfmt(stderr, MM_ERROR, ":146:Please answer yes or no.\n");
    }
}
...
int ShowLeadingWhitespace;
{
    ...
    *txtend = 0;
	...
	pfmt(stderr, MM_WARNING, ":191:Bad encoding value in non-ASCII header string: %s\n", encoding+1);
    }
	...
	if (!fp) {
	    pfmt(stderr, MM_ERROR, ":158:Cannot open temporary file\n");
	} else {
	    char *t;
	    for (t=txt+1; *t; ++t) {
		if (*t == '_') {
	    ...
	    fp = fopen(TmpFile, "r");
	    if (!fp) {
		pfmt(stderr, MM_ERROR, ":158:Could not open temporary file\n");
	    } else {
...
struct part **PartsWritten;
{
	...
	if (prefixed) {
		...
		case 'F':
		    if (UsedBigFile) {
			pfmt(stderr, MM_ERROR, ":192:Bad mailcap entry: %s\n", controlstring);
			ExitWithError(":162:%%n and %%F are incompatible with %%s.  Execution terminated.\n");
		    }
		    if (!UsedLittleFiles) {
			/* Set up parts */
		...
		case '{':
		    s = index(from, '}');
		    if (!s) {
			pfmt(stderr, MM_ERROR, ":193:Ignoring ill-formed parameter reference in mailcap file: %s\n", from);
			break;
		    }
		    ++from;
		    *s = 0;
		...
		case 't':
		    ...
		    break;
		default:
		    pfmt(stderr, MM_ERROR, ":194:Ignoring unrecognized format code in mailcap file: %%%c\n", *from);
		    break;
	    }
	} else if (*from == '%') {
	    prefixed = 1;
...
char *ctype;
{
    ...
    if (!fpout) {
	perror("WriteTmpFile");
	ExitWithError(":158:Cannot open temporary file\n");
    }
    TranslateInputToOutput(InputFP, fpout, EncodingCode, ctype);
    if (ftell(fpout) == 0) retval = 1;
    if (fclose(fpout)) ExitWithError(":164:Cannot write temporary file\n");
    return(retval);
}
...
char *ctype;
{
	...
	pfmt(stderr, MM_WARNING, ":195:Cannot freopen /dev/tty to stdin");
    } else InputFP = stdin;
#endif
}
...
char *Prefix;
{
	...
	if (!freopen("/dev/tty", "r", stdin)){
	    ExitWithError(":165:Don't know how to create a terminal window\n");
	}
	InputFP = stdin;
	pfmt(stderr, MM_WARNING, ":196:reopened /dev/tty, could be strange.\n");
	Prefix[0] = 0;
#endif
    }
}
...
int really;
{
	...
	if (!NeedToBeQuiet(ShortCommand(cmd))) {
	    pfmt(stderr, MM_INFO, ":197:---Executing: %s\n", DoDebug ? cmd : ShortCommand(cmd));
	...
	fflush(stdout); fflush(stderr);
	code = system(cmd);
	if (DoDebug) pfmt(stderr, MM_INFO, ":198:Command exit status: %d\n", code);
#ifdef MSDOS
...
char *name;
{
    ...
    mktemp(name);
	 ...
	 if (DoDebug) pfmt(stderr, MM_INFO, ":199:temp name = \"%s\"\n", name);
#endif
#endif
}
...
char *nameBuf;
{
	...
	pfmt(stderr, MM_ERROR, ":200:Unable to open %s for writing\n", nameBuf);
	exit(1);
    }
...
char *ct;
{
    ...
    *s++ = 0;
	...
	eq = index(s, '=');
	if (!eq) {
	    pfmt(stderr, MM_WARNING, ":201:Ignoring unparsable content-type parameter: '%s'\n", s);
	    ...
	    *eq++ = 0;
	    ...
	    if (DoDebug) pfmt(stderr, MM_INFO, ":202:NEW PARAMETER: %s VALUE: %s\n", s, eq);
	}
	s = t;
    } while (t);
}
...
#if defined(MSDOS) || defined(AMIGA)
    char Buf[100];
    pfmt(stderr, MM_NOSTD, ":203:Press RETURN to go on\n");
    gets(Buf);
#else
    fflush(stdout);
    SaveTtyState();
    if (StartRawStdin() != -1) {
	pfmt(stdout, MM_NOSTD, ":204:--Press any key to go on.--");
    } else {
	pfmt(stdout, MM_NOSTD, ":203:Press RETURN to go on.\n");
    }
    fflush(stdout);
    getchar();
    RestoreTtyState();
