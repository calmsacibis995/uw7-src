...
#ident	"@(#)mail:common/cmd/mail/metamail/richmail/richtext.c	1.1.1.4"
/* @(#)richtext.c	1.5 */
/*
...
char **argv;
{
    ...
    if(term) {
	dum = tigetstr("smkx");
	(void)strcpy(KS, (dum == NULL)? "":dum);

	dum = tigetstr("rmkx");
	(void)strcpy(KE, (dum == NULL)? "":dum);

	dum = tigetstr("smso");
	(void)strcpy(standoutbuf, (dum == NULL)? "":dum);

	dum = tigetstr("rmso");
	(void)strcpy(standendbuf, (dum == NULL)? "":dum);

	dum = tigetstr("bold");
	(void)strcpy(BoldOn, (dum == NULL)? "":dum);

	dum = tigetstr("sgr0");
	(void)strcpy(BoldOff, (dum == NULL)? "":dum);

	dum = tigetstr("smul");
	(void)strcpy(StartUnderline, (dum == NULL)? "":dum);

	dum = tigetstr("rmul");
	(void)strcpy(StopUnderline, (dum == NULL)? "":dum);

	dum = tigetstr("cuf1");
	(void)strcpy(MoveRight, (dum == NULL)? " ":dum);
    } else if (OverStrike) {
#if 0
	...
	dum = StartUnderline;
	if (tgetstr("us", &dum)) /**dum = '\0'*/*dum=*dum; /*else StartUnderline[0] = '\0';*/
	dum = StopUnderline;
    ...
    while((c = richtextlex(InputFP,token,TextEnriched)) != (RCHAR)EOF) {
	if (c == RICHTEXT_COMMAND || c == RICHTEXT_NEG_COMMAND) {
		...
		case 'p':
			...
			while (commct > 0) {
			    for (;;) {
				c = richtextlex(InputFP,token,TextEnriched);
...
static Pause()
{
    ...
    pfmt(stdout, MM_NOSTD, ":219:Press RETURN to continue (or 'q' to quit):");
    fflush(stdout);
    c = getc(stdin);
    if (c == 'q' || c == 'Q') exit(0);
    linesused = 0;
    ...
    *BoldOn, *BoldOff;
{
	...
	*outputString;
	...
	{
	initialized = 1;
	graphicsString = tigetstr("sgr");
	}

    if(graphicsString)
	{
	outputString = tparm(graphicsString, standout, underline, 0, 0, 0, bold, 0, 0, 0);
	controloutput(outputString, 0);
	}
    else
	{
