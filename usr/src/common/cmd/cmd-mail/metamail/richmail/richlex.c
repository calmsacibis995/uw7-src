 ...
 */
RCHAR richtextlex(file,token,TextEnriched)
void *file;
char *token;
int TextEnriched;
{
	...
	if (c == '<') {
		...
		EndInpFile = 1;
		return(richtextlex(file,token,TextEnriched)); /* Flush something out */
	    }
	    NextToken[i] = '\0';
		...
		FlushSize = i - 1;
		FlushStack = FlushSize;
		return(richtextlex(file,token,TextEnriched));
	    }
	    strcpy(token,NextToken);
	    return(cmd);
	} else if (c == SI) {
...
int TextEnriched;
{
    ...
    while ((c = richtextlex(inparam,token,TextEnriched)) != (RCHAR)EOF) {
