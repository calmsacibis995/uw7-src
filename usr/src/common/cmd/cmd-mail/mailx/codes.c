...
#ident	"@(#)mail:common/cmd/mail/metamail/metamail/codes.c	1.1.1.2"
/* @(#)codes.c	1.3 */
/*
...
extern char *index();
static void	output64chunk ARGS((int, int, int, int, FILE *));
...
int PortableNewlines;
{
	...
	if (newline && boundaries && c1 == '-') {
	    ...
	    pfmt(stderr, MM_ERROR, ":1:Ignoring unrecognized boundary line: %s\n", Buf);
	    continue;
	}
	...
	if (c2 == EOF || c3 == EOF || c4 == EOF) {
	    pfmt(stderr, MM_ERROR, ":217:base64 decoder saw premature EOF!\n");
	    return;
	}
	if (c1 == '=' || c2 == '=') {
	    DataDone=1;
