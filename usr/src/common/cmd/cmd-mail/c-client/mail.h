...
#ifdef SCOMS
#define MAILTMPLEN 2048		/* size of a temporary buffer */
#else
...
#ifdef SCOMS
  long (*deliver) (MAILSTREAM *stream,char *mailbox,
		char *sender,FILE *fd);
#endif
};
...
long mail_append_full (MAILSTREAM *stream,char *mailbox,char *flags,char *date,
		       STRING *message);
#ifdef SCOMS
long mail_deliver (MAILSTREAM *stream, char *mailbox,
		char *sender, FILE *fd);
void mail_link_dlls();
#endif
...
/*
 * if you already have a sendmail message this will parse it for deliver()
 * msg contains the entire message, null terminated.
 * date and sender are filled in if they are non-null, rest is set if non-null.
 * returns zero if err.  The date string is converted to
 * a canonical form.
 */

int sendmail_parse_from(char *, char *, char *, char **);
