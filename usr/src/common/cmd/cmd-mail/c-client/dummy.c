...
#ifndef SCOMS
	  (!*s || !stat (s,&sbuf))) ? &dummydriver : NIL;
#else /* return false on all but directories */
	  (!*s || (!stat (s,&sbuf) && S_ISDIR(sbuf.st_mode)))) ?
	    &dummydriver : NIL;
#endif
}
...
#ifdef SCOMS
  /* must be a real valid folder, not just some file we found */
  if (attributes&LATT_NOINFERIORS)
    if (mail_valid(stream,name,0) == 0)
      return T;
#endif
				/* notify main program */
