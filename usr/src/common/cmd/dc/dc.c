...
commnds(){
	...
	while(1){
		...
		case 'Y':
			sdump("stk",*stkptr);
			pfmt(stderr,MM_NOSTD,":362:all %ld rel %ld headmor %ld\n",all,rel,headmor);
			pfmt(stderr,MM_NOSTD,":376:nbytes %ld\n",nbytes);
			pfmt(stderr,MM_NOSTD,":375:longest %ld active %ld maxsize %ld\n", longest,
				active, maxsize);
			pfmt(stderr,MM_NOSTD,":380:new all %d rel %d copy %d more %d lbytes %d\n",
...
casediv:
		...
		case 'v':
			...
			if((c = sbackc(p))<0){
				error(":385:sqrt of negative number\n");
			}
			if(k<savk)n = savk;
			else{
				n = k*2-savk;
		...
		case '^':
			...
			arg1 = pop();
			EMPTY;
			if(sunputc(arg1) != 0)error(":369:exponent not an integer\n");
			arg2 = pop();
			EMPTYR(arg1);
		...
		case 'Q':
			...
			EMPTY;
			if(length(p)>2){
				error(":361:String execution level too large\n");
			}
			rewind(p);
			if((c =  sgetc(p))<0){
				error(":377:negative argument to Q\n");
			}
			release(p);
			while(c-- > 0){
				if(readptr == &readstk[0]){
					error(":359:Empty read-stack\n");
				}
		...
		case 'q':
			...
			continue;
		case 'f':
			if(stkptr == &stack[0])pfmt(stderr,MM_NOSTD,":368:empty stack\n");
			else {
		...
		case 'd':
			if(stkptr == &stack[0]){
				pfmt(stderr,MM_NOSTD,":368:empty stack\n");
				continue;
			}
			q = *stkptr;
			n = length(q);
		...
		case 'S':
			...
			continue;
sempty:
			error(":387:symbol table overflow\n");
		case 's':
			if(stkptr == &stack[0]){
				error(":383:save: no arguments\n");
			}
		...
		case ':':
			...
			if((sfbeg(q) == 0) && ((c = sbackc(q))<0)){
				error(":378:negative index\n");
			}
			if(length(q)>2){
				error(":374:index too big\n");
			}
...
execute:
		...
		case '=':
			...
			continue;
		default:
			pfmt(stderr,MM_NOSTD,":357:%o is unimplemented\n",c);
		}
	}
}
struct blk *
...
struct blk *ddivd,*ddivr;
{
	...
	p = salloc(0);
	if(length(ddivr) == 0){
		pfmt(stderr,MM_NOSTD,":367:divide by 0\n");
		return(p);
	}
	...
	if(under){	/* undershot last - adjust*/
		px = copy(divr,length(divr));	/* don't corrupt ddivr*/
		chsign(px);
		ps = add(px,divd);
		fsfile(ps);
...
char *argv[];
{
	...
	svargc = --argc;
	svargv = argv;
	while(svargc>0 && svargv[1][0] == '-' && !argend){
	...
	if(svargc<=0)curfile = stdin;
		...
		if(stat(svargv[1], &statbuf) == -1) {
			pfmt(stderr, MM_ERROR, ":365:can't stat file \"%s\"\n",
					svargv[1]);
			exit(1);
		}
		if(!S_ISREG(statbuf.st_mode)) {
			pfmt(stderr, MM_ERROR, ":356:\"%s\" is not a regular file\n",
					svargv[1]);
			exit(1);
		}
		if((curfile = fopen(svargv[1],"r")) == NULL){
			pfmt(stderr,MM_ERROR, ":364:can't open file \"%s\"\n",
				svargv[1]);
			exit(1);
		} 
	}
...
{
	if(stkptr == stkend){
		pfmt(stderr,MM_NOSTD,":382:out of stack space\n");
		return;
	}
	stkerr=0;
	*++stkptr = p;
...
struct blk *p;
{
	...
	release(p);
	if(c >= 16){
		pfmt(stderr,MM_NOSTD,":373:hex digit > 16");
		return;
	}
...
load(){
	...
	if(sptr != 0){
		...
		q = salloc(1);
		if(c <= LASTFUN){
			pfmt(stderr,MM_NOSTD,":371:function %c undefined\n",c+'a'-1);
			sputc(q,'c');
			sputc(q,'0');
			sputc(q,' ');
			sputc(q,'1');
...
int size;
{
	...
	if(ptr == 0){
		...
		if((ptr = nalloc(hptr->beg, (unsigned)size)) == NULL){
			pfmt(stderr,MM_NOSTD,":366:copy size %d\n",size);
			ospace("copy");
		}
	}
	if((hdr = hfree) == 0)hdr = morehd();
...
struct blk *hptr;
{
	char *p;
	if (hptr) {
		pfmt(stderr,MM_NOSTD,":358:%s %o rd %o wt %o beg %o last %o\n",s1,hptr,hptr->rd,hptr->wt,hptr->beg,hptr->last);
		p = hptr->beg;
...
char *s;
{
	pfmt(stderr,MM_NOSTD,":389:out of space: %s\n",s);
	pfmt(stderr,MM_NOSTD,":362:all %ld rel %ld headmor %ld\n",all,rel,headmor);
	pfmt(stderr,MM_NOSTD,":376:nbytes %ld\n",nbytes);
	sdump("stk",*stkptr);
	abort();
...
char *s;
{
	...
	pfmt(stderr,MM_NOSTD,":372:got to garbage %s\n",s);
	for(i=0;i<TBLSZ;i++){
		tmps = stable[i];
		if(tmps != 0){
			if(i < ARRAYST){
				do {
					p = tmps->val;
					if(((int)p->beg & 01)  != 0){
						pfmt(stderr,MM_NOSTD,":386:string %o\n",i);
						sdump("odd beg",p);
					}
					redef(p);
					tmps = tmps->next;
				...
				do {
					...
					while((q = getwd(p)) != NULL){
						...
						if(q != 0){
							if(((int)q->beg & 01) != 0){
								pfmt(stderr,MM_NOSTD,":363:array %o elt %d odd\n",i-ARRAYST,ct);
pfmt(stderr,MM_NOSTD,":388:tmps %o p %o\n",tmps,p);
								sdump("elt",q);
							}
							redef(q);
						}
...
struct blk *p;
{
	...
	if ((int)p->beg&01) {
		pfmt(stderr,MM_NOSTD,":381:odd ptr %o hdr %o\n",p->beg,p);
		ospace("redef-bad");
	}
	free(p->beg);
	free(dummy);
...
struct blk *p;
{
	...
	wp = (struct wblk *)p;
	if (wp->rdw == wp->wtw || (wp->rdw+1) > wp->wtw) {
		wp->rdw = wp->wtw;
		return(NULL);
	}
	return(*wp->rdw++);
