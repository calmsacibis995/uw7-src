 ...
 */
	...
	req = (nlm_shareargs *)xmalloc((u_int) sizeof(nlm_shareargs));
	if (!svc_getargs(Transp, xdr_nlm_shareargs, (char *)req)) {
		free_sreq(req);
 ...
 */
	if (SHARE_REQ) {
		...
		if (debug)
			printf("...compatible. new sxx @0x%x\n", sxxh);
		sxh->sxxp->bck = sxxh;
		sxxh->fwd = sxh->sxxp;
		sxh->sxxp = sxxh;
	}
	...
	enum	nlm_stats rc;
{
	...
	if (!(svc_sendreply(T, xdr_nlm_shareres, (caddr_t)&res)))
		svcerr_systemerr(T);
}
	...
	while (ss != NULL) {
		if (((int)ss->sh.mode & (int)s->access) ||
			...
			if (report_sharing_conflicts)
				syslog(LOG_ERR, 
				       gettxt(":135", "%s: File sharing request from %s (mode %d, access %d) conflicts\nwith sharing for %s (mode %d, access %d)\n"),
				       "compatible", 
