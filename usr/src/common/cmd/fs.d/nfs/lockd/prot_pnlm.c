...
struct reclock		*blocking_req = NULL;
	...
	struct reclock *a;
{
	...
	if (act == nlmreply) { /* reply to nlm_transp */
		...
		if (!svc_sendreply(nlm_transp, xdr_reply, (caddr_t)reply))
	...
	int retransmit;
{
	...
	case NLM_UNLOCK_MSG:
		...
		if ((nlm_unlockargs_a = get_nlm_unlockargs()) == NULL) {
			syslog(LOG_ERR, 
			       gettxt(":108","%s: cannot allocate space for %s"),
			       "NLM_UNLOCK_MSG", "nlm_unlockargs_a");
			return (-1);
		}
	...
	case NLM_GRANTED_MSG:
                ...
                if ((nlm_testargs_a = get_nlm_testargs()) == NULL) {
                        syslog(LOG_ERR, 
			       gettxt(":108","%s: cannot allocate space for %s"),
			       "NLM_GRANTED_MSG", "nlm_testargs_a");
			return (-1);
                }
                if (reclocktonlm_testargs(a, nlm_testargs_a) == -1) {
			release_nlm_testargs(nlm_testargs_a);
	...
	if (proc == NLM_LOCK_MSG || proc == NLM_UNLOCK_MSG ||
		...
		proc == NLM_GRANTED_MSG || proc == NLM_LOCK_RECLAIM ) {
		/*
	...
	struct reclock *to;
{
	...
	to->alock.lox.rsys = (long) (((struct sockaddr_in *) nlm_transp->xp_rtaddr.buf)->sin_addr.s_addr);
	...
	struct reclock	*a;
{
	...
	if (debug) {
		printf("enter find_block_req...\n");
printf("base=%d, length=%d, pid=%d, rpid=%d, rsys=%d, type=%d\n",
a->lck.lox.base, a->lck.lox.length, a->lck.lox.pid, a->lck.lox.rpid, a->lck.lox.rsys, a->lck.lox.type);
	}

	while (cur != NULL) {
		if (debug)
printf("base=%d, length=%d, pid=%d, rpid=%d, rsys=%d, type=%d\n",
cur->lck.lox.base, cur->lck.lox.length, cur->lck.lox.pid, cur->lck.lox.rpid, cur->lck.lox.rsys, cur->lck.lox.type);
		/*
