	...
	struct reclock	**list;  /* List to add it too   */
{
	...
	if (p != NULL) {
		for (a = p->lox, spanlist = NULL; a; a = n) {
			...
			/* Case two, */
			/* it completely surrounds one or more locks */
			if (debug)
...
{
	struct data_lock 	*t, *n, *x; /* Some temporaries */
	struct reclock		*tmp_req;
...
out:					/* Remove blocking referencese */
				...
				/* Case three, */
				/* new lock in in the middle of 't' */
				} else {
					/*
					 * This part can fail if there isn't
					 * another lock entry and 'n' lock_refs
					 * available to build it's dependencies.
					 */
					if ((tmp_req = get_reclock()) != NULL) {
						...
						/*
						 * Make sure we don't core dump!
						 */
						if (tmp_p != NULL)
							*tmp_req = *tmp_p;
						if (init_reclock(tmp_req,
						    tmp_p)) {
							x = &tmp_req->lck.lox;
							/* give it a fresh id */
							x->LockID = LockID++;
	...
	u_int n;
{
	...
	if ((a->n_bytes = xmalloc(n)) == NULL) {
		...
		(void) memcpy(a->n_bytes, b, a->n_len);
	return (0);
}
	...
	if (i == 1) {		/* insert! */
		if ((new = find_me(site)) == NULL) { /* not found */
			if ((new = get_me()) == NULL) /* no more me entry */
				...
				if ((new->svr = xmalloc((u_int) (strlen(site)+1)))
					== NULL) {
					used_me--;
					free((char *) new);
					return (-1);
	...
	int i;
{
	...
	default:
		pfmt(stderr, MM_ERROR, 
		     ":118:contact monitor (%d) is not known\n", i);
		abort();
	}
	...
	if ((priv_size = sizeof (struct priv_struct)) > 16) /* move to init */
		pfmt(stderr, MM_WARNING,
		     ":119:problem with private data size (%d) to status monitor\n", priv_size);
	...
	if (resp->res_stat == stat_succ) {
		if (resp->sm_stat == stat_succ) {
			...
			pfmt(stderr, MM_ERROR, 
			     ":120:site %s does not subscribe to status monitor service\n", new->svr);
			return (-1);
		}
	} else {
		pfmt(stderr, MM_ERROR,
		     ":121:lockd cannot contact local statd\n");
		return (-1);
	}
}
