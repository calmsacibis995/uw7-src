	...
	mon *monp;
{
	...
	if ((new = (mon_entry *) malloc (sizeof (mon_entry))) == 0) {
		pfmt(stderr, MM_ERROR, 
		     ":102:%s: %s failed: %s\n",
		     "insert_mon", "malloc", strerror(errno));
		abort();
	}
	...
	int state;
{
	...
	if ((err = myrpc_call(my_idp->my_name, my_idp->my_prog, my_idp->my_vers,
		my_idp->my_proc, xdr_status, &stat, xdr_void, NULL,
		"visible", SM_TCP_TIMEOUT, 0)) != (int) RPC_SUCCESS &&
		err != (int) RPC_TIMEDOUT ) {
		pfmt(stderr, MM_ERROR, 
		     ":146:%s: cannot contact local lockd on %s status change, give up!\n",
		     "rpc.statd", mon_name);
		return (-1);
		}
	else
