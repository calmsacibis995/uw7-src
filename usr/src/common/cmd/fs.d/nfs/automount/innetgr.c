	...
	char *group, *machine, *name, *domain;
{
	...
	if (domain) {
		if (name && !machine) {
			if (lookup_yp(thisdomain, "netgroup.byuser",
				      group, name, domain, &res)) {
				return(res);
			}
		} else if (machine && !name) {
			if (lookup_yp(thisdomain, "netgroup.byhost",
				      group, machine, domain, &res)) {
				return(res);
			}
		}
	}
	...
	char *group;
{
	...
	*d->listp++ = group;
	if (d->listp > d->list + LISTSIZE) {
		syslog(LOG_ERR, gettxt(":97", "%s: recursive overflow\r\n"),
		       "innetgr");
		d->listp--;
		return (0);
	}
	...
	if (err) {
		if (trace > 1) {
			if (err == YPERR_KEY)
			  fprintf(stderr,
				  "%d: innetgr: no such netgroup as %s\n",
				  myid, group);
			else
			  fprintf(stderr, "%d: innetgr: yp_match, %s\n",
				  myid, yperr_string(err));
		}
		d->listp--;
		return(0);
	}
 ...
 */
static
lookup_yp(thisdomain,map,group,name,domain,res)
	char *thisdomain;
	char *map;
	char *group;
	char *name;
	...
	for (i = 0; i < 4; i++) {
		...
		if (!err) {
			*res = inlist(group,val);
			...
			if (trace > 1)
				fprintf(stderr, "%d: yp_match(%s,%s) failed: %s.\n",
					myid, map, key, yperr_string(err));
