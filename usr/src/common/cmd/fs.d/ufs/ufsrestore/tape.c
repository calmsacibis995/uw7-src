...
#ident	"@(#)ufs.cmds:common/cmd/fs.d/ufs/ufsrestore/tape.c	1.4.6.2"
#ident "$Header$"
	...
	char *b;
{
	...
	if (bct < ntrec) {
		memcpy(b, &tbf[(bct++*TP_BSIZE)], (long)TP_BSIZE);
		blksread++;
		return;
	}
	for (i = 0; i < ntrec; i++)
...
getmore:
		...
		if (pipein) {
			...
			if (i % TP_BSIZE != 0)
				panic("partial block read: %d should be %d\n",
					i, ntrec * TP_BSIZE);
			memcpy(&tbf[i], (char *)&endoftapemark,
				(long)TP_BSIZE);
		}
	}
	if (i < 0) {
	...
	if (i == 0) {
		...
		if (rd % TP_BSIZE != 0)
			panic("partial block read: %d should be %d\n",
				rd, ntrec * TP_BSIZE);
		memcpy(&tbf[rd], (char *)&endoftapemark, (long)TP_BSIZE);
	}
	memcpy(b, &tbf[(bct++*TP_BSIZE)], (long)TP_BSIZE);
	blksread++;
}
