...
#ident	"@(#)devmgmt:common/cmd/devmgmt/devreserv/main.c	1.7.11.3"
#ident  "$Header$"
	...
	char  **args;
{
	...
	if (addrlist = (char ***) malloc((unsigned)i*sizeof(char **))) {
		...
		if (*ppp = (char **) malloc((unsigned)(n+1)*sizeof(char *))) {
		     qq = *ppp++;
		     if (*qq++ = strtok(*pp, TOKDELIMS))
			 while (*qq++ = strtok((char *) NULL, TOKDELIMS));
		} else noerror = FALSE;
		*ppp = (char **)NULL; /* mark the end of the list */
	    }
