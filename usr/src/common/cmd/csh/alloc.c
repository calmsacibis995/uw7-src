...
#ident	"@(#)csh:common/cmd/csh/alloc.c	1.2.5.5"
#ident  "$Header$"
...
char **s;
{
	...
	if (s[1])
		SFLAG_N,printf(gettxt(MSG_MALLOC_ST), s[1]);
	for (i = 0; i < NBUCKETS; i++) {
	...
	if (s[1])
		SFLAG_N,printf(gettxt(MSG_USED));
	for (i = 0; i < NBUCKETS; i++) {
		if (s[1])
			printf(" %d", nmalloc[i]);
	...
	if (s[1])
		printf("\n");
	SFLAG_N,printf(gettxt(MSG_TTLUSE), totused, totfree);
#endif OLDMALLOC
}
