...
#ident	"@(#)csh:common/cmd/csh/sh.sem.c	1.3.7.9"
#ident  "$Header$"
	...
	case TPAR:
		...
		if (((t->t_dflg & FTIME) || (t->t_dflg & FPAR) == 0 &&
			...
			if (pid < 0) {
				...
				SFLAG_E,error(gettxt(ER_FAIL_VFORK));
			}
			forked++;
			if (pid) {	/* parent */
	...
	int *pipein, *pipeout;
{
	...
	if (cp = t->t_drit) {
		...
		if((flags & FCAT) && adrof(S_noclobber))
			oflags=O_WRONLY|O_APPEND;
		else
			oflags=O_WRONLY|O_APPEND|O_CREAT;
		
		if ((flags & FCAT) && open_(cp, oflags, 0666) >= 0)
			;
		else {
	...
	register int *pv;
{
	...
	if (pv[0] >= 0 && pv[1] >= 0)
		return;
oops:
	SFLAG_E,error(gettxt(ER_NMAKE_PIPE));
}
	...
	register tchar *cp;
{
	...
	if (type == S_IFCHR || type == S_IFIFO)
		return;
	SFLAG_E,error(gettxt(ER_EXI_FILE), cp);
}
