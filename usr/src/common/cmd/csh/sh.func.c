...
#ident	"@(#)csh:common/cmd/csh/sh.func.c	1.5.5.11"
#ident  "$Header$"
	...
	register struct biltins *bp;
{
	...
	i = blklen(t->t_dcom) - 1;
	if (i < bp->minargs)
		SFLAG_E,bferr(gettxt(ER_FEW_ARG));
	if (i > bp->maxargs)
		SFLAG_E,bferr(gettxt(ER_MANY_ARGS));
	...
	tchar **v;
{
	...
	if (parintr == SIG_IGN)
		return;
	if (setintr && intty)
		SFLAG_E,bferr(gettxt(ER_NUSE_TERM));
	...
	register tchar **v;
{
	...
	if (p == 0)
		...
		if (eq(p, S_alias/*"alias"*/) || eq(p, S_unalias/*"unalias"*/)) {
			setname(p);
			SFLAG_E,bferr(gettxt(ER_DANG_ALIAS));
		}
...
islogin()
{
	...
	if (loginsh)
		return;
	SFLAG_E,error(gettxt(ER_NLOGIN_SHELL));
}
	...
	struct command *kp;
{
	...
	vv = v;
	if (*vv == NOSTR)
		SFLAG_E,bferr(gettxt(ER_EMP_IF));
	if (eq(*vv, S_then /*"then"*/)) {
		if (*++vv)
			SFLAG_E,bferr(gettxt(ER_IMP_THEN));
		setname(S_then/*"then"*/);
		/*
	...
	register tchar **v;
{
	...
	if (*v)
syntax:
		SFLAG_E,error(gettxt(ER_SYNTAX));
...
dobreak()
{
	...
	if (whyles)
		...
		SFLAG_E,bferr(gettxt(ER_NIN_WHILE));
}
	...
	tchar **v;
{
	...
	v++;
	if (*v) {
		set(S_status/*"status"*/, putn(expr(&v)));
		if (*v)
			SFLAG_E,bferr(gettxt(ER_EXP_SYNTAX));
	}
	btoeof();
	if (intty)
		(void) close(SHIN);
	...
	register tchar **v;
{
	...
	if (*cp || strlen_(*v) >= 20 || !letter(**v))
		SFLAG_E,bferr(gettxt(ER_INV_VAR));
	cp = *v++;
	if (v[0][0] != '(' || v[blklen(v) - 1][0] != ')')
		SFLAG_E,bferr(sh_gettxt(ER_NPARN_WORD));
	v++;
	gflag = 0, tglob(v);
	v = glob(v);
	if (v == 0)
		SFLAG_E,bferr(gettxt(ER_NO_MATCH));
...
past:
	...
	case ZIF:
		SFLAG_E,bferr(gettxt(ER_NFND_THEN));

	case ZELSE:
		SFLAG_E,bferr(gettxt(ER_NFND_ENDIF));

	case ZBRKSW:
	case ZSWITCH:
		SFLAG_E,bferr(gettxt(ER_NFND_ENDSW));

	case ZBREAK:
		SFLAG_E,bferr(gettxt(ER_NFND_END));

	case ZGOTO:
		setname(Sgoal);
		SFLAG_E,bferr(gettxt(ER_NFND_LABEL));
	}
	/*NOTREACHED*/
}
	...
	register tchar **v;
{
	...
	if (eq(vp, S_PATH/*"PATH"*/)) {
		...
		if(!setlocale(LC_ALL, "")){
			SFLAG_E,printf(gettxt(ER_INV_LOCALE), lp);
		}
	}
	...
	tchar *name, *val;
{
	...
	blk[1] = 0;
	xfree(ep_);	
	environ = (char **)blkspl_(environ, blk);
	xfree( (void *)oep);
	setenv(name, val);
}
	...
	tchar *name;
{
	...
	for (; *ep; ep++) {
		...
		cp_ = *ep;
		*ep = 0;
		environ = (char **)blkspl_(environ, ep+1);
		*ep = cp_;
		xfree(cp_);
		xfree((void *)oep);
		return;
	...
	register tchar **v;
{
	...
	while (digit(*cp) && *cp != '8' && *cp != '9')
		i = i * 8 + *cp++ - '0';
	if (*cp || i < 0 || i > 0777)
		SFLAG_E,bferr(gettxt(ER_IMP_MASK));
	(void) umask(i);
}
...
struct limits {
	...
	RLIMIT_NOFILE,	S_descriptors/*"descriptors*/,	1,	S_,
	...
	tchar *cp;
{
	...
	if (res)
		return (res);
	SFLAG_E,bferr(gettxt(ER_NO_LIMIT));
	/*NOTREACHED*/
}
	...
	tchar **v;
{
	...
	default:
badscal:
		SFLAG_E,bferr(gettxt(ER_IMP_SCALE));
	}
	return ((int)(f+0.5));
}
	...
	tchar *cp, *str0;
{
	...
	while (*cp && *cp == *str)
		cp++, str++;
	if (*cp)
		SFLAG_E,error(gettxt(ER_BAD_SCALE), str0);
}
	...
	tchar hard;
{
	...
	if (limit == RLIM_INFINITY)
		SFLAG_N,printf(gettxt(MSG_UNLIMIT));
	...
	tchar hard;
{
	...
	if (setrlimit(lp->limconst, &rlim) < 0) {
		SFLAG_E,printf(gettxt(ER_NFND_LIMIT),bname,lp->limname,
		    limit == RLIM_INFINITY ? gettxt(ER_REMV) : gettxt(ER_SET),
		    hard ? gettxt(ER_HARD) : "");
		return (-1);
	}
	return (0);
...
#endif
	if (loginsh)
		SFLAG_E,error(gettxt(ER_NSUP_SHELL));
	untty();
...
retry:
	...
	if (oldisc != NTTYDISC) {
		SFLAG_N,printf(gettxt(MSG_SWTCH_NEW));
		ldisc = NTTYDISC;
	...
	tchar **v;
{
	...
	if (gflag) {
		...
		gargv = 0;
		if (v == 0)
			SFLAG_E,error(gettxt(ER_NO_MATCH));
		v = copyblk(v);
	} else
		trim(v);
	getexit(osetexit);
