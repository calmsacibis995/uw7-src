...
#ident	"@(#)csh:common/cmd/csh/sh.misc.c	1.3.5.7"
#ident  "$Header$"
...
/*
 * This used to check that cp was between "end" and the top of the stack,
 * which broke when memory layout was changed to put the heap ABOVE the
 * stack.  The sbrk() library stub caches the current sbrk(0) value,
 * so the test we're using now is not much more expensive, yet is
 * safer from memory layout changes.
 */
	...
	unsigned i;
{
	...
	child++;
#ifndef debug
	SFLAG_E,error(gettxt(ER_OUT_MEM));
#ifdef lint
	i = i;
#endif
#else
	...
	tchar *name;
{
	...
	setname(name);
	SFLAG_E,bferr(gettxt(ER_UNDEF_VAR));
}
