...
#ident	"@(#)csh:common/cmd/csh/sh.c	1.5.12.1"
#ident  "$Header$"
	...
	char **av;
{
	...
	setlocale(LC_ALL, "");setlabel(S_setlabel);setcat(S_uxue);sh_getwidth();
	...
	/*
	 * If running setuid/setgid, we want to avoid interpreting
	 * imported environment variables for security reasons.
	 * globsafe tells set1() whether or not it is safe to allow
	 * interpretation of assigned strings, e.g. TERM=`/trojan/horse`.
	 */
	setid = (uid != geteuid() || gid != getegid());
	globsafe = !setid;
	...
	/*
	 * Finished importing from environment, so it is now safe for
	 * set1() to permit interpretation of assigned strings.
	 */
	globsafe = 1;
...
notty:
				SFLAG_W,printf(gettxt(ER_NJOB_CNTL));
				tpgrp = -1;
			}
		}
	}
	 ...
	 */
	if (loginsh) {
		SFLAG_N,printf(gettxt(MSG_LOGOUT));
		(void) close(SHIN);
		child++;
		goodbye();
	}
	...
	bool wantnl;
{
	...
	if (setintr) {
		...
		if (pjobs) {
			...
			printf("\n");
			dojobs(jobargv);
			SFLAG_E,bferr(gettxt(ER_INTERRUPT));
		}
	}
...
mailchk()
{
	...
	for (; *vp; vp++) {
		...
		if (stb.st_size == 0 || stb.st_atime >= stb.st_mtime ||
			...
			continue;
		if (cnt == 1)
			SFLAG_N,printf(gettxt(MSG_HAVE_MAIL), new ? sh_gettxt(MSG_NEW) : "");
		else
			SFLAG_N,printf(gettxt(MSG_ARIV_MAIL), new ? sh_gettxt(MSG_NEW_MAIL) : sh_gettxt(MSG_MAIL), *vp);
	}
	chktim = t;
}
...
initdesc()
{
	...
	/* if input is a shell script, it's been taken care already. */
	if (SHIN != FSHIN)		
	...
	(void) fcntl(OLDSTD = dcopy(0, FOLDSTD), F_SETFD,  1);
	closem();
}
...
printprompt()
{
	...
	if (!whyles) {
		 ...
		 */
		for (SFLAG_CN,cp = value(S_prompt /*"prompt"*/); *cp; cp++)
			if (*cp == HIST)
				printf("%d", eventno + 1);
			else {
				if (*cp == '\\' && cp[1] == HIST)
