...
#ident	"@(#)csh:common/cmd/csh/sh.proc.c	1.4.11.2"
#ident  "$Header$"
...
#endif
loop:
	pid = wait9(&w, (setintr ? WNOHANG|WUNTRACED:WNOHANG), &ru);
        /*
	...
	register struct process *pp;
{
	...
	do {
		if ((fp->p_flags&(PFOREGND|PRUNNING)) == PRUNNING)
			SFLAG_E,printf(gettxt(ER_BUG_WJOB));
	} while ((fp = fp->p_friends) != pp);
	/*
...
#endif
	if (pp->P_pid == 0) {
		SFLAG_E,printf(gettxt(ER_BUG_FLUSH));
		return;
	}
	while (pp->P_pid != pp->p_jobid)
		pp = pp->p_friends;
			...
			if (flag&(REASON|AREASON)) {
				...
				case PSIGNALED:
					if ((flag&(REASON|AREASON))
					    && reason != SIGINT
					    && reason != SIGPIPE)
						msg = err_no(SIGNAL_M,pp->p_reason);
					break;
				...
				case PAEXITED:
					if (flag & REASON)
						if (pp->p_reason)
							msg = err_no(EXIT_M,pp->p_reason);
						else
							msg = sh_gettxt(MSG_DONE);
					break;

				default:
					msg = err_no(BUG_M,status);
				}
				{
				  int i,rem,len;
				  ...
				  if (len)
				  {
				    SFLAG_N,printf(format,msg);
				    if (rem > 0)	for (i=0;i<rem;i++) printf(" ");
				  }
				}
			}
		}
prcomd:
		...
		if (flag&(REASON|AREASON) && pp->p_flags&PDUMPED)
			SFLAG_N,printf(sh_gettxt(ER_DMP_CORE));
		if (tp == pp->p_friends) {
			if (flag&AMPERSAND)
				printf(" &");
			if (flag&JOBDIR &&
			    !eq(tp->p_cwd->di_name, dcwd->di_name)) {
				SFLAG_N,printf(sh_gettxt(MSG_WORK_DIR));
		...
		if (tp == pp->p_friends) {
			...
			if (flag&SHELLDIR && !eq(tp->p_cwd->di_name, dcwd->di_name)) {
				SFLAG_N,printf(sh_gettxt(MSG_NWORK_DIR));
	...
	if (v[0] && v[0][0] == '-') {
		...
		if (digit(v[0][1])) {
			...
			if (signum < 0 || signum > NSIG)
				SFLAG_E,bferr(gettxt(ER_BAD_SIGNO));
		} else {
			name = &v[0][1];
			for (signum = 1; signum <= NSIG; signum++) {
			...
			setname(name);
			SFLAG_E,bferr(gettxt(ER_UNKNOWN_S));
		}
gotsig:
		v++;
	} else
	...
	while (*v) {
		...
		if (*cp == '%') {
			...
			case SIGTTOU:
				if ((jobflags & PRUNNING) == 0) {
					/* %s -> %t */
					SFLAG_E,printf(sh_gettxt(ER_ALD_STOP), cp);
					err++;
					goto cont;
				}
			}
			...
			SFLAG_E,bferr(sh_gettxt(ER_ARG_ID));
		else {
			pid = atoi_(cp);
...
panystop(neednl)
{
	...
	for (pp = proclist.p_next; pp; pp = pp->p_next)
		if (pp->p_flags & PSTOPPED)
			SFLAG_E,error(gettxt(ER_STOP_JOB));
}
	...
	tchar *cp;
{
	...
	if (cp == 0 || cp[1] == 0 || eq(cp, S_PARCENTPARCENT /*"%%"*/) ||
				     eq(cp, S_PARCENTPLUS /*"%+"*/)) {
		if (pcurrent == PNULL)
			SFLAG_E,bferr(gettxt(ER_NCUR_JOB));
		return (pcurrent);
	}
	if (eq(cp, S_PARCENTMINUS /*"%-"*/) ||
	    eq(cp, S_PARCENTSHARP /*"%#"*/)) {
		if (pprevious == PNULL)
			SFLAG_E,bferr(gettxt(ER_NPRV_JOB));
		return (pprevious);
	}
	if (digit(cp[1])) {
		...
		for (pp = proclist.p_next; pp; pp = pp->p_next)
			if (pp->p_index == index && pp->P_pid == pp->p_jobid)
				return (pp);
		SFLAG_E,bferr(gettxt(ER_NSUCH_JOB));
	}
	np = PNULL;
	for (pp = proclist.p_next; pp; pp = pp->p_next)
...
match:
				if (np)
					SFLAG_E,bferr(gettxt(ER_AMBIGOUS));
				np = pp;
			}
		}
	if (np)
		return (np);
	if (cp[1] == '?')
		SFLAG_E,bferr(gettxt(ER_NMAT_JOB));
	else
		SFLAG_E,bferr(gettxt(ER_NSUCH_JOB));
	/*NOTREACHED*/
}
...
/*
 * handshake - receive SIGUSR1 handshake from child
 */

static bool gothandshake;
	...
	int wanttty;
{
	...
	/*
	 * If the child process is going to be setting its process group ID,
	 * we will await a handshake from it that tells us it has done so.
	 * This helps us avoid race conditions such as when the group leader
	 * exits before later processes in a pipeline have had a chance to
	 * join its group.
	 */
	dosetpgrp = (wanttty >= 0 && tpgrp >= 0);
	if (dosetpgrp) {
		gothandshake = 0;
		osig = (void (*)()) signal(SIGUSR1, handshake);
		/* osig will be SIG_DFL or SIG_IGN */
	}
	...
	while ((pid = fork()) < 0)
		if (setintr == 0)
			...
			(void) sigsetmask(omask);
			if (dosetpgrp)
				(void) signal(SIGUSR1, osig);
			SFLAG_E,error(gettxt(ER_FAIL_FORK));
		}
	if (pid == 0) {
		settimes();
		...
		if (dosetpgrp) {
			if (!gothandshake) {
				...
				while (!gothandshake)
					sigpause(omask &~ sigmask(SIGUSR1));
				(void) sigsetmask(omask);
			}
			(void) signal(SIGUSR1, osig);
		}
...
okpcntl()
{
	...
	if (tpgrp == -1)
		SFLAG_E,error(gettxt(ER_NCNTL_JOB));
	if (tpgrp == 0)
		SFLAG_E,error(gettxt(ER_NCNTL_JOBS));
}
