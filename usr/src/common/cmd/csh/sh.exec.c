...
#ident	"@(#)csh:common/cmd/csh/sh.exec.c	1.3.7.6"
#ident  "$Header$"
...
#ifdef VPIX
		/*
		 * XXX - This code causes memory leaks when exec succeeds,
		 * if VFORK.  (Memory is allocated using the parent's address
		 * space, and never freed.)
		 */

		dosp = tstostr(NULL, *pv);
		if (dospath(dosp)) {
		    char **dossfx;
			...
			dossav = strtots(NOSTR, *dossfx);
			dossimple = strspl(*av, dossav);
			xfree(dossav);
...
tchar **av;  /* av = arg vector */
{
	...
	if (chk_access (real_name, S_IEXEC) == 3) 
		SFLAG_E,error(gettxt(ER_NO_EXEC), real_name);
...
pexerr()
{
	...
	if (exerr)
		SFLAG_E,bferr(exerr);
	SFLAG_E,bferr(gettxt(ER_NFND_CMD));
}
	...
	register tchar **t;
{
	...
	*ot_str = 0;
	...
	/* XXX - memory leak when exec succeeds, if VFORK */
	...
	case ENOEXEC:
		...
		{       
			...
			if (ff != -1 && read_(ff, &ch, 1) == 1 && !isprint(ch)
			       && !isspace(ch)) {
				SFLAG_W,printf(sh_gettxt(ER_NEXE_BIN));
				Perror(f);
				(void) close(ff);
				return;
			} 
...
/*
 * The hashstat builtin was formerly available only when csh was compiled
 * with -DVFORK.  (Since the hits and misses are tabulated in the child
 * process, they are only available to the parent if it vforked, not if
 * it forked.)  However, since hashstat is documented in the man page it
 * has been made available unconditionally.  When -DVFORK is not used
 * in compilation, hashstat will always report 0 hits, 0 misses, 0%.
 */

hashstat()
{
#ifdef TRACE
	tprintf("TRACE- hashstat()\n");
#endif
	SFLAG_N,printf(gettxt(MSG_HASH_STATIC),
		hits, misses,
		(hits + misses)? 100 * hits / (hits + misses) : 0);
}
