...
#ident	"@(#)csh:common/cmd/csh/sh.dir.c	1.4.6.5"
#ident  "$Header$"
     ...
     tchar **v;
{
	...
	if (*++v == NOSTR) {
		if ((cp = value(S_home)) == NOSTR || *cp == 0)
			SFLAG_E,bferr(sh_gettxt(ER_NO_HOME));
		if (chdir_(cp) < 0)
			SFLAG_E,bferr(sh_gettxt(ER_NCHG_HOME));
		cp = savestr(cp);
     ...
     tchar **v;
{
	...
	if (*++v == NOSTR) {
		if ((dp = dcwd->di_prev) == &dhead)
			dp = dhead.di_prev;
		if (dp == dcwd)
			SFLAG_E,bferr(sh_gettxt(ER_NO_DIR));
		if (chdir_(dp->di_name) < 0)
			Perror(dp->di_name);
	...
	register tchar *cp;
{
	...
	for (dp = dcwd; i != 0; i--) {
		if ((dp = dp->di_prev) == &dhead)
			dp = dp->di_prev;
		if (dp == dcwd)
			SFLAG_E,bferr(gettxt(ER_NDEEP_DSTK));
	}
	return (dp);
}
     ...
     tchar **v;
{
	...
	if (*++v == NOSTR)
		...
		SFLAG_E,bferr(sh_gettxt(ER_INV_ARG));
	if (dp->di_prev == &dhead && dp->di_next == &dhead)
		SFLAG_E,bferr(sh_gettxt(ER_EMP_DSTK));
	if (dp == dcwd) {
