...
#ident	"@(#)csh:common/cmd/csh/sh.set.c	1.3.5.8"
#ident  "$Header$"
	...
	register tchar **v;
{
	...
	do {
		...
		if (op && op != '=')
setsyn:
			SFLAG_E,bferr(gettxt(ER_SYNTAX));
		if (eq(p, S_LPAR/*"("*/)) {
			register tchar **e = v;
			...
			for (;;) {
				if (!*e)
					SFLAG_E,bferr(gettxt(ER_MIS_RPARN));
				if (**e == ')')
					break;
				e++;
			}
	...
	register int *ip;
{
	...
	*cp++ = 0;
	while (*cp && digit(*cp))
		*ip = *ip * 10 + *cp++ - '0';
	if (*cp++ != ']')
		SFLAG_E,bferr(gettxt(ER_SUBSCRIPT));
	return (cp);
}
	...
	tchar *vp;
{
	...
	if (subscr < 1 || subscr > blklen(v->vec))
		SFLAG_E,bferr(gettxt(ER_OUT_SUBS));
	return (v);
}
...
#ifndef MAXDIGS
#define MAXDIGS	11	/* max chars in integer representation, incl. sign */
#endif
...
putn(n)
	register int n;
{
	static tchar number[MAXDIGS + 1];	/* note trailing '\0' */
	div_t result;				/* quot & rem from division */
	int neg;				/* set if original n was -ve */
	tchar *p = &number[MAXDIGS];		/* work backwards from end */

#ifdef TRACE
	tprintf("TRACE- putn()\n");
#endif
	/*
	 * To ensure known division semantics, we work with
	 * negative numbers and use div() rather than the
	 * implementation-defined "/" and "%" operators.
	 * This works whether or not the integer representation is
	 * twos-complement (where -INT_MIN is not representable).
	 */

	if (!(neg = (n < 0)))			/* force n <= 0 */
		n = -n;

	do {
		result = div(n, 10);		/* get quotient, remainder */
		*--p = '0' - result.rem;	/* -9 <= rem <= 0 */
		n = result.quot;		/* quot <= 0 */
	} while (n);

	if (neg)				/* original n was negative */
		*--p = '-';
	...
	register tchar *cp;
{
	...
	return (sign ? -n : n);
badnum:
	SFLAG_E,bferr(gettxt(ER_BAD_NUMBER));
	return (0);
}
...
#endif
	/*
	 * If !globsafe, avoid interpreting the string being assigned.
	 */
	if (globsafe) {
		gflag = 0; tglob(oldv);
		if (gflag) {
	...
	register tchar **v;
{
	...
	if (argv == 0)
		udvar(name);
	if (argv->vec[0] == 0)
		SFLAG_E,bferr(gettxt(ER_NO_WORD));
	lshift(argv->vec, 1);
}
	...
	if (val)
		while (*val) {
			if (strlen_(*val) + strlen_(exppath) + 2 > BUFSIZ) {
				SFLAG_W,printf(gettxt(ER_TRUNC_PATH));
				break;
			}
