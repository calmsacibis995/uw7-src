...
#ident	"@(#)csh:common/cmd/csh/sh.file.c	1.4.6.6"
#ident  "$Header$"
	...
	tchar *recognized_part;
{
	...
	/* Only need to erase the ^[ if IEXTEN and ECHOCTL set */
	if (  ( tty_save.c_lflag & IEXTEN )  &&  ( tty_save.c_lflag & ECHOCTL )  )
	{
		do_erase = 1;
	}
	...
	case 0:				/* erase two characters: ^[ */
		if ( do_erase ) 
			write(unit, "\b\b  \b\b", strlen("\b\b  \b\b"));
		break;

	case 1:				/* overstrike the ^, erase the [ */
		if ( do_erase )
			write(unit, "\b\b", strlen("\b\b"));
...
again:	/* search for matches */
	...
	for (numitems = 0; entry = getentry(dir_fd, looking_for_lognames); ) {
		...
		if (command == LIST) {
			if (numitems >= MAXITEMS) {
				SFLAG_E,printf (gettxt(ER_TOO_MANY),
				    looking_for_lognames ?
					gettxt(ER_NM_PASSWD):gettxt(ER_NM_FILE));
				break;
			}
			if (items == NULL)
	...
	if (command == LIST) {
		qsort( (tchar *)items, numitems, sizeof(items[1]),(int (*)())fcompare);
		/*
