...
#ident	"@(#)csh:common/cmd/csh/sh.dol.c	1.3.5.4"
#ident  "$Header$"
	...
	case '"':
		...
		for (;;) {
			...
			if (c == c1)
				break;
			if (c == '\n' || c == DEOF)
				SFLAG_E,error(gettxt(ER_UNMATCH_C), c1);
			if ((c & (QUOTE|TRIM)) == ('\n' | QUOTE))
				--wp, ++i;
			if (--i <= 0)
				goto toochars;
...
Dgetdol()
{
	...
	case '<'|QUOTE:
		...
		for (np = wbuf; read_(OLDSTD, np, 1) == 1; np++) {
			if (np >= &wbuf[BUFSIZ-1])
				SFLAG_E,error(gettxt(ER_LONG_LINE));
			if (*np <= 0 || *np == '\n')
				break;
		}
		*np = 0;
	...
	default:
		...
		if (digit(c)) {
			...
			if (subscr == 0) {
				...
				if (file == 0)
					SFLAG_E,error(gettxt(ER_NO_FILE));
				setDolp(file);
				goto eatbrac;
			}
			if (bitset)
...
syntax:
				SFLAG_E,error(gettxt(ER_VAR_SYNTAX));
		}
		*np++ = 0;
		unDredc(c);
		vp = adrof(name);
...
oob:
				setname(vp->v_name);
				SFLAG_E,error(gettxt(ER_OUT_SUBS));
			}
			lwb = i;
			if (!*np)
				upb = lwb, np = S_AST/*"*"*/;
...
eatmod:
		...
		if (c == ':') {
			...
			if (!any(c, S_htrqxe))
				SFLAG_E,error(gettxt(ER_BAD_MODF));
			dolmod = c;
			if (c == 'q')
				dolmcnt = 10000;
		} else
     ...
     tchar *term;
{
	...
	for (;;) {
		...
		for (;;) {
			...
			if (c < 0) {
				setname(term);
				SFLAG_E,bferr(gettxt(ER_NO_TERM));
			}
			if (c == '\n')
				break;
			if (c &= TRIM) {
				*lbp++ = c;
				if (--lcnt < 0) {
					setname(S_LESLES/*"<<"*/);
					SFLAG_E,error(gettxt(ER_OVF_LINE));
				} 
			}
		}
		*lbp = 0;
...
/*				if (!any(c, "$\\`"))*/
			...
			*mbp++ = c;
			if (--mcnt == 0) {
				setname(S_LESLES/*"<<"*/);
				SFLAG_E,bferr(gettxt(ER_OVF_LINE));
			}
		}
		*mbp++ = 0;
