...
#ident	"@(#)csh:common/cmd/csh/sh.parse.c	1.3.5.6"
#ident  "$Header$"
	...
	register struct wordent *lex;
{
	...
	if (--aleft == 0)
		SFLAG_E,error(gettxt(ER_LOOP_ALIAS));
	...
	register struct wordent *p2;
{
	...
	for (p = p1; p != p2; p = p->next)
		...
		case ')':
			l--;
			if (l < 0)
				SFLAG_E,error(gettxt(ER_MANY_RPARN));
			continue;
	...
	int flags;
{
	...
	for (p = p1; p != p2; p = p->next)
		...
		case ')':
			l--;
			if (l < 0)
				SFLAG_E,seterr(gettxt(ER_MANY_RPARN));
			continue;
	...
	SFLAG_E,seterr(gettxt(ER_MANY_LPARN));
	return (0);
}
...
again:
	...
	for (p = p1; p != p2; p = p->next) {
		...
		case '(':
			if (l == 0) {
				if (lp != 0 && !specp)
					SFLAG_E,seterr(gettxt(ER_BAD_LPARN));
				lp = p->next;
			}
			l++;
			goto savep;
		...
		case '>':
			...
			if (p->next == p2) {
missfile:
				SFLAG_E,seterr(gettxt(ER_MIS_RNAME));
				continue;
			}
			p = p->next;
...
badout:
				SFLAG_E,seterr(gettxt(ER_AMB_ROUT));
			else
		...
		case '<':
			...
			if ((flags & PHERE) && (t->t_dflg & FHERE))
				SFLAG_E,seterr(gettxt(ER_NUSE_RDIRCT));
			else if ((flags & PIN) || t->t_dlef)
				SFLAG_E,seterr(gettxt(ER_AMB_RIN));
			else
...
savep:
	...
	if (lp != 0 && !specp) {
		if (n != 0)
			SFLAG_E,seterr(gettxt(ER_BAD_LRPARN));
		t->t_dtyp = TPAR;
		t->t_dspr = syn0(lp, rp, PHERE);
	} else {
		if (n == 0)
			SFLAG_E,seterr(gettxt(ER_INV_NCMD));
		t->t_dtyp = TCOM;
	}
	return (t);
}
