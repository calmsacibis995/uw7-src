...
#ident	"@(#)csh:common/cmd/csh/sh.glob.c	1.4.5.5"
#ident  "$Header$"
...
#define sort()	qsort( (tchar *)sortbas, &gargv[gargc] - sortbas, \
		      sizeof(*sortbas), (int (*)())sortscmp), sortbas = &gargv[gargc]
			...
			if (gpathp != gpath + 1) {
				*gpathp = 0;
				if (gethdir(gpath + 1))
					 ...
					 */
					SFLAG_E,error(gettxt(ER_UNKNOWN_U), gpath + 1);
	...
	tchar *p, *s;
{
	...
	case '[':
		for (pe++; *pe && *pe != ']'; pe++)
			continue;
		if (!*pe)
			SFLAG_E,error(gettxt(ER_MIS_LPARN));
		continue;
	}
pend:
	if (brclev || !*pe)
		SFLAG_E,error(gettxt(ER_MIS_MPARN));
	...
	register tchar *s, *p;
{
	...
	for (;;) {
		...
		case '[':
			...
			if (cc == 0)
				SFLAG_E,bferr(gettxt(ER_MIS_LPARN));
			continue;
	...
	tchar *s1, *s2;
{
	...
	if (gnleft <= 0 || ++gargc >= GAVSIZ)
		SFLAG_E,error(gettxt(ER_LONG_ARGS));
	gargv[gargc] = 0;
	...
	tchar c;
{
	...
	if (gpathp >= lastgpathp)
		SFLAG_E,error(gettxt(ER_LONG_PATHNM));
	*gpathp++ = c & TRIM;
	*gpathp = 0;
}
	...
	bool literal;
{
	...
	for (;;) {
		...
		if (!*rp)
oops:
			SFLAG_E,error(gettxt(ER_UNMATCH_Q));
		ep = savestr(lp);
		ep[rp - lp] = 0;
		backeval(ep, literal);
#ifdef GDEBUG
	...
	tchar c;
{
	...
	if (--pnleft <= 0)
		SFLAG_E,error(gettxt(ER_LONG_WORD));
	*pargcp++ = c;
}
...
pword()
{
	...
	psave(0);
	if (pargc == GAVSIZ)
		SFLAG_E,error(gettxt(ER_MANY_WORD_Q));
