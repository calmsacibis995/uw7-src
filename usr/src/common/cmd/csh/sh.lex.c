...
#ident	"@(#)csh:common/cmd/csh/sh.lex.c	1.4.5.7"
#ident  "$Header$"
	...
	for (;;) {
		if (c1) {
			if (c == c1) {
				...
				SFLAG_E,seterrc(gettxt(ER_UNMATCH), c1);
				ungetC(c);
				break;
			}
		...
		if (--i > 0) {
			*wp++ = c;
			...
			SFLAG_E,seterr(gettxt(ER_LONG_WORD));
			wp = &wbuf[1];
			break;
		}
	}
...
getdol()
{
#define	MAXVARNAME	40 /* Max # of chars in variables ?*/
	register tchar *np;
	tchar name[MAXVARNAME];
	register int c;
	int sc;
	bool special = 0;
...
vsyn:
	SFLAG_E,seterr(gettxt(ER_VAR_SYNTAX));
	goto ret;
#undef	MAXVARNAME
}
	...
	tchar *cp;
{
	...
	if (lap != 0 && strlen_(cp) + strlen_(lap) >= BUFSIZ - 4) {
		SFLAG_E,seterr(gettxt(ER_OVF_EXPBUF));
		return;
	}
	if (lap)
...
subst:
	...
	if (sc == '{') {
		c = getC(0);
		if (c != '}')
			SFLAG_E,seterr(gettxt(ER_BAD_FORM));
	}
	exclnxt = hp;
}
	...
	struct wordent *en;
{
	...
	case '&':
		if (slhs[0] == 0) {
			SFLAG_E,seterr(gettxt(ER_NPRV_SUB));
			goto ret;
		}
...
bads:
			lhsb[0] = 0;
			SFLAG_E,seterr(gettxt(ER_BAD_SUB));
			goto ret;
		}
		cp = lhsb;
		for (;;) {
...
/*badlhs:*/
			SFLAG_E,seterr(gettxt(ER_NPRV_LHS));
			goto ret;
		}
		cp = rhsb;
...
/*toorhs:*/
				SFLAG_E,seterr(gettxt(ER_LONG_RHS));
				goto ret;
			}
			if (c == '\\') {
				c = getC(0);
	...
	default:
		if (c == '\n')
			unreadc(c);
		SFLAG_E,seterrc(gettxt(ER_BAD_MOD), c);
		goto ret;
	}
	...
	bool global;
{
	...
	if (didsub == 0)
		SFLAG_E,seterr(gettxt(ER_FAIL_MOD));
	hp->prev = wdp;
	...
	default:
		...
		for (mp = cp; *mp; mp++)
			if (matchs(mp, lhsb)) {
				...
				if (i < 0) {
ovflo:
					SFLAG_E,seterr(gettxt(ER_OVF_SBBUF));
					return (S_ /*""*/);
				}
				*wp = 0;
				(void) strcat_(wp, mp);
	...
	int dol;
{
	...
	if (*al > *ar || *ar > dol) {
bad:
		SFLAG_E,seterr(gettxt(ER_BAD_SEL));
		return (0);
	}
	return (1);
	...
	int sc;
{
	...
	case '?':
		...
		if (np == lhsb) {
			if (lhsb[0] == 0) {
				SFLAG_E,seterr(gettxt(ER_NPRV_SRCH));
				return (0);
			}
		} else
			*np++ = 0;
	...
	tchar *cp;
{
	...
	tprintf("TRACE- noev()\n");
#endif
	SFLAG_E,seterr2(cp,gettxt(ER_NFND_EVENT));
}
...
SFLAG_N,printf(gettxt(MSG_RSET_PGRP), ctpgrp, tpgrp);
					goto reread;
				}
				if (adrof(S_ignoreeof/*"ignoreeof"*/)) {
					if (loginsh)
						SFLAG_A,printf(gettxt(USG_LOGOUT));
					else
						SFLAG_A,printf(gettxt(USG_EXIT));
					reset();
				}
				if (chkstop == 0) {
					panystop(1);
