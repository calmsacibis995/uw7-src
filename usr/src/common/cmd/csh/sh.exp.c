...
#ident	"@(#)csh:common/cmd/csh/sh.exp.c	1.3.5.5"
#ident  "$Header$"
	...
	register tchar ***vp;
{
	...
	if (eq(**vp, S_LBRA /* "{" */)) {
		...
		for (;;) {
			if (!**vp)
				SFLAG_E,bferr(gettxt(ER_MIS_MPARN));
			if (eq(*(*vp)++, S_RBRA /*"}" */))
				break;
		}
		if (ignore&IGNORE)
	...
	if (*cp == '-' && any(cp[1], S_erwxfdzo /*"erwxfdzo" */)) {
		...
		if (cp[2] != '\0')
			SFLAG_E,bferr(gettxt(ER_MALFORM_FILE));
		...
		if (isa(**vp, ANYOP) && stat_(**vp, &stb))
			SFLAG_E,bferr(gettxt(ER_MIS_FNAME));
		dp = *(*vp)++;
