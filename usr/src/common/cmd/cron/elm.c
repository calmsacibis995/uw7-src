...
#ident	"@(#)cron:common/cmd/cron/elm.c	1.4.1.4"
...
/* index points to the front of the index list */
	...
	time_t lb, dt;
{
	 ...
	 */
	n= (struct notice *) MALLOC(sizeof(struct notice), M_ELM);
	n->time = LB;
	n->isdummy = TRUE;
	n->left = NULL;
	nprev = n;
	/* create first dummy key */
	k= (struct key *) MALLOC(sizeof(struct key), M_ELM);
	k->time = LB;
	k->numnote = 1;
	k->notice = n;
	k->left = NULL;
	...
	for (i=1; i<DU; i++) {
		...
		kprev = k;
		n->key = k;
		ind = (struct index *) MALLOC(sizeof(struct index), M_ELM);
		ind->key = k;
		if (indprev == NULL)
			index = ind;
		 else
	...
	time_t	time;
{
	...
	if (k->numnote == NLIM) {
		...
		k2 = (struct key *) MALLOC(sizeof(struct key), M_ELM);
		k2->time = n2->time;
	...
	while (n != NULL) {
		...
		if (n != NULL) {
			...
			if ( (n->key!=NULL) && ((n->key)->numnote==1) ) {
				...
				k->numnote = k->numnote - 1;
				/*
				 * Check if two adjacent sublists can be
				 * merged.  First check left, then check right.
				 */
				kl = k->left;
				kr = k->right;
