...
int	do_snprintf(char *buf, size_t nbytes, const char* fmt, ...);
	...
	while (*s) {
		...
		for (t=fmt; (*t++ = *s) != '\0'; s++) {
			...
			if (*s == '*') {
				x = execute(a);
				a = a->nnext;
				do_snprintf((char *)t-1, RECSIZE - ( t - fmt), "%d", (int) getfval(x));
		...
		*t = '\0';
		...
		if (flag == 0) {
			do_snprintf((char *)p, bufsize - (p - buf), "%s", fmt);
			p += strlen((char*)p);
			continue;
		}
		if (a == NULL)
		...
		a = a->nnext;
		switch (flag) {
		case 1:	do_snprintf((char *)p, bufsize - (p - buf), (char *)fmt, getfval(x)); break;
		case 2:	do_snprintf((char *)p, bufsize - (p - buf), (char *)fmt, (long) getfval(x)); break;
		case 3:	do_snprintf((char *)p, bufsize - (p - buf), (char *)fmt, (int) getfval(x)); break;
		case 4:	do_snprintf((char *)p, bufsize - (p - buf), (char *)fmt, getsval(x)); break;
		case 5: isnum(x) ? do_snprintf((char *)p, bufsize - (p - buf), (char *)fmt, (int) getfval(x))
				 : do_snprintf((char *)p, bufsize - (p - buf), (char *)fmt, getsval(x)[0]);
			break;
		}
		tempfree(x, "");
		p += strlen((char*)p);
...
Cell *print(a,n) Node **a;
{
	...
	for (x = a[0]; x != NULL; x = x->nnext) {
		...
		*/
		if ((y->tval & (ARR|FLD|REC|STR)) == STR)
			fputs((char *)y->sval, fp);
		else if ((y->tval & (ARR|FLD|REC|NUM)) != NUM)
...
static int endfile(afp) struct afile *afp;
{
	int ret;
...
int
do_snprintf(char *target, size_t nbytes, const char *fmt, ...)
{
	va_list ap;
	int ret;
...
#ifdef NO_SNPRINTF
	ret = vsprintf(target, fmt, ap);
#else
	ret = vsnprintf(target, nbytes, fmt, ap);
#endif
	va_end(ap);
