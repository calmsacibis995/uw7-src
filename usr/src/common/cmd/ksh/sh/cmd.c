...
register int sym;
{
	if(sym&SYMFLG)
	{
		...
		p_str(gettxt(E_ENDOFFILE),0);
	else
	{
		if(sym&SYMREP)
			p_char(sym);
		if(sym==NL)
			p_str(gettxt(E_NEWLINE),0);
		else
			p_char(sym);
	}
	p_char('\'');
...
void	sh_syntax()
{
	register const char *cp = gettxt(E_UNEXPECTED);
	register int w = sh.wdval;
	p_setout(ERRIO);
	SFLAG_E;
	p_prp(gettxt(E_SYNBAD));
	if((w==EOFSYM) && sh.owdval)
	{
		w = sh.owdval;
		cp = gettxt(E_UNMATCHED);
	}
	else
		sh.olineno = st.standin->flin;
	if(!(st.states&(PROMPT|PROFILE)))
	{
		p_str(gettxt(E_ATLINE),0);
		p_num(sh.olineno,SP);
	}
	p_str(e_colon,'`');
	if(w)
