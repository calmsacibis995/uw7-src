...
#ident	"@(#)ksh:sh/service.c	1.4.6.7"
 ...
 @*/
{
	...
	{
		if(is_option(RSHFLG))
			sh_fail(name, gettxt(E_RESTRICTED));
	}
	else
	{
		if(path==NULL)
...
@*/
{
	...
	if(strchr(t[0],'/'))
	{
		...
		if(is_option(RSHFLG))
			sh_fail(t[0],gettxt(E_RESTRICTED));
	}
	else
		path=path_get(*t);
#ifdef VFORK
	...
	if(dospath(ap))
	{
		...
		while(**suffix)
		{
			...
			if(canexecute(p)>=0)
			{
				...
				{
					case ENOENT:
						sh_fail(vp,gettxt(E_FOUND));
					default:
						sh_fail(vp,gettxt(E_EXEC));
				}
			}
		}
		t[0] = savet;
...
#ifdef ENAMETOOLONG
		case ENAMETOOLONG:
			xecmsg = gettxt(E_LONGNAME);
			return(prefix);
#endif /* ENAMETOOLONG */
		case ENOMEM:
			sh_fail(p,gettxt(E_TOOBIG));

		case E2BIG:
			sh_fail(p,gettxt(E_ARGLIST));

#ifdef ETXTBSY
		case ETXTBSY:
			sh_fail(p,gettxt(E_TXTBSY));
#endif /* ETXTBSY */
#ifdef ELIBACC
		case ELIBACC:
			sh_fail(p, gettxt(E_LIBACC));

		case ELIBBAD:
			sh_fail(p, gettxt(E_LIBBAD));

		case ELIBSCN:
			sh_fail(p, gettxt(E_LIBSCN));

		case ELIBMAX:
			sh_fail(p, gettxt(E_LIBMAX));
#endif /* ELIBACC */
...
@*/
{
	...
	if(--st.fn_depth==1 && mode==3)
		sh_fail(com[0],gettxt(E_RECURSIVE));
