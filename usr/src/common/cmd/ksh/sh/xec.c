					...
					2,4,8,16, and 32 seconds and then quits */
				{
...
#endif	/* VFORK */
					{
						if((forkcnt *= 2) > FORKLIM)
						{
							...
							{
							case ENOMEM:
								msg = sh_gettxt(E_SWAP);
								break;
							default:
							case EAGAIN:
								msg = sh_gettxt(E_FORK);
								break;
							}
							sh_fail(msg,NIL);
						}
...
#undef c
			...
			case TPROC:
			{
				...
				if(is_abuiltin(np))
				{
					p_setout(ERRIO);
					SFLAG_E;
					p_prp(gettxt(E_RESTRICTED));
					p_str(":",' ');
					p_str(fname,0);
					p_str(gettxt(IS_BUILTIN),NL);
					sh.exitval++;
					break;
				}
