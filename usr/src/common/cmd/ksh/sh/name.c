...
#   endif /* MULTIBYTE */
			...
			else if(n==CSWIDTHNOD)
			{
				if(ed_setwidth(argscan) && !initenv)
					sh_fail(n->namid,gettxt(E_FORMAT));
			}
#endif /* MULTIBYTE */
		}
		return(n);
	...
	if(initenv)
		return(0);
failed:
	sh_fail (argi,(root==sh.alias_tree?gettxt(E_ALINAME):gettxt(E_IDENT)));
	/* NOTREACHED */
	return(0);
}
			...
			if(flag)
			{
				if(isnull(np))
				{
					if(!nam_istype(np,N_FUNCTION))
						goto search;
					a1 = gettxt(IS_UFUNCTION);
				}
				else if(is_abuiltin(np))
					a1 = gettxt(IS_BUILTIN);
				else if(nam_istype(np,N_EXPORT))
					a1 = gettxt(IS_XFUNCTION);
				else
					a1 = gettxt(IS_FUNCTION);
			}
		}
		else
		{
