...
register char *fname;
{
	...
	if(fname)
	{
		...
		if(fd<=0)
			sh_fail(tmp_name,gettxt(E_CREATE));
	}
	io_ftable[fd] = 0;
	return(fd);
}
...
int			mode;
{
	...
	if (fd < 0)
		sh_fail(path, (flags & (O_WRONLY|O_RDWR|O_APPEND|O_CREAT)) ? gettxt(E_CREATE) : gettxt(E_OPEN));
	return(fd);
}
...
register struct ionod	*i;
{
	...
	while(i)
	{
		...
		if(maxtry<=0)
			sh_fail(io_tmpname,gettxt(E_CREATE));
		if(i->iolink)
			free(i->iolink);
	...
	for(;iop;iop=iop->ionxt)
	{
		...
		if(*ion)
		{
			...
			else if(iof&IORDW)
			{
				...
				sh_fail(ion,gettxt(E_RESTRICTED));
			else
			{
				if(iof&IOAPP)
				{
