...
#undef sep
		...
		else if(argp)
		{
			...
			{
				sh_trim(argp);
				sh_fail(id,*argp?argp:gettxt(E_NULLSET));
			}
			else if(c=='=')
			{
				if(n)
...
static void mac_error()
{
	sh_fail(mac_current,gettxt(E_SUBST));
}
