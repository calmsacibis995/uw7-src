	...
	case LOOKUP:
	{
		...
		if(isalpha(c))
		{
			...
			*str = c;
			if(c=='(')
			{
				lvalue->value =gettxt(E_FUNCTION);
				str = *ptr;
				break;
			}
			else if(c=='[')
		...
		else
		{
			...
			default:
				/*
				 * A-Z and a-z represent 10-35 in base 36.
				 * We do not want to use isupper() and islower()
				 * here, since they may include characters other
				 * than A-Z and a-z in non-C locales.
				 */
				if(isdigit(c))
					c -= '0';
				else if(c >= 'A' && c <= 'Z')
