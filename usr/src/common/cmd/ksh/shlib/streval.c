...
#ident	"@(#)ksh:shlib/streval.c	1.2.6.4"
...
number	(*conv)();
{
	...
	convert = conv;
	if(level++ >= MAXLEVEL)
		(void)seterror(sh_gettxt(e_recursive));
	else
	{
		n = expr(0);
		if (peekchr() == ':') (void)seterror(sh_gettxt(E_BADCOLON));
	}
	if (cur.errmsg.value)
	{
...
register int	precedence;
{
...
#ifdef FLOAT
		if(cur.isfloat)
			error(sh_gettxt(E_INCOMPATIBLE));
#endif /* FLOAT */
	...
	while(1)
	{
		...
		case COLON:
			(void)seterror(sh_gettxt(E_BADCOLON));
			break;
#endif
