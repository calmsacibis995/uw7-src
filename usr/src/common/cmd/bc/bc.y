...
#include <pfmt.h>
FILE *in;
#define CARRY_SIZE	(10 * (LINE_MAX+1))
#define STRING_SIZE	(10 * (BC_STRING_MAX+1))
char cary[CARRY_SIZE], *cp = { cary };
char string[STRING_SIZE], *str = { string };
int crs = '0';
...
CRS	:
		={ $$ = cp; *cp++ = crs++; *cp++ = '\0';
			if(crs == '[')crs+= 3;
			if(crs == 'a')crs='{';
			if(crs >= 0241){yyerror(gettxt(":336:program too big",NULL));
				getout(1);
			}
			bstack[bindx++] = lev++; }
	;
...
restart:
	...
	case '"':	
		yylval.cptr = str;
		 while((c=getch()) != '"'){*str++ = c;
			if(str > &string[STRING_SIZE]){yyerror(gettxt(":337:string space exceeded",NULL));
			getout(1);
		}
	}
	 *str++ = '\0';
...
loop:
	...
	sprintf(mbuf,gettxt(":334:can't open input file %s",NULL),sargv[ifile]);
	ln = -1;ss=gettxt(":335:command line",NULL);
	yyerror(mbuf);
	getout(1);
	/*NOTREACHED*/
}
...
va_dcl
{
	...
	while(i-- > 0){
		if( b_sp_nxt >= & b_space[b_sp_max] ) 
			yyerror(gettxt(":333:bundling space exceeded",NULL));
...
yyerror( s ) char *s; {
	if(ifile > sargc)ss=tty;
	pfmt(stderr,MM_NOSTD, ":332:%1$s: %2$d: %3$s\n", ss, ln+1, s);
	(void) fflush(stdout);
	cp = cary;
	crs = rcrs;
	bindx = 0;
...
yyinit(argc,argv) int argc; char *argv[];{
	...
	if(sargc == 0)in=stdin;
		...
		sprintf(mbuf,gettxt(":334:can't open input file %s",NULL),sargv[1]);
		ln = -1; ss=gettxt(":335:command line",NULL);
		yyerror(mbuf);
		getout(1);
	}
...
char **argv;
{
	...
	tty = strdup(gettxt(":338:teletype",NULL));
