%{
#ident	"@(#)ctrace:common/parser.y	1.7"
...
declaration:
	...
	|	TYPEDEF { typename2 = yes; } opt_decl_specifiers {
...
direct_declarator:
		L_IDENT {
			...
			/* save the name because it may be a typedef name */
				...
				nf_name[len] = '\0';
				/* Since the regular express in lex can
				 * match things such as ident[space], any
				 * spaces in nf_name are deleted here 
				 */
				if((tmp_ptr = strrchr(nf_name, ' ')) != NULL)
					*tmp_ptr = '\0';
...
funarg:		funL_LP { 
		...
		parameter_type_list funL_RP  {
			$$.end = $4.end;
			if(only) save_header = save_tmp; 
			para_type = no;  
		}
