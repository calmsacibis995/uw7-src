...
char	BLANKS[100] = "                                                                                                ";
char	GUTTER[WGUTTER] = "     ";
char	DIFF[]	= "/bin/diff -b ";
#define CMDSIZE (sizeof DIFF + PATH_MAX*2 + 2)
char	diffcmd[CMDSIZE];
char	inbuf[10];
...
char	**argv;
{
	...
	if (filebinary(fdes1) || filebinary(fdes2)) {
		if (ferror(fdes1) || ferror(fdes2)) {
		        fprintf(stdout,"Read error in %s: %s\n", ferror(fdes1) ? file1 : file2);
			fclose(fdes1);
			fclose(fdes2);
                        exit(2);
		}
	        if (filebinary(fdes1) && filebinary(fdes2)) 
		   fprintf(stdout,"Binary files %s and %s\n", file1, file2);
                else
		   fprintf(stdout,"Binary files %s %s\n", filebinary(fdes1) ? file1 : file2);
		fclose(fdes1);
		fclose(fdes2);
                exit(1);
	}
	...
	/* Call DIFF command */
	snprintf(diffcmd, CMDSIZE, "%s %s %s", DIFF, file1, file2);
	...
	*/
	...
	while(fgets(diffbuf,LMAX,diffdes) != NULL){
		if(strlen(diffbuf) == LMAX-1 && diffbuf[LMAX-2] != '\n') {
			while(fgets(diffbuf2,LMAX,diffdes) != NULL && diffbuf2[strlen(diffbuf2)-1] != '\n')
				;
		}
		change = ' ';
		com = cmd(diffbuf);
...
put1()
{
	...
	if(fgets(buf1,BMAX,fdes1) != NULL){
		...
		if(change != ' ')
			putmid(1);
		num1++;
		if(strlen(buf1) == BMAX-1 && buf1[BMAX-2] != '\n') {
			fprintf(stderr, "%s: WARNING: input line truncated in %s\n", pgmname, file1); 
			while(fgets(buf1,BMAX,fdes1) != NULL && buf1[strlen(buf1)-1] != '\n')
				;
		}
		return(1);
	} else 
...
put2()
{
	...
	if(fgets(buf2,BMAX,fdes2) != NULL){
		...
		*/
		...
		num2++;
		len1 = 0;
		if(strlen(buf2) == BMAX-1 && buf2[BMAX-2] != '\n') {
			/* read in rest of line (which is truncated in output) */
			fprintf(stderr, "%s: WARNING: input line truncated in %s\n", pgmname, file2); 
			while(fgets(buf2,BMAX,fdes2) != NULL && buf2[strlen(buf2)-1] != '\n')
				;
		}
		return(1);
	} else {
