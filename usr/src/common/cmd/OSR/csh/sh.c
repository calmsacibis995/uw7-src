...
/* #ident	"@(#)csh:sh.c	1.4" */
#pragma comment(exestr, "@(#) sh.c 26.2 96/03/14 ")

/*
 *	@(#) sh.c 1.2 88/03/30 csh:sh.c
 */
/* Copyright (c) 1980 Regents of the University of California */
#include "sh.h"
extern	char	*sbrk();	/* M000 */
 ...
 *	M000	18 Oct 83	andyp
 *	- Took out "login" command, now conditional on LOGIN.  XENIX 3.0
 *	  does not support this as a regular command.
 *	- Removed (v7) assumption that "" == "." for pathnames.
 *	- Removed dependency on pseudo-symbol "end" produced by ld, and
 *	  assumptions about location of stack relative to end of allocated
 *	  data.  This change necessary for fixed-stack machines.
 *	- Changed illegal old-style C-common declarations to externs.
 *	M001	26 Feb 84	andyp
 *	Thanks to Greg Guthrie at Bell Labs, Naperville and net.bugs
 *	- Fix for escaped quotes in quoted strings.
 *	M002	15 May 84	barrys
 *	- Signals are now turned on before executing a newgrp. Before
 *	  this resulted in a user doing a newgrp and then not able
 *	  to perform a quit.
 *	- Changed the minimum number of arguments to newgrp from 1
 *	  to 0.
 *	M003	25 July 84	davep
 *	- source /etc/cshrc for users that have csh as their login shell
 *	  Allows the adminstrator to have default things set up for every user
 *	  Note: This is done before .cshrc or .login so that the user can
 *		override it if necessary.
 *	S004	4 Apr 1989	sco!hoeshuen
 *	- If interactive, set shell variable "cdspell".  If set,
 *	  cd spelling correction occurs.
 *	L005	17 Feb 1994	scol!gregw
 *	- Renamed calloc to Calloc to avoid possible conflict with libc.
 *	L006	27 Nov 1995	scol!ianw
 *	- Changed the type of some automatic variables to static to ensure that
 *	  on return from setjmp they still have the values they had when
 *	  longjmp was called.
 *	L007	14 Mar 1996	scol!ianw
 *	- Fixed a bug introduced by L006 (when a .login file uses the source
 *	  built-in command the user is logged out) by changing the static
 *	  variables to volatile, necessary as srcunit() can be called
 *	  recursively (srcunit() calls process() which can lead to dosource()
 *	  being called which itself calls srcunit()).
 */
	...
	char **av;
{
	register char **v, *cp;
	int nofile = 0;
	volatile int reenter = 0;				/* L006 L007 */
	...
	/* Begin SCO_BASE S004...
	 * If we are reading from a tty, then default is cd spelling
	 * correction on.  Note that if we're not reading from a tty,
	 * then no spelling correction occurs reguardless of "cdspell".
	 */
	if (intty)
		setNS("cdspell");
	/* End S004...
...
char *cp;
{
	 ...
	 */
	pv = (char **) Calloc(i+2, sizeof (char **));		/* L005 */
	dp = cp;
	i = 0;
	for (;;) {
		if ((c = *dp) == ':' || c == 0) {
	...
	bool onlyown;
{
	...
	volatile int oSHIN = -1;				/* L006 L007 */
	int oldintty = intty;
	...
	jmp_buf oldexit;
	volatile int reenter;					/* L006 L007 */
