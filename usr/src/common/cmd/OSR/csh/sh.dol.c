#ident	"@(#)OSRcmds:csh/sh.dol.c	1.1"
/*
 *	@(#) sh.dol.c 25.1 93/07/20 
 *
...
/* #ident	"@(#)csh:sh.dol.c	1.2" */
#pragma comment(exestr, "@(#) sh.dol.c 25.1 93/07/20 ")

/*
 *	@(#) sh.dol.c 1.1 88/03/29 csh:sh.dol.c
 */
/*	@(#)sh.dol.c	2.1	SCCS id keyword	*/
/* Copyright (c) 1980 Regents of the University of California */

/* Modification History
 * S000		7 July 1989	sco!hoeshuen
 * - eat the trailing '}' after expanding an environment variable to
 *   its value, such as in ${ROOT} 
 * S001		28 July 1989	sco!hoeshuen
...
Dgetdol()
{
	...
	char name[4 * MAX_NAME_LEN + 1];			/* L002 */
	int c, sc;
...
Lsyntax:
	...
	if (dimen == 0 && subscr == 0 && c == '[') {
		...
		for (;;) {				/* L002 */
			...
			if (c == '\n' || c == DEOF)
				goto Lsyntax;
			if (np >= &name[4 * MAX_NAME_LEN])   /* L002 */
				goto Lsyntax;
			*np++ = c;
		}
		*np = 0, np = name;
