#ident	"@(#)OSRcmds:csh/sh.lex.c	1.1"
/*
 *	@(#) sh.lex.c 25.3 94/08/12 
 *
...
/* #ident	"@(#)csh:sh.lex.c	1.2" */
#pragma comment(exestr, "@(#) sh.lex.c 25.3 94/08/12 ")

/*
 *	@(#) sh.lex.c 1.1 88/03/29 csh:sh.lex.c
 */
/*	@(#)sh.lex.c	2.1	SCCS id keyword	*/
/* Copyright (c) 1980 Regents of the University of California */

/* Modification History
 * S000		28 July 1989	sco!hoeshuen
 * - added :e modifier support to return the extension part of the filename.
 *   e.g. set i=/mnt/foo.bar, echo $i:e will return bar.
 *   If filename does not have extension, returns null string.
 *
 * L001		1 July 1993	scol!gregw
 * - added some checks for long variable names. From greim@sbsvax.UUCP
 * (Michael Greim).
 *
 * L002		17 February 1994	scol!gregw
 * - Renamed calloc to Calloc to avoid possible conflict with libc.
 *
 * L003		11 August 1994		scol!gregw
 * - When we have ${variable} add test for letter(c) to prevent processing
 *   variable name prematurely.
 */
...
getdol()
{
	register char *np;
	char *ep;						/* L001 */
	char name[4 * MAX_NAME_LEN + 1];			/* L001 */
	register int c;
	int sc;
	bool special = 0, toolong;				/* L001 */
...
*/
	...
	if (c == '[') {
		*np++ = c;
		ep = &np[2 * MAX_NAME_LEN + 8];			/* L001 */
		do {
			c = getC(DOEXCL | DODOL);		/* L001 */
			if (c == '\n') {
				ungetD(c);
				np--;
				goto vsyn;
...
take:
	...
	case 'r':
		...
		goto take;
	/* Begin S000 */
	case 'e':	/* strip off extension part of filename */
		wp = strend(cp);
		for (wp--; wp >= cp && *wp != '.'; wp--)
			if (*wp == '/') /* no extension, return null string */
				return(savestr( (char *) 0));
		if (wp < cp)            /* no extension, return null string */
			return (savestr((char *)0));
		return(savestr(++wp));
	/* Begin S000 */
	}
...
again:
	...
	if (buf >= fblocks) {
		...
		fbuf = nfbuf;
		fbuf[fblocks] = Calloc(BUFSIZ, sizeof (char));	/* L002 */
		fblocks++;
		goto again;
	}
	if (fseekp >= feobp) {
...
settell()
{
	...
	if (lseek(SHIN, 0l, 1) < 0 || errno == ESPIPE)
		return;
	fbuf = (char **) Calloc(2, sizeof (char **));		/* L002 */
	fblocks = 1;
	fbuf[0] = Calloc(BUFSIZ, sizeof (char));		/* L002 */
