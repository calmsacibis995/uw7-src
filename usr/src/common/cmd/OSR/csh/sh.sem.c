#ident	"@(#)OSRcmds:csh/sh.sem.c	1.1"
/*
 *	@(#) sh.sem.c 26.1 95/12/08 
 *
...
/* #ident	"@(#)csh:sh.sem.c	1.2" */
#pragma comment(exestr, "@(#) sh.sem.c 26.1 95/12/08 ")

/*
 * Modification History
 *
 *	L000	04 Dec 1995	scol!ianw
 *	- Restore the foreground process group ID when the controlling
 *	  terminal has no foreground process group (tcgetpgrp returns RESPID).
 */

/*
 *	@(#) sh.sem.c 1.1 88/03/29 csh:sh.sem.c
 */
	...
	case TPAR:
		...
		if (pid != 0) {
			...
			if (t->t_dtyp == TCOM)
				...
				cadd(pid, "()");
			if ((flags & (FPOU|FAND)) == 0) {	/* L000 begin */
				pwait(pid);
				if (pgrp != -1 && tcgetpgrp(SHOUT) == RESPID)
					tcsetpgrp(SHOUT, pgrp);
			}					/* L000 end */
			return;
		}
