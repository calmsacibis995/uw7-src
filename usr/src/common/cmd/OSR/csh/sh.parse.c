#ident	"@(#)OSRcmds:csh/sh.parse.c	1.1"
/*
 *	@(#) sh.parse.c 25.1 94/02/18 
 *
...
/* #ident	"@(#)csh:sh.parse.c	1.2" */
#pragma comment(exestr, "@(#) sh.parse.c 25.1 94/02/18 ")
/*
 * Modification History
 *
 *	L000	17 February	1994
 *	- Renamed calloc to Calloc to avoid possible conflict with libc.
 */

/*
 *	@(#) sh.parse.c 1.1 88/03/29 csh:sh.parse.c
 */
...
again:
	...
	if (n < 0)
		n = 0;
	t = (struct command *) Calloc(1, sizeof (*t));		/* L000 */
	av = (char **) Calloc(n + 1, sizeof (char **));		/* L000 */
	t->t_dcom = av;
	n = 0;
	if (p2->word[0] == ')')
		t->t_dflg = FPAR;
