#ident	"@(#)OSRcmds:ksh/sh/jobs.c	1.1"
#pragma comment(exestr, "@(#) jobs.c 26.1 95/12/07 ")
/*
 ...
 *	- bug fix.  Don't search passed the end of the jobs bitmap
 *	L001	scol!markhe			16 Nov 92
 *	- bug fix for job control.  Suspend on a pipeline or list,
 *	  eg. cat /etc/termcap | pg , would cause the shell to hang in
 *	  waitpid().  When the return status of all stopped processes
 *	  had been returned the waitpid() was still called without
 *	  WNOHANG.  Fix is to remove code that ignores stopped childern
 *	  if none-interactive.  job_wait() is then recursively called with
 *	  pid set to zero, so WNOHANG is used. (This is how the previous
 *	  version of ksh (88e) use to work).
 *	L002	scol!ianw			14 Apr 94
 *	- #ifdef'd out a line that calls the undefined routine sigflag().
 *	L003	scol!ianw			 4 Dec 95
 *	- Ensure the output stream is setup before calling p_str.
 *	- When the controlling terminal has no foreground process group
 *	  (tcgetpgrp returns RESPID), set the foreground process group ID
 *	  to the process group ID.
 *	- Removed L002, libc now contains the sigflag routine.
 */

#include	<sys/param.h>					/* L003 */
#include	<errno.h>
#include	"defs.h"
#include	"jobs.h"
#include	"history.h"
...
#   endif /* alliant */
	...
	if (tcgetpgrp(JOBTTY) == RESPID)			/* L003 */
		tcsetpgrp(JOBTTY, job.mypgid);			/* L003 */
...
#   endif /* apollo */
	...
	while(1)
	{
		...
		if (WIFSTOPPED(wstat))
		{
			...
			/* if(myjob && !(st.states&PROMPT))	L001 */
			/*	continue;			L001 */
...
static int job_alloc()
{
	...
	while(j < JBYTES && job.freejobs[j] == 0xff)		/* L000 */
		j++;
	if(j >= JBYTES)
	{
		register struct process *pw;
