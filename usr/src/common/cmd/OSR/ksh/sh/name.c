#ident	"@(#)OSRcmds:ksh/sh/name.c	1.1"
#pragma comment(exestr, "@(#) name.c 25.3 93/01/20 ")
/*
 ...
 *	- remove quoting from the output of 'whence' and 'type'.  Quoting
 *	  does not work well on the output of 'type', and is not required
 *	  by POSIX2 or XPG4 on the output of 'whence'.
 *	- use strcoll() to sort the nodes in a tree before printing
 *	- make rehash() a global function.  is now called from b_hash() in
 *	  builtin.c
 *	L001	scol!markhe	18 Nov 92
 *	- a variable name is constructed from alphabetics in the portable
 *	  character set and the digits '0' to '9' - so use isp*() macros
 *	- when locale environment variables change, load the new locale
 *	  and/or message catalogue
 *	L002	scol!markhe	 3 Dec 92
 *	- bug fix:
 *	  if FOO is not a variable in the environment, and the following
 *	  is done:
 *			readonly FOO
 *			export FOO
 *			ksh
 *			env
 *	  the enviroment list from 'env' is incomplete.
 *	  The readonly + export does not give FOO a value (it has a null
 *	  ptr).  Invoking another shell marks FOO as imported.  Any
 *	  childern of this shell then have the value of FOO inserted in
 *	  the enviroment list - a null pointer which terminates the list!
 *	  Check for a null ptr before placing on the list.
 *		
 */
...
#   endif /* MULTIBYTE */
			...
			else if (n==LCALLNOD || n==LCMESSNOD)
			{
				setlocale(LC_ALL, argscan);
				catload();
			}
...
int fd;
{
	...
	if(seps==NULL)
		seps = (char*)e_sptbnl;
	/* Note:  IFS is not affected by the locale		L000 */
...
{
	register char *value;
	if(nam_istype(n,N_IMPORT) && n->value.namenv)		/* L002 */
