#ident	"@(#)OSRcmds:ksh/sh/stak.c	1.1"
#pragma comment(exestr, "@(#) stak.c 25.2 92/12/11 ")
/*
 ...
 *
 *	L000	scol!markhe	10 Nov 92
 *	- added new function stakkludge().  After spell correcting a
 *	  pathname/filename we need to replace the incorrect spelling on
 *	  the stack.  We know where the correction must start from but
 *	  none of the existing functions/macros allows us to set this,
 *	  (stakset() cannot easily be used as it changes the stack
 *	  bottom as well).
 *	  stakkludge() sets the stack top to the address given and
 *	  re-calculates the space left on the stack.  note: cannot be
 *	  a macro as needs to access `stakend' in the private stack area.
 */
...
/*								   L000 begin
 *	move the top of the stack to a lower postion, and re-calculate
 *	the space left - called from cd() in sh/builtin.c
 */
void
stakkludge(char *loc)
{
	_stak_cur.staktop = loc;
	_stak_cur.stakleft = _stak_cur.stakend - _stak_cur.staktop;
}								/* L000 end */
