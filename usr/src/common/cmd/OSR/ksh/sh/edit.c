#ident	"@(#)OSRcmds:ksh/sh/edit.c	1.1"
#pragma comment(exestr, "@(#) edit.c 25.3 93/01/20 ")
/*
 ...
 *	- only allow macro names to contain alphabetics from the portable
 *	  character set and the digits '0' to '9'.  Not defined by POSIX2,
 *	  but if the macro name depeneds on the current CTYPE then when
 *	  the CTYPE changes illegal macro names can exist.
...
register int i;
{
	...
	if(i=='_')
		...
		macro[2] = 0;
	if ((ispalnum(i)) &&(np=nam_search(macro,sh.alias_tree,N_NOSCOPE))&&(out=nam_strval(np)))						/* L000 */
	{
#ifdef MULTIBYTE
