#ident	"@(#)OSRcmds:ksh/sh/vi.c	1.1"
#pragma comment(exestr, "@(#) vi.c 25.3 93/01/20 ")
/*
 ...
 *
 *	L000	scol!markhe	14 Jan 93
 *	- made <control>W XPG4 compliant.  Delete characters back to the
 *	 first word boundary, where the boundary is defined by the first
 *	 none blank or punct character.
 */
...
#   define isalph(v)	ispalnum(virtual[v])			/* L000 */
#   define isblank(v)	isspace(virtual[v])
#   define ispnt(v)	ispunct(virtual[v])			/* L000 */
...
register int  cmd;
{
	...
	while( nwords-- && tcur_virt > first_virt )
	{
		...
		if (cmd == 'W') {				/* L000 begin */
			while ((isblank(tcur_virt) || ispnt(tcur_virt)) &&
				tcur_virt >= first_virt)
				--tcur_virt;
		} else						/* L000 end */
...
register int mode;
{
	...
	escape:
		...
		case cntl('W'):		/** delete back word **/
			tmp = cur_virt;				/* L000 begin */
			backword(1, 'W');
			delete(tmp - cur_virt + 1, BAD);	/* L000 end */
			break;
...
find_b:
        ...
        case '%':
	{
		...
		while( tcur_virt <= last_virt
			&& (char *)strchr(paren_chars,virtual[tcur_virt])==(char*)0)
				tcur_virt++;
		if(tcur_virt > last_virt )
			return(BAD);
		nextc = virtual[tcur_virt];
		count = strchr(paren_chars,nextc)-(int)paren_chars;
		if(count < 3)
		{
			incr = 1;
			bound = last_virt;
...
{
	register int v = virtual[c];
	return((v&~STRIP) || ispalnum(v));			/* L000 */
}
