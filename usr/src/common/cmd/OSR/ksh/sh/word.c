#ident	"@(#)OSRcmds:ksh/sh/word.c	1.1"
#pragma comment(exestr, "@(#) word.c 25.3 93/01/20 ")
/*
 ...
 *	- a word consists of characters of alphabetics from the portable
 *	  character set and the digits '0' to '9' - so use ispalpha() and
 *	  ispalnum() macros.
 */
...
sh_lex()
{
	...
	if(!ismeta(c))
	{
		do
		{
			...
			{
				if(staktell()==offset && chk_keywd && ispalpha(c))								/* L000 */
					alpha++;
				stakputc(c);
				if(c == ESCAPE)
					stakputc(io_readc());
