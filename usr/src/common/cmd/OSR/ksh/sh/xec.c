#ident	"@(#)OSRcmds:ksh/sh/xec.c	1.1"
#pragma comment(exestr, "@(#) xec.c 25.4 93/01/20 ")
/*
 ...
 *	L001	scol!markhe (from scol!blf)	4 Nov 92
 *	- A premature SIGALRM during fork()-retries could be missed,
 *	  as it might have blown up something other than the clearly
 *	  intended pause().  Fix by closing window:  Use sighold()
 *	  to stop delivery of the signal until we're ready, then
 *	  wait for that signal with sigpause() rather than pause().
 *	  (carried forward from previous version)
 *	- Don't do a job_wait() whilst retrying forks:  There's no
 *	  way to specify ``wait for at least one process, but allow
 *	  interrupts'' -- see job_wait() comment in "jobs.c".
 *	  (If this could be done the fix would be much better.)
 *	  (carried forward from previous version)
 *	L002	scol!markhe	4 Nov 92
 *	- removed compiler warnings of long/short mismatches
 *	L003	scol!markhe	25 Nov 92
 *	- use correct type for messages
 *	- function names begin with an alphabetic from the portable
 *	  charcater set
			...
			case TFORK:
			{
					...
					2,4,8,16, and 32 seconds and then quits */
				{
...
#endif	/* VFORK */
					{
						...
						if(sh.trapnote&SIGSET)
							sh_exit(SIGFAIL);
						sighold(SIGALRM);   /* L001 */
						alarm(forkcnt);
						sigpause(SIGALRM);  /* L001 */
						/* job_wait((pid_t)1); L001 */
						/* pause(); 	       L001 */
						alarm(0);
					}
				}
...
#define c	type
						...
						{
							p_setout(ERRIO);
							p_list(nargs,arglist=args,1);								/* L003 */
							refresh = 0;
						}
						sh_prompt(1);
...
#undef c
...
#endif /* POSIX */
			{
				...
				at = Hz*(ta.time-tb.time);	    /* L000 */
				at +=  ((Hz*((time_t)(ta.millitm-tb.millitm)))/1000);								    /* L000 */
#else
				at -= bt;
#endif	/* _sys_timeb_ */
				p_setout(ERRIO);
		...
		if(mainloop && com0)
		{
			...
			if(strlen(com[argn-1]) < TMPSIZ)
			{
				...
				sh.lastarg = (char *)strcpy(locbuf,com[argn-1]);
			}
			else
			{
