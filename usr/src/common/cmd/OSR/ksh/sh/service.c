#ident	"@(#)OSRcmds:ksh/sh/service.c	1.1"
#pragma comment(exestr, "@(#) service.c 25.3 93/01/20 ")
/*
 ...
 *	- added Xnet //machine_name/path support
 *	  (carried forward from previous version)
 *	L001	scol!markhe	4 Jan 92
...
extern	int	isremote;					/* L000 */
extern char cwdname[];						/* L000 */
...
int flag;
{
	...
	if(sh.pwd)
		 ...
		 */
		return((isremote>0) ? (char *) cwdname: sh.pwd);/* L000 end */
	while(1) 
	{
...
@*/
{
	...
	/*							   L000 begin
	 * use cwdname instead of sh.pwd because needs to compare "file"
	 * relative to the full pathname, including the machine name.
	 * Otherwise, if "file" is //machine_name and sh.pwd is /, the
	 * comparison will result in "machine_name".
	 */
	if(!*(pwd=cwdname))					/* L000 end */
		return(fp);
	while(*pwd==*fp)
	{
		if(*pwd++==0)
	...
	/*							   L000 begin
	 * if this is on a remote machine then `cwdname' is simply
	 * //machine/, the trailing slash not allowing the correct
	 * path to be calculated.  Therefore move `fp' back to
	 * point to the previous character (which should be a slash)
	 */
	if (isremote && *pwd == 0 && *(pwd-1) == '/')
		fp--;						/* L000 end */
	if(*pwd==0 && *fp == '/')
	{
		while(*++fp=='/');
