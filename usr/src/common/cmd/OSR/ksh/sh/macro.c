#ident	"@(#)OSRcmds:ksh/sh/macro.c	1.1"
#pragma comment(exestr, "@(#) macro.c 25.3 93/01/20 ")
/*
 ...
 *
 *	L000	scol!markhe	24 Nov 92
 *	- accept alphabetics in portable character set and digits '0' to '9'
 *	  only.  Required for POSIX2 and XPG4.
 *	L001	scol!markhe	23 Dec 92
 *	- the (current) POSIX 1003.2 Draft 11 does *not* specify that
 *	  ${a#b#c} causes removal of pattern 'b' followed by removal
 *	  of pattern 'c'.  The #ifdef POSIX code was allowing this (must
 *	  have been specified in an earlier draft, but I cannot find it
 *	  anywhere).  Changed code "back" so above parameter expansion
 *	  uses the pattern "b#c" in its matching.
 */
