#ident	"@(#)OSRcmds:ksh/include/defs.h	1.1"
#pragma comment(exestr, "@(#) defs.h 25.5 94/08/24 ")
/*
 ...
 *	- added extra parameter to pathcanon() prototype - see shlib/cannon.c
 *	  for reason
 *	L001	scol!markhe	11 Nov 92
 *	- added macro ENTRY() for initialising message strings (messages
 *	  are not initialised in internationalised version, as the defaults
 *	  are set in catload.c).
 *	- p_list() now takes an extra argument to indicate if numbering is
 *	  required
 *	- added prototypes for; p_qstr() (print quoted strings) and rehash()
 *	  (which has been changed from an alias to a built-in)
 *	L002	scol!anthonys	26 May 94
 *	- Added a new flag which is used to indicate the "shell" should
 *	  behave as required by POSIX.2
 *	L003	scol!anthonys	4 July 94
 *	- added new function permtostr, used by umask.
 *	
 */
...
typedef	const char *	MSG;					/* L001 begin */
#ifdef	INTL
#  define	ENTRY(var, string)	(var)
#else
#  define	 ENTRY(var, string)	(var) = (string)
#endif	/* INTL */						/* L001 end */
...
#endif /* apollo */
#define Nolog	28
#define Pshell	29	/* when set, this "shell" conforms to POSIX.2 */ /* L002 */
...
/* print interface routines */
    ...
    extern void p_list(int,char*[],int);			/* L001 */
...
#ifdef	POSIX							/* L001 begin */
    extern void p_qstr(char*,int);
#endif	/* POSIX */						/* L001 end */
#else
    extern void p_flush();
    extern void p_list();
    extern void p_nchr();
...
#ifdef PROTO
    extern void			rehash(struct namnod *);	/* L001 */
...
/* pathname handling routines */
    ...
    extern char		*pathcanon(char*, int);			/* L000 */
...
/* frequently referenced routines */
    ...
    extern char 	*permtostr(mode_t);			/* L003 */
    ...
    /* extern char 	*strchr(const char*,int); - Was redefined in UW */
    /* extern int		strlen(const char*); - Was redefined in UW */
    extern int		strmatch(const char*,const char*);
    /* extern char 	*strrchr(const char*,int); - Was redefined in UW */
    /* extern char 	*strcpy(char*,const char*); - Was redefined in UW */
    /* extern char 	*strncpy(char*,const char*,int); - Was redefined in UW */
