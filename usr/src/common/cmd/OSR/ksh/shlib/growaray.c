#ident	"@(#)OSRcmds:ksh/shlib/growaray.c	1.1"
#pragma comment(exestr, "@(#) growaray.c 25.4 93/07/07 ")
/*
 ...
 *	- "defs.h" also include "shtype.h" as well, so no longer need
 *	  local defintions of astchar() and round()
 *	L001	scol!ianm	14 Jun 93
 *	- fixed memory leak associated with last array element.
 */
...
#endif /* MULTIDIM */
	{
		...
		if(flag==A_DELETE)
		{
			...
			if(--ap->nelem == 0)
			{
				free((char*)ap);
				/*
				 * Must free the storage associated with the
				 * last array element.
				 */
				if (nv)				/* L001 vvv */
				{
