#ident	"@(#)OSRcmds:compress/huf-coding.c	1.1"
#pragma comment(exestr, "@(#) huf-coding.c 25.1 92/12/01 ")
/*
...
int decoded;    /* for use in lzh-decode.c */
	...
	if (bufsiz == 0) {
		...
		while ((buf = (uchar *) malloc (bufsiz)) == NULL) {
			...
			if (bufsiz < 4 * (unsigned) 1024) 
				FatalError (MSGSTR(COMPRESS_NOMEM,
						"No memory available"));
								/* L001 */
		}
	}
