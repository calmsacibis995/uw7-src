#ident	"@(#)OSRcmds:sh/xec.c	1.1"
#pragma comment(exestr, "@(#) xec.c 26.1 95/12/08 ")
/*
 ...
 * 
 * 	L001	13nov90		scol!hughd
 *		- move #include <sys/stat.h> up from cd(dir), this way
 *		  we don't get umask compiler error with XPG3 sys/stat.h
 *	L002	15mar92		scol!markhe
 *		- added support for symbolic modes to umask.  Needed for
 *		  POSIX.2 and XPG4.
 *	S003	16mar95		sco!vinces
 *		- added logical path processing to argument syntax.
 *		  renamed cd() to physical_cd(), and modified it to update the
 *		  original dir name with changes due to spell checking. Look
 *		  for replacement cd() function in pwd.c.
 *	L004	04dec95		scol!ianw
 *		- Restore the foreground process group ID when the controlling
 *		  terminal has no foreground process group (tcgetpgrp returns
 *		  RESPID).
 */
...
#include	"sym.h"
#include	"hash.h"
#include	<limits.h>					/* L002 */
#include	<ctype.h>					/* L002 */
#include	<sys/types.h>
#include	<sys/times.h>
#include	<sys/stat.h>		/* S000 L001 */
#include	<sys/param.h>					/* L004 */
#include	<libgen.h>
#include        "../include/sym_mode.h"				/* L002 */
		...
		case TCOM:
			{
				...
				short	comtype;
								/* S003 */
				unsigned int physical = (flags & logflg)?0:1;
				...
				{		
					...
					else if (comtype == BUILTIN)
					{
						...
						case SYSCD:	
								/* S003 begin */
							/* make cd have arguments affects argn value */
							/* we'll have to evaluate what this means to */
							/* the code that follows - hashing and all   */
							if (a1 && *a1 == '-') {
								if (*(a1+1) == 'P') {
									physical = 1;
									a1 = com[2];
							...
							if (flags & rshflg)	
								failed(com[0], restricted);	
							if ((a1 && *a1) || (a1 == 0 && (a1 = homenod.namval)))	
							{	
						...
						/* S000	SCO_BASE chdir -> cd */
						/* S003 - new arg to cd() */
						while ((f = (cd(curstak(), physical) < 0)) && cdpath);
...
i = (i * 10) + (long)toint(c);	
						...
						{ 	
							...
							if (isdigit(*a1)) {
								errno = 0;
								flag = (mode_t) strtol(a1, &endptr, 8);
								if (errno != 0 || *endptr != '\0')
									error(badumask);
							} else {
								flag = umask(0);
								if (strperm(a1, ~flag, &flag) == -1) {
									umask(flag);
									error(badumask);
								}
								flag = ~flag;
							...
							umask(flag = umask(0));	
							if (symbolic_mode) {
								a1 = (unsigned char *)permtostr(flag);
								prs_buff(a1);
								prc_buff(NL);
							} else {
								prc_buff('0');	
								for (j = 6; j >= 0; j -= 3)	
									prc_buff(todigit((flag >> j) & 07));	
								prc_buff(NL);	
							}
						}	
						break;
						...
						case SYSGETOPT: {
							...
							if(argn > 3) {
								com[2] = dolv[0];
								getoptval = getopt(argn-2, (char *const *)&com[2], (char *)com[1]);
							}
							else
								getoptval = getopt(dolc+1, (char *const *)dolv, (char *)com[1]);
							if(getoptval == -1) {
								itos(optind);
								assign(n, numbuf);
								n = lookup(varnam);
		...
		case TFORK:
			...
			if (parent)
			{
				...
				if ((treeflgs & (FAMP | FPOU)) == 0) {
					await(parent, 0);
					if (pgrp != -1 && tcgetpgrp(output) == RESPID)
						tcsetpgrp(output, pgrp);
				}				/* L004 end */
			...
			else	/* this is the forked branch (child) of execute */
			{
				...
				if (treeflgs & FPIN)
				{
					rename((char *)pf1[INPIPE], (char *)0);
					close(pf1[OTPIPE]);
				}
				if (treeflgs & FPOU)
				{
					close(pf2[INPIPE]);
					rename((char *)pf2[OTPIPE], (char *)1);
				}
				/*
				 * default std input for &
				 */
				if (treeflgs & FINT && ioset == 0)
					rename((char *)chkopen(devnull), (char *)0);
				/*
				 * io redirection
				 */
				initio(t->treio, 0);
		...
		case TSW:
			{
				...
				while (regp)
				{
					...
					while (rex)
					{
						...
						if (gmatch((char *)r, (char *)(s = macro(rex->argval))) || (char *)(trim(s), eq(r, s)))
						{
...
#define	EXECUTE	01		/* is it searchable */
	...
	/*
	 * S003 
	 * use temporary string "tmpdir" to keep track of modifications due to
	 * directory name spelling checks. Previous overloading of "dir" didn't
	 * properly update the directory name for the higher scope functions.
	 */
	char *tmpdir;

	if((rval=chdir(dir)) < 0) {
		if((tmpdir=spname(dir)) && (stat(tmpdir, &sb)>=0) && flags&prompt) {
			...
			if(c!='n' && c!='N'){
				prs("ok\n");
				if ( (rval = chdir(tmpdir)) >= 0)
				{
				   movstr(tmpdir,locstak());
				   movstr(tmpdir, dir);
				}

			    /* locstak() used assuming cd()
			     * only gets called in the execute() switch.
			     * tdystak() is called at end of execute()
			     * routine.
			     */
			}
			while (c != '\n' && c != EOF)
