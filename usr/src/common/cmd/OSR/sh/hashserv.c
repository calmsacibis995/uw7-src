#ident	"@(#)OSRcmds:sh/hashserv.c	1.1"
#pragma comment(exestr, "@(#) hashserv.c 25.1 94/08/24 ")
/*
...
/* #ident	"@(#)sh:hashserv.c	1.5.1.1" */
/*
 *	UNIX shell
 *
 *	Modification history
 *
 *	L000 30-Sep-91	scol!anthonys
 *	- Modified chk_access() to use SCO's eaccess() instead of stat'ing
 *	  the file.  This has the desired effect of enabling the correct
 *	  checking for supplementary groups.
 *	L001	scol!hughd	24aug94
 *	- removed spurious second arg from one of the calls to nextpath()
...
int mode, regflag;
{	
	int ftype;					/* vvv L000 begin vvv */
	struct stat statb;
	if (stat((char *)name, &statb) == 0) {		/* L001 */
		ftype = statb.st_mode & S_IFMT;
		if(mode == S_IEXEC && regflag && ftype != S_IFREG)
			return(2);
		if(eaccess(name, mode >> 6) == 0) {
			return(0);
		}
	}						/* ^^^ L000 end   ^^^ */
