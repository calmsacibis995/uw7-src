#ident	"@(#)sccs:cmd/delta.c	6.13.4.1"
...
char *file;
{
	...
	if ((ser = sidtoser(&pp->pf_gsid,&gpkt)) == 0 ||
		sidtoser(&pp->pf_nsid,&gpkt))
			fatal(":57:invalid sid in p-file (de3)");
		...
		if (status) {		/* diff failed */
			...
			*/
			if (((status >> 8) & 0377) == 32) { /* 'execl' failed */
				fatal(":58:cannot execute '%s' (de12)",Diffpgm);
			...
			*/
			if (difflim -= 500) {	/* reduce segmentation */
				pfmt(stderr, MM_ERROR,
					":59:'%s' failed, re-trying, segmentation = %d (de13)\n",
						Diffpgm,difflim);
				fclose(Xiop);	/* set up */
...
*/
	if(Sflags[CMFFLAG - 'a']) {
		if (Mrs) {
			 error(":64:input CMR's ignored");
			 Mrs = "";
		}
		if(!deltack(pkt->p_file,Cmrs,Nsid,Sflags[CMFFLAG - 'a'])) {
			 fatal(":65:Delta denied due to CMR difficulties");
...
static char ambig[] = ":66:ambiguous `r' keyletter value (de15)";
...
struct sid *sp;
{
			...
			if (sp->s_rel == 0) {
				if (++cnt) {
					...
					if (fclose(out) == EOF)
						xmsg(outname, "rdpfile");
					fclose(in);
					fatal(":67:missing -r argument (de1)");
	...
	fclose(in);
	if (!goodpf.pf_user[0])
		fatal(":68:login name or SID specified not in p-file (de2)");
...
int difflim;
{
	...
	if ((i = fork()) < 0) {
		close(pfd[0]);
		close(pfd[1]);
		fatal(":69:cannot fork, try again (de11)");
	}
	else if (i == 0) {
		close(pfd[0]);
		close(1);
...
struct sid *sidp;
{
	...
	if (pkt->p_cutoff > pkt->p_idel[n].i_datetime)
		...
		case SX_EMPTY:
			...
			break;
		case APPLY:
			fatal(":71:internal error in delta/enter() (de5)");

			break;
		case NOAPPLY:
			fatal(":72:internal error in delta/enter() (de6)");

			break;
		default:
			fatal(":73:internal error in delta/enter() (de7)");
...
static char bd[] = ":74:leading SOH char in line %d of file `%s' not allowed (de14)";
