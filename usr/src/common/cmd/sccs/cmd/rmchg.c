...
char *argv[];
{
	...
	(void)setcat("uxepu");
	sprintf(label,"%s%s","UX:",(char*)basename(argv[0]));
	(void)setlabel(label);
	...
	*/
	...
	if(HADM && HADZ)
	{
		fatal(":125:Cassi not compatible with cmrs on command line");
...
char *file;
{
	...
	if (!exists(file)) {
		fatal(":128:file %s does not exist (rc2)",file);
	...
	*/
	...
	if (user != fowner && user != downer)
		if (!equal(Pgmr,logname())) {
			fatal(":130:you are neither owner nor '%s' (rc4)",Pgmr);
	...
	if (D_type == 'R') {
		...
		for (n = maxser(&gpkt); n > D_serial; n--) {
			...
			if (ptr->i_pred == D_serial)
				fatal(":131:not a 'leaf' delta (rc5)");
		}
		...
		*/
		if ((Mrs != NULL) && (*Mrs != '\0')) {
			if (!(p = Sflags[VALFLAG - 'a']))
				fatal(":132:MRs not allowed (rc6)");
			if (*p && valmrs(&gpkt,p))
				fatal(":133:invalid MRs (rc7)");
		}
	...
	*/
	if((HADZ && !Sflags[CMFFLAG - 'a']) ||
		...
		{
			fatal(":134:invalid use of 'z' flag \n CASSI incompatibility\n");
		...
		if(HADZ)
			...
			/*now process the testcmr table and send messages*/
				...
				{
					fatal(":135:uneditable cmr for CASSI controlled file- Change not allowed");
	...
	if (numdelts < 2 && found == YES)
		...
		fatal(":136:may not remove the last delta (rc13)");
...
struct packet *pkt;
{
	...
	if((HADZ) && (pkt->p_line[1] == MRNUM) && (D_type == 'D') && (!doneesc))
		{
		 ...
		 if(!esccmfdelt(pkt))
			{
			 fatal(":137:bad cdc replace of cmr's (CASSI)");
	...
	if (pkt->p_line[1] == MRNUM) {		/* line read is MR line */
		...
		while (*p)
			p++;
		if (*(p - 2) == DELIVER)
			fatal(":138:delta specified has delivered MR (rc9)");
		 ...
		 if(!holdptr[1])
			{
			 ...
			 pfmt(stdout, MM_NOSTD,
				":139:only one cmr left for this delta no change possible \n");
...
struct packet *pkt;
{
	...
	if(!(fred=(char *)gf(Sflags[CMFFLAG - 'a'])))
		fatal(":142:No fred file found- see sccs administrator");
...
msg(app,name,cmrs,stats,sids,fred)
	...
	*ptr=(char)NULL;
	...
	if(!(fd=fopen(holdfred,"a")))
		{
			fatal(":144:CASSI msg not writable \n");
			return(0);
		}
	...
	fclose(fd);
	 if(noexist) /*new termLOG make owner of /BD/source owner of file */
	 {
		if(stat(dir,&stbuf) == -1)
		{
			fatal(":145:Cassi BD/source not writable\n");
		}
		chmod(holdfred,0666);
	...
	while (fgets(rline,sizeof(line),in) != NULL) {
		...
		if (sp->s_rel == pf.pf_gsid.s_rel &&
			...
			sp->s_seq == pf.pf_gsid.s_seq) {
				fclose(in);
				fatal(":146:being edited -- sid is in p-file (rc12)");
