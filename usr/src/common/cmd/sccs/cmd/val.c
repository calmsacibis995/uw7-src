...
register char	*file;
{
	...
	if (*inp_line)
		printf("%s\n\n",inp_line);
	if (code & NAME_ERR){
		pfmt(stderr,MM_ERROR,":156:%s: %cM%c, -m mismatch",file,percent,percent);
		fprintf(stderr,"\n");
	}
	if (code & TYPE_ERR){
		pfmt(stderr,MM_ERROR,":157:%s: %cY%c, -y mismatch",file,percent,percent);
		fprintf(stderr,"\n");
	}
	if (code & NONEXSID_ERR){
		pfmt(stderr,MM_ERROR,":158:%s: SID nonexistent",file);
		fprintf(stderr,"\n");
	}
	if (code & INVALSID_ERR){
		pfmt(stderr,MM_ERROR,":159:%s: SID invalid or ambiguous",file);
		fprintf(stderr,"\n");
	}
	if (code & FILENAM_ERR){
		pfmt(stderr,MM_ERROR,":160:%s: can't open file or file not SCCS",file);
		fprintf(stderr,"\n");
	}
	if (code & CORRUPT_ERR){
		pfmt(stderr,MM_ERROR,":161:%s: corrupted SCCS file",file);
		fprintf(stderr,"\n");
	}
	if (code & UNKDUP_ERR){
		pfmt(stderr,MM_ERROR,":162:%s: Unknown or duplicate keyletter argument",file);
		fprintf(stderr,"\n");
	}
	if (code & FILARG_ERR){
		pfmt(stderr,MM_ERROR,":163:%s: missing file argument",file);
		fprintf(stderr,"\n");
	}
	return;
}
