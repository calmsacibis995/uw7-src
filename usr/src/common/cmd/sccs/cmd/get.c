...
register char *argv[];
{
	...
	for(i=1; i<argc; i++)
		if(argv[i][0] == '-' && (c=argv[i][1])) {
			...
			if (testmore) {
				testmore = 0;
				if (*p) {
					fatal(":51:value after %c arg (cm8)",c);
	...
	if (HADE && HADM)
		fatal(":78:e not allowed with m (ge3)");
	if (HADE)
		HADK = 1;
	if (!HADS)
		verbosity = -1;
	...
	*/
	...
	if (!HADA)
		...
		if ((ser = Ser) > maxser(&gpkt))
			fatal(":79:serial number too large (ge19)");
	...
	if (!(HADP || HADG))
		if (exists(Gfile) && (S_IWRITE & Statbuf.st_mode)) {
			Fflags = FTLEXIT | FTLMSG | FTLCLN;
			fatal(":80:writable `%s' exists (ge4)",Gfile);
		}
	if (gpkt.p_verbose) {
	...
	if (!HADG || HADL) {
		...
		*/
		if (stat(file, &tstatbuf) < 0) {
			fatal(":81:could not stat `%s' (ge16)", file);
		}
...
int branch;
{
	...
	*/
	...
	if (sidtoser(&pkt->p_reqsid,pkt) || in_pfile(&pkt->p_reqsid,pkt))
		fatal(":83:bad SID calculated in newsid()");
}
...
struct sid *sidp;
{
	...
	case APPLY:
		sid_ba(sidp,str);
		fatal(":84:%s already included (ge9)",str);
		break;
	case NOAPPLY:
		sid_ba(sidp,str);
		fatal(":85:%s already excluded (ge10)",str);
		break;
	default:
		fatal(":86:internal error in get/enter() (ge11)");
		break;
	}
}
		...
		if(lp[0] == '%' && lp[1] != 0 && lp[2] == '%') {
			...
			case 'P':
				...
				if(getcwd(Olddir,sizeof(Olddir)) == NULL)
					fatal(":87:getcwd() failed (ge20)");

				if(chdir(Dir) != 0)
					fatal(":88:cannot change directory (ge21)");
...
static	char	warn[] = ":89:being edited: `%s' (ge18)\n";
			...
			if (!(Sflags[JOINTFLAG - 'a'])) {
				if ((pf.pf_gsid.s_rel == pkt->p_gotsid.s_rel &&
				   ...
				   pf.pf_nsid.s_seq == pkt->p_reqsid.s_seq)) {
					fclose(in);
					fatal(":90:being edited: `%s' (ge17)",line);

				}
				if (!equal(pf.pf_user,user))
					pfmt(stderr,MM_WARNING,warn,line);

			}
			else pfmt(stderr,MM_WARNING,warn,line);
		}
...
cmrinsert ()
{
	...
	if ( HADP && ( ! HADZ))	/* no CMFFLAG and no place to prompt. */
		fatal(":93:Background CASSI get with no CMRs\n");

retry:
	if (cmr[0] == NULL) {					/* No CMR list.  Make one. */
		if(HADZ && ((!isatty(0)) || (!isatty(1))))
		{
			fatal(":94:Background CASSI get with invalid CMR\n");

		}
		pfmt (stdout, MM_NOSTD,
			":95:Input Comma Separated List of CMRs: ");
	...
	if(!bad && holdcmr[1]) {
		...
		if((isatty(0)) && (isatty(1)))
			if(!valid)
				pfmt(stdout,MM_NOSTD,
					":97:Must enter at least one valid CMR.\n");

			else
				pfmt(stdout,MM_NOSTD,
					":98:Re-enter invalid CMRs, or press return.\n");
