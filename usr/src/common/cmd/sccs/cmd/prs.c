...
char *argv[];
{
	...
	*/
		...
		if (argv[j][0] == '-' && (c = argv[j][1])) {
			...
			case 'c':	/* cutoff date[time] */
				if((*p) && (date_ab(p,&Date_time) != -1))
					...
					fatal(":104:invalid cutoff date (prs4)");
	...
	if(HADR && HADC)
		fatal(":105:can't specify cutoff date and SID (prs5)");
	if(HADC && (!HADL) && (!HADE))
		fatal(":106:must specify -e or -l with -c (prs6)");
		...
		if(lp[0] == ':' && lp[1] != 0 && lp[2] == ':') {
			...
			case 256*'N'+'P':	/* :PN: Full pathname of File */
				...
				if(getcwd(Olddir,sizeof(Olddir)) == NULL)
					fatal(":107:getcwd() failed (prs2)");

				if(chdir(Dir) != 0)
					fatal(":108:cannot change directory (prs3)");
...
register struct packet *pkt;
{
	...
	*/
	if (odt.d_sid.s_rel == 0)
		fatal(":109:nonexistent SID (prs1)");
	else
	...
	*/
	...
	if (i == 0) {
		...
		*/
		...
		fatal(":39:cannot execute '%s'",Getpgm);
	}
	else {
		wait(&status);
		return;
...
printflags()
{
	...
	if (Sflags[BRCHFLAG - 'a'])	/* check for 'branch' flag */
		pfmt(stdout,MM_NOSTD,":111:branch\n");
	if ((k = (Sflags[CEILFLAG - 'a'])))	/* check for 'ceiling flag */
		pfmt(stdout,MM_NOSTD,":112:ceiling\t%s\n",k);
	if ((k = (Sflags[DEFTFLAG - 'a'])))  /* check for 'default SID' flag */
		pfmt(stdout,MM_NOSTD,":113:default SID\t%s\n",k);
