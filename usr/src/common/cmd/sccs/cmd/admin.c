#ident	"@(#)sccs:cmd/admin.c	6.14.2.4"
...
static char	*uuname;
static char b_dir[]  =  ":1:directory `%s' specified as `%c' keyletter value (ad29)";
...
char *argv[];
{
	...
	*/
	for(j=1; j<argc; j++)
		if(argv[j][0] == '-' && (c = argv[j][1])) {
			...
			case 'd':	/* flags to be deleted */
				...
				case LOCKFLAG:
					if (*p) {
						...
						*/
						repl(p,',',' ');
						if (!val_list(p))
							fatal(":4:bad list format (ad27)");

						if (!range(p))
							fatal(":5:element in list out of range (ad28)");
				...
				default:
					fatal(":6:unknown flag (ad3)");
				}

				if (rm_flag[f - 'a']++)
					fatal(":7:flag twice (ad4)");
				break;

			case 'f':	/* flags to be added */
				testklt = 0;
				if (!(f = *p))
					fatal(":8:f has no argument (ad5)");
				...
				case CMFFLAG:			/* CMFNET MODS */
					if (*p)				/* CMFNET MODS */
						...
						fatal (":10:No application with application flag.");

					if (gf (CMFAPPL) == (char*) NULL)
						fatal (":11:No .FRED file exists for this application.");

					break;				/* END CMFNET MODS */

				case FLORFLAG:	/* floor */
					if ((i = patoi(p)) == -1)
						fatal(":12:floor not numeric (ad22)");

					if (((int) size(p) > 5) || (i < MINR) ||
							(i > MAXR))
						fatal(":13:floor out of range (ad23)");

					break;

				case CEILFLAG:	/* ceiling */
					if ((i = patoi(p)) == -1)
						fatal(":14:ceiling not numeric (ad24)");

					if (((int) size(p) > 5) || (i < MINR) ||
							(i > MAXR))
						fatal(":15:ceiling out of range (ad25)");

					break;

				case DEFTFLAG:	/* default sid */
					if (!(*p))
						fatal(":16:no default sid (ad14)");
			...
			case 'a':	/* user-name allowed to make deltas */
				testklt = 0;
				if (!(*p))
					fatal(":19:bad a argument (ad8)");

				if (asub > MAXNAMES)
					fatal(":20:too many 'a' keyletters (ad9)");
			...
			case 'e':	/* user-name to be removed */
				testklt = 0;
				if (!(*p))
					fatal(":21:bad e argument (ad10)");

				if (esub > MAXNAMES)
					fatal(":22:too many 'e' keyletters (ad11)");

				enames[esub++] = p;
				break;

			default:
				fatal(":23:unknown key letter (cm1)");
			}

			if (had[c - 'a']++ && testklt++)
				fatal(":24:key letter twice (cm2)");
			argv[j] = 0;
		}
		else
			num_files++;
	...
	if (num_files == 0)
		fatal(":25:missing file arg (cm3)");

	if ((HADY || HADM) && ! (HADI || HADN))
		fatal(":26:illegal use of 'y' or 'm' keyletter (ad30)");

	if (HADI && num_files > 1) /* only one file allowed with `i' */
		fatal(":27:more than one file (ad15)");
	if ((HADI || HADN) && ! logname())
		fatal(":28:USER ID not in password file (cm9)");
...
char	*afile;
{
	...
	if (HADI && had_dir) /* directory not allowed with `i' keyletter */
		fatal(":29:directory named with `i' keyletter (ad26)");
	...
	if (HADI || HADN) {
			...
			fatal(":30:Can't have two verification routines.");


		if (HADM && !VFLAG && !had_flag[CMFFLAG - 'a'])
			fatal(":31:MRs not allowed (de8)");

		if (VFLAG && !HADM)
			fatal(":32:MRs required (de10)");

	}

	if (!HADI && HADR)
		fatal(":33:r only allowed with i (ad16)");

	if (HADN && HADT && !(*tfile))
		fatal(":34:t has no argument (ad17)");

	if (HADN && HADD)
		fatal(":35:d not allowed with n (ad18)");

	if (HADN && fexists) {
		fatal(":36:file %s exists (ad19)",afile);
	}

	if (!HADN && !fexists) {
		fatal(":37:file %s does not exist (ad20)",afile);
	...
	*/
	...
	if (!HADH && lockit(copy(auxf(afile,'z'),Zhold),(unsigned)2,getpid(),uuname))
		fatal(":41:cannot create lock file (cm4)");
		...
		if (Mrs) {
			if (had_flag[CMFFLAG - 'a']) {	/* CMF check routine */
				if (cmrcheck (Mrs, CMFAPPL) != 0) {	/* check them */
					fatal (":42:Bad CMR number(s).");

					}
				}
			mrfixup();
			if (z && valmrs(&gpkt,z))
				fatal(":43:invalid MRs (de9)");
			putmrs(&gpkt);
		}
			...
			*/
			if (!Did_id) {
				if (had_flag[IDFLAG - 'a'])
					if(!(flag_p[IDFLAG -'a']))
						fatal(":45:no id keywords (cm6)");

					else
						fatal(":46:invalid id keywords (cm10)");

				else
					pfmt(stderr, MM_WARNING,
						":47:No id keywords (cm7)\n");
			}
...
char *lastchp;
{
	...
	for (k = 1; fgets(strp,len,inptr); k++)
	{
		...
		if (*strp == CTLCHAR)
		{
			...
			{
				fatal(":48:%s illegal data on line %d (ad21)",
	...
	if (error_line != 0) 
	{
		fatal(":49:%s line %d too long (ad32)",
			file, error_line);
	}
	return(k - 1);
}
...
char *str;
{
	...
	*p++ = COMMENTS;
	*p++ = ' ';
	copy(gettxt("uxepu:253","date and time created"),p);
	while (*p++)
		;
	--p;
	*p++ = ' ';

	lcltm = localtime(&dt->d_datetime);
	strftime( p, BUFSIZ, gettxt("uxepu:99", "%y/%m/%d %T"), lcltm);

	while (*p++)
		;
	--p;
	*p++ = ' ';
	copy(gettxt("uxepu:254","by"),p);
	while (*p++)
		;
	--p;
	*p++ = ' ';
