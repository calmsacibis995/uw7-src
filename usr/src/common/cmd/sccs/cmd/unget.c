#ident	"@(#)sccs:cmd/unget.c	6.9.1.1"
...
struct sid *sp;
{
		...
		if (equal(pf.pf_user,user)) {
			...
			if (sp->s_rel == 0) {
				if (++cnt) {
					fclose(out);
					fclose(in);
					fatal(":151:SID must be specified (un1)");
	...
	if (!goodpf.pf_user[0])
		if (!name)
			fatal(":152:login name not in p-file (un2)");
		else fatal(":153:specified SID not in p-file (un3)");
	return(&goodpf);
}
...
struct packet *pkt;
{
	...
	if(!(in = fopen(auxf(pkt->p_file,'p'),"r")))
		pfmt(stderr,MM_ERROR,
			":154:No outstanding deltas for: %s\n",pkt->p_file);
