...
register char *argv[];
{
	...
	if (HADP && HADC)
		fatal(":52:can't have both -p and -c (cb2)");
	setsig();
	Fflags &= ~FTLEXIT;
	Fflags |= FTLJMP;
	iop = stdout;
...
char *file;
{
	...
	if (HADP) {
		...
		if (!(n = sidtoser(&sid, &gpkt)))
			fatal(":54:sid doesn't exist (cb3)");
		while (n <= maxser(&gpkt)) {
	...
	if (!(Val_ptr = Sflags[VALFLAG - 'a']))
		Val_ptr = Blank;
	if(Sflags[ENCODEFLAG - 'a'] && (atoi(Sflags[ENCODEFLAG - 'a']) == 1)) /* if file is encoded force -b option */
	...
	fprintf(iop, "$c\n");
	pfmt(iop, MM_NOSTD,
		":56:*** DELTA TABLE PRIOR TO COMBINE ***\n");
	fprintf(iop, ".\n");
	fprintf(iop, "w\n");
	fprintf(iop, "q\n");
	fprintf(iop, "!\n");
