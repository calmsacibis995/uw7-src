#ident	"@(#)sccs:lib/comobj/dodelt.c	6.6.1.2"
		...
		if (pkt->p_idel == 0) {
			if (Timenow < dt.d_datetime)
				pfmt(stderr,MM_ERROR,
					":202:Time stamp later than current clock time (co10)\n");
			...
			if (pkt->p_line[0] != CTLCHAR)
				...
				case IGNORE:
					if (dt.d_type == 'D') {
                                                if(!rdp->i_ixg)
                                                {
rdp->i_ixg = (struct ixg *)fmalloc(sizeof(struct ixg));
						    zero(rdp->i_ixg,sizeof(struct ixg));
                                                }
