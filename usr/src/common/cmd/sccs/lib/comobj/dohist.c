#ident	"@(#)sccs:lib/comobj/dohist.c	6.9.1.1"
...
char *file;
{
	...
	if (!Comments) {
		if (doprmt)
			pfmt(stdout, MM_NOSTD,":204:comments? ");
		sprintf(line,"\n");
...
char *result;
{
	...
	*/
	...
	if ((had_standinp && (!HADY || (Domrs && !HADM)))) {
		Ffile = 0;
		fatal(":205:standard input specified w/o -y and/or -m keyletter (de16)");
	...
	*/
	...
	while (!done && fgets(line,sz,stdin) != NULL) {
		...
		if (*--p == '\n') {
			...
			fatal(":206:line too long (co18)");
		if ((int) (size(line) + size(result)) > RESPSIZE)
			fatal(":207:response too long (co19)");
		strcat(result,line);
	}
	return(result);
}
...
char *pgm;
{
	...
	if (p = Sflags[TYPEFLAG - 'a'])
		...
		Varg[2] = Null;
	if ((i = fork()) < 0) {
		fatal(":208:cannot fork; try again (co20)");
	}
	else if (i == 0) {
		for (i = 4; i < 15; i++)
			(void) close(i);
	...
	for (ap = p; *p; p++) {
		if (*p == ' ' || *p == '\t') {
			if (argv >= &Varg[(NVARGS - 1)])
				fatal(":209:too many MRs (co21)");
			c = *p;
			*p = 0;
			if ((len = size(ap)) > LENMR)
				fatal(":210:MR number too long (co24)");
