...
static const char useexcl[] = ":891:c, e, f, h, n, C, and D options are mutually exclusive\n";

static char default_timefmt[] = "%a %b %e %T %Y\n";
...
char **argv;
{
	...
	while((c = getopt(argc, argv, "D:bC:cefhilnrS:stw")) != EOF)	{
		switch(c)	{
		case 'D':
			if (opt || hflag)	{
				pfmt(stderr, MM_ERROR, useexcl);
				usage_done();
			}
			opt = D_IFDEF;
			wantelses = 1;
		...
		case 'C':
			...
			context = (int)strtol(argp, &termp, 10);
			argp = termp;
			if ((*argp != '\0') || (context < 0)) {
				pfmt(stderr, MM_ERROR, useC);
				usage_done();
	...
	else if ((stb1.st_mode & S_IFMT) == S_IFIFO)	{
		pfmt(stderr, MM_WARNING,
			":892:Ignoring named pipe: %s\n", file1);
		status = 0;
		done();
	}
	if (!strcmp(file2, "-")) {
	...
	else if ((stb2.st_mode & S_IFMT) == S_IFIFO)	{
		pfmt(stderr, MM_WARNING,
			":892:Ignoring named pipe: %s\n", file2);
		status = 0;
		done();
...
change(a,b,c,d)
{
	...
	if (anychange == 0) {
		anychange = 1;
		if(opt == D_CONTEXT) {
			fmtp = gettxt(":819", default_timefmt);
			(void)cftime(cbuf, fmtp, &stb1.st_mtime);
			printf("*** %s	%s", file1, cbuf);
			(void)cftime(cbuf, fmtp, &stb2.st_mtime);
			printf("--- %s	%s", file2, cbuf);

			context_vec_start = (struct context_vec *) 
						malloc((MAX_CONTEXT + 1) *
...
void usage_done()
{
	pfmt(stderr, MM_ACTION, ":890:Usage:\n    diff [-bilrstw -S name] [-c|-e|-f|-h|-n|-C number|-D string] file1 file2\n");
	done();
}
