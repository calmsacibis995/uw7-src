...
char *prm;
{
	 ...
	 * The path is taken to be on the local system if the specified
	 * host name matches Myname to a maximum of MAXBASENAME characters,
	 * since Myname is truncated to that size.  For example, if the local
	 * system's name is "thisisaverylongnodename" then Myname will be
	 * "thisisaverylon" and a path like "thisisaverylongish!path" is
	 * treated as local.
	 */
	if (((n = strlen(Myname)) == p - buf || n == MAXBASENAME)
	    && strncmp(Myname, buf, n) == SAME)
	    return(chkFile(p+1));  /* yes, so recurse with thing after '!' */
	else
	    return(0);		   /* not my system - not my worry */
}
