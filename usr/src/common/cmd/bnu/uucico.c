...
char **envp;
{
	...
	*xflag = NULLCHAR;
	...
	call.telno = NULL;	/* no telno */
	call.caller_telno = NULL;	/* no caller telno */
	call.sysname = NULL;	/* no sysname */
	call.function = F_UUCICO; /* want chat and file transfer */
	call.class = NULL;
	call.protocol = NULL;
	call.pinfo_len = 0;
	call.pinfo = NULL;
	...
	if (Role == SLAVE) {
		...
		(void) sprintf(lockname, "%.*s", SYSNSIZE + 2, Rmtname);
			/* restrict the lockname to 9 chars, so that
			   it does not exceed 14 chars */
 ...
 */
	...
	if (Role == MASTER) {
		...
		if (*JobGrade != NULLCHAR) {
			...
			CDEBUG(1, "No jobs queued for %s\n", Rmtname);
			cleanup(102);
		}

		Nstat.t_scall = times(&Nstat.t_tga);
		Ofn = Ifn = dials(&call, &ret_call, &dstatus); 
		Nstat.t_ecall = times(&Nstat.t_tga);
		if (Ofn < 0) {
