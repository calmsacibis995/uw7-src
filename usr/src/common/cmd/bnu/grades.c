...
struct cs_struct *scfile;
{
	...
	(void) pfmt(stderr,MM_WARNING|MM_NOGET,"%s (%ld bytes) %s (%ld bytes) %s (%s).\n%s\n",
		gettxt(grd_errid[RESTP1], grd_err[RESTP1]), scfile->jsize,
		gettxt(grd_errid[RESTP2], grd_err[RESTP2]), climit,
		gettxt(grd_errid[RESTP3], grd_err[RESTP3]), scfile->sgrade,
		gettxt(grd_errid[RESTP4], grd_err[RESTP4]));
...
char *grade;
{
	...
	while (rdfulline(cfd, line, BUFSIZ) != 0) {
		...
		if (upermit(carray, na) != FAIL) {
			...
			(void) fclose(cfd);
			(void) pfmt(stderr, MM_ERROR|MM_NOGET,"%s: %s <%s>.\n %s\n%s\n",
				Progname,
				gettxt(grd_errid[PERMP1], grd_err[PERMP1]),
				grade,
				gettxt(grd_errid[PERMP2], grd_err[PERMP2]),
				gettxt(grd_errid[PERMP3], grd_err[PERMP3]));
			return(5);
	...
	(void) pfmt(stderr,MM_ERROR|MM_NOGET,"%s: %s <%s> %s\n%s\n", Progname,
		gettxt(grd_errid[NEXTP1], grd_err[NEXTP1]), grade,
		gettxt(grd_errid[NEXTP2], grd_err[NEXTP2]),
		gettxt(grd_errid[NEXTP3], grd_err[NEXTP3]));
	return(6);
}
