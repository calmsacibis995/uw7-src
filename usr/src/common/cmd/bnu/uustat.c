...
char **envp;
{
	...
	while ((i = getopt(argc, argv, "acjk:mnpr:qs:u:x:t:d:S:")) != EOF) {
		...
		case 's':
			...
			if (versys(Rmtname)) {
				pfmt(stderr, MM_ERROR, ":38:Invalid system\n");
				exit(1);
			}
			Sysopt = 1;
			break;
		...
		case 'u':
			...
			if(gninfo(User, &chkid, chkname)) {
				pfmt(stderr, MM_ERROR, ":39:Invalid user\n");
				exit(1);
			}
			Uopt = 1;
			execute = 1;
	...
	while (gnamef(spooldir, f) == TRUE) {
	 ...
	 /* f will contain remote machine names */
		...
		/* The X files are in /var/spool/uucp/system directory
		   check for them */
		if (gradef[1] == '.') {
                   if (gradef[0] == XQTPRE) {
                        m->xcount++;
                        if ( (i = _age(".",gradef)) > m->x_age)
                                m->x_age = i;
                   }
                }
	    }
	...
	if (Jobcount && (jobcount != 0))
		pfmt(stdout, MM_NOSTD, ":40:job count = %d\n",jobcount);

	/* for Kill or Rejuvenate - will not get here unless it failed */
	if (Kill || Rejuvenate) {
	    pfmt(stderr, MM_ERROR, ":41:Cannot find Job %s; Not %s\n", Jobid,
		Kill ? gettxt(":64", "killed") : gettxt(":65", "rejuvenated"));
	    exit(1);
	}
	...
	if (s.st_size == 0 && User[0] == '\0') { /* dummy D. for polling */
		...
		{ 
		   statefound = state(dir, file);		
			...
			{
			   if (statefound == 1)
				(void) strcat(outbuf, gettxt(":61", "queued"));
			   else if (statefound == 2)
				(void) strcat(outbuf, gettxt(":62", "running"));
			   else if (statefound == 3)
				(void) strcat(outbuf, gettxt(":63", "interrupted"));
			}
		}
...
char *file, *dir;
{
	...
	if (stat(fullname, &s) != 0) {
	     /* error - can't stat */
	    pfmt(stderr, MM_ERROR, ":42:Cannot stat:%s, errno (%d)--cannot %s it!\n",
		fullname, errno, Kill ? gettxt(":79", "kill") : gettxt(":80", "rejuvenate"));
	    exit(1);
	}

	fp = fopen(fullname, "r");
	if (fp == NULL) {
	    pfmt(stderr, MM_ERROR, ":43:Cannot read:%s, errno (%d)--cannot %s it!\n",
		fullname, errno, Kill ? gettxt(":79", "kill") : gettxt(":80", "rejuvenate"));
	    exit(1);
	}
	...
	while (fgets(buf, BUFSIZ, fp) != NULL) {
	    if (sscanf(buf,"%s%s%s%s%s%s", type, file1, file2,
	      user, opt, file3) <6) {
		pfmt(stderr, MM_ERROR, ":44:Bad format:%s, errno (%d)--cannot %s it!\n",
		    fullname, errno, Kill ? gettxt(":79", "kill") : gettxt(":80", "rejuvenate"));
	        exit(1);
	    }
	    ...
	    if (first) {
			...
			pfmt(stderr, MM_ERROR,
			    ":45:Not owner, uucp or root - cannot %s job %s\n",
    			    Kill ? gettxt(":79", "kill") : gettxt(":80", "rejuvenate"), Jobid);
		    exit(1);
		}
		first = 0;
	    }
		...
		/* program error?? */
		pfmt(stderr, MM_ERROR, ":46:Cannot %s, File ( %s), errno (%d)\n",
		    Kill ? gettxt(":79", "kill") : gettxt(":80", "rejuvenate"), rfullname, errno);
		exit(1);
	    }
	}
	...
	if (ret != 0) {
	    /* program error?? */
		pfmt(stderr, MM_ERROR, ":46:Cannot %s, File ( %s), errno (%d)\n",
		    Kill ? gettxt(":79", "kill") : gettxt(":80", "rejuvenate"), fullname, errno);
	    exit(1);
	}
	...
	fclose(fp);
	if (!nonotf) {
		pfmt(stdout, MM_NOSTD, ":48:Job: %s successfully %s\n", Jobid,
		    Kill ? gettxt(":64", "killed") : gettxt(":65", "rejuvenated"));
		}
	exit(0);
}
...
char	*name;
{
	...
	if (m-M >= UUSTAT_TBL) {
		...
		pfmt(stderr, MM_WARNING,
		    ":49: Table Overflow--output not complete\n");
	    }
	    else
...
/*	if (m->locked && m->type != SS_INPROGRESS) */
	if (m->locked)
		pfmt(stdout, MM_NOSTD, ":50:Locked ");
	if (m->stst[0] != '\0') {
		printf("%s ", m->stst);
		switch (m->type) {
		case SS_SEQBAD:
		...
		case SS_CHAT_FAILED:
			...
			if (t > 0) {
				minimum = (t + 59) / 60;
				pfmt(stdout, MM_NOSTD, ":66:Retry: %d:%2.2d", minimum/60, minimum%60);
			}
			if (m->count > 1)
				pfmt(stdout, MM_NOSTD, ":51: Count: %d", m->count);
		}
	}
	putchar('\n');
	return;
...
lckpid()
{
    ...
    *strrchr(lckdir, '/') = '\0';
	...
	if (EQUALSN("LCK.", f, 4) || EQUALSN("LK.", f, 3)) {
	    ...
	    (void) close(fd);
	    if (ret != -1) {
		/* report only if pid is active; else skip it */
	        if ((ret1=kill(pid, 0)) == 0 || errno == EPERM) {
	           printf("%s: ", f);
		   printf("%ld\n", (long) pid);
		   for(i=0; i<MAXLOCKS; i++) {
		     if (list[i] == pid)
...
char *a,*b;
{
	return(strcoll(((struct m *) a)->mach,((struct m *) b)->mach));
}
...
if (fp == NULL)
   {
	pfmt(stderr, MM_ERROR, ":52:Cannot open account log\n");
		return;
   }
	...
	if (State && !Uopt)
	  {
	   if (EQUALS(Loginuser, loginame))
		{
			pfmt(stdout, MM_NOSTD, ":53:%s completed\n",jobid);
			jobcount++;
		}
	  }
	else
...
long timerange;
{
	...
	if (fp == NULL)
 	  {
		pfmt(stderr, MM_ERROR, ":54:Cannot open performance log\n");
			return(0);
	   }
...
queuetime()
{
	...
	":55:average queue time to [%s] for last [%ld] minutes:  %6.2f seconds\n",
	Rmtname, calcnum, avgqtime);
	pfmt(stdout, MM_NOSTD,
 	":56:data gathered from %s:%s to %s:%s GMT\n",
...
xfertime()
{
	...
	pfmt(stdout, MM_NOSTD,
	":57:average transfer rate with [ %s ] for last [%ld] minutes: %6.2f bytes/sec\n",
...
errortn()
{
	...
	pfmt(stderr, MM_ACTION, ":58:Usage: uustat [-q] or [-m] or [-kJOB [-n]] or [-rJOB [-n]] or [-p]\n");
	pfmt(stderr, MM_NOSTD, ":59:\t\tor\n\t\t   Usage: uustat [-a] [-sSYSTEM [-j]] [-uUSER] [-SSTATE]\n");
	pfmt(stderr, MM_NOSTD, ":60:\t\tor\n\t\t   Usage: uustat -tSYSTEM [-d number] [-c]\n");
	return;
}
