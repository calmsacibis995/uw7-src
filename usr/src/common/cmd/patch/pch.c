...
char *filename;
{
    ...
    if (pfp == Nullfp)
	fatal2(":44:patch file %s not found\n", filename);
...
{ 0,0 }, { 0,0 },
{ ":50", "  Looks like a context diff to me...\n" },
{ ":51", "  The next patch looks like a context diff to me...\n" },
{ ":52", "  Looks like a normal diff to me...\n" }, 
{ ":53", "  The next patch looks like a normal diff to me...\n" },
{ ":54", "  Looks like an ed script to me...\n" },
{ ":55", "  The next patch looks like an ed script to me...\n" },
{ ":56", "  Looks like a new-style context diff to me...\n" },
{ ":57", "  The next patch looks like a new-style context diff to me...\n" }
};
...
there_is_another_patch()
{
	...
	if (verbose)
	    pfmt(stderr, MM_NOSTD, ":46:done\n");
	return FALSE;
    }
    if (verbose)
	pfmt(stderr, MM_NOSTD, ":47:Hmm...");
    diff_type = intuit_diff_type();
    if (!diff_type) {
	if (p_base != 0L) {
	    if (verbose)
		pfmt(stderr, MM_NOSTD,
			":48:  Ignoring the trailing text.\ndone\n");
	}
	else
	    pfmt(stderr, MM_NOSTD,
		":49:  I can't seem to find a patch in there anywhere.\n");
	return FALSE;
    }
    if (verbose)
	fprintf(stderr, gettxt(messages[diff_type*2 + (p_base != 0L)].mesno, 
		   messages[diff_type*2 + (p_base != 0L)].dflt));
    if (p_indent && verbose)
	if (p_indent == 1)
	    pfmt(stderr, MM_NOSTD, ":58:(Patch is indented 1 space.)\n");
	else
	    pfmt(stderr, MM_NOSTD, ":59:(Patch is indented %d spaces.)\n",
		p_indent);
    skip_to(p_start,p_sline);
    while (filearg[0] == Nullch) {
	if (force) {
	    pfmt(stderr, MM_NOSTD, ":60:No file to patch.  Skipping...\n");
	    filearg[0] = savestr(bestguess);
	    return TRUE;
	}
	ask1(":61:File to patch: ");
	if (*buf != '\n') {
	    if (bestguess)
		free(bestguess);
	...
	if (filearg[0] == Nullch) {
	    ask2(":62:No file found--skip this patch? [%s] ",
		nl_langinfo(NOSTR));
	    if (strcmp(buf, nl_langinfo(NOSTR)) == 0) {	/* not yes */
		continue;
	    }
	    if (verbose)
		pfmt(stderr, MM_NOSTD, ":63:Skipping patch...\n");
...
long file_line;
{
	...
	Fseek(pfp, p_base, 0);
	pfmt(stderr, MM_NOSTD, ":64:The text leading up to this was:\n");
	pfmt(stderr, MM_NOSTD, ":99:--------------------------\n");
	while (ftell(pfp) < file_pos) {
	    ret = fgets(buf, sizeof buf, pfp);
	    assert(ret != Nullch);
	    pfmt(stderr, MM_NOSTD, ":100:|%s", buf);
	}
	pfmt(stderr, MM_NOSTD, ":99:--------------------------\n");
    }
    else
...
another_hunk()
{
	...
	while (p_end < p_max) {
		...
		if (p_max - p_end < 4)
		    ...
		    fatal1(":65:Unexpected end of file in patch.\n");
		}
	    }
	    p_end++;
		...
		if (strnEQ(buf, "********", 8)) {
			...
			fatal2(":66:Unexpected end of hunk at line %ld.\n",
			    p_input_line);
		}
		if (p_end != 0) {
		    if (repl_beginning && repl_could_be_missing) {
		    ...
		    fatal3(":67:Unexpected *** at line %ld: %s",
			p_input_line, buf);
		}
		context = 0;
		...
		if (buf[1] == '-') {
			...
			if (p_end == 1) {
				...
				fatal3(":68:Duplicate \"---\" at line %ld--check line numbers at line %ld.\n",
				    p_input_line, p_hunk_beg + repl_beginning);
			    }
			    else {
				if (p_end <= p_ptrn_lines)
					fatal3(":69:Premature \"---\" at line %ld--check line numbers at line %ld.\n",
					    p_input_line, p_hunk_beg);
			    	else
					fatal3(":70:Overdue \"---\" at line %ld--check line numbers at line %ld.\n",
					    p_input_line, p_hunk_beg);
			    }
			}
		    }
			...
			fatal4(":71:Hunk too large (%ld lines) at line %ld: %s",
	...
	if (p_end >=0 && !repl_beginning)
	    fatal2(":72:No --- found in patch at line %ld\n", pch_hunk_beg());
	...
	if (diff_type == CONTEXT_DIFF &&
	  (fillcnt || (p_first > 1 && ptrn_copiable > 2*p_context)) ) {
	    if (verbose) {
		pfmt(stderr, MM_NOSTD, 
":73:(Fascinating--this is really a new-style context diff but without\nthe telltale extra asterisks on the *** line that usually indicate\nthe new style...)\n");
	    }
	...
	if (fillcnt) {
		...
		while (fillsrc <= p_end && p_char[fillsrc] != ' ')
		    fillsrc++;
		if (fillsrc > p_end)
		    fatal2(":74:Replacement text or line numbers mangled in hunk at line %ld\n",
			p_hunk_beg);
	...
	for (i=1; i<=p_ptrn_lines; i++) {
	    ...
	    p_input_line++;
	    if (ret == Nullch)
		fatal2(":75:Unexpected end of file in patch at line %ld.\n",
		  p_input_line);
	    if (*buf != '<')
		fatal2(":76:< expected at line %ld of patch.\n", p_input_line);
	...
	if (hunk_type == 'c') {
		...
		fatal2(":75:Unexpected end of file in patch at line %ld.\n",
		    p_input_line);
	    if (*buf != '-')
		fatal2(":77:--- expected at line %ld of patch.\n",
			p_input_line);
	}
	Sprintf(buf, "--- %ld,%ld\n", min, max);
	...
	for (i++; i<=p_end; i++) {
		...
		fatal2(":75:Unexpected end of file in patch at line %ld.\n",
		    p_input_line);
	    if (*buf != '>')
		fatal2(":78:> expected at line %ld of patch.\n", p_input_line);
	    p_line[i] = savestr(buf+2);
	    if (out_of_mem) {
		p_end = i-1;
		return FALSE;
	...
	if (!pch_swap())
	    pfmt(stderr, MM_NOSTD,
		":79:Not enough memory to swap next hunk!\n");
#ifdef DEBUGGING
    if (debug & 2) {
	int i;
	char special;
	...
	for (i=0; i <= p_end; i++) {
		...
		special = ' ';
	    pfmt(stderr, MM_NOSTD, ":101:%3d %c %c %s", i, p_char[i], special, p_line[i]);
	    Fflush(stderr);
	}
    }
#endif
...
malformed:
    fatal3(":80:Malformed patch at line %ld: %s", p_input_line, buf);
...
do_ed_script()
{
    ...
    if (skip_rest_of_patch)
	return;
    pfmt(pipefp, MM_NOSTD, ":102:w\n");
    pfmt(pipefp, MM_NOSTD, ":103:q\n");
    Fflush(pipefp);
    Pclose(pipefp);
    ignore_signals();
    if (oopt != NULL)
