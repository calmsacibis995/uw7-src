...
char *pat;
{
	...
	if ((err = regcomp(rp, pat, REG_ANGLES | REG_ESCNL | REG_NOSUB)) != 0) {
		if (opt == 'h') {
...
for (j = 1; j < argc; j++) {
	i = 0;
	if (!last && argv[j][i] == '-' && (c = argv[j][i + 1])) {
		switch(c) {
			case '-':
				if (argv[j][i + 2] != '\0')
					optmsg(c, "");
				last = 1; /* end of options */
				break;
			case 'h':
			h_again:;
				switch(argv[j][i + 2]) {
...
/*		DO WHILE THERE IS INPUT
	...
	while (( p = fgetws(line,sizeof(line)/sizeof(line[0]),iptr)) != NULL) {
