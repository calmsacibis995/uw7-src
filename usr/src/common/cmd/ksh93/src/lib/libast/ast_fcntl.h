#ident	"@(#)ksh93:src/lib/libast/ast_fcntl.h	1.1"

/* : : generated by proto : : */
/* : : generated from features/fcntl.c by iffe version 05/09/95 : : */
#ifndef _def_fcntl_ast
#if !defined(__PROTO__)
#if defined(__STDC__) || defined(__cplusplus) || defined(_proto) || defined(c_plusplus)
#if defined(__cplusplus)
#define __MANGLE__	"C"
#else
#define __MANGLE__
#endif
#define __STDARG__
#define __PROTO__(x)	x
#define __OTORP__(x)
#define __PARAM__(n,o)	n
#if !defined(__STDC__) && !defined(__cplusplus)
#if !defined(c_plusplus)
#define const
#endif
#define signed
#define void		int
#define volatile
#define __V_		char
#else
#define __V_		void
#endif
#else
#define __PROTO__(x)	()
#define __OTORP__(x)	x
#define __PARAM__(n,o)	o
#define __MANGLE__
#define __V_		char
#define const
#define signed
#define void		int
#define volatile
#endif
#if defined(__cplusplus) || defined(c_plusplus)
#define __VARARG__	...
#else
#define __VARARG__
#endif
#if defined(__STDARG__)
#define __VA_START__(p,a)	va_start(p,a)
#else
#define __VA_START__(p,a)	va_start(p)
#endif
#endif

#define _def_fcntl_ast	1
                  

#if defined(__STDPP__directive) && defined(__STDPP__hide)
__STDPP__directive pragma pp:hide chmod creat fcntl mkdir mkfifo mknod open umask
#else
#define chmod	______chmod
#define creat	______creat
#define fcntl	______fcntl
#define mkdir	______mkdir
#define mkfifo	______mkfifo
#define mknod	______mknod
#define open	______open
#define umask	______umask
#endif 

#include <ast_fs.h>	/* <fcntl.h> includes <sys/stat.h>! */
#include <fcntl.h>

#if defined(_AST_H) || defined(_POSIX_SOURCE)
#define _AST_mode_t	mode_t
#else
#define _AST_mode_t	int
#endif
#if defined(__STDPP__directive) && defined(__STDPP__hide)
__STDPP__directive pragma pp:nohide chmod creat fcntl mkdir mkfifo mknod open umask
extern __MANGLE__ int	creat __PROTO__((const char*, _AST_mode_t));
extern __MANGLE__ int	fcntl __PROTO__((int, int, ...));
extern __MANGLE__ int	open __PROTO__((const char*, int, ...));
#else
#ifdef	creat
#undef	creat
extern __MANGLE__ int	creat __PROTO__((const char*, _AST_mode_t));
#endif 
#ifdef	fcntl
#undef	fcntl
extern __MANGLE__ int	fcntl __PROTO__((int, int, ...));
#endif 
#ifdef	open
#undef	open
extern __MANGLE__ int	open __PROTO__((const char*, int, ...));
#endif 

#undef	_AST_mode_t
#undef	chmod
#undef	mkdir
#undef	mkfifo
#undef	mknod
#undef	umask
#endif 

#endif
