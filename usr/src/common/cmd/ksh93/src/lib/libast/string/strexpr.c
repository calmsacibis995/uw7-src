#ident	"@(#)ksh93:src/lib/libast/string/strexpr.c	1.1"
#pragma prototyped
/*
 * G. S. Fowler
 * D. G. Korn
...
expr(register Expr_t* ex, register int precedence)
{
	...
	case 0:
		...
		error(ex, gettxt(":265","more tokens expected"));
	case '-':
		n = -expr(ex, 13);
		break;
	case '+':
	...
	for (;;)
	{
		...
		case 0:
			goto done;
		case ')':
			if (!precedence) error(ex, gettxt(":266","too many )'s"));
			goto done;
		case '(':
			n = expr(ex, 1);
			if (getchr(ex) != ')')
			{
				ungetchr(ex);
				error(ex, gettxt(":267","closing ) expected"));
			}
		gotoperand:
			if (operand) error(ex, gettxt(":268","operator expected"));
			operand = 1;
			continue;
		case '?':
			...
			else
			{
				...
				{
					ungetchr(ex);
					error(ex, gettxt(":269",": expected for ? operator"));
				}
				if (n)
				{
					n = x;
		...
		case '=':
		case '!':
			if (peekchr(ex) != '=') error(ex, gettxt(":270","operator syntax error"));
		...
		case '%':
			...
			x = expr(ex, 12);
			if (c == '*') n *= x;
			else if (x == 0) error(ex, gettxt(":271","divide by zero"));
			else if (c == '/') n /= x;
			else n %= x;
			break;
		default:
			...
			if (ex->nextchr == pos) error(ex, gettxt(":272","syntax error"));
			goto gotoperand;
		}
		if (ex->errmsg) return(0);
		if (!operand) error(ex, gettxt(":273","operand expected"));
	}
 done:
	ungetchr(ex);
	if (!operand) error(ex, gettxt(":273","operand expected"));
...
strexpr(const char* s, char** end, long(*convert)(const char*, char**, void*), void* handle)
{
	...
	n = expr(&ex, 0);
	if (peekchr(&ex) == ':')
		seterror(&ex, gettxt(":274", "invalid use of :"));
	if (ex.errmsg)
	{
