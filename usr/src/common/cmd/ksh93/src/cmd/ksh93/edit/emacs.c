#ident	"@(#)ksh93:src/cmd/ksh93/edit/emacs.c	1.1"
#pragma prototyped
...
static void draw(Draw_t);
static int escape(genchar*, int);
static void putstring(char*);
static void search(genchar*,int);
static void setcursor(int, int);
static void show_info(const char*);
static void xcommands(int);
...
#   endif /* SHOPT_MULTIBYTE */
	...
	while ((c = ed_getchar(0)) != (-1))
	{
		...
		case cntl(']') :
			...
			{
				tmpbuf[0] = c;
				for(tmpctr = 1; tmpctr <= MB_CUR_MAX; tmpctr++)
				{
					if (mblen(tmpbuf, tmpctr) != tmpctr)
					{
						if ((lookahead - tmpctr) < 0)
							break;
						tmpbuf[tmpctr] = 
						 -previous[--lookahead];
					}
				}
				if (mbtowc(&tmpgen, tmpbuf, MB_CUR_MAX) < 1)
					tmpgen = c;
			} else
				tmpgen = c;
...
static void putstring(register char *sp)
{
	register int c;
	while (c= *sp++)
		 putchar(c);
...
static int escape(register genchar *out,int count)
{
...
static void xcommands(int count)
{
...
static void search(genchar *out,int direction)
{
...
static void draw(Draw_t option)
{
#define	NORMAL ' '
#define	LOWER  '<'
#define	BOTH   '*'
...
static void setcursor(register int newp,int c)
{
...
static int isword(register int i)
{
