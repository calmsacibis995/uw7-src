...
#define	NLOGS		100		/* max number of log files */
	...
	char **argv;
{
	...
	(void)setcat("uxsyslogd");
	(void)setlabel("UX:syslogd");
	...
	if (funix < 0) {
		(void) sprintf(line, gettxt(":1","cannot open %s"), LogName);
		logerror(line);
	...
	if (ioctl(funix, I_STR, &str) < 0) {
		logerror(gettxt(":2","cannot register to log console messages"));
...
{
	(void) fprintf(stderr,
	    gettxt(":3","usage: syslogd [-d] [-mmarkinterval] [-ppath] [-fconffile]\n"));
	exit(1);
}
...
doalarm()
{
	...
	if (Marking) {
		MarkTimer -= curalarm;
		if (MarkTimer <= 0) {
			logmsg(LocalHostName, LOG_INFO, gettxt(":5","-- MARK --"),
			    ADDDATE|MARK);
...
flushmsg()
{
	...
	if (PrevCount == 0)
		return;
	if (PrevCount > 1)
		(void) sprintf(PrevLine+16, gettxt(":6","last message repeated %d times"),
		    PrevCount);
	PrevCount = 0;
	...
	char *type;
{
	...
	if (t_errno == 0 || t_errno == TSYSERR) {
		if (errno == 0)
			...
			(void) sprintf(buf, gettxt(":7","syslogd: %s: error %d"), type, errno);
		else
...
die(sig)
{
	...
	if (sig) {
		...
		flushmsg();
		(void) sprintf(buf, gettxt(":8","going down on signal %d"), sig);
		errno = 0;
		logerror(buf);
	}
	exit(0);
...
nofile:
	...
	logmsg(LocalHostName, LOG_SYSLOG|LOG_INFO, gettxt(":9","syslogd: restart"), ADDDATE);
	...
	register struct filed *f;
{
	...
	for (p = line; *p && *p != '\t';) {
		...
		*bp = '\0';
		...
		if (pri < 0) {
			(void) sprintf(xbuf, gettxt(":10","line %d: unknown priority name \"%s\""),
			    lineno,  buf);
			logerror(xbuf);
			return;
		}
		...
		while (*p && !strchr("\t.;", *p)) {
			...
			*bp = '\0';
			if (*buf == '*')
				...
				if (i < 0) {
					(void) sprintf(xbuf, gettxt(":11","line %d: unknown facility name \"%s\""),
					    lineno, buf);
					logerror(xbuf);
					return;
				}
	...
	{
	case '\0':
		(void) sprintf(xbuf, gettxt(":12","line %d: no action part"), lineno);
		errno = 0;
		logerror(xbuf);
		break;
	...
	case '@':
		...
		if (gethost(p, &nbp, &ncp) < 0) {
			(void) sprintf(xbuf, gettxt(":13","line %d: unknown host %s"),
			    lineno, p);
			errno = 0;
			logerror(xbuf);
			break;
		...
		if (ismyaddr(nbp, ncp)) {
			...
			(void) sprintf(xbuf,
			    gettxt(":14","line %d: host %s is this host - logging loop"),
			    lineno, p);
			errno = 0;
			logerror(xbuf);
			break;
...
#ifndef FULL_IPV6
			/*
			 * Unless we have a full IPv6 implementation, skip IPv6
			 * entry.
			 */
			if (strcmp(ncp->nc_protofmly, NC_INET6) == 0)
				continue;
#endif /* FULL_IPV6 */
	...
	int first;
{
	...
	if (Ninputs > NINLOGS) {
		(void) sprintf(line, gettxt(":15","No free listener slots for transport %s\n"),
			ncp->nc_netid);
		if (first)
			logerror(line);
		dprintf(line);
	...
	if ((fd = t_open(ncp->nc_device, O_RDWR, NULL)) < 0) {
		(void) sprintf(line, gettxt(":16","%s: open failed errno=%d t_errno=%d\n"), ncp->nc_device, errno, t_errno);
		if (first)
				logerror(line);
		dprintf(line);
		return;
	...
	if (t_bind(fd, &bind, bound) < 0) {
		(void) sprintf(line, gettxt(":17","cannot bind for transport %s\n"),
				ncp->nc_netid);
		if (first)
			logerror(line);
		dprintf(line);
	...
	if ((bind.addr.len != bound->addr.len) ||
		...
		(void) sprintf(line, gettxt(":18","bind address in use for transport %s\n"),
				ncp->nc_netid);
		if (first)
			logerror(line);
		dprintf(line);
