...
char *format(s,a) char *s; NODE *a;
{
	...
	while (*s) {
		...
		*t = '\0';
		...
		{
			/* check for overflow BEFORE we shove it in p! */
			xs = getsval(x);
			if (xs)
			{
				if ((strlen(xs) + strlen(fmt) -2) >= RECSIZE)	
					error(FATAL, "string %.40s ... too long to print", xs);
				sprintf(p, fmt, xs);
			}
		}
		tempfree(x);
...
CELL *print(a,n) NODE **a;
{
	...
	for (x=a[0]; x!=NULL; x=x->nnext) {
		...
		if(xs)
		{
			if ((strlen(xs) + strlen(s)) >= RECSIZE)
				error(FATAL, "string %.40s ... too long to print", s);
			strcat(s, xs);
		}
		tempfree(y);
		if (x->nnext == NULL)
		{
			if ((strlen(s) + strlen(ORS)) >= RECSIZE)
				error(FATAL, "string %.40s ... too long to print", s);
			strcat(s, *ORS);
		}
		else
		{
			if ((strlen(s) + strlen(OFS)) >= RECSIZE)
				error(FATAL, "string %.40s ... too long to print", s);
			strcat(s, *OFS);
		}
	}
