#ident	"@(#)oawk:tran.c	1.1.2.4"
#ident "$Header$"
#include "stdio.h"
#include "awk.def"
#include "awk.h"

#define goodstr(foo)	((foo) == NULL ? "" : (foo))
...
char *s;
{
	dprintf("setsval: %o %s\n", vp, s, NULL);
        if (vp->tval & ARR)
                error(FATAL, "illegal reference to array %s", vp->nval);
        if ((vp->tval & (NUM | STR)) == 0)
                error(FATAL, "funny variable %o: %s %s %g %o", vp, goodstr(vp->nval),
                        goodstr(vp->sval), vp->fval, vp->tval);
	if (vp == recloc)
		error(FATAL, "can't set $0");
	vp->tval &= ~NUM;
	vp->tval |= STR;
...
register CELL *vp;
{
	...
	dprintf("getsval: %o", vp, NULL, NULL);
        if (vp->tval & ARR)
                error(FATAL, "illegal reference to array %s", goodstr(vp->nval));
        if ((vp->tval & (NUM | STR)) == 0)
                error(FATAL, "funny variable %o: %s %s %g %o", vp, goodstr(vp->nval),
                        goodstr(vp->sval), vp->fval, vp->tval);
	if ((vp->tval & STR) == 0) {
