...
char *	argv[];
{
	...
	if (c != EOF || optind == argc - 1 && strcmp(argv[optind], "=") == 0) {
		(void) fprintf(stderr, "%s: usage is %s [ -v ] [ -c cutoffyear ] zonename ...\n",
			argv[0], argv[0]);
		exit(1);
	}
	if (cutoff != NULL)
	...
	*/
	...
	for (i = optind; i < argc; ++i) {
		...
		tzequals = imalloc(strlen(argv[i]) + 4 + 1);
		if (tzequals == NULL) {
...
time_t	t;
{
	...
	if (tmp->tm_isdst)
		(void) printf(" %s", tzname[1]);
	else
		(void) printf(" %s", tzname[0]);
	if (v)	{
		(void) printf(" isdst=%d", tmp->tm_isdst);
		if (tmp->tm_isdst)
			(void) printf(" gmtoff=%ld", 0 - altzone);
		else
			(void) printf(" gmtoff = %ld", 0 - timezone);
	}
	(void) printf("\n");
}
