...
int tftp_backoff[REXMTSHIFT + 1] = { 1, 2, 4, 8, 16, 32 };

int	rexmtshift 	= 0;
int	rexmtval 	= 5;
int maxtimeout	= REXMTMAX;
...
timer()
{
	if (++rexmtshift > REXMTSHIFT) {
		printf("Transfer timed out.\n");
		longjmp(toplevel, -1);
	}
	if (rexmtval < tftp_backoff[rexmtshift])
		rexmtval = tftp_backoff[rexmtshift];
	if (rexmtval < REXMTMIN)
		rexmtval = REXMTMIN;
	else if (rexmtval > maxtimeout)
		rexmtval = maxtimeout;
...
send_data:
		...
		for ( ; ; ) {
			...
			alarm(0);
			rexmtshift = 0;	/* success reset timeout */
			if (n < 0) {
			...
			if (ap->th_opcode == ACK) {
				...
				/* The following two lines have been
				   commented out to prevent the
				   occurrence of the sorcerer's 
				   apprentice syndrome
