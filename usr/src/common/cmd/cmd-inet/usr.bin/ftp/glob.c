	...
	register char *v;
{
	...
	if (globcnt == 0 && (gflag&1)) {
		blkfree(gargv, gargv == agargv ? 1 : 0), gargv = 0;
		return (0);
	} else
...
patherr1:
	closedir(dirp);
patherr2:
	globerr = gettxt(":236", "Bad directory components");
}
	...
	register char *s1, *s2;
{
	...
	if (len >= gnleft || gargc >= GAVSIZ - 1)
		globerr = gettxt(":237", "Arguments too long");
	else {
		gargc++;
		gnleft -= len;
		gargv[gargc] = 0;
	...
	char c;
{
	...
	if (gpathp >= lastgpathp)
		globerr = gettxt(":238", "Pathname too long");
	else {
		*gpathp++ = c;
		*gpathp = 0;
	}
