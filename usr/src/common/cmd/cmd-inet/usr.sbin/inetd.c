 ...
 *	server program arguments	starting with argument 0
 *
 * for rpc services:
...
#define	TOOMANY		900		/* don't start more than TOOMANY */
...
struct	servtab {
	...
	int	se_recvifindex;		/* if nonzero, set IP_RECVIFINDEX */
...
#define CMD_OPTIONS "dstl:"
	...
	if (debug && standalone) {
		debugfp = stderr;
		setbuf(debugfp, NULL); /* make it unbuffered */
	}
	...
	if (!standalone) {
		...
		if (s == NULL || *s == '\0') {
			syslog(LOG_ERR,
			       "No port monitor tag in the environment");
			exit(-1);
		}
		...
		if (debug) {
			sprintf(logfile, "%s%s/log", LOGDIR, Tag);
			if ((debugfp = fopen(logfile ,"a")) == NULL) {
				debug = 0;
				syslog(LOG_WARNING,
	     "inetd: cannot open debug file <%s>, errno: %d. Turning off debug",
				logfile, errno);
			}
			else {
				setbuf(debugfp, NULL); /* make it unbuffered */
	...
	for (;;) {
	    ...
	    size_t remaddrlen = sizeof (rem_addr);
...
#define SWAP(a, b) { void *c = a; a = b; b = c; }
			if (cp->se_user)
				SWAP(sep->se_user, cp->se_user);
			if (cp->se_server)
				SWAP(sep->se_server, cp->se_server);
			SWAP(sep->se_argv, cp->se_argv);
			sigsetmask(omask);
			freeconfig(cp);
			if (debug)
...
setsockopt(fd, SOL_SOCKET, opt, (char *)&on, sizeof (on))
		...
		if (sep->se_recvifindex
			&& setsockopt(sep->se_fd, IPPROTO_IP, IP_RECVIFINDEX, 
			(char *)&on, sizeof(on)) < 0) {
			syslog(LOG_ERR, "setsockopt (IP_RECVIFINDEX): %m");
		}
	...
	register struct servtab *sep;
{
	...
	if (sep->se_socktype == SOCK_TLI) {
		...
		addr.sin_port = 0;
		addr.sin_addr.s_addr = htonl(INADDR_ANY);
		if (bind(sep->se_fd, (struct sockaddr *)&addr,
			 sizeof(addr)) < 0) {
			syslog(LOG_ERR, "%s/%s: bind: %m",
			       sep->se_service, sep->se_proto);
			(void) close(sep->se_fd);
		...
		if (getsockname(sep->se_fd, (struct sockaddr *)&addr,
				&len) != 0) {
...
more:
	...
	arg = skip(&cp);
	/*
	 * Look for options following socket type.
	 */
	if (opt = strchr(arg, '/')) {
		*opt++ = '\0';
	}
	if (strcmp(arg, "stream") == 0)
		...
		sep->se_socktype = 0;
	/*
	 * Check options.  "i" is the only option currently recognized.
	 * "i" causes IP_RECVIFINDEX to be set on the socket.
	 */
	if (opt && strchr(opt, 'i')) {
		sep->se_recvifindex = 1;
	}
	else {
		sep->se_recvifindex = 0;
	}
	...
	if (strcmp(sep->se_server, "internal") == 0) {
		...
		sep->se_bi = NULL;
#define INCRMNT	5
	sep->se_argv = (char **)malloc(INCRMNT * sizeof(char *));
	if (sep->se_argv == (char **)0) {
		syslog(LOG_ERR, "Out of memory.");
		exit(-1);
	}
	argc = 0;
	for (arg = skip(&cp); cp; arg = skip(&cp)) {
		if ((argc % INCRMNT) == INCRMNT - 1) {
			sep->se_argv = (char **)realloc((void *)sep->se_argv,
				(argc/INCRMNT + 2) * INCRMNT * sizeof(char *));
			if (sep->se_argv == (char **)0) {
				syslog(LOG_ERR, "Out of memory.");
				exit(-1);
			}
