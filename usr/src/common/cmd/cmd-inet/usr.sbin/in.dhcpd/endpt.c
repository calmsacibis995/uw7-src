 ...
 *	passed from inetd or tlid.  Returns 1 if so, 0 if not.
 *
...
send_packet(int endpt, struct sockaddr_in *dest, void *buf, int len)
{
	...
	if (t_sndudata(endpt, &udata) == -1) {
		report(LOG_ERR, "Unable to send packet: %s",
			t_strerror(t_errno));
		return -1;
	}
...
handle_tli_event(int endpt)
{
	...
	case T_UDERR:
		...
		errno = uderr.error;
		report(LOG_WARNING, "Error sending to %s: %m",
...
get_endpt(u_short port)
{
	...
	if ((endpt = socket(AF_INET, SOCK_DGRAM, 0)) == -1) {
		report(LOG_ERR, "Unable to create UDP socket: %m");
		return -1;
	}
...
send_packet(int endpt, struct sockaddr_in *dest, void *buf, int len)
{
	if (sendto(endpt, buf, len, 0, (struct sockaddr *) dest,
		...
		report(LOG_ERR, "sendto failed: %m");
		return -1;
	}
	else {
		return 0;
	...
	u_long *ifindex)
{
	...
	if ((rlen = recvmsg(endpt, &mh, 0)) < 0) {
		report(LOG_ERR, "recvmsg failed: %m");
		return -1;
	}
...
is_endpt(int fd)
{
	...
	if (getsockname(fd, (struct sockaddr *) &sin, &len) == -1) {
		if (errno != EBADF && errno != ENOTSOCK) {
			report(LOG_ERR, "getsockname failed: %m");
		}
		return 0;
	}
