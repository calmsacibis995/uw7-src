...
init_igmp()
{
	...
	mylog(LOG_ERR, errno, "IGMP socket");
    ...
    int recvlen;
{
    ...
    if (ip->ip_p == 0) {
	if (src == 0 || dst == 0)
	    mylog(LOG_WARNING, 0, "kernel request not accurate");
	else
    ...
    mylog(LOG_DEBUG, 0, "RECV %s from %-15s to %s",
    ...
    int datalen;
{
	...
	if (errno == ENETDOWN)
	    ...
	    mylog(igmp_log_level(type, code), errno,
		"sendto to %s on %s",
    ...
    mylog(LOG_DEBUG, 0, "SENT %s from %-15s to %s",
