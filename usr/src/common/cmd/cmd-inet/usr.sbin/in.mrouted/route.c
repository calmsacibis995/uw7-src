 ...
 */
static int
init_children_and_leaves( register struct rtentry *r, register vifi_t parent)
{
 ...
 */
void
add_vif_to_routes( register vifi_t vifi)
{
 ...
 */
void
delete_vif_from_routes(register vifi_t vifi)
{
    register struct rtentry *r;
 ...
 */
void
delete_neighbor_from_routes( register u_int32 addr, register vifi_t vifi)
{
    register struct rtentry *r;
    register struct uvif *v;
 ...
 */
static int
find_route( register u_int32 origin, register u_int32 mask)
{
    register struct rtentry *r;
 ...
 */
static void
discard_route( register struct rtentry *prev_r)
{
    register struct rtentry *r;
 ...
 */
void
update_route(u_int32 origin, u_int32 mask, u_int metric, u_int32 src, vifi_t vifi)
{
    register struct rtentry *r;
    u_int adj_metric;
 ...
 */
void
accept_report(u_int32 src, u_int32 dst, register char *p,
				register int datalen, u_int32 level)
{
    vifi_t vifi;
    ...
    mylog(LOG_DEBUG, 0, "Updating %d routes from %s to %s", nrt,
		inet_fmt(src, s1), inet_fmt(dst, s2));
    for (i = 0; i < nrt; ++i) {
	if (i != 0 && rt[i].origin == rt[i-1].origin &&
		      rt[i].mask == rt[i-1].mask) {
	    mylog(LOG_WARNING, 0, "%s reports duplicate route for %s",
 ...
 */
static int
report_chunk(register struct rtentry *start_rt, vifi_t vifi, u_int32 dst)
{
    register struct rtentry *r;
    register char *p;
    register int i;
...
report_next_chunk()
{
    ...
    n = min;
    mylog(LOG_INFO, 0, "update %d starting at %d of %d",
