...
void k_set_rcvbuf(int bufsize)
{
	...
	mylog(LOG_ERR, errno, "setsockopt SO_RCVBUF %u", bufsize);
}
...
void k_hdr_include(int bool)
{
	...
	mylog(LOG_ERR, errno, "setsockopt IP_HDRINCL %u", bool);
#endif
}
...
void k_set_ttl(int t)
{
	...
	mylog(LOG_ERR, errno, "setsockopt IP_MULTICAST_TTL %u", ttl);
}
...
void k_set_loop(int l)
{
	...
	mylog(LOG_ERR, errno, "setsockopt IP_MULTICAST_LOOP %u", loop);
}
...
void k_set_if(u_int32 ifa)
{
	...
	mylog(LOG_ERR, errno, "setsockopt IP_MULTICAST_IF %s",
	    		    inet_fmt(ifa, s1));
}
...
void k_join(u_int32 grp, u_int32 ifa)
{
	...
	mylog(LOG_WARNING, errno, "can't join group %s on interface %s",
...
void k_leave(u_int32 grp, u_int32 ifa)
{
    ...
    if (setsockopt(igmp_socket, IPPROTO_IP, IP_DROP_MEMBERSHIP,
		   (char *)&mreq, sizeof(mreq)) < 0)
	mylog(LOG_WARNING, errno, "can't leave group %s on interface %s",
				inet_fmt(grp, s1), inet_fmt(ifa, s2));
}
...
void k_init_dvmrp(void)
{
    ...
    if (setsockopt(igmp_socket, IPPROTO_IP, MRT_INIT,
		   (char *)&v, sizeof(int)) < 0)
#endif
	mylog(LOG_ERR, errno, "can't enable Multicast routing in kernel");
}
...
void k_stop_dvmrp(void)
{
    if (setsockopt(igmp_socket, IPPROTO_IP, MRT_DONE,
		   (char *)NULL, 0) < 0)
	mylog(LOG_WARNING, errno, "can't disable Multicast routing in kernel");
}
...
void k_add_vif(vifi_t vifi, struct uvif *v)
{
    ...
    if (setsockopt(igmp_socket, IPPROTO_IP, MRT_ADD_VIF,
		   (char *)&vc, sizeof(vc)) < 0)
	mylog(LOG_ERR, errno, "setsockopt MRT_ADD_VIF");
}
...
void k_del_vif(vifi_t vifi)
{
	...
	mylog(LOG_ERR, errno, "setsockopt MRT_DEL_VIF");
}
...
#endif
	mylog(LOG_WARNING, errno, "setsockopt MRT_ADD_MFC");
    }
}
...
#endif
	mylog(LOG_WARNING, errno, "setsockopt MRT_DEL_MFC");
    }
