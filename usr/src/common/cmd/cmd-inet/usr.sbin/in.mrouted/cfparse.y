...
va_dcl
{
	...
	mylog(LOG_ERR,0,"%s: %s near line %d", configfilename, buf, lineno);
}
...
va_dcl
{
	...
	mylog(LOG_WARNING,0,"%s: %s near line %d", configfilename, buf, lineno);
}
...
char *s;
{
	mylog(LOG_ERR, 0, "%s: %s near line %d", configfilename, s, lineno);
}
...
config_vifs_from_file()
{
	...
	if ((f = fopen(configfilename, "r")) == NULL) {
	    if (errno != ENOENT)
		mylog(LOG_ERR, errno, "can't open %s", configfilename);
	    return;
	}
