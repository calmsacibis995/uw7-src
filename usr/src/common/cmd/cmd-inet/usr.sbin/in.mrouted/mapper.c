...
#ifdef __STDC__
void
mylog(int severity, int syserr, char *format, ...)
{
	va_list ap;
	char    fmt[100];
...
/*VARARGS3*/
void 
mylog(severity, syserr, format, va_alist)
	int     severity, syserr;
	char   *format;
	va_dcl
{
...
void accept_group_report(src, dst, group, r_type)
    ...
    int r_type;
{
    mylog(LOG_INFO, 0, "ignoring IGMP group membership report from %s to %s",
...
void accept_probe(src, dst, p, datalen, level)
    ...
    int datalen;
{
    mylog(LOG_INFO, 0, "ignoring DVMRP probe from %s to %s",
	inet_fmt(src, s1), inet_fmt(dst, s2));
}
...
void accept_report(src, dst, p, datalen, level)
    ...
    int datalen;
{
    mylog(LOG_INFO, 0, "ignoring DVMRP routing report from %s to %s",
	inet_fmt(src, s1), inet_fmt(dst, s2));
}
...
#define GET_ADDR(a) (a = ((u_int32)*p++ << 24), a += ((u_int32)*p++ << 16),\
	...
	if (datalen < 4 + 3) {
	    mylog(LOG_WARNING, 0, "received truncated interface record from %s",
		inet_fmt(src, s1));
	    return;
	}
	...
	while (ncount--) {
	    ...
	    if (datalen < 4) {
		mylog(LOG_WARNING, 0, "received truncated neighbor list from %s",
		    inet_fmt(src, s1));
		return;
	    }
...
{
    mylog(LOG_NOTICE, 0, "network marked down...");
}
    ...
    char *argv[];
{
	...
	if (count < 0) {
	    ...
	    mylog(LOG_DEBUG, 0, "Timed out receiving neighbor lists");
