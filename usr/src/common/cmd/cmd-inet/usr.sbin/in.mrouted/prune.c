...
send_prune( struct gtable *gt)
{
	...
	*p++ = ((char *)&(tmp))[i];
    ...
    mylog(LOG_DEBUG, 0, "sent prune for (%s %s)/%d on vif %d to %s",
...
send_graft( struct gtable *gt)
{
	...
	*p++ = ((char *)&(gt->gt_mcastgrp))[i];
    ...
    mylog(LOG_DEBUG, 0, "sent graft for (%s %s) to %s on vif %d",
	inet_fmts(gt->gt_route->rt_origin, gt->gt_route->rt_originmask, s1),
	inet_fmt(gt->gt_mcastgrp, s2),
...
send_graft_ack( u_int32 src, u_int32 dst, u_int32 origin, u_int32 grp)
{
	...
	*p++ = ((char *)&(grp))[i];
    ...
    mylog(LOG_DEBUG, 0, "sent graft ack for (%s, %s) to %s",
...
add_table_entry(u_int32 origin, u_int32 mcastgrp)
{
	...
	if (gt == NULL)
	    mylog(LOG_ERR, 0, "ran out of memory");
	...
	*gtnp = gt;
	...
	if (r) {
		...
		mylog(LOG_WARNING, 0, "Entry for (%s %s) (rt:%x) exists (rt:%x)",
	...
	*stnp = st;
...
#endif
	mylog(LOG_WARNING, 0, "kernel entry already exists for (%s %s)",
    ...
    mylog(LOG_DEBUG, 0, "add cache entry (%s %s) gm:%x, parent-vif:%d",
	inet_fmt(origin, s1),
...
reset_neighbor_state( vifi_t vifi, u_int32 addr)
{
	...
	if (vifi == r->rt_parent) {
		...
		mylog(LOG_DEBUG, 0, "reset_neighbor_state parent reset (%s %s)",
		    inet_fmts(r->rt_origin, r->rt_originmask, s1),
		    inet_fmt(g->gt_mcastgrp, s2));
		...
		mylog(LOG_DEBUG, 0, "reset member state (%s %s) gm:%x",
		    inet_fmts(r->rt_origin, r->rt_originmask, s1),
...
del_table_entry( struct rtentry *r, u_int32 mcastgrp, u_int  del_flag)
{
	...
	g = r->rt_groups;
	while (g) {
	    mylog(LOG_DEBUG, 0, "del_table_entry deleting (%s %s)",
		inet_fmts(r->rt_origin, r->rt_originmask, s1),
		inet_fmt(g->gt_mcastgrp, s2));
	    st = g->gt_srctbl;
	    while (st) {
		if (k_del_rg(st->st_origin, g) < 0) {
		    mylog(LOG_WARNING, errno,
...
update_table_entry( struct rtentry *r)
{
	...
	mylog(LOG_DEBUG, 0, "updating cache entries (%s %s) gm:%x",
	    inet_fmts(r->rt_origin, r->rt_originmask, s1),
	    inet_fmt(g->gt_mcastgrp, s2),
	    g->gt_grpmems);
...
update_lclgrp( vifi_t vifi, u_int32 mcastgrp)
{
    ...
    mylog(LOG_DEBUG, 0, "group %s joined on vif %d",
	...
	if (g->gt_mcastgrp == mcastgrp &&
	    ...
	    prun_add_ttls(g);
	    mylog(LOG_DEBUG, 0, "update lclgrp (%s %s) gm:%x",
		inet_fmts(r->rt_origin, r->rt_originmask, s1),
		inet_fmt(g->gt_mcastgrp, s2), g->gt_grpmems);
...
delete_lclgrp( vifi_t vifi, u_int32 mcastgrp)
{
    ...
    mylog(LOG_DEBUG, 0, "group %s left on vif %d",
	inet_fmt(mcastgrp, s1), vifi);
	...
	if (g->gt_mcastgrp == mcastgrp) {
		...
		mylog(LOG_DEBUG, 0, "delete lclgrp (%s %s) gm:%x",
		    inet_fmts(r->rt_origin, r->rt_originmask, s1),
		    inet_fmt(g->gt_mcastgrp, s2), g->gt_grpmems);
...
accept_prune( u_int32 src, u_int32 dst, char *p, int datalen)
{
    ...
    mylog(LOG_DEBUG, 0, "%s on vif %d prunes (%s %s)/%d",
	...
	if (!VIFM_ISSET(vifi, r->rt_children)) {
	    mylog(LOG_WARNING, 0, "prune received from non-child %s for (%s %s)",
		inet_fmt(src, s1), inet_fmt(prun_src, s2),
		inet_fmt(prun_grp, s3));
	    return;
	}
	if (VIFM_ISSET(vifi, g->gt_scope)) {
	    mylog(LOG_WARNING, 0, "prune received from %s on scoped grp (%s %s)",
		inet_fmt(src, s1), inet_fmt(prun_src, s2),
		inet_fmt(prun_grp, s3));
	    return;
	}
	if ((pt = find_prune_entry(src, g->gt_pruntbl)) != NULL) {
	     ...
	     */
	    if (pt->pt_timer > 10) {
		mylog(LOG_WARNING, 0, "%s %d from %s for (%s %s)/%d %s %d %s %x",
	    ...
	    if (pt == NULL)
	      mylog(LOG_ERR, 0, "pt: ran out of memory");
	...
	if (stop_sending && !grplst_mem(vifi, prun_grp)) {
	    VIFM_CLR(vifi, g->gt_grpmems);
	    mylog(LOG_DEBUG, 0, "prune (%s %s), stop sending on vif %d, gm:%x",
		inet_fmts(r->rt_origin, r->rt_originmask, s1),
	 ...
	 * downstream.
	 */
	mylog(LOG_DEBUG, 0, "%s (%s %s)/%d from %s",
...
chkgrp_graft( vifi_t	vifi, u_int32	mcastgrp)
{
	...
	if (g->gt_mcastgrp == mcastgrp && VIFM_ISSET(vifi, r->rt_children))
		...
		mylog(LOG_DEBUG, 0, "chkgrp graft (%s %s) gm:%x",
		    inet_fmts(r->rt_origin, r->rt_originmask, s1),
		    inet_fmt(g->gt_mcastgrp, s2), g->gt_grpmems);
...
accept_graft(u_int32 src, u_int32 dst, char *p, int datalen)
{
    ...
    mylog(LOG_DEBUG, 0, "%s on vif %d grafts (%s %s)",
	inet_fmt(src, s1), vifi,
	...
	if (VIFM_ISSET(vifi, g->gt_scope)) {
	    mylog(LOG_WARNING, 0, "graft received from %s on scoped grp (%s %s)",
	...
	while ((pt = *ptnp) != NULL) {
		...
		*ptnp = pt->pt_next;
		...
		VIFM_SET(vifi, g->gt_grpmems);
		mylog(LOG_DEBUG, 0, "accept graft (%s %s) gm:%x",
		    inet_fmts(r->rt_origin, r->rt_originmask, s1),
		    inet_fmt(g->gt_mcastgrp, s2), g->gt_grpmems);
	 ...
	 */
	send_graft_ack(dst, src, graft_src, graft_grp);
	mylog(LOG_DEBUG, 0, "%s (%s %s) from %s",
...
accept_g_ack( u_int32  src, u_int32 dst, char *p, int datalen)
{
    ...
    mylog(LOG_DEBUG, 0, "%s on vif %d acks graft (%s, %s)",
	inet_fmt(src, s1), vifi,
	...
	mylog(LOG_WARNING, 0, "%s (%s, %s) from %s",
...
steal_sources(struct rtentry *rt)
{
	...
	if ((rt->rt_origin & rp->rt_originmask) == rp->rt_origin) {
	    mylog(LOG_DEBUG, 0, "Route for %s stealing sources from %s",
...
age_table_entry()
{
    ...
    mylog(LOG_DEBUG, 0, "ageing entries");
	...
	if (gt->gt_prsent_timer > 0) {
	    ...
	    if (gt->gt_prsent_timer <= 0) {
		mylog(LOG_DEBUG, 0, "upstream prune tmo (%s %s)",
		    inet_fmts(r->rt_origin, r->rt_originmask, s1),
		    inet_fmt(gt->gt_mcastgrp, s2));
	...
	while ((pt = *ptnp) != NULL) {
		...
		mylog(LOG_DEBUG, 0, "expire prune (%s %s) from %s on vif %d", 
		    inet_fmts(r->rt_origin, r->rt_originmask, s1),
		    inet_fmt(gt->gt_mcastgrp, s2),
	...
	if (gt->gt_timer <= 0) {
		...
		if (sg_req.pktcnt == st->st_pktcnt) {
		    *stnp = st->st_next;
		    mylog(LOG_DEBUG, 0, "age_table_entry deleting (%s %s)",
			inet_fmt(st->st_origin, s1),
			inet_fmt(gt->gt_mcastgrp, s2));
		    if (k_del_rg(st->st_origin, gt) < 0) {
			mylog(LOG_WARNING, errno,
	    ...
	    mylog(LOG_DEBUG, 0, "timeout cache entry (%s, %s)",
		inet_fmts(r->rt_origin, r->rt_originmask, s1),
		inet_fmt(gt->gt_mcastgrp, s2));
...
expire_prune(vifi_t vifi, struct gtable *gt)
{
     ...
     */
    if (gt->gt_prsent_timer > 0) {
        mylog(LOG_DEBUG, 0, "prune expired with %d left on %s",
    ...
    if (!VIFM_ISSET(vifi, gt->gt_grpmems)) {
        ...
        mylog(LOG_DEBUG, 0, "forw again (%s %s) gm:%x vif:%d",
	inet_fmts(rt->rt_origin, rt->rt_originmask, s1),
		...
		char *data, u_int no, int datalen)
{
    ...
    if (datalen == QLEN) {
	type = QUERY;
	mylog(LOG_DEBUG, 0, "Initial traceroute query rcvd from %s to %s",
	    inet_fmt(src, s1), inet_fmt(dst, s2));
    }
    else if ((datalen - QLEN) % RLEN == 0) {
	type = RESP;
	mylog(LOG_DEBUG, 0, "In-transit traceroute query rcvd from %s to %s",
	    inet_fmt(src, s1), inet_fmt(dst, s2));
	if (IN_MULTICAST(ntohl(dst))) {
	    mylog(LOG_DEBUG, 0, "Dropping multicast response");
	    return;
	}
    }
    else {
	mylog(LOG_WARNING, 0, "%s from %s to %s",
	...
	mylog(LOG_DEBUG, 0, "packet with all reports filled in");
	return;
    }

    mylog(LOG_DEBUG, 0, "s: %s g: %s d: %s ", inet_fmt(qry->tr_src, s1),
	    inet_fmt(group, s2), inet_fmt(qry->tr_dst, s3));
    mylog(LOG_DEBUG, 0, "rttl: %d rd: %s", qry->tr_rttl,
	    inet_fmt(qry->tr_raddr, s1));
    mylog(LOG_DEBUG, 0, "rcount:%d, qid:%06x", rcount, qry->tr_qid);

    /* determine the routing table entry for this traceroute */
    rt = determine_route(qry->tr_src);
    if (rt) {
	mylog(LOG_DEBUG, 0, "rt parent vif: %d rtr: %s metric: %d",
		rt->rt_parent, inet_fmt(rt->rt_gateway, s1), rt->rt_metric);
	mylog(LOG_DEBUG, 0, "rt origin %s",
		inet_fmts(rt->rt_origin, rt->rt_originmask, s1));
    } else
	mylog(LOG_DEBUG, 0, "...no route");
	...
	if (oqid == qry->tr_qid) {
	     ...
	     * it is not fatal.
	     */
	    mylog(LOG_DEBUG, 0, "ignoring duplicate traceroute packet");
	    return;
	}

	if (rt == NULL) {
	    mylog(LOG_DEBUG, 0, "Mcast traceroute: no route entry %s",
	...
	if (vifi == NO_VIF) {
	    ...
	    mylog(LOG_DEBUG, 0, "Destination %s not an interface",
		   inet_fmt(qry->tr_dst, s1));
	    if (IN_MULTICAST(ntohl(dst)))
		return;
	    errcode = TR_WRONG_IF;
	} else if (rt != NULL && !VIFM_ISSET(vifi, rt->rt_children)) {
	    mylog(LOG_DEBUG, 0, "Destination %s not on forwarding tree for src %s",
	 ...
	 */
	if ((vifi = find_vif(src, dst)) == NO_VIF) {
	    mylog(LOG_DEBUG, 0, "Wrong interface for packet");
	    errcode = TR_WRONG_IF;
	}
    }   
    ...
    mylog(LOG_DEBUG, 0, "Sending traceroute response");
...
sendit:
     ...
     */
    mylog(LOG_DEBUG, 0, "rcount:%d, no:%d", rcount, no);
	 ...
	 */
	if (phys_vif != -1) {
	    mylog(LOG_DEBUG, 0, "Sending reply to %s from %s",
	    ...
	    mylog(LOG_INFO, 0, "No enabled phyints -- %s",
			"dropping traceroute reply");
    } else {
	mylog(LOG_DEBUG, 0, "Sending %s to %s from %s",
