...
static int SetTimer __P((vifi_t vifi, struct listaddr *g));
...
init_vifs()
{
	...
	mylog(LOG_ERR, errno, "UDP socket");
    mylog(LOG_INFO,0,"Getting vifs from kernel interfaces");
    config_vifs_from_kernel();
    mylog(LOG_INFO,0,"Getting vifs from %s",configfilename);
    ...
    if (enabled_vifs < 2)
	mylog(LOG_ERR, 0, "can't forward: %s",
    ...
    mylog(LOG_INFO, 0, "Installing vifs in mrouted...");
    for (vifi = 0, v = uvifs; vifi < numvifs; ++vifi, ++v) {
	if (!(v->uv_flags & VIFF_DISABLED)) {
	    if (!(v->uv_flags & VIFF_DOWN)) {
		if (v->uv_flags & VIFF_TUNNEL)
		    mylog(LOG_INFO, 0, "vif #%d, tunnel %s -> %s", vifi,
				inet_fmt(v->uv_lcl_addr, s1),
				inet_fmt(v->uv_rmt_addr, s2));
		else
		    mylog(LOG_INFO, 0, "vif #%d, phyint %s", vifi,
				inet_fmt(v->uv_lcl_addr, s1));
		start_vif2(vifi);
	    } else mylog(LOG_INFO, 0,
...
init_installvifs()
{
    ...
    mylog(LOG_INFO, 0, "Installing vifs in kernel...");
    for (vifi = 0, v = uvifs; vifi < numvifs; ++vifi, ++v) {
	if (!(v->uv_flags & VIFF_DISABLED)) {
	    if (!(v->uv_flags & VIFF_DOWN)) {
		if (v->uv_flags & VIFF_TUNNEL)
 ...
 */
static void
send_probe_on_vif(register struct uvif *v)
{
    register char *p;
 ...
 */
vifi_t
find_vif( register u_int32 src, register u_int32 dst)
{
    register vifi_t vifi;
    register struct uvif *v;
...
accept_group_report(u_int32 src, u_int32 dst, u_int32 group, int r_type)
{
	...
	if (g == NULL)
	    mylog(LOG_ERR, 0, "ran out of memory");    /* fatal */
...
accept_neighbor_request(u_int32 src, u_int32 dst)
{
	...
	if ((udp = socket(AF_INET, SOCK_DGRAM, 0)) < 0
	    ...
	    mylog(LOG_WARNING, errno, "Determining local address");
	    close(udp);
	    return;
	}
	close(udp);
...
accept_info_request(u_int32 src, u_int32 dst, u_char *p, int datalen)
{
	...
	switch (*p) {
	    ...
	    default:
		mylog(LOG_INFO, 0, "ignoring unknown info type %d", *p);
		break;
	}
	*(q+1) = len++;
	outlen += len * 4;
...
{
    mylog(LOG_INFO, 0, "ignoring spurious DVMRP neighbor list from %s to %s",
...
{
    mylog(LOG_INFO, 0, "ignoring spurious DVMRP neighbor list2 from %s to %s",
	inet_fmt(src, s1), inet_fmt(dst, s2));
}
...
{
    mylog(LOG_INFO, 0, "ignoring spurious DVMRP info reply from %s to %s",
	inet_fmt(src, s1), inet_fmt(dst, s2));
}
 ...
 */
int
update_neighbor(vifi_t vifi, u_int32 addr, int msgtype,
			char *p, int datalen, u_int32 level)
{
    register struct uvif *v;
	 ...
	 */
	mylog(LOG_DEBUG, 0, "New neighbor %s on vif %d v%d.%d nf 0x%02x",
