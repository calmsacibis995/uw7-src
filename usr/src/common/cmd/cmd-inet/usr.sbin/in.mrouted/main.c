...
usage:	fprintf(stderr, 
    ...
    mylog(LOG_NOTICE, 0, "%s", versionstring);
    ...
    if ((((vers >> 8) & 0xff) != 3) ||
	 ((vers & 0xff) != 5))
	mylog(LOG_ERR, 0, "kernel (v%d.%d)/mrouted (v%d.%d) version mismatch",
...
#endif
   {
            if (errno != EINTR) /* SIGALRM is expected */
                mylog(LOG_WARNING, errno, "select failed");
            continue;
        }
	...
	if (FD_ISSET(igmp_socket, &rfds)) {
	    recvlen = recvfrom(igmp_socket, recv_buf, RECV_BUF_SIZE,
			       0, NULL, &dummy);
	    if (recvlen < 0) {
		if (errno != EINTR) mylog(LOG_ERR, errno, "recvfrom");
		continue;
	    }
#ifdef SYSV
	    ...
	    if (sigprocmask(SIG_BLOCK, &block, &oblock) < 0)
		    mylog(LOG_ERR, errno, "sigprocmask");
#else
...
static void
    ...
    int i;
{
    mylog(LOG_NOTICE, 0, "%s exiting", versionstring);
    cleanup();
    _exit(1);
}
    ...
    int i;
{
    ...
    mylog(LOG_NOTICE, 0, "%s restart", versionstring);
    ...
    u_int32 origin, mcastgrp;
{
	...
	if ((f = fopen("/tmp/mrouted.clog", "w")) == NULL) {
	    mylog(LOG_ERR, errno, "open /tmp/mrouted.clog");
	}
    }
