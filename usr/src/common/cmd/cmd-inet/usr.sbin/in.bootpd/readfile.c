...
/*
 * List of subnet entrys used for default by subnet response.
 */
struct host_list *subnets = NULL;
...
/*
 * This routine clears a list of hosts.  It frees all the host structs
 * and list members and resets the pointer to NULL.
 */

void list_Reset(hlist_ptr)
struct host_list **hlist_ptr;
{
    struct host_list *hlist_save;
    while (*hlist_ptr) {
	free_host((hash_datum *) (*hlist_ptr)->host_ptr);
	hlist_save = *hlist_ptr;
	*hlist_ptr = hlist_save->next;
	free((char *) hlist_save);
    }	
}
    ...
    int force;
{
	...
	/* Insert in subnet list if this is net entry. */
	if (hp->linkcount <= 1) {
	    if(!strncmp(SUBNETKEY, hp->hostname->string, strlen(SUBNETKEY))) {
		hlp = (struct host_list *) smalloc(sizeof(struct host_list));
		hlp->host_ptr = hp;
		hlp->next = subnets;
		subnets = hlp;
	    }
    ...
    struct host *hp;
{
    ...
    /*
     * Macros for use in the switch below:
     */
...
#define PARSE_STR(MEMBER) do {			\
	...
	if (optype == OP_ADDITION) {
		...
		if (!strncmp(tmpstr, "rfc", 3)) {
		    bcopy(vm_rfc1048, hp->vm_cookie, 4);
		} else if (!strncmp(tmpstr, "cmu", 3)) {
...
PRIVATE boolean hwinscmp(d1, d2)
    hash_datum *d1, *d2;
{
...
char **src;
{
    ...
    /*
     * Macros for use in this function:
     */
...
/*
 * Use ip_addr to lookup host entry in list of subnet entries.
 */

struct host *lookup_subnet(ip_addr)
    ulong ip_addr;
{
    struct host_list *look = subnets;
    struct host *hp;


    while (look != NULL) {
	hp = look->host_ptr;
	if ((hp->iaddr.s_addr & hp->subnet_mask.s_addr) ==
	    (ip_addr & hp->subnet_mask.s_addr)) {
		return(hp);
	}
	look = look->next;
    }
