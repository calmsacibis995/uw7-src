...
static	struct sockaddr_in hostaddr = { sizeof(struct sockaddr_in),
				       0 };		/* host address */
#else
...
#else
	if (WriteFile((HANDLE)sockfd, (char *)xdata, xdatalen,
			(LPDWORD)&NumberOfBytesWritten, NULL) == FALSE) {
#endif /* SYS_WINNT */
...
again:
	...
	if (n == -1) {
#else
	if(ReadFile((HANDLE)sockfd, (char *)&rpkt, sizeof(rpkt), 
					(LPDWORD)&n, NULL) == FALSE) {
#endif /* SYS_WINNT */
		warning("read", "", "");
		return -1;
	}
...
#else
			(void) ReadFile((HANDLE)sockfd, junk, sizeof junk, NULL, NULL);
#endif /* SYS_WINNT */
	} while (res > 0);
	...
	FILE *fp;
{
	...
	extern char *Version;
#else
	char *Version = "version=3.4n (beta multicast)";
#endif
