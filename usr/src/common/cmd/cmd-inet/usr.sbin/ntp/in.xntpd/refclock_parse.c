#ident "@(#)refclock_parse.c	1.2"

#if defined(REFCLOCK) && defined(PARSE)
/*
 * /src/NTP/REPOSITORY/v3/xntpd/refclock_parse.c,v 3.87 1995/12/17 22:54:34 kardel Exp
 *
 * refclock_parse.c,v 3.87 1995/12/17 22:54:34 kardel Exp
 *
...
static char rcsid[]="refclock_parse.c,v 3.87 1995/12/17 22:54:34 kardel Exp";
#endif
  ...
  struct refclockstat *out;
{
  ...
  if (out)
      ...
      for (i = 0; i <= CEVNT_MAX; i++)
	{
      ...
      sprintf(tt, "refclock_driver_version=\"refclock_parse.c,v 3.87 1995/12/17 22:54:34 kardel Exp\"");
  ...
  int event;
{
  switch (event)
	...
	while (*iv)
	  {
	    ...
	    if (rtc < 0)
	      {
		syslog(LOG_ERR, "PARSE receiver #%d: trimbletaip_event: failed to send cmd to clock: %m", CLK_UNIT(parse->peer));
		return;
	      }
	    else
	      {
		if (rtc != strlen(*iv))
		  {
		    syslog(LOG_ERR, "PARSE receiver #%d: trimbletaip_event: failed to send cmd incomplete (%d of %d bytes sent)",
	...
	NLOG(NLOG_CLOCKINFO)
	  ERR(ERR_BADIO)
	    syslog(LOG_ERR, "PARSE receiver #%d: trimbletaip_event: RECEIVER INITIALIZED",
		   CLK_UNIT(parse->peer));
      }
      break;
