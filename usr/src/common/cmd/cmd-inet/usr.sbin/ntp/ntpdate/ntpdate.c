	...
	char *argv[];
{
	 ...
	 */
	while ((c = ntp_getopt(argc, argv, "a:bBde:k:o:p:qst:vu")) != EOF)
		switch (c) {
		case 'a':
			c = atoi(ntp_optarg);
			sys_authenticate = 1;
...
#define	LOG_NTP	LOG_DAEMON
	...
	if (!SetPriorityClass(process_handle, (DWORD) REALTIME_PRIORITY_CLASS)) {
			syslog(LOG_ERR, "SetPriorityClass failed: %m");
	}
#endif /* SYS_WINNT */
...
clock_adjust()
{
	...
	if (dostep) {
		if (simple_query || l_step_systime(&server->offset)) {
			syslog(LOG_NOTICE, "step time server %s offset %s sec",
			    ntoa(&server->srcadr),
		...
		if (simple_query || l_adj_systime(&server->offset)) {
			syslog(LOG_NOTICE, "adjust time server %s offset %s sec",
			    ntoa(&server->srcadr),
			    lfptoa(&server->offset, 6));
		}
#else
    ...
    DWORD dwUser, DWORD dw1, DWORD dw2)
{
	...
	if (initializing2) {
			...
			if (!SetThreadPriority(TimerThreadHandle, (DWORD) THREAD_PRIORITY_HIGHEST))
			 	syslog(LOG_ERR, "SetThreadPriority: %m");
...
input_handler()
{
	...
	for (;;) {
		...
		rb->recv_length = recvfrom(fd, (char *)&rb->recv_pkt,
		    ...
		    (struct sockaddr *)&rb->srcadr, (size_t *)&fromlen);
		if (rb->recv_length == -1) {
			rb->next = freelist;
			freelist = rb;
			free_recvbufs++;
