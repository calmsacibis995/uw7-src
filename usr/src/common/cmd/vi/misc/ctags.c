 ...
 *
 * One shortcoming of this routine is that it fails to detect
 * a function when a comment is placed between the function
 * name and the '('.
 */
...
ret:
	*lp = --sp;
	if (c == '\n')
		lineno--;
	ungetc(c,inf);
	if ((bad) && (update_lineftell == TRUE)) {
		/*
		 * we must update lineftell in the case that this
		 * was not the start of a function but we processed
		 * a carriage return before we found that out; if
		 * this is not done and the next token is the start
		 * of a function, we will output the line preceding
		 * the function name instead of the correct line.
		 */
		lineftell = ftell(inf);
	}
