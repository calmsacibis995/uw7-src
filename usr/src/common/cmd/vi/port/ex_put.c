...
#ident	"@(#)vi:port/ex_put.c	1.22.1.18"
#ident  "$Header$"
...
#include <wctype.h>
/*
 * Because putchar is a macro in <stdio.h>, and because some versions of
 * <wctype.h> include <stdio.h>, we undef putchar here.
 */
#undef putchar
...
register wchar_t c;
{
	...
	if ((int)(c & QUOTE))
		...
		putchar('^'), c = ctlof(c);
        else if (((c & ~0x7f) != 0) && (putoctal || !wisprint(c))) {
	/*
	 * The above checks are performed to find if it's necessary to print
	 * the octal representation of the character.  Because the C locale
	 * does not include 8-bit characters, LANG (actually LC_CTYPE must
	 * be set in order to put 8-bit characters through.  This is break
	 * with past practice in 1.x (because of a bug) and 2.0 but is 
	 * the correct behavior for vi.
	 */
		outchar('\\');
		outchar(((c >> 6) & 07) + '0');
		outchar(((c >> 3) & 07) + '0');
...
ostart()
{
	...
	/*
	if (!intty)
		error(":282", "Open and visual must be used interactively");
	*/
	gTTY(2);
	normtty++;
	f = tty;
