...
#ident	"@(#)vi:port/ex_cmdsub.c	1.17.1.14"
#ident "$Header$"
	...
	line *addrt;
{
	...
	if (adt < ad1) {
		...
		dot = ++adt;		/* set to the 1st of the moved lines */
		if (adt != ad1) {
			reverse(adt, ad1);
			reverse(ad1, ad2);
			reverse(adt, ad2);
	...
	register struct maps *mp;
{
	...
	if (msnext - mapspace + strlen((const char *)dest) + strlen((const char *)src) + strlen((const char *)dname) + 3 > (unsigned)MAXCHARMACS)
