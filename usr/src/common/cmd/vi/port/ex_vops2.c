...
#ident	"@(#)vi:port/ex_vops2.c	1.21.2.11"
#ident  "$Header$"
	...
	int cnt, indent;
{
	...
	if (cursor < linebuf) cursor = linebuf;
				...
				if (outcol+(backsl?OCOLUMNS:0) - width >= OCOLUMNS - value(vi_WRAPMARGIN)) {
					 ...
					 */
						...
						startlength = length - 1;
							...
							(startlength > length)){
						/*
						 * Only insert ctrl-v before
						 * escape if the escape is not
						 * the character that caused the
						 * wrap.  If it caused the wrap,
						 * it couldn't have been pre-
						 * ceded by ctrl-v since the
						 * code for ctrl-v bypasses the
						 * wrap code.  If it didn't
						 * cause the wrap, it must have
						 * been preceded by ctrl-v or
						 * we would have returned to
						 * command mode.
						 */
								c[0] = CTRL('v');
								macpush(c, 0);
