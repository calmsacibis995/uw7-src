...
#ident	"@(#)vi:port/ex_temp.c	1.19.1.9"
#ident "$Header$"
#include "ex.h"
#include "ex_temp.h"
#include "ex_vis.h"
...
synctmp()
{
	...
	*zero = (line) H.Time;
	for (a = zero, bp = blocks; a <= dol; a += BUFSIZ / sizeof *a, bp++) {
		/* check for walking off the end of the array */
		/* leave last entry as -1 from previous loop iteration */
		if ((bp - blocks) == (LBLKS-1) ) {
			error(EXmtmptoolinesid, EXmtmptoolines);
			break;
		}
