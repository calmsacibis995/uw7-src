...
#ident	"@(#)vi:port/ex_vget.c	1.20.1.15"
#ident "$Header$"
	...
	register unsigned char *buf, *str;
{
	...
	if ((unsigned char)buf[128] == 0200)
		return;
	if (strlen((const char *)buf) + strlen((const char *)str) + 1 >= (unsigned) VBSIZE) {
		buf[128] = 0200;
		return;
	}
		...
		if (p = maps[d].cap) {
			 ...
			 */
			 ...
			 && strcmp((char *)maps[d].descr, (const char *)maps[d].cap)) {
				...
				if(strlen((const char *)vmacbuf)  + strlen((const char *)st) > (unsigned) BUFSIZ) 
...
#endif
	if ((vmacp ? strlen((const char *)vmacp) : 0) + strlen((const char *)st) > (unsigned) BUFSIZ)
		error(EXmmactoolongid, EXmmactoolong);
	if (vmacp) {
